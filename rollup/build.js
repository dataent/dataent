...
function concatenate_files() {
	...
	const files_to_concat = Object.keys(get_build_json('dataent'))
		.filter(filename => filename.startsWith('concat:'));

	files_to_concat.forEach(output_file => {
		const input_files = get_build_json('dataent')[output_file];

		const file_content = input_files.map(file_name => {
			let prefix = get_app_path('dataent');
			if (file_name.startsWith('node_modules/')) {
				prefix = path.resolve(get_app_path('dataent'), '..');
			}
...
function show_production_message() {
	const production = process.env.DATAENT_ENV === 'production';
