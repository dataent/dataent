...
class TestAddressTemplate(unittest.TestCase):
	...
	def test_default_is_unset(self):
		a = dataent.get_doc("Address Template", "India")
		a.is_default = 1
		a.save()

		b = dataent.get_doc("Address Template", "Brazil")
		b.is_default = 1
		b.save()

		self.assertEqual(dataent.db.get_value("Address Template", "India", "is_default"), 0)
	...
	def make_default_address_template(self):
		...
		if not dataent.db.exists('Address Template', 'Brazil'):
			dataent.get_doc({
				"doctype": "Address Template",
				"country": 'Brazil',
				"template": template
