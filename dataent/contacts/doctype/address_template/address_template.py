...
class AddressTemplate(Document):
	def validate(self):
		...
		self.defaults = dataent.db.get_values("Address Template", {"is_default":1, "name":("!=", self.name)})
		if not self.is_default:
			if not self.defaults:
				self.is_default = 1
				if cint(dataent.db.get_single_value('System Settings', 'setup_complete')):
					dataent.msgprint(_("Setting this Address Template as default as there is no other default"))
	...
	def on_update(self):
		if self.is_default and self.defaults:
			for d in self.defaults:
				dataent.db.set_value("Address Template", d[0], "is_default", 0)

	def on_trash(self):
		if self.is_default:
			dataent.throw(_("Default Address Template cannot be deleted"))
