...
def load_address_and_contact(doc, key=None):
	...
	address_list = dataent.get_all("Address", filters=filters, fields=["*"])
	...
	contact_list = dataent.get_all("Contact", filters=filters, fields=["*"])
...
def has_permission(doc, ptype, user):
	...
	for df in (links.get("permitted_links") + links.get("not_permitted_links")):
		...
		if name and dataent.has_permission(doctype, ptype, doc=name):
			return True
...
def get_permitted_and_not_permitted_links(doctype):
	...
	allowed_doctypes = dataent.permissions.get_doctypes_with_read()
...
def delete_contact_and_address(doctype, docname):
	for parenttype in ('Contact', 'Address'):
		items = dataent.db.sql_list("""select parent from `tabDynamic Link`
			where parenttype=%s and link_doctype=%s and link_name=%s""",
			(parenttype, doctype, docname))

		for name in items:
			doc = dataent.get_doc(parenttype, name)
			if len(doc.links)==1:
				doc.delete()
...
def filter_dynamic_link_doctypes(doctype, txt, searchfield, start, page_len, filters):
	...
	doctypes = dataent.db.get_all("DocField", filters=filters, fields=["parent"],
	...
	_doctypes = dataent.db.get_all("Custom Field", filters=filters, fields=["dt"],
		as_list=True)
...
def set_link_title(doc):
	...
	for link in doc.links:
		if not link.link_title:
			linked_doc = dataent.get_doc(link.link_doctype, link.link_name)
