...
from dataent.contacts.report.addresses_and_contacts.addresses_and_contacts import get_data

def get_custom_linked_doctype():
	if bool(dataent.get_all("DocType", filters={'name':'Test Custom Doctype'})):
		return
...
def create_linked_address(link_list):
	if dataent.flags.test_address_created:
		return

	address = dataent.get_doc({
		"doctype": "Address",
	...
	address.insert()
	dataent.flags.test_address_created = True

def create_linked_contact(link_list):
	if dataent.flags.test_contact_created:
		return
	...
	contact.insert()
	dataent.flags.test_contact_created = True
