...
from dataent.utils import get_site_path
from dataent import conf

END_LINE = '<!-- dataent: end-file -->'
TASK_LOG_MAX_AGE = 86400  # 1 day in seconds
redis_server = None

@dataent.whitelist()
def get_pending_tasks_for_doc(doctype, docname):
	return dataent.db.sql_list("select name from `tabAsync Task` where status in ('Queued', 'Running') and reference_doctype=%s and reference_name=%s", (doctype, docname))
...
def publish_progress(percent, title=None, doctype=None, docname=None, description=None):
	publish_realtime('progress', {'percent': percent, 'title': title, 'description': description},
		user=dataent.session.user, doctype=doctype, docname=docname)
...
def publish_realtime(event=None, message=None, room=None,
	...
	if event is None:
		if getattr(dataent.local, "task_id", None):
			event = "task_progress"
		else:
			event = "global"
	...
	if not room:
		if not task_id and hasattr(dataent.local, "task_id"):
			task_id = dataent.local.task_id
	...
	if after_commit:
		...
		if not params in dataent.local.realtime_log:
			dataent.local.realtime_log.append(params)
	else:
...
def emit_via_redis(event, message, room):
	...
	try:
		r.publish('events', dataent.as_json({'event': event, 'message': message, 'room': room}))
...
class FileAndRedisStream(FileIO):
	...
	def write(self, data):
		...
		if dataent.local.task_id:
			put_log(self.count, data, task_id=dataent.local.task_id)
			self.count += 1
		return ret
...
def get_task_log_file_path(task_id, stream_type):
	logs_dir = dataent.utils.get_site_path('task-logs')
...
def can_subscribe_doc(doctype, docname, sid):
	if os.environ.get('CI'):
		return True
	from dataent.sessions import Session
	from dataent.exceptions import PermissionError
	session = Session(None, resume=True).get_session_data()
	if not dataent.has_permission(user=session.user, doctype=doctype, doc=docname, ptype='read'):
...
def get_doc_room(doctype, docname):
	return ''.join([dataent.local.site, ':doc:', doctype, '/', docname])

def get_user_room(user):
	return ''.join([dataent.local.site, ':user:', user])

def get_site_room():
	return ''.join([dataent.local.site, ':all'])

def get_task_progress_room(task_id):
	return "".join([dataent.local.site, ":task_progress:", task_id])

# dataent.chat
def get_chat_room(room):
	room = ''.join([dataent.local.site, ":room:", room])

	return room
# end dataent.chat room