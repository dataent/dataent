...
class Workflow(Document):
	...
	def on_update(self):
		...
		dataent.cache().delete_key('workflow_' + self.name) # clear cache created in model/workflow.py
	...
	def create_custom_field_for_workflow_state(self):
		...
		if not meta.get_field(self.workflow_state_field):
			...
			dataent.msgprint(_("Created Custom Field {0} in {1}").format(self.workflow_state_field,
				self.document_type))
	...
	def update_default_workflow_status(self):
		...
		for d in states:
			if not d.doc_status in docstatus_map:
				dataent.db.sql("""update `tab%s` set `%s` = %s where \
	...
	def validate_docstatus(self):
		def get_state(state):
			...
			dataent.throw(dataent._("{0} not a valid State").format(state))
		...
		for t in self.transitions:
			...
			if state.doc_status=="2":
				dataent.throw(dataent._("Cannot change state of Cancelled Document. Transition row {0}").format(t.idx))

			if state.doc_status=="1" and next_state.doc_status=="0":
				dataent.throw(dataent._("Submitted Document cannot be converted back to draft. Transition row {0}").format(t.idx))

			if state.doc_status=="0" and next_state.doc_status=="2":
				dataent.throw(dataent._("Cannot cancel before submitting. See Transition {0}").format(t.idx))

	def set_active(self):
		if int(self.is_active or 0):
			# clear all other
			dataent.db.sql("""update tabWorkflow set is_active=0
				where document_type=%s""",
				self.document_type)

@dataent.whitelist()
def get_fieldnames_for(doctype):
	return [f.fieldname for f in dataent.get_meta(doctype).fields \
