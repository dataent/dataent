...
class PropertySetter(Document):
	...
	def validate_fieldtype_change(self):
		if self.field_name in not_allowed_fieldtype_change and \
			...
			dataent.throw(_("Field type cannot be changed for {0}").format(self.field_name))

	def delete_property_setter(self):
		"""delete other property setters on this, if this is new"""
		if self.get('__islocal'):
			dataent.db.sql("""delete from `tabProperty Setter` where
	...
	def get_property_list(self, dt):
		return dataent.db.sql("""select fieldname, label, fieldtype
		from tabDocField
		where parent=%s
	...
	def get_setup_data(self):
		return {
			'doctypes': [d[0] for d in dataent.db.sql("select name from tabDocType")],
			'dt_properties': self.get_property_list('DocType'),
			'df_properties': self.get_property_list('DocField')
		}

	def get_field_ids(self):
		return dataent.db.sql("select name, fieldtype, label, fieldname from tabDocField where parent=%s", self.doc_type, as_dict = 1)

	def get_defaults(self):
		if not self.field_name:
			return dataent.db.sql("select * from `tabDocType` where name=%s", self.doc_type, as_dict = 1)[0]
		else:
			return dataent.db.sql("select * from `tabDocField` where fieldname=%s and parent=%s",
				(self.field_name, self.doc_type), as_dict = 1)[0]

	def on_update(self):
		if dataent.flags.in_patch:
...
def make_property_setter(doctype, fieldname, property, value, property_type, for_doctype = False, 
	...
	property_setter = dataent.get_doc({
