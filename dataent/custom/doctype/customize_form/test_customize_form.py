...
from dataent.core.doctype.doctype.doctype import InvalidFieldNameError
...
class TestCustomizeForm(unittest.TestCase):
	...
	def tearDown(self):
		dataent.delete_doc("Custom Field", "Event-test_custom_field")
		dataent.db.commit()
		dataent.clear_cache(doctype="Event")

	def get_customize_form(self, doctype=None):
		d = dataent.get_doc("Customize Form")
		if doctype:
			d.doc_type = doctype
	...
	def test_fetch_to_customize(self):
		...
		self.assertEqual(len(d.get("fields")),
			len(dataent.get_doc("DocType", d.doc_type).fields) + 1)
	...
	def test_save_customization_property(self):
		d = self.get_customize_form("Event")
		self.assertEquals(dataent.db.get_value("Property Setter",
	...
	def test_save_customization_custom_field_property(self):
		d = self.get_customize_form("Event")
		self.assertEquals(dataent.db.get_value("Custom Field", "Event-test_custom_field", "reqd"), 0)

		custom_field = d.get("fields", {"fieldname": "test_custom_field"})[0]
		custom_field.reqd = 1
		d.run_method("save_customization")
		self.assertEquals(dataent.db.get_value("Custom Field", "Event-test_custom_field", "reqd"), 1)
	...
	def test_save_customization_new_field(self):
		...
		d.run_method("save_customization")
		self.assertEquals(dataent.db.get_value("Custom Field",
		...
		dataent.delete_doc("Custom Field", "Event-test_add_custom_field_via_customize_form")
		self.assertEquals(dataent.db.get_value("Custom Field",
	...
	def test_save_customization_remove_field(self):
		...
		self.assertEqual(dataent.db.get_value("Custom Field", custom_field.name), None)

		dataent.local.test_objects["Custom Field"] = []
	...
	def test_reset_to_defaults(self):
		...
		dataent.local.test_objects["Property Setter"] = []
