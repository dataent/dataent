...
dataent.provide("dataent.customize_form");

dataent.ui.form.on("Customize Form", {
	onload: function(frm) {
		dataent.customize_form.add_fields_help(frm);
		...
		frm.set_query("doc_type", function() {
			return {
				...
				filters: [
					['DocType', 'issingle', '=', 0],
					['DocType', 'custom', '=', 0],
					['DocType', 'name', 'not in', dataent.model.core_doctypes_list],
					['DocType', 'restrict_to_domain', 'in', dataent.boot.active_domains]
				]
			};
		});
	...
	refresh: function(frm) {
		...
		if(frm.doc.doc_type) {
			dataent.customize_form.set_primary_action(frm);
			...
			frm.add_custom_button(__('Reset to defaults'), function() {
				dataent.customize_form.confirm(__('Remove all customizations?'), frm);
			}, "fa fa-eraser", "btn-default");

			frm.add_custom_button(__('Set Permissions'), function() {
				dataent.set_route('permission-manager', frm.doc.doc_type);
			...
			if(dataent.boot.developer_mode) {
				frm.add_custom_button(__('Export Customizations'), function() {
					dataent.prompt(
						...
						function(data) {
							dataent.call({
								method: 'dataent.modules.utils.export_customizations',
								args: {
		...
		if(frm.doc.doc_type) {
			var fields = $.map(frm.doc.fields,
					function(df) { return dataent.model.is_value_type(df.fieldtype) ? df.fieldname : null; });
			fields = ["", "name", "modified"].concat(fields);
			frm.set_df_property("sort_field", "options", fields);
		}

		if(dataent.route_options && dataent.route_options.doc_type) {
			setTimeout(function() {
				frm.set_value("doc_type", dataent.route_options.doc_type);
...
dataent.ui.form.on("Customize Form Field", {
	before_fields_remove: function(frm, doctype, name) {
		var row = dataent.get_doc(doctype, name);
		if(!(row.is_custom_field || row.__islocal)) {
			dataent.msgprint(__("Cannot delete standard field. You can hide it if you want"));
			throw "cannot delete custom field";
		}
	},
	fields_add: function(frm, cdt, cdn) {
		var f = dataent.model.get_doc(cdt, cdn);
		f.is_custom_field = 1;
	}
});

dataent.customize_form.set_primary_action = function(frm) {
	frm.page.set_primary_action(__("Update"), function() {
		if(frm.doc.doc_type) {
			return frm.call({
				...
				callback: function(r) {
					if(!r.exc) {
						dataent.customize_form.clear_locals_and_refresh(frm);
						frm.script_manager.trigger("doc_type");
					}
				}
			});
...
dataent.customize_form.confirm = function(msg, frm) {
	...
	var d = new dataent.ui.Dialog({
		...
		primary_action: function() {
			return frm.call({
				...
				callback: function(r) {
					if(r.exc) {
						dataent.msgprint(r.exc);
					} else {
						d.hide();
						dataent.show_alert({message:__('Customizations Reset'), indicator:'green'});
						dataent.customize_form.clear_locals_and_refresh(frm);
					}
				}
			});
	...
	dataent.customize_form.confirm.dialog = d;
	d.show();
}

dataent.customize_form.clear_locals_and_refresh = function(frm) {
	// clear doctype from locals
	dataent.model.clear_doc("DocType", frm.doc.doc_type);
	delete dataent.meta.docfield_copy[frm.doc.doc_type];

	frm.refresh();
}

dataent.customize_form.add_fields_help = function(frm) {
	...
	$('#fields_help').click(function() {
			...
			"<table cellspacing='25'>\
				...
				<tr>\
					...
					<td><a class='link_type' \
							onclick='dataent.customize_form.fields_help_dialog.hide()'\
		...
		dataent.customize_form.fields_help_dialog = d;
