...
def add(doctype, name, user=None, read=1, write=0, share=0, everyone=0, flags=None, notify=0):
	...
	if share_name:
		doc = dataent.get_doc("DocShare", share_name)
	else:
		doc = dataent.new_doc("DocShare")
		doc.update({
			"user": user,
...
def remove(doctype, name, user, flags=None):
	share_name = dataent.db.get_value("DocShare", {"user": user, "share_name": name,
		"share_doctype": doctype})

	if share_name:
		dataent.delete_doc("DocShare", share_name, flags=flags)
...
def set_permission(doctype, name, user, permission_to, value=1, everyone=0):
	...
	if not share_name:
		...
		share = dataent.get_doc("DocShare", share_name)
...
def get_shared(doctype, user=None, rights=None):
	...
	return dataent.db.sql_list("""select share_name from tabDocShare
...
def get_shared_doctypes(user=None):
	...
	return dataent.db.sql_list("select distinct share_doctype from tabDocShare where (user=%s or everyone=1)", user)

def get_share_name(doctype, name, user, everyone):
	if cint(everyone):
		share_name = dataent.db.get_value("DocShare", {"everyone": 1, "share_name": name,
			"share_doctype": doctype})
	else:
		share_name = dataent.db.get_value("DocShare", {"user": user, "share_name": name,
			"share_doctype": doctype})
...
def check_share_permission(doctype, name):
	...
	if not dataent.has_permission(doctype, ptype="share", doc=name):
		dataent.throw(_("No permission to {0} {1} {2}".format("share", doctype, name)), dataent.PermissionError)
