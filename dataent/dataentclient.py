...
'''
DataentClient is a library that helps you connect with other dataent systems
'''

class AuthError(Exception):
	pass

class DataentException(Exception):
	pass

class DataentClient(object):
	...
	def insert(self, doc):
		...
		res = self.session.post(self.url + "/api/resource/" + doc.get("doctype"),
			data={"data":dataent.as_json(doc)}, verify=self.verify, headers=self.headers)
		return self.post_process(res)
	...
	def update(self, doc):
		...
		res = self.session.put(url, data={"data":dataent.as_json(doc)}, verify=self.verify, headers=self.headers)
		return self.post_process(res)
	...
	def migrate_doctype(self, doctype, filters=None, update=None, verbose=1, exclude=None, preprocess=None):
		...
		for doc in docs:
			...
			if doctype != "User" and not dataent.db.exists("User", doc.get("owner")):
				dataent.get_doc({"doctype": "User", "email": doc.get("owner"),
	...
	def migrate_single(self, doctype):
		...
		doc.modified = dataent.db.get_single_value(doctype, "modified")
		dataent.get_doc(doc).insert()
	...
	def post_process(self, response):
		...
		if rjson and ("exc" in rjson) and rjson["exc"]:
			try:
				...
				exc = 'DataentClient Request Failed\n\n' + exc
			except Exception:
				exc = rjson["exc"]

			raise DataentException(exc)
		if 'message' in rjson:
...
class DataentOAuth2Client(DataentClient):
