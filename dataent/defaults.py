...
from dataent.cache_manager import clear_defaults_cache, common_default_keys
...
def set_user_default(key, value, user=None, parenttype=None):
	set_default(key, value, user or dataent.session.user, parenttype)

def add_user_default(key, value, user=None, parenttype=None):
	add_default(key, value, user or dataent.session.user, parenttype)

def get_user_default(key, user=None):
	user_defaults = get_defaults(user or dataent.session.user)
	...
	if is_a_user_permission_key(key):
		if d and isinstance(d, (list, tuple)) and len(d)==1:
			...
			d = user_defaults.get(dataent.scrub(key), None)
...
def is_a_user_permission_key(key):
	return ":" not in key and key != dataent.scrub(key)

def not_in_user_permission(key, value, user=None):
	# returns true or false based on if value exist in user permission
	user = user or dataent.session.user
	user_permission = get_user_permissions(user).get(dataent.unscrub(key)) or []
...
def get_user_permissions(user=None):
	from dataent.core.doctype.user_permission.user_permission \
		import get_user_permissions as _get_user_permissions
	'''Return dataent.core.doctype.user_permissions.user_permissions._get_user_permissions (kept for backward compatibility)'''
...
def get_defaults(user=None):
	...
	if not user:
		user = dataent.session.user if dataent.session else "Guest"
...
def clear_user_default(key, user=None):
	clear_default(key, parent=user or dataent.session.user)
...
def clear_default(key=None, value=None, parent=None, name=None, parenttype=None):
	...
	dataent.db.sql("""delete from tabDefaultValue where {0}""".format(" and ".join(conditions)),
		tuple(values))
...
def get_defaults_for(parent="__default"):
	"""get all defaults"""
	defaults = dataent.cache().hget("defaults", parent)

	if defaults==None:
		# sort descending because first default must get precedence
		res = dataent.db.sql("""select defkey, defvalue from `tabDefaultValue`
			where parent = %s order by creation""", (parent,), as_dict=1)

		defaults = dataent._dict({})
		for d in res:
		...
		dataent.cache().hset("defaults", parent, defaults)
...
def _clear_cache(parent):
	if parent in common_default_keys:
		...
		dataent.clear_cache(user=parent)
