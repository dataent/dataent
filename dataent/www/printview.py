...
import dataent, os, copy, json, re
from dataent import _

from dataent.modules import get_doc_path
from dataent.utils import cint, strip_html
...
def get_context(context):
	...
	if not ((dataent.form_dict.doctype and dataent.form_dict.name) or dataent.form_dict.doc):
		return {
			"body": """<h1>Error</h1>
				<p>Parameters doctype and name required</p>
				<pre>%s</pre>""" % repr(dataent.form_dict)
		}

	if dataent.form_dict.doc:
		doc = dataent.form_dict.doc
	else:
		doc = dataent.get_doc(dataent.form_dict.doctype, dataent.form_dict.name)
	...
	return {
		"body": get_html(doc, print_format = print_format,
			meta=meta, trigger_print = dataent.form_dict.trigger_print,
			no_letterhead=dataent.form_dict.no_letterhead),
		"css": get_print_style(dataent.form_dict.style, print_format),
		"comment": dataent.session.user,
		"title": doc.get(meta.title_field) if meta.title_field else doc.name,
		"has_rtl": True if dataent.local.lang in ["ar", "he", "fa"] else False
	}

def get_print_format_doc(print_format_name, meta):
	"""Returns print format document"""
	if not print_format_name:
		print_format_name = dataent.form_dict.format \
	...
	if print_format_name == "Standard":
		...
		try:
			return dataent.get_doc("Print Format", print_format_name)
...
def get_html(doc, name=None, print_format=None, meta=None,
	...
	if not dataent.flags.ignore_print_permissions:
		validate_print_permission(doc)

	if doc.meta.is_submittable:
		if doc.docstatus==0 and not cint(print_settings.allow_print_for_draft):
			dataent.throw(_("Not allowed to print draft documents"), dataent.PermissionError)

		if doc.docstatus==2 and not cint(print_settings.allow_print_for_cancelled):
			dataent.throw(_("Not allowed to print cancelled documents"), dataent.PermissionError)
	...
	letter_head = dataent._dict(get_letter_head(doc, no_letterhead) or {})

	if letter_head.content:
		letter_head.content = dataent.utils.jinja.render_template(letter_head.content, {"doc": doc.as_dict()})

	if letter_head.footer:
		letter_head.footer = dataent.utils.jinja.render_template(letter_head.footer, {"doc": doc.as_dict()})
	...
	for field in meta.fields:
		if field.fieldtype=='Text Editor':
			...
			if value and '<!-- markdown -->' in value:
				doc.set(field.fieldname, dataent.utils.md_to_html(value))
...
def get_html_and_style(doc, name=None, print_format=None, meta=None,
	...
	if isinstance(doc, string_types) and isinstance(name, string_types):
		doc = dataent.get_doc(doc, name)

	if isinstance(doc, string_types):
		doc = dataent.get_doc(json.loads(doc))

	print_format = get_print_format_doc(print_format, meta=meta or dataent.get_meta(doc.doctype))
	return {
...
def get_letter_head(doc, no_letterhead):
	...
	if doc.get("letter_head"):
		return dataent.db.get_value("Letter Head", doc.letter_head, ["content", "footer"], as_dict=True)
	else:
		return dataent.db.get_value("Letter Head", {"is_default": 1}, ["content", "footer"], as_dict=True) or {}

def get_print_format(doctype, print_format):
	if print_format.disabled:
		dataent.throw(_("Print Format {0} is disabled").format(print_format.name),
			dataent.DoesNotExistError)

	# server, find template
	path = os.path.join(get_doc_path(dataent.db.get_value("DocType", doctype, "module"),
		"Print Format", print_format.name), dataent.scrub(print_format.name) + ".html")
	...
	if os.path.exists(path):
		...
		if print_format.html:
			...
			dataent.throw(_("No template found at path: {0}").format(path),
				dataent.TemplateNotFoundError)
...
def get_visible_columns(data, table_meta, df):
	...
	columns = []
	doc = data[0] or dataent.new_doc(df.options)
