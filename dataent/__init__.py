...
"""
globals attached to dataent module
...
def _(msg, lang=None):
	...
	from dataent.translate import get_full_dict
...
def get_lang_dict(fortype, name=None):
	...
	from dataent.translate import get_dict
	return get_dict(fortype, name)

def set_user_lang(user, user_language=None):
	"""Guess and set user language for the session. `dataent.local.lang`"""
	from dataent.translate import get_user_lang
...
def init(site, sites_path=None, new_site=False):
	"""Initialize dataent for the current site. Reset thread locals `dataent.local`"""
...
def connect(site=None, db_name=None):
	...
	:param site: If site is given, calls `dataent.init`.
	:param db_name: Optional. Will use from `site_config.json`."""
	from dataent.database import Database
	if site:
		init(site)
...
def cache():
	...
	global redis_server
	if not redis_server:
		from dataent.utils.redis_wrapper import RedisWrapper
...
def get_traceback():
	...
	from dataent.utils import get_traceback
...
def msgprint(msg, title=None, raise_exception=0, as_table=False, indicator=None, alert=False):
	...
	"""
	from dataent.utils import encode
...
def throw(msg, exc=ValidationError, title=None):
	...
	:param msg: Message.
	:param exc: Exception class. Default `dataent.ValidationError`"""
	msgprint(msg, raise_exception=exc, title=title, indicator='red')

def emit_js(js, user=False, **kwargs):
	from dataent.realtime import publish_realtime
	if user == False:
		user = session.user
...
def create_folder(path, with_init=False):
	...
	from dataent.utils import touch_file
...
def get_user():
	from dataent.utils.user import UserPermissions
...
def get_roles(username=None):
	...
	if username:
		...
		return dataent.permissions.get_roles(username)
	else:
...
def sendmail(recipients=[], sender="", subject="No Subject", message="No Message",
	...
	if as_markdown:
		message = dataent.utils.md_to_html(message)

	if not delayed:
		now = True

	from dataent.email import queue
...
def only_for(roles):
	"""Raise `dataent.PermissionError` if the user does not have any of the given **Roles**.
...
def clear_cache(user=None, doctype=None):
	...
	import dataent.cache_manager
	if doctype:
		dataent.cache_manager.clear_doctype_cache(doctype)
		reset_metadata_version()
	elif user:
		dataent.cache_manager.clear_user_cache(user)
	else: # everything
		from dataent import translate
		dataent.cache_manager.clear_user_cache()
...
def has_permission(doctype=None, ptype="read", doc=None, user=None, verbose=False, throw=False):
	"""Raises `dataent.PermissionError` if not permitted.
	...
	import dataent.permissions
	out = dataent.permissions.has_permission(doctype, ptype, doc=doc, verbose=verbose, user=user)
	if throw and not out:
		if doc:
			dataent.throw(_("No permission for {0}").format(doc.doctype + " " + doc.name))
		else:
...
def new_doc(doctype, parent_doc=None, parentfield=None, as_dict=False):
	...
	from dataent.model.create_new import get_new_doc
	return get_new_doc(doctype, parent_doc, parentfield, as_dict=as_dict)

def set_value(doctype, docname, fieldname, value=None):
	"""Set document value. Calls `dataent.client.set_value`"""
	import dataent.client
	return dataent.client.set_value(doctype, docname, fieldname, value)
...
def get_doc(*args, **kwargs):
	"""Return a `dataent.model.document.Document` object of the given type and name.
	...
	Examples:
		...
		todo = dataent.get_doc("ToDo", "TD0001")

	"""
	import dataent.model.document
	doc = dataent.model.document.get_doc(*args, **kwargs)
...
def get_single(doctype):
	"""Return a `dataent.model.document.Document` object of the given Single doctype."""
	return get_doc(doctype, doctype)

def get_meta(doctype, cached=True):
	"""Get `dataent.model.meta.Meta` instance of given doctype name."""
	import dataent.model.meta
	return dataent.model.meta.get_meta(doctype, cached=cached)

def get_meta_module(doctype):
	import dataent.modules
	return dataent.modules.load_doctype_module(doctype)

def delete_doc(doctype=None, name=None, force=0, ignore_doctypes=None, for_reload=False,
	ignore_permissions=False, flags=None, ignore_on_trash=False, ignore_missing=True):
	"""Delete a document. Calls `dataent.model.delete_doc.delete_doc`.
	...
	import dataent.model.delete_doc
	dataent.model.delete_doc.delete_doc(doctype, name, force, ignore_doctypes, for_reload,
...
def reload_doc(module, dt=None, dn=None, force=False, reset_permissions=False):
	...
	import dataent.modules
	return dataent.modules.reload_doc(module, dt, dn, force=force, reset_permissions=reset_permissions)

def rename_doc(*args, **kwargs):
	"""Rename a document. Calls `dataent.model.rename_doc.rename_doc`"""
...
def get_all_apps(with_internal_apps=True, sites_path=None):
	...
	if "dataent" in apps:
		apps.remove("dataent")
	apps.insert(0, 'dataent')

	return apps

def get_installed_apps(sort=False, dataent_last=False):
	...
	if dataent_last:
		if 'dataent' in installed:
			installed.remove('dataent')
		installed.append('dataent')
...
def get_hooks(hook=None, default=None, app_name=None):
	...
	def load_app_hooks(app_name=None):
		...
		for app in [app_name] if app_name else get_installed_apps(sort=True):
			app = "dataent" if app=="webnotes" else app
			try:
...
def get_file_items(path, raise_not_found=False, ignore_empty_lines=True):
	...
	if content:
		content = dataent.utils.strip(content)
...
def compare(val1, condition, val2):
	"""Compare two values using `dataent.utils.compare`
	...
	"""
	import dataent.utils
	return dataent.utils.compare(val1, condition, val2)
...
def redirect_to_message(title, html, http_status_code=None, context=None, indicator_color=None):
	...
	Example Usage:
		dataent.redirect_to_message(_('Thank you'), "<div><p>You will receive an email at test@example.com</p></div>")
...
def build_match_conditions(doctype, as_condition=True):
	...
	import dataent.desk.reportview
	return dataent.desk.reportview.build_match_conditions(doctype, as_condition=as_condition)

def get_list(doctype, *args, **kwargs):
	"""List database query via `dataent.model.db_query`. Will also check for permissions.
	...
	Example usage:
		...
		# simple dict filter
		dataent.get_list("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

		# filter as a list of lists
		dataent.get_list("ToDo", fields="*", filters = [["modified", ">", "2014-01-01"]])

		# filter as a list of dicts
		dataent.get_list("ToDo", fields="*", filters = {"description": ("like", "test%")})
	"""
	import dataent.model.db_query
	return dataent.model.db_query.DatabaseQuery(doctype).execute(None, *args, **kwargs)

def get_all(doctype, *args, **kwargs):
	"""List database query via `dataent.model.db_query`. Will **not** check for permissions.
	Parameters are same as `dataent.get_list`
	...
	Example usage:
		...
		# simple dict filter
		dataent.get_all("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

		# filter as a list of lists
		dataent.get_all("ToDo", fields=["*"], filters = [["modified", ">", "2014-01-01"]])

		# filter as a list of dicts
		dataent.get_all("ToDo", fields=["*"], filters = {"description": ("like", "test%")})
	"""
...
def get_value(*args, **kwargs):
	...
	Alias for `dataent.db.get_value`
...
def get_test_records(doctype):
	...
	from dataent.modules import get_doctype_module, get_module_path
...
def format_value(*args, **kwargs):
	...
	import dataent.utils.formatters
	return dataent.utils.formatters.format_value(*args, **kwargs)
...
def get_print(doctype=None, name=None, print_format=None, style=None, html=None, as_pdf=False, doc=None, output = None, no_letterhead = 0):
	...
	from dataent.utils.pdf import get_pdf
...
def attach_print(doctype, name, file_name=None, print_format=None, style=None, html=None, doc=None, lang=None, print_letterhead=True):
	from dataent.utils import scrub_urls
...
def publish_progress(*args, **kwargs):
	...
	"""
	import dataent.realtime
	return dataent.realtime.publish_progress(*args, **kwargs)
...
def publish_realtime(*args, **kwargs):
	...
	return dataent.realtime.publish_realtime(*args, **kwargs)

def local_cache(namespace, key, generator, regenerate_if_none=False):
	"""A key value store for caching within a request

	:param namespace: dataent.local.cache[namespace]
	:param key: dataent.local.cache[namespace][key] used to retrieve value
...
def enqueue(*args, **kwargs):
	...
	'''
	import dataent.utils.background_jobs
	return dataent.utils.background_jobs.enqueue(*args, **kwargs)
...
def enqueue_doc(*args, **kwargs):
	...
	'''
	import dataent.utils.background_jobs
	return dataent.utils.background_jobs.enqueue_doc(*args, **kwargs)
...
def logger(module=None, with_more_info=True):
	...
	from dataent.utils.logger import get_logger
...
def get_active_domains():
	from dataent.core.doctype.domain_settings.domain_settings import get_active_domains
...
def get_version(doctype, name, limit = None, head = False, raise_err = True):
	...
	Example
	>>> dataent.get_version('User', 'foobar@gmail.com')
	>>>
	[
		{
	...
	if meta.track_changes:
		...
		from dataent.chat.util import squashify, dictify, safe_json_loads
...
def parse_json(val):
	from dataent.utils import parse_json
...
def mock(type, size = 1, locale = 'en'):
	...
	from dataent.chat.util import squashify
