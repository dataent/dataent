...
from dataent.modules import scrub, get_module_path
from dataent.utils import flt, cint, get_html_format, cstr, get_url_to_form
...
from dataent.permissions import get_role_permissions
from six import string_types, iteritems
from datetime import timedelta
from dataent.utils.file_manager import get_file
from dataent.utils import gzip_decompress

def get_report_doc(report_name):
	doc = dataent.get_doc("Report", report_name)
	doc.custom_columns = []

	if doc.report_type == 'Custom Report':
		custom_report_doc = doc
		reference_report = custom_report_doc.reference_report
		doc = dataent.get_doc("Report", reference_report)
		doc.custom_report = report_name
		doc.custom_columns = custom_report_doc.json
		doc.is_custom_report = True

	if not doc.is_permitted():
		dataent.throw(_("You don't have access to Report: {0}").format(report_name), dataent.PermissionError)

	if not dataent.has_permission(doc.ref_doctype, "report"):
		dataent.throw(_("You don't have permission to get a report on: {0}").format(doc.ref_doctype),
			dataent.PermissionError)

	if doc.disabled:
		dataent.throw(_("Report {0} is disabled").format(report_name))
...
def generate_report_result(report, filters=None, user=None):
	...
	if report.report_type == "Query Report":
		if not report.query:
			status = "error"
			dataent.msgprint(_("Must specify a Query to run"), raise_exception=True)

		if not report.query.lower().startswith("select"):
			status = "error"
			dataent.msgprint(_("Query must be a SELECT"), raise_exception=True)

		result = [list(t) for t in dataent.db.sql(report.query, filters)]
		columns = [cstr(c[0]) for c in dataent.db.get_description()]
	else:
		module = report.module or dataent.db.get_value("DocType", report.ref_doctype, "module")
		if report.is_standard == "Yes":
			...
			# The JOB
			res = dataent.get_attr(method_name)(dataent._dict(filters))
			...
			dataent.cache().hset('report_execution_time', report.name, execution_time)
	...
	return {
		...
		"status": status,
		"execution_time": dataent.cache().hget('report_execution_time', report.name) or 0
	}
...
def get_script(report_name):
	...
	return {
		...
		"execution_time": dataent.cache().hget('report_execution_time', report_name) or 0
	}
...
def run(report_name, filters=None, user=None):
	...
	if not user:
		user = dataent.session.user
	if not dataent.has_permission(report.ref_doctype, "report"):
		dataent.msgprint(_("Must have report permission to access this report."),
...
def get_prepared_report_result(report, filters, dn="", user=None):
	...
	if dn:
		# Get specified dn
		doc = dataent.get_doc("Prepared Report", dn)
	else:
		# Only look for completed prepared reports with given filters.
		doc_list = dataent.get_all("Prepared Report",
			filters={
				"status": "Completed",
		...
		if doc_list:
			# Get latest
			doc = dataent.get_doc("Prepared Report", doc_list[0])

	if doc:
		try:
			# Prepared Report data is stored in a GZip compressed JSON file
			attached_file_name = dataent.db.get_value("File", {"attached_to_doctype": doc.doctype, "attached_to_name":doc.name}, "name")
			...
			dataent.delete_doc("Prepared Report", doc.name)
			dataent.db.commit()
			doc = None
...
def export_query():
	...
	data = dataent._dict(dataent.local.form_dict)
	...
	if isinstance(data.get("report_name"), string_types):
		...
		dataent.permissions.can_export(
			dataent.get_cached_value('Report', report_name, 'ref_doctype'),
			raise_exception=True
		)
	...
	if file_format_type == "Excel":
		...
		dataent.response['filename'] = report_name + '.xlsx'
		dataent.response['filecontent'] = xlsx_file.getvalue()
...
def get_report_module_dotted_path(module, report_name):
	return dataent.local.module_app[scrub(module)] + "." + scrub(module) \
...
def get_data_for_custom_field(doctype, field):
	...
	value_map = dataent._dict(dataent.get_all(doctype,
		fields=["name", field],
		as_list=1))
...
def save_report(reference_report, report_name, columns):
	...
	docname = dataent.db.exists("Report", report_name)
	if docname:
		report = dataent.get_doc("Report", {'report_name': docname, 'is_standard': 'No', 'report_type': 'Custom Report'})
		report.update({"json": columns})
		report.save()
		dataent.msgprint(_("Report updated successfully"))

		return docname
	else:
		new_report = dataent.get_doc({
			'doctype': 'Report',
			'report_name': report_name,
			'json': columns,
		...
		dataent.msgprint(_("{0} saved successfully".format(new_report.name)))
...
def get_filtered_data(ref_doctype, columns, data, user):
	...
	shared = dataent.share.get_shared(ref_doctype, user)
	columns_dict = get_columns_dict(columns)

	role_permissions = get_role_permissions(dataent.get_meta(ref_doctype), user)
		...
		if not matched_for_doctype:
			for match_filters in filter_list:
				...
				for dt, idx in linked_doctypes.items():
					...
					if dt in match_filters and cell_value not in match_filters.get(dt) and dataent.db.exists(dt, cell_value):
						match = False
						break
	...
	for idx, col in enumerate(columns):
		...
		# dict
			...
			if "fieldname" not in col_dict:
				col_dict["fieldname"] = dataent.scrub(col_dict["label"])
...
def get_user_match_filters(doctypes, user):
	...
	for dt in doctypes:
		filter_list = dataent.desk.reportview.build_match_conditions(dt, user, False)
		if filter_list:
