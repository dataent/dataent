...
def runserverobj(method, docs=None, dt=None, dn=None, arg=None, args=None):
	...
	if dt: # not called from a doctype (from a page)
		...
		doc = dataent.get_doc(dt, dn)

	else:
		doc = dataent.get_doc(json.loads(docs))
		doc._original_modified = doc.modified
		doc.check_if_latest()

	if not doc.has_permission("read"):
		dataent.msgprint(_("Not permitted"), raise_exception = True)
	...
	if doc:
		...
		if r:
			#build output as csv
			if cint(dataent.form_dict.get('as_csv')):
				make_csv_output(r, doc.doctype)
			else:
				dataent.response['message'] = r
...
def make_csv_output(res, dt):
	...
	dataent.response['doctype'] = dt.replace(' ','')
