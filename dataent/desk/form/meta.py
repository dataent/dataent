...
import dataent, os
from dataent.model.meta import Meta
from dataent.modules import scrub, get_module_path, load_doctype_module
from dataent.utils import get_html_format
from dataent.translate import make_dict_from_messages, extract_messages_from_code
from dataent.model.utils import render_include
from dataent.build import scrub_html_template
...
def get_meta(doctype, cached=True):
	...
	if cached and not dataent.conf.developer_mode:
		meta = dataent.cache().hget("form_meta", doctype)
		if meta:
			meta = FormMeta(meta)
		else:
			meta = FormMeta(doctype)
			dataent.cache().hset("form_meta", doctype, meta.as_dict())
	else:
		meta = FormMeta(doctype)

	if dataent.local.lang != 'en':
		meta.set_translations(dataent.local.lang)
...
class FormMeta(Meta):
	...
	def add_custom_script(self):
		...
		# custom script
		custom = dataent.db.get_value("Custom Script", {"dt": self.name,
	...
	def add_search_fields(self):
		...
		for df in self.get("fields", {"fieldtype": "Link", "options":["!=", "[Select]"]}):
			if df.options:
				search_fields = dataent.get_meta(df.options).search_fields
				if search_fields:
	...
	def add_linked_document_type(self):
		for df in self.get("fields", {"fieldtype": "Link"}):
			if df.options:
				try:
					df.linked_document_type = dataent.get_meta(df.options).document_type
				except dataent.DoesNotExistError:
					# edge case where options="[Select]"
					pass

	def load_print_formats(self):
		print_formats = dataent.db.sql("""select * FROM `tabPrint Format`
	...
	def load_workflows(self):
		...
		if workflow_name and dataent.db.exists("Workflow", workflow_name):
			workflow = dataent.get_doc("Workflow", workflow_name)
			workflow_docs.append(workflow)

			for d in workflow.get("states"):
				workflow_docs.append(dataent.get_doc("Workflow State", d.state))
