...
from dataent.boot import get_allowed_pages, get_allowed_reports
from dataent.desk.doctype.desktop_icon.desktop_icon import set_hidden, clear_desktop_icons_cache
...
def hide_module(module):
	set_hidden(module, dataent.session.user, 1)
...
def build_standard_config(module, doctype_info):
	...
	if not dataent.db.get_value("Module Def", module):
		dataent.throw(_("Module Not Found"))
...
def apply_permissions(data):
	default_country = dataent.db.get_default("country")

	user = dataent.get_user()
...
def get_last_modified(doctype):
	def _get():
		try:
			last_modified = dataent.get_all(doctype, fields=["max(modified)"], as_list=True, limit_page_length=1)[0][0]
	...
	last_modified = dataent.cache().hget("last_modified", doctype, _get)
...
def get_report_list(module, is_standard="No"):
	...
	reports =  dataent.get_list("Report", fields=["name", "ref_doctype", "report_type"], filters=
