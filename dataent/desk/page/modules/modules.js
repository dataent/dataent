dataent.pages['modules'].on_page_load = function(wrapper) {
	...
	dataent.modules_page = page;
	dataent.module_links = {};
	page.section_data = {};
	...
	page.add_menu_item(__('Set Desktop Icons'), function() {
		dataent.dataent_toolbar.modules_select
			.show(dataent.session.user);
	});

	if(dataent.user.has_role('System Manager')) {
		page.add_menu_item(__('Install Apps'), function() {
			dataent.set_route("applications");
		});
	}

	page.get_page_modules = () => {
		return dataent.get_desktop_icons(true)
	...
	page.main.on("click", '.module-section-link[data-type="help"]', function() {
		dataent.help.show_video($(this).attr("data-youtube-id"));
		return false;
	});
	...
	var show_section = function(module_name) {
		...
		if(module_name in page.section_data) {
			...
			page.main.empty();
			return dataent.call({
				method: "dataent.desk.moduleview.get",
				args: {
					module: module_name
				},
				callback: function(r) {
					var m = dataent.get_module(module_name);
	...
	var render_section = function(m) {
		...
		page.main.html(dataent.render_template('modules_section', m));

		// if(dataent.utils.is_xs() || dataent.utils.is_sm()) {
		...
		dataent.app.update_notification_count_in_modules();
	};

	var process_data = function(module_name, data) {
		dataent.module_links[module_name] = [];
		data.forEach(function(section) {
			section.items.forEach(function(item) {
				...
				if(item.type==="doctype") {
					...
					dataent.module_links[module_name].push(item.name);
				}
				if(!item.route) {
					if(item.link) {
						...
						if(dataent.model.is_single(item.doctype)) {
							item.route = 'Form/' + item.doctype;
						} else {
							if (item.filters) {
								dataent.route_options=item.filters;
							}
				...
				(item.doctype && dataent.model.can_read(item.doctype))) {
					item.shown = true;
				}
			});
		});
...
dataent.pages['modules'].on_page_show = function(wrapper) {
	let route = dataent.get_route();
	let modules = dataent.modules_page.get_page_modules().map(d => d.module_name);
	$("body").attr("data-sidebar", 1);
	if(route.length > 1) {
		// activate section based on route
		let module_name = route[1];
		if(modules.includes(module_name)) {
			dataent.modules_page.activate_link(
				dataent.modules_page.sidebar.find('.module-link[data-name="'+ module_name +'"]'));
		} else {
			dataent.throw(__(`Module ${module_name} not found.`));
		}
	} else if(dataent.modules_page.last_link) {
		// open last link
		dataent.set_route('modules', dataent.modules_page.last_link.attr('data-name'));
	} else {
		// first time, open the first page
		dataent.modules_page.activate_link(dataent.modules_page.sidebar.find('.module-link:first'));
	}
};
