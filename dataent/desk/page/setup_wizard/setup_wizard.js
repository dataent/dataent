dataent.provide("dataent.setup");
dataent.provide("dataent.setup.events");
...
dataent.setup = {
	...
	on: function(event, fn) {
		if(!dataent.setup.events[event]) {
			dataent.setup.events[event] = [];
		}
		dataent.setup.events[event].push(fn);
	},
	add_slide: function(slide) {
		dataent.setup.slides.push(slide);
	},

	remove_slide: function(slide_name){
		dataent.setup.slides = dataent.setup.slides.filter((slide) => slide.name !== slide_name);
	},

	run_event: function(event) {
		$.each(dataent.setup.events[event] || [], function(i, fn) {
			fn();
		});
	}
}

dataent.pages['setup-wizard'].on_page_load = function(wrapper) {
	let requires = (dataent.boot.setup_wizard_requires || []);


	dataent.require(requires, function() {
		dataent.call({
			method: "dataent.desk.page.setup_wizard.setup_wizard.load_languages",
			freeze: true,
			callback: function(r) {
				dataent.setup.data.lang = r.message;

				dataent.setup.run_event("before_load");
				var wizard_settings = {
					parent: wrapper,
					slides: dataent.setup.slides,
					slide_class: dataent.setup.SetupWizardSlide,
					unidirectional: 1,
					before_load: ($footer) => {
				...
				dataent.wizard = new dataent.setup.SetupWizard(wizard_settings);
				dataent.setup.run_event("after_load");
				// dataent.wizard.values = test_values_edu;
				let route = dataent.get_route();
				if(route) {
					dataent.wizard.show_slide(route[1]);
				}
			}
		});
	});
...
dataent.pages['setup-wizard'].on_page_show = function(wrapper) {
	if(dataent.get_route()[1]) {
		dataent.wizard && dataent.wizard.show_slide(dataent.get_route()[1]);
	}
};

dataent.setup.on("before_load", function() {
	// load slides
	dataent.setup.slides_settings.forEach((s) => {
		if(!(s.name==='user' && dataent.boot.developer_mode)) {
			// if not user slide with developer mode
			dataent.setup.add_slide(s);
		}
	});
});

dataent.setup.SetupWizard = class SetupWizard extends dataent.ui.Slides {
	constructor(args = {}) {
		super(args);
		$.extend(this, args);
		...
		dataent.set_route("setup-wizard/0");
	}
	...
	handle_enter_press(e) {
		if (e.which === dataent.ui.keyCode.ENTER) {
			var $target = $(e.target);
	...
	before_show_slide() {
		if(!this.welcomed) {
			dataent.set_route(this.page_name);
			return false;
		}
		return true;
	}
	...
	show_slide(id) {
		...
		super.show_slide(id);
		dataent.set_route(this.page_name, id + "");
	}
	...
	refresh_slides() {
		...
		this.update_values();
		dataent.setup.slides = [];
		dataent.setup.run_event("before_load");

		dataent.setup.slides = this.get_setup_slides_filtered_by_domain();

		this.slides = dataent.setup.slides;
		dataent.setup.run_event("after_load");
	...
	action_on_complete() {
		...
		return dataent.call({
			method: "dataent.desk.page.setup_wizard.setup_wizard.setup_complete",
	...
	post_setup_success() {
		...
		if(dataent.setup.welcome_page) {
			localStorage.setItem("session_last_route", dataent.setup.welcome_page);
		}
		setTimeout(function() {
			// Reload
	...
	listen_for_setup_stages() {
		dataent.realtime.on("setup_task", (data) => {
	...
	get_setup_slides_filtered_by_domain() {
		var filtered_slides = [];
		dataent.setup.slides.forEach(function(slide) {
			if(dataent.setup.domains) {
				let active_domains = dataent.setup.domains;
				if (!slide.domains ||
	...
	show_working_state() {
		...
		this.$working_state = this.get_message(
			...
			__("Starting Dataent ...")).appendTo(this.parent);
	...
	attach_abort_button() {
		...
		this.$abort_btn.on('click', () => {
			...
			this.container.show();
			dataent.set_route(this.page_name, this.slides.length - 1);
		});
...
dataent.setup.SetupWizardSlide = class SetupWizardSlide extends dataent.ui.Slide {
	...
	set_init_values () {
		var me = this;
		// set values from dataent.setup.values
		if(dataent.wizard.values && this.fields) {
			this.fields.forEach(function(f) {
				var value = dataent.wizard.values[f.fieldname];
				if(value) {
...
// Dataent slides settings
// ======================================================

dataent.setup.slides_settings = [
	{
		...
		onload: function(slide) {
			...
			language_field.set_input(dataent.setup.data.default_language || "English");

			if (!dataent.setup._from_load_messages) {
				language_field.$input.trigger("change");
			}
			delete dataent.setup._from_load_messages;
			moment.locale("en");
		},

		setup_fields: function(slide) {
			dataent.setup.utils.setup_language_field(slide);
			dataent.setup.utils.bind_language_events(slide);
		},
	},

	{
		...
		onload: function(slide) {
			if(dataent.setup.data.regional_data) {
				this.setup_fields(slide);
			} else {
				dataent.setup.utils.load_regional_data(slide, this.setup_fields);
			}
		},

		setup_fields: function(slide) {
			dataent.setup.utils.setup_region_fields(slide);
			dataent.setup.utils.bind_region_events(slide);
		}
	},

	{
		...
		onload: function(slide) {
			if(dataent.session.user!=="Administrator") {
				...
				if(dataent.boot.user.first_name || dataent.boot.user.last_name) {
					slide.form.fields_dict.full_name.set_input(
						[dataent.boot.user.first_name, dataent.boot.user.last_name].join(' ').trim());
				}

				var user_image = dataent.get_cookie("user_image");
				...
				dataent.setup.utils.load_user_details(slide, this.setup_fields);
			}
		},

		setup_fields: function(slide) {
			if(dataent.setup.data.full_name) {
				slide.form.fields_dict.full_name.set_input(dataent.setup.data.full_name);
			}
			if(dataent.setup.data.email) {
				let email = dataent.setup.data.email;
				slide.form.fields_dict.email.set_input(email);
				if (dataent.get_gravatar(email, 200)) {
					var $attach_user_image = slide.form.fields_dict.attach_user_image.$wrapper;
					$attach_user_image.find(".missing-image").toggle(false);
					$attach_user_image.find("img").attr("src", dataent.get_gravatar(email, 200));
					$attach_user_image.find(".img-container").toggle(true);
				}
			}
		},
...
dataent.setup.utils = {
	load_regional_data: function(slide, callback) {
		dataent.call({
			method:"dataent.geo.country_info.get_country_timezone_info",
			callback: function(data) {
				dataent.setup.data.regional_data = data.message;
				callback(slide);
			}
		});
	},
	...
	load_user_details: function(slide, callback) {
		dataent.call({
			method: "dataent.desk.page.setup_wizard.setup_wizard.load_user_details",
			freeze: true,
			callback: function(r) {
				dataent.setup.data.full_name = r.message.full_name;
				dataent.setup.data.email = r.message.email;
				callback(slide);
			}
		})
	},
	...
	setup_language_field: function(slide) {
		var language_field = slide.get_field("language");
		language_field.df.options = dataent.setup.data.lang.languages;
	...
	setup_region_fields: function(slide) {
		...
		*/
		var data = dataent.setup.data.regional_data;
		...
		slide.get_input("currency").empty()
			.add_options(dataent.utils.unique([""].concat($.map(data.country_info,
		...
		if(dataent.wizard.values.country) {
			country_field.set_input(dataent.wizard.values.country);
		} else if (data.default_country) {
			country_field.set_input(data.default_country);
		}

		if(dataent.wizard.values.currency) {
			slide.get_field("currency").set_input(dataent.wizard.values.currency);
		}

		if(dataent.wizard.values.timezone) {
			slide.get_field("timezone").set_input(dataent.wizard.values.timezone);
		}
	...
	bind_language_events: function(slide) {
		slide.get_input("language").unbind("change").on("change", function() {
			...
			slide.language_call_timeout = setTimeout (() => {
				...
				dataent._messages = {};
				dataent.call({
					method: "dataent.desk.page.setup_wizard.setup_wizard.load_messages",
					freeze: true,
					args: {
						language: lang
					},
					callback: function(r) {
						dataent.setup._from_load_messages = true;
						dataent.wizard.refresh_slides();
					}
				});
			}, 500);
		});
	...
	bind_region_events: function(slide) {
		...
		*/
		slide.get_input("country").on("change", function() {
			...
			dataent.boot.sysdefaults.date_format = (data.country_info[country].date_format
				|| "dd-mm-yyyy");
		});
		...
		slide.get_input("currency").on("change", function() {
			...
			if (!currency) return;
			dataent.model.with_doc("Currency", currency, function() {
				dataent.provide("locals.:Currency." + currency);
				var currency_doc = dataent.model.get_doc("Currency", currency);
				...
				dataent.boot.sysdefaults.number_format = number_format;
