...
import dataent, json
from dataent.utils import cstr, unique, cint
...
def sanitize_searchfield(searchfield):
	...
	def _raise_exception(searchfield):
		dataent.throw(_('Invalid Search Field {0}').format(searchfield), dataent.DataError)
...
def search_link(doctype, txt, query=None, filters=None, page_length=20, searchfield=None, reference_doctype=None, ignore_user_permissions=False):
	...
	dataent.response['results'] = build_for_autosuggest(dataent.response["values"])
	del dataent.response["values"]
...
def search_widget(doctype, txt, query=None, searchfield=None, start=0,
	...
	standard_queries = dataent.get_hooks().standard_queries or {}

	if query and query.split()[0].lower()!="select":
		# by method
		dataent.response["values"] = dataent.call(query, doctype, txt,
		...
		if query:
			dataent.throw(_("This query style is discontinued"))
			# custom query
			# dataent.response["values"] = dataent.db.sql(scrub_custom_query(query, searchfield, txt))
		else:
			...
			formatted_fields.append("""locate("{_txt}", `tab{doctype}`.`name`) as `_relevance`""".format(
				_txt=dataent.db.escape((txt or "").replace("%", "")), doctype=dataent.db.escape(doctype)))


			# In order_by, `idx` gets second priority, because it stores link count
			from dataent.model.db_query import get_order_by
			...
			values = dataent.get_list(doctype,
				filters=filters,
			...
			if as_dict:
				for r in values:
					r.pop("_relevance")
				dataent.response["values"] = values
			else:
				dataent.response["values"] = [r[:-1] for r in values]
