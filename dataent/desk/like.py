...
def _toggle_like(doctype, name, add, user=None):
	...
	try:
		liked_by = dataent.db.get_value(doctype, name, "_liked_by")

		# CHANGED: Allow someone to like their own documents as it also works as a bookmark
		# if owner==dataent.session.user and add=="Yes":
		# 	dataent.throw(_("You cannot like something that you created"))
		...
		dataent.db.set_value(doctype, name, "_liked_by", json.dumps(liked_by), update_modified=False)
...
def remove_like(doctype, name):
	...
	# remove Comment
	dataent.delete_doc("Communication", [c.name for c in dataent.get_all("Communication",
		filters={
