...
def get_form_params():
	...
	for field in fields:
		...
		df = dataent.get_meta(parenttype).get_field(fieldname)
...
def compress(data, args = {}):
	...
	from dataent.desk.query_report import add_total_row
	...
	if args.get("add_total_row"):
		meta = dataent.get_meta(args.doctype)
...
def save_report():
	...
	if dataent.db.exists('Report', data['name']):
		d = dataent.get_doc('Report', data['name'])
	else:
		d = dataent.new_doc('Report')
		d.report_name = data['name']
		d.ref_doctype = data['doctype']

	d.report_type = "Report Builder"
	d.json = data['json']
	dataent.get_doc(d).save()
	dataent.msgprint(_("{0} is saved").format(d.name))
	return d.name
...
def export_query():
	...
	dataent.permissions.can_export(doctype, raise_exception=True)

	if 'selected_items' in form_params:
		si = json.loads(dataent.form_dict.get('selected_items'))
	...
	if file_format_type == "CSV":
		...
		# convert to csv
		import csv
		from dataent.utils.xlsxutils import handle_html

		f = StringIO()
		writer = csv.writer(f)
		for r in data:
			# encode only unicode type strings and not int, floats etc.
			writer.writerow([handle_html(dataent.as_unicode(v)).encode('utf-8') \
				if isinstance(v, string_types) else v for v in r])

		f.seek(0)
		dataent.response['result'] = text_type(f.read(), 'utf-8')
		dataent.response['type'] = 'csv'
		dataent.response['doctype'] = doctype
		...
		dataent.response['filename'] = doctype + '.xlsx'
...
def delete_items():
	...
	il = sorted(json.loads(dataent.form_dict.get('items')), reverse=True)
	doctype = dataent.form_dict.get('doctype')

	failed = []

	for i, d in enumerate(il):
		try:
			dataent.delete_doc(doctype, d)
			if len(il) >= 5:
				dataent.publish_realtime("progress",
...
def get_sidebar_stats(stats, doctype, filters=[]):
	cat_tags = dataent.db.sql("""select tag.parent as category, tag.tag_name as tag
...
def get_filter_dashboard_data(stats, doctype, filters=[]):
	...
	for tag in tags:
		...
		if tag["type"] not in ['Date', 'Datetime']:
			tagcount = dataent.get_list(doctype,
		...
		if tag["type"] not in ['Check','Select','Date','Datetime','Int',
			...
			if stats[tag["name"]]:
				data =["No Data", dataent.get_list(doctype,
					fields=[tag["name"], "count(*)"],
