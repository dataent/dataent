...
def get_parent_dt(dt):
	pdt = ''
	if dataent.db.sql('select name from `tabDocType` where istable=1 and name=%s', dt):
		import dataent.model.meta
		return dataent.model.meta.get_parent_dt(dt)
	return pdt
...
def get_sql_meta(tl):
	...
	for dt in tl:
		...
		res = dataent.db.sql("select fieldname, label, fieldtype, options, width \
...
def build_description_simple():
	...
	for m in dataent.db.get_description():
		colnames.append(m[0])
...
def build_description_standard(meta, tl):
	...
	desc = dataent.db.get_description()
...
def runquery(q='', ret=0, from_export=0):
	...
	formatted = cint(dataent.form_dict.get('formatted'))

	# CASE A: Simple Query
	# --------------------
	if dataent.form_dict.get('simple_query') or dataent.form_dict.get('is_simple'):
		if not q: q = dataent.form_dict.get('simple_query') or dataent.form_dict.get('query')
		if q.split()[0].lower() != 'select':
			raise Exception('Query must be a SELECT')

		as_dict = cint(dataent.form_dict.get('as_dict'))
		res = dataent.db.sql(q, as_dict = as_dict, as_list = not as_dict, formatted=formatted)
	...
	# -----------------------
		...
		if not q: q = dataent.form_dict.get('query')
		...
		q = q.replace('__user', dataent.session.user)
		q = q.replace('__today', dataent.utils.nowdate())

		res = dataent.db.sql(q, as_list=1, formatted=formatted)
	...
	qm = dataent.form_dict.get('query_max') or ''
	if qm and qm.strip():
		if qm.split()[0].lower() != 'select':
			raise Exception('Query (Max) must be a SELECT')
		if not dataent.form_dict.get('simple_query'):
			qm = add_match_conditions(qm, tl)

		out['n_values'] = dataent.utils.cint(dataent.db.sql(qm)[0][0])
...
def runquery_csv():
	...
	q = dataent.form_dict.get('query')

	rep_name = dataent.form_dict.get('report_name')
	if not dataent.form_dict.get('simple_query'):
...
def add_limit_to_query(query, args):
	...
	if args.get('limit_page_length'):
		...
		import dataent.utils
		args['limit_start'] = dataent.utils.cint(args.get('limit_start'))
		args['limit_page_length'] = dataent.utils.cint(args.get('limit_page_length'))
