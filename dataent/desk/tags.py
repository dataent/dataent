...
def check_user_tags(dt):
	...
	try:
		dataent.db.sql("select `_user_tags` from `tab%s` limit 1" % dt)
...
def get_tagged_docs(doctype, tag):
	dataent.has_permission(doctype, throw=True)

	return dataent.db.sql("""SELECT name
		FROM `tab{0}`
...
def get_tags(doctype, txt, cat_tags):
	...
	try:
		for _user_tags in dataent.db.sql_list("""select DISTINCT `_user_tags`
			from `tab{0}`
			where _user_tags like '%{1}%'
			limit 50""".format(dataent.db.escape(doctype), dataent.db.escape(txt))):
...
class DocTags:
	...
	def get_tag_fields(self):
		...
		return dataent.db.get_value('DocType', self.dt, 'tag_fields')

	def get_tags(self, dn):
		"""returns tag for a particular item"""
		return (dataent.db.get_value(self.dt, dn, '_user_tags', ignore=1) or '').strip()
	...
	def update(self, dn, tl):
		...
		if not tl:
			...
			tags = ',' + ','.join(tl)
		try:
			dataent.db.sql("update `tab%s` set _user_tags=%s where name=%s" % \
				(self.dt,'%s','%s'), (tags , dn))
			doc= dataent.get_doc(self.dt, dn)
