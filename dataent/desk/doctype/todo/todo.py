...
class ToDo(Document):
	def validate(self):
		...
		if self.is_new():
			...
			if self.assigned_by == self.owner:
				assignment_message = dataent._("{0} self assigned this task: {1}").format(get_fullname(self.assigned_by), self.description)
			else:
				assignment_message = dataent._("{0} assigned {1}: {2}").format(get_fullname(self.assigned_by), get_fullname(self.owner), self.description)
			...
			if self.get_db_value("status") != self.status:
				self._assignment = {
					"text": dataent._("Assignment closed by {0}".format(get_fullname(dataent.session.user))),
	...
	def on_trash(self):
		...
		dataent.db.sql("""update `tabCommunication` set link_doctype=null, link_name=null
	...
	def add_assign_comment(self, text, comment_type):
		...
		dataent.get_doc(self.reference_type, self.reference_name).add_comment(comment_type, text)
	...
	def update_in_reference(self):
		...
		try:
			assignments = [d[0] for d in dataent.get_all("ToDo",
				filters={
			...
			assignments.reverse()
			dataent.db.set_value(self.reference_type, self.reference_name,
				"_assign", json.dumps(assignments), update_modified=False)

		except Exception as e:
			if e.args[0] == 1146 and dataent.flags.in_install:
				# no table
				return
...
def on_doctype_update():
	dataent.db.add_index("ToDo", ["reference_type", "reference_name"])
