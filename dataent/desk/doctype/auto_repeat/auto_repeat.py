...
from dataent.desk.form import assign_to
from dataent.utils.jinja import validate_template
from dateutil.relativedelta import relativedelta
from dataent.utils.user import get_system_managers
from dataent.utils import cstr, getdate, split_emails, add_days, today, get_last_day, get_first_day
from dataent.model.document import Document
from dataent.core.doctype.communication.email import make
...
class AutoRepeat(Document):
	...
	def validate_reference_doctype(self):
		if not dataent.get_meta(self.reference_doctype).has_field('auto_repeat'):
			dataent.throw(_("Add custom field Auto Repeat in the doctype {0}").format(self.reference_doctype))
		...
		if self.repeat_on_day and self.next_schedule_date:
			...
			if next_date.day != self.repeat_on_day:
				...
				if self.repeat_on_day < lastday:
					...
					dataent.throw(_("Next Date's day and Repeat on Day of Month must be equal"))
			...
			if self.recipients:
				...
				for email in email_list:
					if not validate_email_add(email):
						dataent.throw(_("{0} is an invalid email address in 'Recipients'").format(email))
			else:
	...
	def update_auto_repeat_id(self):
		dataent.db.set_value(self.reference_doctype, self.reference_document, "auto_repeat", self.name)
	...
	def link_party(self):
		reference = dataent.get_meta(self.reference_doctype)
		for field in reference.fields:
			if field.options in ['Customer', 'Supplier', 'Employee']:
				self.reference_party_doctype = field.options
				self.reference_party = dataent.db.get_value(self.reference_doctype, self.reference_document, field.fieldname)
				break
...
def create_repeated_entries(data):
	...
	while schedule_date <= getdate(today()) and not dataent.db.get_value('Auto Repeat', data.name, 'disabled'):
		create_documents(data, schedule_date)
		schedule_date = get_next_schedule_date(schedule_date, data.frequency, data.repeat_on_day)

		if schedule_date and not dataent.db.get_value('Auto Repeat', data.name, 'disabled'):
			dataent.db.set_value('Auto Repeat', data.name, 'next_schedule_date', schedule_date)
			dataent.db.commit()

def get_auto_repeat_entries(date):
	return dataent.db.sql(""" select * from `tabAuto Repeat`
...
def create_documents(data, schedule_date):
	try:
		...
		dataent.db.rollback()
		dataent.db.begin()
		dataent.log_error(dataent.get_traceback(), _("Recurring document creation failure"))
		disable_auto_repeat(data)
		dataent.db.commit()
		if data.reference_document and not dataent.flags.in_test:
			notify_error_to_user(data)

def disable_auto_repeat(data):
	auto_repeat = dataent.get_doc('Auto Repeat', data.name)
...
def notify_error_to_user(data):
	...
	if party_type:
		party = dataent.db.get_value(data.reference_doctype, data.reference_document, party_type)
...
def send_notification(new_rv, auto_repeat_doc, print_format='Standard'):
	...
	if not auto_repeat_doc.subject:
		...
		subject = dataent.render_template(auto_repeat_doc.subject, {'doc': new_rv})

	if not auto_repeat_doc.message:
		message = _("Please find attached {0} #{1}").format(new_rv.doctype, new_rv.name)
	elif "{" in auto_repeat_doc.message:
		message = dataent.render_template(auto_repeat_doc.message, {'doc': new_rv})

	attachments = [dataent.attach_print(new_rv.doctype, new_rv.name,
...
def notify_errors(doc, doctype, party, owner, name):
	...
	dataent.sendmail(recipients + [dataent.db.get_value("User", owner, "email")],
					subject=_("[Urgent] Error while creating recurring %s for %s" % (doctype, doc)),
					message=dataent.get_template("templates/emails/recurring_document_failed.html").render({
						"type": _(doctype),
						"name": doc,
						"party": party or "",
						"auto_repeat": name
	...
	try:
		...
		dataent.log_error(dataent.get_traceback(), _("Recurring Documents Failed"))
...
def make_auto_repeat(doctype, docname):
	doc = dataent.new_doc('Auto Repeat')

	reference_doc = dataent.get_doc(doctype, docname)
...
def auto_repeat_doctype_query(doctype, txt, searchfield, start, page_len, filters):
	return dataent.db.sql("""select parent from `tabDocField`
...
def get_contacts(reference_doctype, reference_name):
	docfields = dataent.get_meta(reference_doctype).fields
	...
	if contact_fields:
		...
		for contact_field in contact_fields:
			contacts.append(dataent.db.get_value(reference_doctype, reference_name, contact_field))
	else:
		return []
	...
	if contacts:
		...
		for contact in contacts:
			emails.append(dataent.db.get_value("Contact", contact, "email_id"))
...
def update_reference(docname, reference):
	try:
		dataent.db.set_value("Auto Repeat", docname, "reference_document", reference)
		return "success"
...
def generate_message_preview(reference_dt, reference_doc, message=None, subject=None):
	doc = dataent.get_doc(reference_dt, reference_doc)
	subject_preview = _("Please add a subject to your email")
	msg_preview = dataent.render_template(message, {'doc': doc})
	if subject:
		subject_preview = dataent.render_template(subject, {'doc': doc})
