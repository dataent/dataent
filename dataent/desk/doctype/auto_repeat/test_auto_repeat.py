...
from dataent.desk.doctype.auto_repeat.auto_repeat import get_auto_repeat_entries, create_repeated_entries, disable_auto_repeat
from dataent.utils import today, add_days, getdate, add_months
...
class TestAutoRepeat(unittest.TestCase):
	def setUp(self):
		if not dataent.db.sql('select name from `tabCustom Field` where name="auto_repeat"'):
			add_custom_fields()
	...
	def monthly_auto_repeat(self, doctype, docname, start_date, end_date):
		...
		for data in get_auto_repeat_entries(today()):
			create_repeated_entries(data)
		docnames = dataent.get_all(doc.reference_doctype, {'auto_repeat': doc.name})
	...
	def test_notification_is_attached(self):
		...
		linked_comm = dataent.db.exists("Communication", dict(reference_doctype="ToDo", reference_name=new_todo))
...
def make_auto_repeat(**args):
	...
	doc = dataent.get_doc({
		...
		'reference_document': args.reference_document or dataent.db.get_value('ToDo', {'docstatus': 1}, 'name'),
