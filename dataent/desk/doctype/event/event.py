...
from dataent.utils import (getdate, cint, add_months, date_diff, add_days,
	nowdate, get_datetime_str, cstr, get_datetime, now_datetime, format_datetime)
from dataent.model.document import Document
from dataent.utils.user import get_enabled_system_users
...
class Event(Document):
	def validate(self):
		...
		if self.starts_on and self.ends_on and get_datetime(self.starts_on) > get_datetime(self.ends_on):
			dataent.msgprint(dataent._("Event end must be after start"), raise_exception=True)
		...
		if getdate(self.starts_on) != getdate(self.ends_on) and self.repeat_on == "Every Day":
			dataent.msgprint(dataent._("Every day events should finish on the same day."), raise_exception=True)

	def on_update(self):
		self.sync_communication()

	def on_trash(self):
		communications = dataent.get_all("Communication", dict(reference_doctype=self.doctype, reference_name=self.name))
		if communications:
			for communication in communications:
				dataent.get_doc("Communication", communication.name).delete()

	def sync_communication(self):
		if self.event_participants:
			for participant in self.event_participants:
				communication_name = dataent.db.get_value("Communication", dict(reference_doctype=self.doctype, reference_name=self.name, timeline_doctype=participant.reference_doctype, timeline_name=participant.reference_docname), "name")
				if communication_name:
					communication = dataent.get_doc("Communication", communication_name)
					self.update_communication(participant, communication)
				else:
					meta = dataent.get_meta(participant.reference_doctype)
...
def delete_communication(event, reference_doctype, reference_docname):
	deleted_participant = dataent.get_doc(reference_doctype, reference_docname)
	if isinstance(event, string_types):
		event = json.loads(event)

	communication_name = dataent.db.get_value("Communication", dict(reference_doctype=event["doctype"], reference_name=event["name"], timeline_doctype=deleted_participant.reference_doctype, timeline_name=deleted_participant.reference_docname), "name")
	if communication_name:
		deletion = dataent.get_doc("Communication", communication_name).delete()
...
def get_permission_query_conditions(user):
	...
	return """(tabEvent.event_type='Public' or tabEvent.owner='%(user)s')""" % {
			"user": dataent.db.escape(user),
			"roles": "', '".join([dataent.db.escape(r) for r in dataent.get_roles(user)])
		}
...
def send_event_digest():
	...
	for user in get_enabled_system_users():
		...
		if events:
			dataent.set_user_lang(user.name, user.language)
			...
			dataent.sendmail(
				...
				subject=dataent._("Upcoming Events for Today"),
				template="upcoming_events",
				args={
					'events': events,
				},
				header=[dataent._("Events in Today's Calendar"), 'blue']
			)
...
def get_events(start, end, user=None, for_reminder=False, filters=None):
	...
	events = dataent.db.sql("""select name, subject, description, color,
		...
		order by starts_on""".format(
			...
			roles=", ".join('"{}"'.format(dataent.db.escape(r)) for r in roles)
		), {
			"start": start,
			"end": end,
			"user": user,
...
def delete_events(ref_type, ref_name, delete_event=False):
	participations = dataent.get_all("Event Participants", filters={"reference_doctype": ref_type, "reference_docname": ref_name,
		"parenttype": "Event"}, fields=["parent", "name"])

	if participations:
		for participation in participations:
			if delete_event:
				dataent.delete_doc("Event", participation.parent, for_reload=True)
			else:
				total_participants = dataent.get_all("Event Participants", filters={"parenttype": "Event", "parent": participation.parent})

				if len(total_participants) <= 1:
					dataent.db.sql("DELETE FROM `tabEvent` WHERE `name` = %(name)s", {'name': participation.parent})

				dataent.db.sql("DELETE FROM `tabEvent Participants ` WHERE `name` = %(name)s", {'name': participation.name})
