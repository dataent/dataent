...
import dataent
from dataent.utils import time_diff_in_seconds, now, now_datetime, DATETIME_FORMAT
...
def get_notifications():
	if (dataent.flags.in_install or
		not dataent.db.get_single_value('System Settings', 'setup_complete')):
		return {
			"open_count_doctype": {},
			"open_count_module": {},
			"open_count_other": {},
	...
	for name in groups:
		count = cache.hget("notification_count:" + name, dataent.session.user)
...
def get_new_messages():
	last_update = dataent.cache().hget("notifications_last_update", dataent.session.user)
	now_timestamp = now()
	dataent.cache().hset("notifications_last_update", dataent.session.user, now_timestamp)
	...
	return dataent.db.sql("""select sender_full_name, content
		from `tabCommunication`
			...
			and creation > %s
			order by creation desc""", (dataent.session.user, last_update), as_dict=1)
	...
	for m in notification_map:
		try:
			if m in notification_count:
				...
				dataent.cache().hset("notification_count:" + m, dataent.session.user, open_count[m])
			...
			if d in notification_count:
				...
				try:
					if isinstance(condition, dict):
						result = len(dataent.get_list(d, fields=["name"],
					...
					dataent.cache().hset("notification_count:" + d, dataent.session.user, result)
	...
	for doctype in config.targets:
		if doctype in can_read:
			if doctype in notification_percent:
				...
				try:
					if isinstance(condition, dict):
						doc_list = dataent.get_list(doctype, fields=["name", target_field, value_field],
...
def clear_notifications(user=None):
	...
	dataent.publish_realtime('clear_notifications')

def delete_notification_count_for(doctype):
	dataent.cache().delete_key("notification_count:" + doctype)
	dataent.publish_realtime('clear_notifications')
...
def get_notification_info_for_boot():
	...
	conditions = {}
	module_doctypes = {}
	doctype_info = dict(dataent.db.sql("""select name, module from tabDocType"""))
...
def get_open_count(doctype, name, items=[]):
	...
	if dataent.flags.in_migrate or dataent.flags.in_install:
		return {
			'count': []
		}

	dataent.has_permission(doc=dataent.get_doc(doctype, name), throw=True)
	...
	for d in items:
		...
		if filters:
			...
			total = len(dataent.get_all(d, fields='name',
	...
	module = dataent.get_meta_module(doctype)
