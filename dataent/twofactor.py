...
from dataent.utils import get_url, get_datetime, time_diff_in_seconds
...
def toggle_two_factor_auth(state, roles=[]):
	...
	for role in roles:
		role = dataent.get_doc('Role', {'role_name': role})
		role.two_factor_auth = state
		role.save(ignore_permissions=True)

def two_factor_is_enabled(user=None):
	'''Returns True if 2FA is enabled.'''
	enabled = int(dataent.db.get_value('System Settings', None, 'enable_two_factor_auth') or 0)
	if enabled:
		bypass_two_factor_auth = int(dataent.db.get_value('System Settings', None, 'bypass_2fa_for_retricted_ip_users') or 0)
		if bypass_two_factor_auth:
			...
			if restrict_ip_list:
				for ip in restrict_ip_list:
					if dataent.local.request_ip.startswith(ip):
						enabled = False
						break
...
def cache_2fa_data(user, token, otp_secret, tmp_id):
	...
	if verification_method in ['SMS', 'Email']:
		expiry_time = 300
		dataent.cache().set(tmp_id + '_token', token)
		dataent.cache().expire(tmp_id + '_token', expiry_time)
	else:
		expiry_time = 180
	for k, v in iteritems({'_usr': user, '_pwd': pwd, '_otp_secret': otp_secret}):
		dataent.cache().set("{0}{1}".format(tmp_id, k), v)
		dataent.cache().expire("{0}{1}".format(tmp_id, k), expiry_time)
...
def get_otpsecret_for_(user):
	...
	otp_secret = dataent.db.get_default(user + '_otpsecret')
	if not otp_secret:
		otp_secret = b32encode(os.urandom(10)).decode('utf-8')
		dataent.db.set_default(user + '_otpsecret', otp_secret)
		dataent.db.commit()
	return otp_secret

def get_verification_method():
	return dataent.db.get_value('System Settings', None, 'two_factor_method')

def confirm_otp_token(login_manager, otp=None, tmp_id=None):
	'''Confirm otp matches.'''
	if not otp:
		otp = dataent.form_dict.get('otp')
	if not otp:
	...
	if not tmp_id:
		tmp_id = dataent.form_dict.get('tmp_id')
	hotp_token = dataent.cache().get(tmp_id + '_token')
	otp_secret = dataent.cache().get(tmp_id + '_otp_secret')
	if not otp_secret:
	...
	if totp.verify(otp):
		...
		if not dataent.db.get_default(login_manager.user + '_otplogin'):
			dataent.db.set_default(login_manager.user + '_otplogin', 1)
...
def get_verification_obj(user, token, otp_secret):
	...
	if verification_method == 'SMS':
		...
		if not dataent.db.get_default(user + '_otplogin'):
...
def process_2fa_for_sms(user, token, otp_secret):
	...
	phone = dataent.db.get_value('User', user, ['phone', 'mobile_no'], as_dict=1)
...
def process_2fa_for_otp_app(user, otp_secret, otp_issuer):
	...
	if dataent.db.get_default(user + '_otplogin'):
...
def process_2fa_for_email(user, token, otp_secret, otp_issuer, method='Email'):
	...
	status = True
	prompt = ''
	if method == 'OTP App' and not dataent.db.get_default(user + '_otplogin'):
...
def get_email_subject_for_2fa(kwargs_dict):
	...
	subject_template = _('Login Verification Code from {}').format(dataent.db.get_value('System Settings', 'System Settings', 'otp_issuer_name'))
...
def get_email_subject_for_qr_code(kwargs_dict):
	...
	subject_template = _('One Time Password (OTP) Registration Code from {}').format(dataent.db.get_value('System Settings', 'System Settings', 'otp_issuer_name'))
	subject = render_string_template(subject_template, kwargs_dict)
	return subject
...
def get_link_for_qrcode(user, totp_uri):
	...
	lifespan = int(dataent.db.get_value('System Settings', 'System Settings', 'lifespan_qrcode_image'))
	if lifespan<=0:
		lifespan = 240
...
def send_token_via_email(user, token, otp_secret, otp_issuer, subject=None, message=None):
	...
	enqueue(method=dataent.sendmail, queue='short', timeout=300, event=None,
...
def qrcode_as_png(user, totp_uri):
	...
	file_path = os.path.join(dataent.get_site_path('public', 'files'), file_obj.file_name)
	url = qrcreate(totp_uri)
...
def create_barcode_folder():
	...
	folder = dataent.db.exists('File', {'file_name': folder_name})
	if folder:
		return folder
	folder = dataent.get_doc({
			'doctype': 'File',
...
def delete_qrimage(user, check_expiry=False):
	...
	user_barcodes = dataent.get_all('File', {'attached_to_doctype': 'User',
	...
	for barcode in user_barcodes:
		if check_expiry and not should_remove_barcode_image(barcode):
			continue
		barcode = dataent.get_doc('File', barcode.name)
		dataent.delete_doc('File', barcode.name, ignore_permissions=True)
...
def delete_all_barcodes_for_users():
	...
	users = dataent.get_all('User', {'enabled':1})
	for user in users:
...
def should_remove_barcode_image(barcode):
	...
	if isinstance(barcode, string_types):
		barcode = dataent.get_doc('File', barcode)
	lifespan = dataent.db.get_value('System Settings', 'System Settings', 'lifespan_qrcode_image')
	if time_diff_in_seconds(get_datetime(), barcode.creation) > int(lifespan):
		return True
	return False

def disable():
	dataent.db.set_value('System Settings', None, 'enable_two_factor_auth', 0)
