...
class OAuthWebRequestValidator(RequestValidator):
	...
	def validate_client_id(self, client_id, request, *args, **kwargs):
		...
		cli_id = dataent.db.get_value("OAuth Client",{ "name":client_id })
		if cli_id:
			request.client = dataent.get_doc("OAuth Client", client_id).as_dict()
			return True
		else:
			return False
	...
	def validate_redirect_uri(self, client_id, redirect_uri, request, *args, **kwargs):
		...
		redirect_uris = dataent.db.get_value("OAuth Client", client_id, 'redirect_uris').split(get_url_delimiter())
	...
	def get_default_redirect_uri(self, client_id, request, *args, **kwargs):
		...
		redirect_uri = dataent.db.get_value("OAuth Client", client_id, 'default_redirect_uri')
		return redirect_uri

	def validate_scopes(self, client_id, scopes, client, request, *args, **kwargs):
		# Is the client allowed to access the requested scopes?
		client_scopes = dataent.db.get_value("OAuth Client", client_id, 'scopes').split(get_url_delimiter())
	...
	def get_default_scopes(self, client_id, request, *args, **kwargs):
		...
		scopes = dataent.db.get_value("OAuth Client", client_id, 'scopes').split(get_url_delimiter())
	...
	def save_authorization_code(self, client_id, code, request, *args, **kwargs):
		...
		oac = dataent.new_doc('OAuth Authorization Code')
	...
	def authenticate_client(self, request, *args, **kwargs):
		...
		if request.client_id:
			...
			if "refresh_token" in dataent.form_dict:
				oc = dataent.get_doc("OAuth Client", dataent.db.get_value("OAuth Bearer Token", {"refresh_token": dataent.form_dict["refresh_token"]}, 'client'))
			elif "token" in dataent.form_dict:
				oc = dataent.get_doc("OAuth Client", dataent.db.get_value("OAuth Bearer Token", dataent.form_dict["token"], 'client'))
			else:
				oc = dataent.get_doc("OAuth Client", dataent.db.get_value("OAuth Bearer Token", dataent.get_request_header("Authorization").split(" ")[1], 'client'))
		try:
		...
		return dataent.session.user == unquote(cookie_dict.get('user_id', "Guest"))

	def authenticate_client_id(self, client_id, request, *args, **kwargs):
		cli_id = dataent.db.get_value('OAuth Client', client_id, 'name')
		if not cli_id:
			# Don't allow public (non-authenticated) clients
			return False
		else:
			request["client"] = dataent.get_doc("OAuth Client", cli_id)
			return True
	...
	def validate_code(self, client_id, code, client, request, *args, **kwargs):
		...
		validcodes = dataent.get_all("OAuth Authorization Code", filters={"client": client_id, "validity": "Valid"})
		...
		if code in checkcodes:
			request.scopes = dataent.db.get_value("OAuth Authorization Code", code, 'scopes').split(get_url_delimiter())
			request.user = dataent.db.get_value("OAuth Authorization Code", code, 'user')
			return True
		else:
			return False

	def confirm_redirect_uri(self, client_id, code, redirect_uri, client, *args, **kwargs):
		saved_redirect_uri = dataent.db.get_value('OAuth Client', client_id, 'default_redirect_uri')
	...
	def save_bearer_token(self, token, request, *args, **kwargs):
		...
		otoken = dataent.new_doc("OAuth Bearer Token")
		otoken.client = request.client['name']
		try:
			otoken.user = request.user if request.user else dataent.db.get_value("OAuth Bearer Token", {"refresh_token":request.body.get("refresh_token")}, "user")
		except Exception as e:
			otoken.user = dataent.session.user
		...
		default_redirect_uri = dataent.db.get_value("OAuth Client", request.client['name'], "default_redirect_uri")
	...
	def invalidate_authorization_code(self, client_id, code, request, *args, **kwargs):
		...
		dataent.db.set_value("OAuth Authorization Code", code, "validity", "Invalid")
		dataent.db.commit()
	...
	def validate_bearer_token(self, token, scopes, request):
		...
		otoken = dataent.get_doc("OAuth Bearer Token", token)
		token_expiration_local = otoken.expiration_time.replace(tzinfo=pytz.timezone(dataent.utils.get_time_zone()))
		token_expiration_utc = token_expiration_local.astimezone(pytz.utc)
		is_token_valid = (dataent.utils.datetime.datetime.utcnow().replace(tzinfo=pytz.utc) < token_expiration_utc) \
			and otoken.status != "Revoked"
		client_scopes = dataent.db.get_value("OAuth Client", otoken.client, 'scopes').split(get_url_delimiter())
		are_scopes_valid = True
		for scp in scopes:
			are_scopes_valid = are_scopes_valid and True if scp in client_scopes else False
	...
	def get_original_scopes(self, refresh_token, request, *args, **kwargs):
		...
		# request.
		obearer_token = dataent.get_doc("OAuth Bearer Token", {"refresh_token": refresh_token})
	...
	def revoke_token(self, token, token_type_hint, request, *args, **kwargs):
		...
		if token_type_hint == "access_token":
			otoken = dataent.db.set_value("OAuth Bearer Token", token, 'status', 'Revoked')
		elif token_type_hint == "refresh_token":
			otoken = dataent.db.set_value("OAuth Bearer Token", {"refresh_token": token}, 'status', 'Revoked')
		else:
			otoken = dataent.db.set_value("OAuth Bearer Token", token, 'status', 'Revoked')
		dataent.db.commit()
	...
	def validate_refresh_token(self, refresh_token, client, request, *args, **kwargs):
		...
		otoken = dataent.get_doc("OAuth Bearer Token", {"refresh_token": refresh_token, "status": "Active"})
	...
	def validate_silent_login(self, request):
		...
		Method is used by:
		    ...
		    - OpenIDConnectHybrid
		"""
		if dataent.session.user == "Guest" or request.prompt.lower() == "login":
			return False
		else:
			return True
	...
	def validate_user_match(self, id_token_hint, scopes, claims, request):
		...
		Method is used by:
		    ...
		    - OpenIDConnectHybrid
		"""
		if id_token_hint and id_token_hint == dataent.db.get_value("User Social Login", {"parent":dataent.session.user, "provider": "dataent"}, "userid"):
			return True
		else:
			return False
...
def delete_oauth2_data():
	...
	code_list = dataent.get_all("OAuth Authorization Code", filters={"validity":"Invalid"})
	token_list = dataent.get_all("OAuth Bearer Token", filters={"status":"Revoked"})
	if len(code_list) > 0:
		commit_code = True
	if len(token_list) > 0:
		commit_token = True
	for code in code_list:
		dataent.delete_doc("OAuth Authorization Code", code["name"])
	for token in token_list:
		dataent.delete_doc("OAuth Bearer Token", token["name"])
