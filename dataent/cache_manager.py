...
def clear_global_cache():
	from dataent.website.render import clear_cache as clear_website_cache

	clear_doctype_cache()
	clear_website_cache()
	dataent.cache().delete_value(["app_hooks", "installed_apps",
...
def clear_defaults_cache(user=None):
	if user:
		for p in ([user] + common_default_keys):
			dataent.cache().hdel("defaults", p)
	elif dataent.flags.in_install!="dataent":
		dataent.cache().delete_key("defaults")

def clear_document_cache():
	dataent.local.document_cache = {}
	dataent.cache().delete_key("document_cache")

def clear_doctype_cache(doctype=None):
	cache = dataent.cache()

	if getattr(dataent.local, 'meta_cache') and (doctype in dataent.local.meta_cache):
		del dataent.local.meta_cache[doctype]
	...
	if doctype:
		...
		for dt in dataent.db.sql("""select parent from tabDocField
