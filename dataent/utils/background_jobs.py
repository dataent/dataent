...
def enqueue(method, queue='default', timeout=None, event=None,
	...
	'''
		...
		:param now: if now=True, the method is executed via dataent.call
	...
	if now or dataent.flags.in_migrate:
		return dataent.call(method, **kwargs)
	...
	if enqueue_after_commit:
		if not dataent.flags.enqueue_after_commit:
			dataent.flags.enqueue_after_commit = []

		dataent.flags.enqueue_after_commit.append({
			"queue": queue,
			"is_async": is_async,
			"timeout": timeout,
			"queue_args":queue_args
		})
		return dataent.flags.enqueue_after_commit
	else:
...
def enqueue_doc(doctype, name=None, method=None, queue='default', timeout=300,
	...
	enqueue('dataent.utils.background_jobs.run_doc_method', doctype=doctype, name=name,
		doc_method=method, queue=queue, timeout=timeout, now=now, **kwargs)

def run_doc_method(doctype, name, doc_method, **kwargs):
	getattr(dataent.get_doc(doctype, name), doc_method)(**kwargs)
...
def get_redis_conn():
	if not hasattr(dataent.local, 'conf'):
		raise Exception('You need to call dataent.init')

	elif not dataent.local.conf.redis_queue:
		raise Exception('redis_queue missing in common_site_config.json')

	global redis_connection

	if not redis_connection:
		redis_connection = redis.from_url(dataent.local.conf.redis_queue)

	return redis_connection

def enqueue_test_job():
	enqueue('dataent.utils.background_jobs.test_job', s=100)
