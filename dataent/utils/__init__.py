...
import dataent
from dataent.utils.identicon import Identicon
from email.utils import parseaddr, formataddr
# utility functions like cint, int, flt, etc.
from dataent.utils.data import *
...
def get_fullname(user=None):
	...
	if not hasattr(dataent.local, "fullnames"):
		dataent.local.fullnames = {}

	if not dataent.local.fullnames.get(user):
		p = dataent.db.get_value("User", user, ["first_name", "last_name"], as_dict=True)
		if p:
			dataent.local.fullnames[user] = " ".join(filter(None,
				[p.get('first_name'), p.get('last_name')])) or user
		else:
			dataent.local.fullnames[user] = user

	return dataent.local.fullnames.get(user)
...
def get_email_address(user=None):
	...
	return dataent.db.get_value("User", user, ["email"], as_dict=True).get("email")
...
def validate_email_address(email_str, throw=False):
	...
	def _check(e):
		...
		if not _valid:
			if throw:
				dataent.throw(dataent._("{0} is not a valid Email Address").format(e),
					dataent.InvalidEmailAddressError)
			return None
		else:
			return matched
...
def get_defaults(key=None):
	...
	"""
	return dataent.db.get_defaults(key)

def set_default(key, val):
	"""
	Set / add a default value to defaults`
	"""
	return dataent.db.set_default(key, val)
...
def get_path(*path, **kwargs):
	...
	if not base:
		base = dataent.local.site_path
	return os.path.join(base, *path)

def get_site_base_path(sites_dir=None, hostname=None):
	return dataent.local.site_path
...
def get_bench_path():
	return os.path.realpath(os.path.join(os.path.dirname(dataent.__file__), '..', '..', '..'))
...
def get_test_client():
	from dataent.app import application
	return Client(application)

def get_hook_method(hook_name, fallback=None):
	method = (dataent.get_hooks().get(hook_name))
	if method:
		method = dataent.get_attr(method[0])
		return method
	if fallback:
		return fallback
...
def call_hook_method(hook, *args, **kwargs):
	out = None
	for method_name in dataent.get_hooks(hook):
		out = out or dataent.get_attr(method_name)(*args, **kwargs)

	return out

def update_progress_bar(txt, i, l):
	if not getattr(dataent.local, 'request', None):
		lt = len(txt)
		if lt < 36:
			txt = txt + " "*(36-lt)
	...
	if os.path.exists(print_path):
		...
		for include_directive, path in re.findall("""({% include ['"]([^'"]*)['"] %})""", html_format):
			for app_name in dataent.get_installed_apps():
				include_path = dataent.get_app_path(app_name, *path.split(os.path.sep))
...
def get_sites(sites_path=None):
	if not sites_path:
		sites_path = getattr(dataent.local, 'sites_path', None) or '.'
...
def markdown(text, sanitize=True, linkify=True):
	html = dataent.utils.md_to_html(text)
...
def get_site_info():
	...
	users = dataent.get_all('User', filters={'user_type': 'System User', 'name': ('not in', STANDARD_USERS)},
	...
	space_usage = dataent._dict((dataent.local.conf.limits or {}).get('space_usage', {}))
