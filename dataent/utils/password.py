...
def get_decrypted_password(doctype, name, fieldname='password', raise_exception=True):
	auth = dataent.db.sql('''select `password` from `__Auth`
	...
	if auth and auth[0][0]:
		...
		dataent.throw(_('Password not found'), dataent.AuthenticationError)

def set_encrypted_password(doctype, name, pwd, fieldname='password'):
	dataent.db.sql("""insert into __Auth (doctype, name, fieldname, `password`, encrypted)
...
def check_password(user, pwd, doctype='User', fieldname='password'):
	'''Checks if user and password are correct, else raises dataent.AuthenticationError'''

	auth = dataent.db.sql("""select name, `password` from `__Auth`
	...
	if not auth or not passlibctx.verify(pwd, auth[0].password):
		raise dataent.AuthenticationError(_('Incorrect User or Password'))
...
def delete_login_failed_cache(user):
	dataent.cache().hdel('last_login_tried', user)
	dataent.cache().hdel('login_failed_count', user)
	dataent.cache().hdel('locked_account_time', user)
...
def update_password(user, pwd, doctype='User', fieldname='password', logout_all_sessions=False):
	...
	if logout_all_sessions:
		from dataent.sessions import clear_sessions
		clear_sessions(user=user, keep_current=True, force=True)

def delete_all_passwords_for(doctype, name):
	try:
		dataent.db.sql("""delete from __Auth where doctype=%(doctype)s and name=%(name)s""",
...
def rename_password(doctype, old_name, new_name):
	...
	dataent.db.sql("""update __Auth set name=%(new_name)s
...
def rename_password_field(doctype, old_fieldname, new_fieldname):
	dataent.db.sql('''update `__Auth` set fieldname=%(new_fieldname)s
...
def create_auth_table():
	...
	dataent.db.sql_ddl("""create table if not exists __Auth (
...
def decrypt(pwd):
	try:
		...
		dataent.throw(_('Encryption key is invalid, Please check site_config.json'))
