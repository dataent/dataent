...
import dataent
import json, os
from dataent.modules import scrub, get_module_path, utils
from dataent.custom.doctype.customize_form.customize_form import doctype_properties, docfield_properties
...
def reset_all():
	for doctype in dataent.db.get_all('DocType', dict(custom=0)):
		print(doctype.name)
...
def remove_duplicate_fields(doctype):
	for field in dataent.db.sql('''select fieldname, count(1) as cnt from tabDocField where parent=%s group by fieldname having cnt > 1''', doctype):
		dataent.db.sql('delete from tabDocField where fieldname=%s and parent=%s limit 1', (field[0], doctype))
...
def get_custom_docfield_properties():
	fields_meta = dataent.get_meta('Custom Field').fields
	fields = {}
...
def setup_perms_for(doctype):
	perms = dataent.get_all('DocPerm', fields='*', filters=dict(parent=doctype), order_by='idx asc')
	# get default perms
	try:
	...
	if len(standard_perms) != len(perms):
		...
		for i, p in enumerate(perms):
			...
			for fieldname in dataent.get_meta('DocPerm').get_fieldnames_with_value():
