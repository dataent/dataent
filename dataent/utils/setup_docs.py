...
import os, json, dataent, shutil
from dataent.utils import markdown
...
class setup_docs(object):
	def __init__(self, app, target_app):
		...
		dataent.flags.web_pages_folders = ['docs',]
		dataent.flags.web_pages_apps = [self.app,]

		self.hooks = dataent.get_hooks(app_name = self.app)
	...
	def setup_app_context(self):
		...
		self.app_context =  {
			...
			"get_doctype_app": dataent.get_doctype_app
		}
	...
	def add_sidebars(self):
		...
		for basepath, folders, files in os.walk(self.docs_path): # pylint: disable=unused-variable
			with open(os.path.join(basepath, '_sidebar.json'), 'w') as sidebarfile:
				sidebarfile.write(dataent.as_json([
	...
	def make_home_pages(self):
		...
		with open(os.path.join(self.path, "index.html"), "w") as home:
			home.write(dataent.render_template("templates/autodoc/dev_home.html",
			self.app_context))
	...
	def build_user_docs(self):
		...
		# contents
		shutil.copy(os.path.join(dataent.get_app_path("dataent", "templates", "autodoc",
			...
			if f.endswith(".py"):
				...
				if not os.path.exists(module_doc_path):
					...
					with open(module_doc_path, "wb") as f:
						...
						f.write(dataent.render_template("templates/autodoc/pymodule.html",
	...
	def make_folder(self, path, template=None, context=None):
		...
		if not os.path.exists(path):
			...
			with open(index_html_path, "w") as f:
				f.write(dataent.render_template(template, context))
		...
		if not os.path.exists(model_path):
			...
			if os.path.exists(model_json_path):
				...
				with open(model_path, "wb") as f:
					...
					context.update(self.app_context)
					f.write(dataent.render_template("templates/autodoc/doctype.html",
						context).encode("utf-8"))

def get_version(app="dataent"):
	try:
		return dataent.get_attr(app + ".__version__")
