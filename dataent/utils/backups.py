...
from dataent.utils import cstr, get_url, now_datetime
...
class BackupGenerator:
	...
	def zip_files(self):
		for folder in ("public", "private"):
			files_path = dataent.get_site_path(folder, "files")
			...
			err, out = dataent.utils.execute_in_shell(cmd_string)
	...
	def take_dump(self):
		...
		args = dict([item[0], dataent.utils.esc(item[1], '$ ')]
...
def get_backup():
	...
	"""
	#if verbose: print dataent.db.cur_db_name + " " + conf.db_password
	delete_temp_backups()
	odb = BackupGenerator(dataent.conf.db_name, dataent.conf.db_name,\
						  dataent.conf.db_password, db_host = dataent.db.host)
	odb.get_backup()
	recipient_list = odb.send_email()
	dataent.msgprint(_("Download link for your backup will be emailed on the following email address: {0}").format(', '.join(recipient_list)))
...
def new_backup(older_than=6, ignore_files=False, backup_path_db=None, backup_path_files=None, backup_path_private_files=None, force=False):
	delete_temp_backups(older_than = dataent.conf.keep_backups_for_hours or 24)
	odb = BackupGenerator(dataent.conf.db_name, dataent.conf.db_name,\
						  dataent.conf.db_password,
						  backup_path_db=backup_path_db, backup_path_files=backup_path_files,
						  backup_path_private_files=backup_path_private_files,
						  db_host = dataent.db.host)
...
def get_backup_path():
	backup_path = dataent.utils.get_site_path(conf.get("backup_path", "private/backups"))
	return backup_path
