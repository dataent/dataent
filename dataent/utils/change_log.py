...
from dataent import _, safe_decode


def get_change_log(user=None):
	if not user: user = dataent.session.user

	last_known_versions = dataent._dict(json.loads(dataent.db.get_value("User",
	...
	if "dataent" in current_versions:
		set_in_change_log("dataent", current_versions["dataent"], change_log)
...
def update_last_known_versions():
	dataent.db.set_value("User", dataent.session.user, "last_known_versions",
...
def check_for_update():
	updates = dataent._dict(major=[], minor=[], patch=[])
	apps = get_versions()
	...
	for app in apps:
		...
		for update_type in updates:
			if github_version.__dict__[update_type] > instance_version.__dict__[update_type]:
				updates[update_type].append(dataent._dict(
...
def add_message_to_redis(update_json):
	...
	user_list = [x.name for x in dataent.get_all("User", filters={"enabled": True})]
	system_managers = [user for user in user_list if 'System Manager' in dataent.get_roles(user)]
...
def show_update_popup():
	...
	if update_message:
		dataent.msgprint(update_message, title=_("New updates are available"), indicator='green')
