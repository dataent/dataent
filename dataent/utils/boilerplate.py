...
import dataent, os, re
from dataent.utils import touch_file, cstr
...
def make_boilerplate(dest, app_name):
	...
	dataent.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, dataent.scrub(hooks.app_title)),
		with_init=True)
	dataent.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "templates"), with_init=True)
	dataent.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "www"))
	dataent.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "templates",
		"pages"), with_init=True)
	dataent.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "templates",
		"includes"))
	dataent.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "config"), with_init=True)
	dataent.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public",
		"css"))
	dataent.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public",
		"js"))
	...
	with open(os.path.join(dest, hooks.app_name, ".gitignore"), "w") as f:
		f.write(dataent.as_unicode(gitignore_template.format(app_name = hooks.app_name)))
...
# 	"dataent.desk.doctype.event.event.get_events": "{app_name}.event.get_events"
# }}
