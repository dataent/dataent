...
class SignupDisabledError(dataent.PermissionError): pass

def get_oauth2_providers():
	out = {}
	providers = dataent.get_all("Social Login Key", fields=["*"])
...
def get_oauth_keys(provider):
	...
	# try conf
	keys = dataent.conf.get("{provider}_login".format(provider=provider))

	if not keys:
		# try database
		client_id, client_secret = dataent.get_value("Social Login Key", provider, ["client_id", "client_secret"])
...
def get_oauth2_authorize_url(provider, redirect_to):
	...
	state = { "site": dataent.utils.get_url(), "token": dataent.generate_hash(), "redirect_to": redirect_to 	}

	dataent.cache().set_value("{0}:{1}".format(provider, state["token"]), True, expires_in_sec=120)
...
def get_redirect_uri(provider):
	...
	if keys and keys.get("redirect_uri"):
		...
		return dataent.utils.get_url(redirect_uri)
...
def get_info_via_oauth(provider, code, decoder=None, id_token=False):
	...
	if not (info.get("verified_email") or info.get("verified")):
		dataent.throw(_("Email not verified with {0}").format(provider.title()))
...
def login_oauth_user(data=None, provider=None, state=None, email_id=None, key=None, generate_login_token=False):
	...
	# if email_id and key:
	# 	data = json.loads(dataent.db.get_temp(key))
	#	# What if data is missing because of an invalid key
	# 	data["email"] = email_id
	#
	# elif not (data.get("email") and get_first_name(data)) and not dataent.db.exists("User", data.get("email")):
	# 	# ask for user email
	# 	key = dataent.db.set_temp(json.dumps(data))
	# 	dataent.db.commit()
	# 	dataent.local.response["type"] = "redirect"
	# 	dataent.local.response["location"] = "/complete_signup?key=" + key
	# 	return
	...
	if not (state and state["token"]):
		dataent.respond_as_web_page(_("Invalid Request"), _("Token is missing"), http_status_code=417)
		return

	token = dataent.cache().get_value("{0}:{1}".format(provider, state["token"]), expires=True)
	if not token:
		dataent.respond_as_web_page(_("Invalid Request"), _("Invalid Token"), http_status_code=417)
		return

	user = get_email(data)

	if not user:
		dataent.respond_as_web_page(_("Invalid Request"), _("Please ensure that your profile has an email address"))
		return
	...
	try:
		...
		return dataent.respond_as_web_page("Signup is Disabled", "Sorry. Signup from Website is disabled.",
			success=False, http_status_code=403)

	dataent.local.login_manager.user = user
	dataent.local.login_manager.post_login()

	# because of a GET request!
	dataent.db.commit()

	if dataent.utils.cint(generate_login_token):
		login_token = dataent.generate_hash(length=32)
		dataent.cache().set_value("login_token:{0}".format(login_token), dataent.local.session.sid, expires_in_sec=120)

		dataent.response["login_token"] = login_token


	else:
		redirect_to = state.get("redirect_to")
		redirect_post_login(
			desk_user=dataent.local.response.get('message') == 'Logged In',
...
def update_oauth_user(user, data, provider):
	...
	if not dataent.db.exists("User", user):
		...
		# is signup disabled?
		if dataent.utils.cint(dataent.db.get_single_value("Website Settings", "disable_signup")):
		...
		user.update({
			...
			"enabled": 1,
			"new_password": dataent.generate_hash(get_email(data)),
		...
		if not user.enabled:
			dataent.respond_as_web_page(_('Not Allowed'), _('User {0} is disabled').format(user.email))
			return False
	...
	if provider=="facebook" and not user.get_social_login_userid(provider):
	...
	elif provider=="dataent" and not user.get_social_login_userid(provider):
		save = True
		...
		save = True
		user_id_property = dataent.db.get_value("Social Login Key", provider, "user_id_property") or "sub"
...
def redirect_post_login(desk_user, redirect_to=None):
	...
	dataent.local.response["location"] = redirect_to
