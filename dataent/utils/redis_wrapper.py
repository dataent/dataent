...
class RedisWrapper(redis.Redis):
	...
	def make_key(self, key, user=None, shared=False):
		...
		return "{0}|{1}".format(dataent.conf.db_name, key).encode('utf-8')
	...
	def get_value(self, key, generator=None, user=None, expires=False):
		...
		:param expires: If the key is supposed to be with an expiry, don't store it in dataent.local
		"""
		original_key = key
		key = self.make_key(key, user)

		if key in dataent.local.cache:
			val = dataent.local.cache[key]
	...
	def get_keys(self, key):
		...
		try:
			...
			return [k for k in list(dataent.local.cache) if regex.match(k.decode())]
	...
	def hset(self, name, key, value, shared=False):
		...
		# set in local
		if not _name in dataent.local.cache:
			dataent.local.cache[_name] = {}
		dataent.local.cache[_name][key] = value
	...
	def hget(self, name, key, generator=None, shared=False):
		...
		if key in dataent.local.cache[_name]:
			return dataent.local.cache[_name][key]
	...
	def hdel(self, name, key, shared=False):
		...
		if _name in dataent.local.cache:
			if key in dataent.local.cache[_name]:
				del dataent.local.cache[_name][key]
		try:
	...
	def hdel_keys(self, name_starts_with, key):
		...
		for name in dataent.cache().get_keys(name_starts_with):
			name = name.split("|", 1)[1]
			self.hdel(name, key)
