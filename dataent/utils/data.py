...
def _get_time_zone():
	return dataent.db.get_system_setting('time_zone') or 'Asia/Kolkata' # Default to India ?!

def get_time_zone():
	if dataent.local.flags.in_test:
		return _get_time_zone()

	return dataent.cache().get_value("time_zone", _get_time_zone)
...
def now():
	...
	if dataent.flags.current_date:
		return getdate(dataent.flags.current_date).strftime(DATE_FORMAT) + " " + \
...
def get_user_format():
	if getattr(dataent.local, "user_format", None) is None:
		dataent.local.user_format = dataent.db.get_default("date_format")

	return dataent.local.user_format or "yyyy-mm-dd"
...
def formatdate(string_date=None, format_string=None):
	...
	try:
		formatted_date = babel.dates.format_date(date, format_string, locale=(dataent.local.lang or "").replace("-", "_"))
...
def format_time(txt):
	try:
		formatted_time = babel.dates.format_time(get_time(txt), locale=(dataent.local.lang or "").replace("-", "_"))
	except UnknownLocaleError:
...
def format_datetime(datetime_string, format_string=None):
	...
	try:
		formatted_datetime = babel.dates.format_datetime(datetime, format_string, locale=(dataent.local.lang or "").replace("-", "_"))
	except UnknownLocaleError:
...
def global_date_format(date, format="long"):
	...
	date = getdate(date)
	formatted_date = babel.dates.format_date(date, locale=(dataent.local.lang or "en").replace("-", "_"), format=format)
	return formatted_date
...
def round_based_on_smallest_currency_fraction(value, currency, precision=2):
	smallest_currency_fraction_value = flt(dataent.db.get_value("Currency",
...
def fmt_money(amount, precision=None, currency=None):
	...
	"""
	number_format = dataent.db.get_default("number_format") or "#,###.##"
	if precision is None:
		precision = cint(dataent.db.get_default('currency_precision')) or None
	...
	if decimal_str:
		...
		if precision > 2:
			if len(decimals) < 3:
				if currency:
					fraction  = dataent.db.get_value("Currency", currency, "fraction_units", cache=True) or 100
	...
	if currency and dataent.defaults.get_global_default("hide_currency_symbol") != "Yes":
		symbol = dataent.db.get_value("Currency", currency, "symbol", cache=True) or currency
...
def money_in_words(number, main_currency = None, fraction_currency=None):
	...
	"""
	from dataent.utils import get_defaults
	_ = dataent._
	...
	if not fraction_currency:
		fraction_currency = dataent.db.get_value("Currency", main_currency, "fraction", cache=True) or _("Cent")

	number_format = dataent.db.get_value("Currency", main_currency, "number_format", cache=True) or \
		dataent.db.get_default("number_format") or "#,###.##"
...
def in_words(integer, in_million=True):
	...
	"""
	locale = 'en_IN' if not in_million else dataent.local.lang
	integer = int(integer)
	try:
...
def is_html(text):
	if not isinstance(text, dataent.string_types):
		return False
...
def comma_or(some_list):
	return comma_sep(some_list, dataent._("{0} or {1}"))

def comma_and(some_list):
	return comma_sep(some_list, dataent._("{0} and {1}"))
...
def comma_sep(some_list, pattern):
	if isinstance(some_list, (list, tuple)):
		...
		if not some_list:
			...
			return pattern.format(", ".join(dataent._(s) for s in some_list[:-1]), some_list[-1])
	else:
		return some_list
...
def get_url(uri=None, full_address=False):
	...
	if not host_name:
		if hasattr(dataent.local, "request") and dataent.local.request and dataent.local.request.host:
			protocol = 'https://' if 'https' == dataent.get_request_header('X-Forwarded-Proto', "") else 'http://'
			...
			if dataent.local.conf.ssl_certificate:
				...
				if domain and dataent.local.site.endswith(domain) and dataent.local.conf.wildcard.get('ssl_certificate'):
					protocol = 'https://'

			host_name = protocol + dataent.local.site

		else:
			host_name = dataent.db.get_value("Website Settings", "Website Settings",
				"subdomain")
	...
	if not (dataent.conf.restart_supervisor_on_update or dataent.conf.restart_systemd_on_update) and host_name and not url_contains_port(host_name) and port:
...
def get_filter(doctype, f):
	...
	"""
	from dataent.model import default_fields, optional_fields
	...
	if not isinstance(f, (list, tuple)):
		dataent.throw(dataent._("Filter must be a tuple or list (in a list)"))
	...
	if len(f) == 3:
		...
		dataent.throw(dataent._("Filter must have 4 values (doctype, fieldname, operator, value): {0}").format(str(f)))

	f = dataent._dict(doctype=f[0], fieldname=f[1], operator=f[2], value=f[3])
...
def sanitize_column(column_name):
	...
	def _raise_exception():
		dataent.throw(_("Invalid field name {0}").format(column_name), dataent.DataError)
