...
import redis
import json
from dataent.utils import cint, strip_html_tags
from dataent.model.base_document import get_controller
from dataent.model.db_schema import varchar_len
...
def setup_global_search_table():
	...
	:return:
	"""
	if not '__global_search' in dataent.db.get_tables():
		dataent.db.sql('''create table __global_search(
			doctype varchar(100),
...
def get_doctypes_with_global_search(with_child_tables=True):
	...
	def _get():
		...
		for d in dataent.get_all('DocType', fields=['name', 'module'], filters=filters):
			meta = dataent.get_meta(d.name)
			if len(meta.get_global_search_fields()) > 0:
				global_search_doctypes.append(d)

		installed_apps = dataent.get_installed_apps()
		module_app = dataent.local.module_app

		doctypes = [
			d.name for d in global_search_doctypes
			if module_app.get(dataent.scrub(d.module))
			and module_app[dataent.scrub(d.module)] in installed_apps
		]

		return doctypes

	return dataent.cache().get_value('doctypes_with_global_search', _get)
...
def rebuild_for_doctype(doctype):
	...
	if dataent.local.conf.get('disable_global_search'):
		return

	def _get_filters():
		filters = dataent._dict({ "docstatus": ["!=", 2] })
	...
	if cint(meta.istable) == 1:
		parent_doctypes = dataent.get_all("DocField", fields="parent", filters={
			"fieldtype": "Table",
			"options": doctype
		})
	...
	all_records = dataent.get_all(doctype, fields=fieldnames, filters=_get_filters())
		...
		if content:
			...
			try:
				if hasattr(get_controller(doctype), "is_website_published") and meta.allow_guest_to_view:
					d = dataent.get_doc(doctype, doc.name)
			...
			all_contents.append({
				...
				"name": dataent.db.escape(doc.name),
				"content": dataent.db.escape(' ||| '.join(content or '')),
				"published": published,
				"title": dataent.db.escape(title or '')[:int(varchar_len)],
				"route": dataent.db.escape(route or '')[:int(varchar_len)]
			})
	if all_contents:
...
def get_children_data(doctype, meta):
	...
	"""
	all_children = dataent._dict()
	child_search_fields = dataent._dict()

	for child in meta.get_table_fields():
		child_meta = dataent.get_meta(child.options)
		...
		if search_fields:
			...
			child_records = dataent.get_all(child.options, fields=child_fieldnames, filters={
				"docstatus": ["!=", 1],
				"parenttype": doctype
			})

			for record in child_records:
				all_children.setdefault(record.parent, dataent._dict())\
...
def update_global_search(doc):
	...
	if content:
		...
		try:
			...
			dataent.cache().lpush('global_search_queue', json.dumps(value))
...
def sync_global_search():
	...
	:return:
	"""
	while dataent.cache().llen('global_search_queue') > 0:
		value = json.loads(dataent.cache().lpop('global_search_queue').decode('utf-8'))
		sync_value(value)
	...
	for r in results:
		try:
			if dataent.get_meta(r.doctype).image_field:
				r.image = dataent.db.get_value(r.doctype, r.name, dataent.get_meta(r.doctype).image_field)
		except Exception:
