...
import mimetypes
import io
from dataent.utils import get_hook_method, get_files_path, random_string, encode, cstr, call_hook_method, cint
...
class MaxFileSizeReachedError(dataent.ValidationError):
	pass


def get_file_url(file_data_name):
	data = dataent.db.get_value("File", file_data_name, ["file_name", "file_url"], as_dict=True)
	return data.file_url or data.file_name


def upload():
	# get record details
	dt = dataent.form_dict.doctype
	dn = dataent.form_dict.docname
	file_url = dataent.form_dict.file_url
	filename = dataent.form_dict.filename
	dataent.form_dict.is_private = cint(dataent.form_dict.is_private)

	if not filename and not file_url:
		dataent.msgprint(_("Please select a file or url"),
	...
	comment = {}
	if dt and dn:
		comment = dataent.get_doc(dt, dn).add_comment("Attachment",
...
def save_url(file_url, filename, dt, dn, folder, is_private, df=None):
	...
	# 	dataent.msgprint("URL must start with 'http://' or 'https://'")
	# 	return None, None

	file_url = unquote(file_url)
	file_size = dataent.form_dict.file_size
...
def get_uploaded_content():
	# should not be unicode when reading a file, hence using dataent.form
	if 'filedata' in dataent.form_dict:
		if "," in dataent.form_dict.filedata:
			dataent.form_dict.filedata = dataent.form_dict.filedata.rsplit(",", 1)[1]
		dataent.uploaded_content = base64.b64decode(dataent.form_dict.filedata)
		dataent.uploaded_filename = dataent.form_dict.filename
		return dataent.uploaded_filename, dataent.uploaded_content
	else:
		dataent.msgprint(_('No file attached'))
		return None, None
...
def get_file_data_from_hash(content_hash, is_private=0):
	for name in dataent.db.sql_list("select name from `tabFile` where content_hash=%s and is_private=%s", (content_hash, is_private)):
		b = dataent.get_doc('File', name)
		return {k: b.get(k) for k in dataent.get_hooks()['write_file_keys']}
	return False
...
def check_max_file_size(content):
	...
	if file_size > max_file_size:
		dataent.msgprint(_("File size exceeded the maximum allowed size of {0} MB").format(
...
def remove_all(dt, dn, from_delete=False):
	...
	try:
		for fid in dataent.db.sql_list("""select name from `tabFile` where
...
def remove_file_by_url(file_url, doctype=None, name=None):
	if doctype and name:
		fid = dataent.db.get_value("File", {"file_url": file_url,
...
def remove_file(fid, attached_to_doctype=None, attached_to_name=None, from_delete=False):
	...
	if not (attached_to_doctype and attached_to_name):
		attached = dataent.db.get_value("File", fid,
	...
	if attached_to_doctype and attached_to_name and not from_delete:
		doc = dataent.get_doc(attached_to_doctype, attached_to_name)
		ignore_permissions = doc.has_permission("write") or False
		if dataent.flags.in_web_form:
			ignore_permissions = True
		if not file_name:
			file_name = dataent.db.get_value("File", fid, "file_name")
		comment = doc.add_comment("Attachment Removed", _("Removed {0}").format(file_name))

	dataent.delete_doc("File", fid, ignore_permissions=ignore_permissions)
...
def delete_file(path):
	...
	if path:
		if ".." in path.split("/"):
			dataent.msgprint(_("It is risky to delete this file: {0}. Please contact your System Manager.").format(path))
...
def get_file_path(file_name):
	...
	f = dataent.db.sql("""select file_url from `tabFile`
	...
	if file_path.startswith("/private/files/"):
		...
		dataent.throw(_("There is some problem with the file url: {0}").format(file_path))
...
def get_file_name(fname, optional_suffix):
	...
	n_records = dataent.db.sql("select name from `tabFile` where file_name=%s", fname)
...
def download_file(file_url):
	...
	Method : GET
	Endpoint : dataent.utils.file_manager.download_file
	URL Params : file_name = /path/to/file relative to site path
	"""
	file_doc = dataent.get_doc("File", {"file_url":file_url})
	file_doc.check_permission("read")
	path = os.path.join(get_files_path(), os.path.basename(file_url))

	with open(path, "rb") as fileobj:
		filedata = fileobj.read()
	dataent.local.response.filename = os.path.basename(file_url)
	dataent.local.response.filecontent = filedata
	dataent.local.response.type = "download"

def extract_images_from_doc(doc, fieldname):
	content = doc.get(fieldname)
	content = extract_images_from_html(doc, content)
	if dataent.flags.has_dataurl:
		doc.set(fieldname, content)


def extract_images_from_html(doc, content):
	dataent.flags.has_dataurl = False
	...
	def _save_file(match):
		...
		if not dataent.flags.has_dataurl:
			dataent.flags.has_dataurl = True
...
def add_attachments(doctype, name, attachments):
	...
	for a in attachments:
		if isinstance(a, string_types):
			attach = dataent.db.get_value("File", {"name":a}, ["file_name", "file_url", "is_private"], as_dict=1)
