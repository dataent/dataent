...
from dataent.utils.minify import JavascriptMinify
import warnings
...
import os, dataent, json, shutil, re
...
def bundle(no_compress, app=None, make_copy=False, restore=False, verbose=False):
	...
	check_yarn()
	dataent.commands.popen(command, cwd=dataent_app_path)
...
def watch(no_compress):
	...
	dataent.commands.popen('{pacman} run watch'.format(pacman=pacman), cwd = dataent_app_path)
					...
					for target, sources in iteritems(json.loads(f.read())):
						...
						for source in sources:
							if isinstance(source, list):
								s = dataent.get_pymodule_path(source[0], *source[1].split("/"))
							else:
...
def html_to_js_template(path, content):
	'''returns HTML template content as Javascript code, adding it to `dataent.templates`'''
	return """dataent.templates["{key}"] = '{content}';\n""".format(\
