...
class LDAPSettings(Document):
	def validate(self):
		...
		if not self.flags.ignore_mandatory:
			if self.ldap_search_string and self.ldap_search_string.endswith("={0}"):
				...
				dataent.throw(_("LDAP Search String needs to end with a placeholder, eg sAMAccountName={0}"))
	...
	def connect_to_ldap(self, base_dn, password):
		try:
			...
			dataent.throw(msg, title=_("LDAP Not Installed"))
		except ldap3.core.exceptions.LDAPInvalidCredentialsResult:
			dataent.throw(_("Invalid username or password"))
		except Exception as ex:
			dataent.throw(_(str(ex)))
	...
	def get_ldap_client_settings():
		...
		if ldap.enabled:
			result["enabled"] = True
			result["method"] = "dataent.integrations.doctype.ldap_settings.ldap_settings.login"
		return result
	...
	def sync_roles(self, user, additional_groups=None):
		...
		all_mapped_roles = {r.epaas_role for r in self.ldap_groups}
		matched_roles = {r.epaas_role for r in self.ldap_groups if r.ldap_group.lower() in lower_groups}
	...
	def create_or_update_user(self, user_data, groups=None):
		user = None
		if dataent.db.exists("User", user_data['email']):
			user = dataent.get_doc("User", user_data['email'])
	...
	def authenticate(self, username, password):
		...
		if not self.enabled:
			dataent.throw(_("LDAP is not enabled."))
