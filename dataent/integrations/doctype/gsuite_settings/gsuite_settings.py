...
class GSuiteSettings(Document):
	...
	def get_access_token(self):
		if not self.refresh_token:
			raise dataent.ValidationError(_("Google GSuite is not configured."))
		data = {
...
def gsuite_callback(code=None):
	...
	if code is None:
		return {
			'url': 'https://accounts.google.com/o/oauth2/v2/auth?access_type=offline&response_type=code&client_id={}&scope={}&redirect_uri={}?cmd=dataent.integrations.doctype.gsuite_settings.gsuite_settings.gsuite_callback'.format(doc.client_id, SCOPES, get_request_site_address(True))
			}
	else:
		try:
			data = {'code': code,
				...
				'redirect_uri': get_request_site_address(True) + '?cmd=dataent.integrations.doctype.gsuite_settings.gsuite_settings.gsuite_callback',
				'grant_type': 'authorization_code'}
			r = requests.post('https://www.googleapis.com/oauth2/v4/token', data=data).json()
			dataent.db.set_value("Gsuite Settings", None, "authorization_code", code)
			if 'refresh_token' in r:
				dataent.db.set_value("Gsuite Settings", None, "refresh_token", r['refresh_token'])
			dataent.db.commit()
			return
...
def run_gsuite_script(option, filename = None, template_id = None, destination_id = None, json_data = None):
	gdoc = dataent.get_doc('GSuite Settings')
	if gdoc.script_url:
		data = {
			'exec': option,
			'filename': filename,
		...
		try:
			...
			dataent.throw(r.text)

		return r
	else:
		dataent.throw(_('Please set script URL on Gsuite Settings'))
