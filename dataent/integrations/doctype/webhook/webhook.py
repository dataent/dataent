...
class Webhook(Document):
	...
	def on_update(self):
		dataent.cache().delete_value('webhooks')

	def validate_docevent(self):
		if self.webhook_doctype:
			is_submittable = dataent.get_value("DocType", self.webhook_doctype, "is_submittable")
			if not is_submittable and self.webhook_docevent in ["on_submit", "on_cancel", "on_update_after_submit"]:
				dataent.throw(_("DocType must be Submittable for the selected Doc Event"))
	...
	def validate_request_url(self):
		try:
			...
			dataent.throw(_("Check Request URL"), exc=e)
	...
	def validate_repeating_fields(self):
		...
		if len(webhook_data)!= len(set(webhook_data)):
			dataent.throw(_("Same Field is entered more than once"))
