...
from dataent.utils import get_url, call_hook_method
from dataent.integrations.utils import create_request_log, create_payment_gateway
...
class BraintreeSettings(Document):
	...
	def validate_transaction_currency(self, currency):
		if currency not in self.supported_currencies:
			dataent.throw(_("Please select another payment method. Stripe does not support transactions in currency '{0}'").format(currency))
	...
	def create_payment_request(self, data):
		...
		try:
			...
			return{
				"redirect_to": dataent.redirect_to_message(_('Server Error'), _("There seems to be an issue with the server's braintree configuration. Don't worry, in case of failure, the amount will get refunded to your account.")),
				"status": 401
			}
	...
	def create_charge_on_braintree(self):
		...
		if result.is_success:
			...
			error_log = dataent.log_error("code: " + str(result.transaction.processor_response_code) + " | text: " + str(result.transaction.processor_response_text), "Braintree Payment Error")
			self.integration_request.db_set('error', error_log.error, update_modified=False)
		else:
			self.integration_request.db_set('status', 'Failed', update_modified=False)
			for error in result.errors.deep_errors:
				error_log = dataent.log_error("code: " + str(error.code) + " | message: " + str(error.message), "Braintree Payment Error")
				self.integration_request.db_set('error', error_log.error, update_modified=False)
		...
		if self.flags.status_changed_to == "Completed":
			...
			if self.data.reference_doctype and self.data.reference_docname:
				...
				try:
					...
					braintree_success_page = dataent.get_hooks('braintree_success_page')
					if braintree_success_page:
						custom_redirect_to = dataent.get_attr(braintree_success_page[-1])(self.data)
				except Exception:
					dataent.log_error(dataent.get_traceback())
...
def get_gateway_controller(doc):
	payment_request = dataent.get_doc("Payment Request", doc)
	gateway_controller = dataent.db.get_value("Payment Gateway", payment_request.payment_gateway, "gateway_controller")
	return gateway_controller

def get_client_token(doc):
	gateway_controller = get_gateway_controller(doc)
	settings = dataent.get_doc("Braintree Settings", gateway_controller)
