...
from dataent.integrations.utils import make_post_request
from rq.timeouts import JobTimeoutException
from dataent.utils import (cint, split_emails, get_request_site_address,
...
class DropboxSettings(Document):
	def onload(self):
		if not self.app_access_key and dataent.conf.dropbox_access_key:
...
def take_backups_if(freq):
	if dataent.db.get_value("Dropbox Settings", None, "backup_frequency") == freq:
...
def update_file_dropbox_status(file_name):
	dataent.db.set_value("File", file_name, 'uploaded_to_dropbox', 1, update_modified=False)

def is_fresh_upload():
	file_name = dataent.db.get_value("File", {'uploaded_to_dropbox': 1}, 'name')
...
def set_dropbox_access_token(access_token):
	dataent.db.set_value("Dropbox Settings", None, 'dropbox_access_token', access_token)
	dataent.db.commit()
