...
from dataent.integrations.utils import create_request_log, make_post_request, create_payment_gateway
from dataent.utils import get_url, call_hook_method, cint, get_datetime


api_path = '/api/method/dataent.integrations.doctype.paypal_settings.paypal_settings'
...
class PayPalSettings(Document):
	...
	def setup_sandbox_env(self, token):
		data = json.loads(dataent.db.get_value("Integration Request", token, "data"))
		setattr(self, "use_sandbox", cint(dataent._dict(data).use_sandbox) or 0)
	...
	def validate_transaction_currency(self, currency):
		if currency not in self.supported_currencies:
			dataent.throw(_("Please select another payment method. PayPal does not support transactions in currency '{0}'").format(currency))
	...
	def get_paypal_params_and_url(self):
		...
		if hasattr(self, "use_sandbox") and self.use_sandbox:
			params.update({
				"USER": dataent.conf.sandbox_api_username,
				"PWD": dataent.conf.sandbox_api_password,
				"SIGNATURE": dataent.conf.sandbox_signature
			})
	...
	def validate_paypal_credentails(self):
		...
		try:
			...
			dataent.throw(_("Invalid payment gateway credentials"))
...
def get_paypal_and_transaction_details(token):
	...
	integration_request = dataent.get_doc("Integration Request", token)
...
def manage_recurring_payment_profile_status(profile_id, action, args, url):
	...
	if response.get("ACK")[0] != "Success" and response.get("L_ERRORCODE0", [])[0] != '11556':
		dataent.throw(_("Failed while amending subscription"))
...
def ipn_handler():
	try:
		...
		dataent.enqueue(method='dataent.integrations.doctype.paypal_settings.paypal_settings.handle_subscription_notification',
