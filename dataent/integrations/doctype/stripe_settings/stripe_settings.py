...
from dataent.utils import get_url, call_hook_method, cint, flt
from dataent.integrations.utils import make_get_request, make_post_request, create_request_log, create_payment_gateway
	...
	def validate_stripe_credentails(self):
		if self.publishable_key and self.secret_key:
			...
			try:
				...
				dataent.throw(_("Seems Publishable Key or Secret Key is wrong !!!"))
	...
	def validate_minimum_transaction_amount(self, currency, amount):
		if currency in self.currency_wise_minimum_charge_amount:
			if flt(amount) < self.currency_wise_minimum_charge_amount.get(currency, 0.0):
				dataent.throw(_("For currency {0}, the minimum transaction amount should be {1}").format(currency,
	...
	def create_charge_on_stripe(self):
		...
		try:
			...
			if charge.captured == True:
				...
				dataent.log_error(charge.failure_message, 'Stripe Payment not completed')
...
def get_gateway_controller(doctype, docname):
	...
	gateway_controller = dataent.db.get_value("Payment Gateway", reference_doc.payment_gateway, "gateway_controller")
