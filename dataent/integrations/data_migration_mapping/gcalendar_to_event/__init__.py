...
from dataent.utils import add_days


def pre_process(events):
	if events["status"] == "cancelled":
		if dataent.db.exists("Event", dict(gcalendar_sync_id=events["id"])):
			e = dataent.get_doc("Event", dict(gcalendar_sync_id=events["id"]))
		...
		default_tz = dataent.db.get_value("System Settings", None, "time_zone")
