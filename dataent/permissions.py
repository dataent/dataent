...
def check_admin_or_system_manager(user=None):
	...
	if ("System Manager" not in dataent.get_roles(user)) and (user!="Administrator"):
		dataent.throw(_("Not permitted"), dataent.PermissionError)

def print_has_permission_check_logs(func):
	def inner(*args, **kwargs):
		dataent.flags['has_permission_check_logs'] = []
		result = func(*args, **kwargs)
		self_perm_check = True if not kwargs.get('user') else kwargs.get('user') == dataent.session.user
		...
		if not result and self_perm_check and raise_exception:
			msgprint(('<br>').join(dataent.flags.get('has_permission_check_logs', [])))
		dataent.flags.pop('has_permission_check_logs', None)
		return result
	return inner
...
def has_permission(doctype, ptype="read", doc=None, verbose=False, user=None, raise_exception=True):
	...
	def false_if_not_shared():
		if ptype in ("read", "write", "share", "email", "print"):
			shared = dataent.share.get_shared(doctype, user,
...
def get_role_permissions(doctype_meta, user=None):
	...
	if isinstance(doctype_meta, string_types):
		doctype_meta = dataent.get_meta(doctype_meta) # assuming doctype name was passed
...
def has_user_permission(doc, user=None):
	...
	apply_strict_user_permissions = dataent.get_system_settings('apply_strict_user_permissions')
...
def has_controller_permissions(doc, ptype, user=None):
	...
	methods = dataent.get_hooks("has_permission").get(doc.doctype, [])

	if not methods:
		return None

	for method in methods:
		controller_permission = dataent.call(dataent.get_attr(method), doc=doc, ptype=ptype, user=user)
...
def get_all_perms(role):
	...
	perms = dataent.get_all('DocPerm', fields='*', filters=dict(role=role))
	custom_perms = dataent.get_all('Custom DocPerm', fields='*', filters=dict(role=role))
	doctypes_with_custom_perms = dataent.db.sql_list("""select distinct parent
...
def get_roles(user=None, with_standard=True):
	...
	def get():
		if user == 'Administrator':
			return [r[0] for r in dataent.db.sql("select name from `tabRole`")] # return all available roles
		else:
			return [r[0] for r in dataent.db.sql("""select role from `tabHas Role`
				where parent=%s and role not in ('All', 'Guest')""", (user,))] + ['All', 'Guest']

	roles = dataent.cache().hget("roles", user, get)
...
def get_perms_for(roles, perm_doctype='DocPerm'):
	...
	return dataent.db.get_all(perm_doctype, fields=['*'], filters=filters)

def get_doctypes_with_custom_docperms():
	'''Returns all the doctypes with Custom Docperms'''

	doctypes = dataent.db.get_all('Custom DocPerm', fields=['parent'], distinct=1)
...
def set_user_permission_if_allowed(doctype, name, user, with_message=False):
	if get_role_permissions(dataent.get_meta(doctype), user).set_user_permissions!=1:
...
def add_user_permission(doctype, name, user, ignore_permissions=False, applicable_for=None):
	'''Add user permission'''
	from dataent.core.doctype.user_permission.user_permission import user_permission_exists

	if not user_permission_exists(user, doctype, name, applicable_for):
		if not dataent.db.exists(doctype, name):
			dataent.throw(_("{0} {1} not found").format(_(doctype), name), dataent.DoesNotExistError)
...
def remove_user_permission(doctype, name, user):
	user_permission_name = dataent.db.get_value('User Permission',
		dict(user=user, allow=doctype, for_value=name))
	dataent.delete_doc('User Permission', user_permission_name)

def clear_user_permissions_for_doctype(doctype, user=None):
	user_permissions_for_doctype = dataent.db.get_list('User Permission',
		dict(user=user, allow=doctype))
	for d in user_permissions_for_doctype:
		dataent.delete_doc('User Permission', d.name)

def can_import(doctype, raise_exception=False):
	if not ("System Manager" in dataent.get_roles() or has_permission(doctype, "import")):
		if raise_exception:
			raise dataent.PermissionError("You are not allowed to import: {doctype}".format(doctype=doctype))
		else:
			return False
	return True

def can_export(doctype, raise_exception=False):
	if not ("System Manager" in dataent.get_roles() or has_permission(doctype, "export")):
		if raise_exception:
			raise dataent.PermissionError("You are not allowed to export: {doctype}".format(doctype=doctype))
		else:
			return False
	return True
...
def update_permission_property(doctype, role, permlevel, ptype, value=None, validate=True):
	...
	from dataent.core.doctype.doctype.doctype import validate_permissions_for_doctype
	out = setup_custom_perms(doctype)

	name = dataent.get_value('Custom DocPerm', dict(parent=doctype, role=role,
		permlevel=permlevel))
...
def setup_custom_perms(parent):
	...
	if not dataent.db.exists('Custom DocPerm', dict(parent=parent)):
		copy_perms(parent)
		return True
...
def add_permission(doctype, role, permlevel=0):
	...
	if dataent.db.get_value('Custom DocPerm', dict(parent=doctype, role=role,
		permlevel=permlevel, if_owner=0)):
		return

	custom_docperm = dataent.get_doc({
...
def copy_perms(parent):
	...
	for d in dataent.get_all('DocPerm', fields='*', filters=dict(parent=parent)):
		custom_perm = dataent.new_doc('Custom DocPerm')
...
def reset_perms(doctype):
	...
	dataent.db.sql("""delete from `tabCustom DocPerm` where parent=%s""", doctype)

def get_linked_doctypes(dt):
	return list(set([dt] + [d.options for d in
		dataent.get_meta(dt).get("fields", {
			"fieldtype":"Link",
...
def push_perm_check_log(log):
	if dataent.flags.get('has_permission_check_logs') == None: return
	dataent.flags.get('has_permission_check_logs').append(_(log))
