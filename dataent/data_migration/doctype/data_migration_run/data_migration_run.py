...
import dataent, json, math
from dataent.model.document import Document
from dataent import _
from dataent.utils import get_source_value, cstr
...
class DataMigrationRun(Document):
	...
	def enqueue_next_mapping(self):
		...
		if next_mapping_name:
			...
			dataent.enqueue_doc(self.doctype, self.name, 'run_current_mapping', now=dataent.flags.in_test)
		else:
			self.complete()
	...
	def enqueue_next_page(self):
		...
		if(percent_complete < 100):
			dataent.publish_realtime(self.trigger_name,
				{"progress_percent": percent_complete}, user=dataent.session.user)
	...
	def get_last_modified_condition(self):
		last_run_timestamp = dataent.db.get_value('Data Migration Run', dict(
	...
	def complete(self):
		...
		dataent.publish_realtime(self.trigger_name,
			{"progress_percent": 100}, user=dataent.session.user)
	...
	def execute_postprocess(self, status):
		...
		if postprocess_method_path:
			dataent.get_attr(postprocess_method_path)({
				"status": status,
				"stats": {
	...
	def get_plan(self):
		if not hasattr(self, 'plan'):
			self.plan = dataent.get_doc('Data Migration Plan', self.data_migration_plan)
		return self.plan
		...
		if hasattr(self, 'mappings'):
			for m in self.mappings:
				if m.name == mapping_name:
					return m
		return dataent.get_doc('Data Migration Mapping', mapping_name)
	...
	def get_data(self, filters):
		...
		data = []
		doclist = dataent.get_all(mapping.local_doctype,
			filters=filters, or_filters=or_filters,
			start=start, page_length=mapping.page_length)

		for d in doclist:
			doc = dataent.get_doc(mapping.local_doctype, d['name'])
			data.append(doc)
		return data

	def get_new_local_data(self):
		'''Fetch newly inserted local data using `dataent.get_all`. Used during Push'''
		mapping = self.get_mapping(self.current_mapping)
	...
	def get_updated_local_data(self):
		'''Fetch local updated data using `dataent.get_all`. Used during Push'''
		mapping = self.get_mapping(self.current_mapping)
		filters = mapping.get_filters() or {}
	...
	def get_deleted_local_data(self):
		'''Fetch local deleted data using `dataent.get_all`. Used during Push'''
		mapping = self.get_mapping(self.current_mapping)
		...
		data = dataent.get_all('Deleted Document', fields=['name', 'data'],
			filters=filters)
	...
	def get_count(self, mapping):
		...
		to_insert = dataent.get_all(mapping.local_doctype, ['count(name) as total'],
			filters=filters, or_filters=or_filters)[0].total

		to_delete = dataent.get_all('Deleted Document', ['count(name) as total'],
	...
	def get_connection(self):
		if not hasattr(self, 'connection'):
			self.connection = dataent.get_doc('Data Migration Connector',
	...
	def _push_insert(self):
		...
		for d in data:
			...
			try:
				...
				dataent.db.set_value(mapping.local_doctype, d.name,
			...
			if migration_id_value:
				try:
					if not local_doc_exists(mapping, migration_id_value):
						...
						# set migration id
						dataent.db.set_value(mapping.local_doctype, local_doc.name,
...
def local_doc_exists(mapping, migration_id_value):
	return dataent.db.exists(mapping.local_doctype, {
		mapping.migration_id_field: migration_id_value
	})
