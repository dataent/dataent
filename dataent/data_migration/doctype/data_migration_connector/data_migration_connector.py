...
from dataent.modules.export_file import create_init_py
from .connectors.base import BaseConnection
from .connectors.dataent_connection import DataentConnection

class DataMigrationConnector(Document):
	def validate(self):
		if not (self.python_module or self.connector_type):
			dataent.throw(_('Enter python module or select connector type'))

		if self.python_module:
			try:
				get_connection_class(self.python_module)
			except:
				dataent.throw(dataent._('Invalid module path'))
	...
	def get_connection(self):
		if self.python_module:
			...
			self.connection = DataentConnection(self)
