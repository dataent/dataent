...
import dataent
from dataent.data_migration.doctype.data_migration_connector.connectors.base import BaseConnection
import googleapiclient.discovery
import google.oauth2.credentials
from googleapiclient.errors import HttpError
import time
from datetime import datetime
from dataent.utils import add_days, add_years
from dataent.desk.doctype.event.event import has_permission

class CalendarConnector(BaseConnection):
	def __init__(self, connector):
		self.connector = connector
		settings = dataent.get_doc("GCalendar Settings", None)

		self.account = dataent.get_doc("GCalendar Account", connector.username)
	...
	def check_remote_calendar(self):
		def _create_calendar():
			timezone = dataent.db.get_value("System Settings", None, "time_zone")
			calendar = {
			...
			try:
				...
				dataent.db.set_value("GCalendar Account", self.account.name, "gcalendar_id", created_calendar["id"])
			except Exception:
	...
	def insert(self, doctype, doc):
		if doctype == 'Events':
			d = dataent.get_doc("Event", doc["name"])
			if has_permission(d, self.account.name):
				try:
					...
					dataent.log_error(dataent.get_traceback(), "GCalendar Synchronization Error")
		...
		while True:
			try:
				...
				if err.resp.status in [410]:
					...
					dataent.log_error(err.resp, "GCalendar Events Fetch Error")
			...
			if not page_token:
				if events.get('nextSyncToken'):
					dataent.db.set_value("GCalendar Account", self.connector.username, "next_sync_token", events.get('nextSyncToken'))
				break
		return list(results)
	...
	def update_events(self, doctype, doc, migration_id):
		try:
			...
			try:
				...
				dataent.log_error(e, "GCalendar Synchronization Error")
		except HttpError as err:
			if err.resp.status in [404]:
				self.insert_events(doctype, doc, migration_id)
			else:
				dataent.log_error(err.resp, "GCalendar Synchronization Error")
	...
	def return_recurrence(self, doctype, doc):
		e = dataent.get_doc(doctype, doc.name)
