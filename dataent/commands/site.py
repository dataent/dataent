...
from dataent.commands.scheduler import _is_scheduler_enabled
from dataent.limits import update_limits, get_limits
from dataent.installer import update_site_config
from dataent.utils import touch_file, get_site_path
...
from dataent.exceptions import SQLError
...
def _new_site(db_name, site, mariadb_root_username=None, mariadb_root_password=None, admin_password=None,
	...
	"""Install a new Dataent site"""

	if not db_name:
		db_name = hashlib.sha1(site.encode()).hexdigest()[:16]

	from dataent.installer import install_db, make_site_dirs
	...
	try:
		...
		apps_to_install = ['dataent'] + (dataent.conf.get("install_apps") or []) + (list(install_apps) or [])
		for app in apps_to_install:
			_install_app(app, verbose=verbose, set_as_patched=not source_sql)

		dataent.utils.scheduler.toggle_scheduler(enable_scheduler)
		dataent.db.commit()

		scheduler_status = "disabled" if dataent.utils.scheduler.is_scheduler_disabled() else "enabled"
		print("*** Scheduler is", scheduler_status, "***")

	except dataent.exceptions.ImproperDBConfigurationError:
...
def restore(context, sql_file_path, mariadb_root_username=None, mariadb_root_password=None, db_name=None, verbose=None, install_app=None, admin_password=None, force=None, with_public_files=None, with_private_files=None):
	...
	dataent.init(site=site)
	_new_site(dataent.conf.db_name, site, mariadb_root_username=mariadb_root_username,
...
def _reinstall(site, admin_password=None, mariadb_root_username=None, mariadb_root_password=None, yes=False, verbose=False):
	...
	dataent.init(site=site)
	_new_site(dataent.conf.db_name, site, verbose=verbose, force=True, reinstall=True, install_apps=installed,
...
def run_patch(context, module):
	...
	for site in context.sites:
		...
		try:
			dataent.connect()
			dataent.modules.patch_handler.run_single(module, force=context.force)
		finally:
			dataent.destroy()
...
def reload_doc(context, module, doctype, docname):
	...
	for site in context.sites:
		try:
			dataent.init(site=site)
			dataent.connect()
			dataent.reload_doc(module, doctype, docname, force=context.force)
			dataent.db.commit()
		finally:
			dataent.destroy()
...
def _drop_site(site, root_login='root', root_password=None, archived_sites_path=None, force=False):
	...
	from dataent.installer import get_root_connection
	...
	if not archived_sites_path:
		archived_sites_path = os.path.join(dataent.get_app_path('dataent'), '..', '..', '..', 'archived_sites')
...
def move(dest_dir, site):
	...
	dataent.init(site)
	old_path = dataent.utils.get_site_path()
...
def publish_realtime(context, event, message, room, user, doctype, docname, after_commit):
	...
	from dataent import publish_realtime
	for site in context.sites:
		try:
			dataent.init(site=site)
			dataent.connect()
