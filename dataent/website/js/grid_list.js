export default function make_datatable(container, doctype) {
	...
	const bind_events = () => {
		...
		$('.btn-delete').on('click', () => {
			...
			if(web_form_names_to_delete.length) {
				dataent.call({
					type:"POST",
					method: "dataent.website.doctype.web_form.web_form.delete_multiple",
					args: {
	...
	return new Promise(resolve => {
		dataent.call({
			method: 'dataent.website.doctype.web_form.web_form.get_in_list_view_fields',
			args: { doctype },
			callback: (r) => {
				const docfields = r.message;

				dataent.call({
					method: 'dataent.www.list.get_list_data',
					args: {
						doctype,
