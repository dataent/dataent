dataent.ui.form.on('Website Settings', {
	...
	set_banner_from_image: function(frm) {
		if (!frm.doc.banner_image) {
			dataent.msgprint(__("Select a Brand Image first."));
		}
	...
	set_parent_label_options: function(frm) {
		dataent.meta.get_docfield("Top Bar Item", "parent_label", frm.docname).options =
	...
	set_parent_options: function(frm, doctype, name) {
		var item = dataent.get_doc(doctype, name);
...
dataent.ui.form.on('Top Bar Item', {
