...
from dataent.utils import today, cint, global_date_format, get_fullname, strip_html_tags, markdown, sanitize_html
from dataent.website.utils import find_first_image, get_comment_list
...
class BlogPost(WebsiteGenerator):
	...
	def make_route(self):
		if not self.route:
			return dataent.db.get_value('Blog Category', self.blog_category,
	...
	def validate(self):
		...
		# update posts
		dataent.db.sql("""update tabBlogger set posts=(select count(*) from `tabBlog Post`
	...
	def get_context(self, context):
		...
		if self.blogger:
			context.blogger_info = dataent.get_doc("Blogger", self.blogger).as_dict()
...
def get_list_context(context=None):
	...
	category = sanitize_html(dataent.local.form_dict.blog_category or dataent.local.form_dict.category)
	if category:
		...
		blogger = dataent.db.get_value("Blogger", {"name": dataent.local.form_dict.blogger}, "full_name")
		list_context.sub_title = _("Posts by {0}").format(blogger)
		list_context.title = blogger

	elif dataent.local.form_dict.txt:
		list_context.sub_title = _('Filtered by "{0}"').format(sanitize_html(dataent.local.form_dict.txt))
	...
	list_context.update(dataent.get_doc("Blog Settings", "Blog Settings").as_dict(no_default_fields=True))
	return list_context

def get_children():
	return dataent.db.sql("""select route as name,
...
def clear_blog_cache():
	for blog in dataent.db.sql_list("""select route from
...
def get_blog_category(route):
	return dataent.db.get_value("Blog Category", {"name": route}, "title") or route

def get_blog_list(doctype, txt=None, filters=None, limit_start=0, limit_page_length=20, order_by=None):
	conditions = []
	if filters:
		if filters.blogger:
			conditions.append('t1.blogger="%s"' % dataent.db.escape(filters.blogger))
		if filters.blog_category:
			conditions.append('t1.blog_category="%s"' % dataent.db.escape(filters.blog_category))

	if txt:
		conditions.append('(t1.content like "%{0}%" or t1.title like "%{0}%")'.format(dataent.db.escape(txt)))
	...
	posts = dataent.db.sql(query, as_dict=1)
	...
	for post in posts:
		...
		post.category = dataent.db.get_value('Blog Category', post.blog_category,
