...
from dataent.utils import get_datetime, now, strip_html
from dataent.utils.jinja import render_template
from dataent.website.doctype.website_slideshow.website_slideshow import get_slideshow
from dataent.website.router import resolve_route
from dataent.website.utils import extract_title, find_first_image, get_comment_list
...
class WebPage(WebsiteGenerator):
	...
	def check_for_redirect(self, context):
		if "<!-- redirect:" in context.main_section:
			dataent.local.flags.redirect_location = \
	...
	def validate_dates(self):
		if self.end_date:
			...
			if self.published and now() > self.end_date:
				...
				dataent.msgprint(_("Clearing end date, as it cannot be in the past for published pages."))


def check_publish_status():
	web_pages = dataent.get_all("Web Page", fields=["name", "published", "start_date", "end_date"])
...
def check_broken_links():
	cnt = 0
	for p in dataent.db.sql("select name, main_section from `tabWeb Page`", as_dict=True):
		for link in re.findall('href=["\']([^"\']*)["\']', p.main_section):
			if link.startswith("http"):
				try:
					res = requests.get(link)
				except requests.exceptions.SSLError:
					res = dataent._dict({"status_code": "SSL Error"})
				except requests.exceptions.ConnectionError:
					res = dataent._dict({"status_code": "Connection Error"})
