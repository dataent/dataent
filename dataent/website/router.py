...
from dataent.website.utils import can_cache, delete_page_cache, extract_title
...
def get_page_context(path):
	page_context = None
	if can_cache():
		page_context_cache = dataent.cache().hget("page_context", path) or {}
		page_context = page_context_cache.get(dataent.local.lang, None)

	if not page_context:
		page_context = make_page_context(path)
		if can_cache(page_context.no_cache):
			page_context_cache[dataent.local.lang] = page_context
			dataent.cache().hset("page_context", path, page_context_cache)
...
def make_page_context(path):
	...
	context.pathname = dataent.local.path
...
def get_page_context_from_doctype(path):
	...
	if page_info:
		return dataent.get_doc(page_info.get("doctype"),
...
def get_all_page_context_from_doctypes():
	...
	routes = dataent.cache().get_value("website_generator_routes")
	if not routes:
		routes = get_page_info_from_doctypes()
		dataent.cache().set_value("website_generator_routes", routes)
...
def get_page_info_from_doctypes(path=None):
	...
	for doctype in get_doctypes_with_web_view():
		...
		try:
			for r in dataent.db.sql("""select route, name, modified from `tab{0}`
...
def get_pages(app=None):
	...
	return dataent.cache().get_value('website_pages', lambda: _build(app))
...
def get_pages_from_path(start, app, app_path):
	...
	if os.path.exists(start_path):
		for basepath, folders, files in os.walk(start_path):
			...
			for fname in files:
				fname = dataent.utils.cstr(fname)
				if not '.' in fname:
					continue
...
def setup_source(page_info):
	...
	if page_info.template.endswith('.md'):
		source = dataent.utils.md_to_html(source)
...
def get_doctypes_with_web_view():
	...
	def _get():
		...
		doctypes += [d.name for d in dataent.get_all('DocType', 'name, module',
			dict(has_web_view=1)) if dataent.local.module_app[dataent.scrub(d.module)] in installed_apps]
		return doctypes

	return dataent.cache().get_value('doctypes_with_web_view', _get)
...
def get_start_folders():
	return dataent.local.flags.web_pages_folders or ('www', 'templates/pages')