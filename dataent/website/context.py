...
from dataent.website.doctype.website_settings.website_settings import get_website_settings
from dataent.website.router import get_page_context
...
def get_context(path, args=None):
	...
	if hasattr(dataent.local, 'request'):
		...
		context["path"] = dataent.local.request.path.strip('/ ')
	else:
		context["path"] = path
	...
	# set using dataent.respond_as_web_page
	if hasattr(dataent.local, 'response') and dataent.local.response.get('context'):
		context.update(dataent.local.response.context)
	...
	if module:
		...
		if hasattr(module, "get_context"):
			try:
				...
				if ret:
					context.update(ret)
			except (dataent.PermissionError, dataent.DoesNotExistError, dataent.Redirect):
				raise
			except:
...
def build_context(context):
	...
	context.update(dataent.local.conf.get("website_context") or {})
	...
	if context.doc:
		...
		context_controller_hooks = dataent.get_hooks("extend_website_page_controller_context") or {}
	...
	if not context.base_template_path:
		app_base = dataent.get_hooks("base_template")
...
def add_sidebar_and_breadcrumbs(context):
	...
	from dataent.website.router import get_page_info_from_template
	if context.show_sidebar:
		context.no_cache = 1
		add_sidebar_data(context)
	else:
		if context.basepath:
			hooks = dataent.get_hooks('look_for_sidebar_json')
...
def add_sidebar_data(context):
	...
	if context.show_sidebar and context.website_sidebar:
		context.sidebar_items = dataent.get_all('Website Sidebar Item',
			filters=dict(parent=context.website_sidebar), fields=['title', 'route', '`group`'],
			order_by='idx asc')

	if not context.sidebar_items:
		sidebar_items = dataent.cache().hget('portal_menu_items', dataent.session.user)
		if sidebar_items == None:
			sidebar_items = []
			roles = dataent.get_roles()
			portal_settings = dataent.get_doc('Portal Settings', 'Portal Settings')
			...
			dataent.cache().hset('portal_menu_items', dataent.session.user, sidebar_items)
...
def add_metatags(context):
	...
	if tags:
		...
		if tags.get("image"):
			tags["og:image"] = tags["twitter:image:src"] = tags["image"] = dataent.utils.get_url(tags.get("image"))
