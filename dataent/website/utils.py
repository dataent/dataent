...
def can_cache(no_cache=False):
	if dataent.conf.disable_website_cache or dataent.conf.developer_mode:
		return False
	if getattr(dataent.local, "no_cache", False):
		return False
	return not no_cache
...
def get_home_page():
	if dataent.local.flags.home_page:
		return dataent.local.flags.home_page

	def _get_home_page():
		home_page = None

		get_website_user_home_page = dataent.get_hooks('get_website_user_home_page')
		if get_website_user_home_page:
			home_page = dataent.get_attr(get_website_user_home_page[-1])(dataent.session.user)
		...
		if not home_page:
			home_page = dataent.db.get_value("Website Settings", None, "home_page") or "login"

		home_page = home_page.strip('/')

		return home_page

	return dataent.cache().hget("home_page", dataent.session.user, _get_home_page)

def is_signup_enabled():
	if getattr(dataent.local, "is_signup_enabled", None) is None:
		dataent.local.is_signup_enabled = True
		if dataent.utils.cint(dataent.db.get_value("Website Settings",
			"Website Settings", "disable_signup")):
				dataent.local.is_signup_enabled = False

	return dataent.local.is_signup_enabled
...
def get_next_link(route, url_prefix=None, app=None):
	...
	if next_item:
		if next_item.route and next_item.title:
			html = ('<p class="btn-next-wrapper">' + dataent._("Next")\
...
def get_full_index(route=None, app=None):
	...
	from dataent.website.router import get_pages

	if not dataent.local.flags.children_map:
		def _build():
			children_map = {}
			added = []
		...
		children_map = dataent.cache().get_value('website_full_index', _build)

		dataent.local.flags.children_map = children_map

	return dataent.local.flags.children_map
