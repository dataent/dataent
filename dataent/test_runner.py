...
import xmlrunner
import importlib
from dataent.modules import load_doctype_module, get_module_name
...
def run_tests_for_doctype(doctypes, verbose=False, tests=(), force=False, profile=False):
	...
	for doctype in doctypes:
		...
		if force:
			for name in dataent.db.sql_list("select name from `tab%s`" % doctype):
				dataent.delete_doc(doctype, name, force=True)
...
def run_setup_wizard_ui_test(app=None, verbose=False, profile=False):
	...
	dataent.flags.run_setup_wizard_ui_test = 1
...
def run_ui_tests(app=None, test=None, test_list=None, verbose=False, profile=False):
	...
	module = importlib.import_module('dataent.tests.ui.test_test_runner')
	dataent.flags.ui_test_app = app
	if test_list:
		dataent.flags.ui_test_list = test_list
	else:
		dataent.flags.ui_test_path = test
...
def _run_unittest(modules, verbose=False, tests=(), profile=False):
	...
	dataent.flags.tests_verbose = verbose
...
def _add_test(app, path, filename, verbose, test_suite=None, ui_tests=False):
	...
	app_path = dataent.get_pymodule_path(app)
...
def make_test_records(doctype, verbose=0, force=False):
	...
	if dataent.flags.skip_test_records:
		return
	...
	for options in get_dependencies(doctype):
		...
		if not options in dataent.local.test_objects:
			dataent.local.test_objects[options] = []
...
def get_dependencies(doctype):
	...
	for df in meta.get_table_fields():
		link_fields.extend(dataent.get_meta(df.options).get_link_fields())
...
def add_to_test_record_log(doctype):
	...
	if not doctype in test_record_log:
		dataent.flags.test_record_log.append(doctype)
...
def get_test_record_log():
	...
	if 'test_record_log' not in dataent.flags:
		if os.path.exists(dataent.get_site_path('.test_log')):
			with open(dataent.get_site_path('.test_log'), 'r') as f:
				dataent.flags.test_record_log = f.read().splitlines()
		else:
			dataent.flags.test_record_log = []

	return dataent.flags.test_record_log