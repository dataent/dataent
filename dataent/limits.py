...
from dataent.utils import now_datetime, getdate, flt, cint, get_fullname
from dataent.installer import update_site_config
from dataent.utils.data import formatdate
from dataent.utils.user import get_enabled_system_users, disable_users
from dataent.utils.__init__ import get_site_info
import os, subprocess, json
from six.moves.urllib.parse import parse_qsl, urlsplit, urlunsplit, urlencode
from six import string_types

class SiteExpiredError(dataent.ValidationError):
...
def check_if_expired():
	...
	dataent.throw(message, SiteExpiredError)
...
def get_upgrade_url(upgrade_url):
	...
	params.update({
		...
		'country': dataent.db.get_value("System Settings", "System Settings", 'country')
	})
...
def get_limits():
	...
	'''
	return dataent._dict(dataent.local.conf.limits or {})
...
def validate_space_limit(file_size):
	...
	# in MB
	usage = dataent._dict(limits.space_usage or {})
	if not usage:
		# first time
		usage = dataent._dict(update_space_usage())

	file_size = file_size / (1024.0 ** 2)

	if flt(flt(usage.total) + file_size, 2) > space_limit:
		# Stop from attaching file
		dataent.throw(_("You have exceeded the max space of {0} for your plan. {1}.").format(
	...
	# update files size in dataent subscription
	usage.files_size = flt(usage.files_size) + file_size
	update_limits({ 'space_usage': usage })

def update_space_usage():
	# public and private files
	files_size = get_folder_size(dataent.get_site_path("public", "files"))
	files_size += get_folder_size(dataent.get_site_path("private", "files"))

	backup_size = get_folder_size(dataent.get_site_path("private", "backups"))
...
def update_site_usage():
	data = get_site_info()
	# exists = os.path.isfile(dataent.get_site_path("site_data.json"))
	with open(os.path.join(dataent.get_site_path(), 'site_data.json'), 'w') as outfile:
