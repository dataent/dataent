...
from dataent.utils import (cint, flt, now, cstr, strip_html,
	sanitize_html, sanitize_email, cast_fieldtype)
from dataent.model import default_fields
from dataent.model.naming import set_new_name
from dataent.model.utils.link_count import notify_link_count
from dataent.modules import load_doctype_module
from dataent.model import display_fieldtypes
from dataent.model.db_schema import type_map, varchar_len
from dataent.utils.password import get_decrypted_password, set_encrypted_password

_classes = {}

def get_controller(doctype):
	"""Returns the **class** object of the given DocType.
	For `custom` type, returns `dataent.model.document.Document`.
	...
	if not doctype in _classes:
		module_name, custom = dataent.db.get_value("DocType", doctype, ("module", "custom"), cache=True) \
			or ["Core", False]
...
class BaseDocument(object):
	...
	def meta(self):
		if not hasattr(self, "_meta"):
			self._meta = dataent.get_meta(self.doctype)
	...
	def get_db_value(self, key):
		return dataent.db.get_value(self.doctype, self.name, key)
		...
		for fieldname in self.meta.get_valid_columns():
			...
			if df:
				...
				if isinstance(d[fieldname], list) and df.fieldtype != 'Table':
					dataent.throw(_('Value for {0} cannot be a list').format(_(df.label)))
	...
	def get_valid_columns(self):
		if self.doctype not in dataent.local.valid_columns:
			...
			dataent.local.valid_columns[self.doctype] = valid

		return dataent.local.valid_columns[self.doctype]
	...
	def db_insert(self):
		...
		columns = list(d)
		try:
			dataent.db.sql("""insert into `tab{doctype}`
			...
			if e.args[0]==1062:
				if "PRIMARY" in cstr(e.args[1]):
					...
					raise dataent.DuplicateEntryError(self.doctype, self.name, e)
	...
	def show_unique_validation_message(self, e):
		...
		# created using dataent.db.add_unique
		...
		dataent.msgprint(_("{0} must be unique".format(label or fieldname)))

		# this is used to preserve traceback
		raise dataent.UniqueValidationError(self.doctype, self.name, e)

	def update_modified(self):
		'''Update modified timestamp'''
		self.set("modified", now())
		dataent.db.set_value(self.doctype, self.name, 'modified', self.modified, update_modified=False)
	...
	def _get_missing_mandatory_fields(self):
		...
		def get_msg(df):
			if df.fieldtype == "Table":
				...
				return "{}: {} {} #{}: {}: {}".format(_("Error"), dataent.bold(_(self.doctype)),
		...
		if self.meta.istable:
			for fieldname in ("parent", "parenttype"):
				if not self.get(fieldname):
					missing.append((fieldname, get_msg(dataent._dict(label=fieldname))))
		...
		for df in (self.meta.get_link_fields()
			...
			if docname:
				if df.fieldtype=="Link":
					doctype = df.options
					if not doctype:
						dataent.throw(_("Options not set for link field {0}").format(df.fieldname))
				else:
					doctype = self.get(df.options)
					if not doctype:
						dataent.throw(_("{0} must be set first").format(self.meta.get_label(df.options)))
				...
				if not fields_to_fetch:
					...
					values = dataent._dict(name=dataent.db.get_value(doctype, docname,
						'name', cache=True))
				else:
					...
					values = dataent.db.get_value(doctype, docname,
						values_to_fetch, as_dict=True)

				if dataent.get_meta(doctype).issingle:
					values.name = doctype
				...
				if values:
					...
					if not values.name:
						...
						and (is_submittable or self.meta.is_submittable) and dataent.get_meta(doctype).is_submittable
						and cint(dataent.db.get_value(doctype, docname, "docstatus"))==2):
	...
	def _validate_selects(self):
		...
		for df in self.meta.get_select_fields():
			...
			if value not in options and not (dataent.flags.in_test and value.startswith("_T-")):
				# show an elaborate message
				prefix = _("Row #{0}:").format(self.idx) if self.get("parentfield") else ""
				label = _(self.meta.get_label(df.fieldname))
				comma_options = '", "'.join(_(each) for each in options)

				dataent.throw(_('{0} {1} cannot be "{2}". It should be one of "{3}"').format(prefix, label,
					value, comma_options))

	def _validate_constants(self):
		if dataent.flags.in_import or self.is_new() or self.flags.ignore_validate_constants:
			return

		constants = [d.fieldname for d in self.meta.get("fields", {"set_only_once": ('=',1)})]
		if constants:
			values = dataent.db.get_value(self.doctype, self.name, constants, as_dict=True)
		...
		for fieldname in constants:
			...
			if self.get(fieldname) != value:
				dataent.throw(_("Value cannot be changed for {0}").format(self.meta.get_label(fieldname)),
	...
	def _validate_update_after_submit(self):
		...
		db_values = dataent.get_doc(self.doctype, self.name).as_dict()
		...
		for key in self.as_dict():
			...
			if df and not df.allow_on_submit and (self.get(key) or db_value):
				...
				if self_value != db_value:
					dataent.throw(_("Not allowed to change {0} after submission").format(df.label),
						dataent.UpdateAfterSubmitError)
	...
	def get_formatted(self, fieldname, doc=None, currency=None, absolute_value=False, translated=False):
		...
		if not df and fieldname in default_fields:
			from dataent.model.meta import get_default_df
	...
	def reset_values_if_no_permlevel_access(self, has_access_to, high_permlevel_fields):
		...
		if to_reset:
			if self.is_new():
				...
				ref_doc = dataent.new_doc(self.doctype)
			else:
	...
	def _extract_images_from_text_editor(self):
		from dataent.utils.file_manager import extract_images_from_doc
...
def _filter(data, filters, limit=None):
	...
	for d in data:
		...
		for f, fval in iteritems(_filters):
			if not dataent.compare(getattr(d, f, None), fval[0], fval[1]):
				add = False
				break
