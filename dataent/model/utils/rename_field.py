...
from dataent.utils.password import rename_password_field
from dataent.model.utils.user_settings import update_user_settings_data, sync_user_settings


def rename_field(doctype, old_fieldname, new_fieldname):
	"""This functions assumes that doctype is already synced"""

	meta = dataent.get_meta(doctype, cached=False)
...
def update_reports(doctype, old_fieldname, new_fieldname):
	...
	reports = dataent.db.sql("""select name, ref_doctype, json from tabReport
	...
	for r in reports:
		...
		if report_dict.get("updated"):
			...
			dataent.db.sql("""update `tabReport` set `json`=%s where name=%s""", (new_val, r.name))

def update_users_report_view_settings(doctype, ref_fieldname, new_fieldname):
	user_report_cols = dataent.db.sql("""select defkey, defvalue from `tabDefaultValue` where
...
def update_property_setters(doctype, old_fieldname, new_fieldname):
	dataent.db.sql("""update `tabProperty Setter` set field_name = %s
		where doc_type=%s and field_name=%s""", (new_fieldname, doctype, old_fieldname))

	dataent.db.sql('''update `tabCustom Field` set insert_after=%s
...
def update_user_settings(doctype, old_fieldname, new_fieldname):
	...
	user_settings = dataent.db.sql(''' select user, doctype, data from `__UserSettings`
