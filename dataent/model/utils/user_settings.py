...
from dataent import safe_decode
...
def get_user_settings(doctype, for_update=False):
	user_settings = dataent.cache().hget('_user_settings',
		'{0}::{1}'.format(doctype, dataent.session.user))

	if user_settings is None:
		user_settings = dataent.db.sql('''select data from __UserSettings
			where user=%s and doctype=%s''', (dataent.session.user, doctype))
...
def update_user_settings(doctype, user_settings, for_update=False):
	...
	dataent.cache().hset('_user_settings', '{0}::{1}'.format(doctype, dataent.session.user),
		json.dumps(current))
...
def sync_user_settings():
	...
	for key, data in iteritems(dataent.cache().hgetall('_user_settings')):
		...
		dataent.db.sql('''insert into __UserSettings (user, doctype, data) values (%s, %s, %s)
			...
			if view_settings and view_settings.get("filters"):
				...
				for view_filter in view_filters:
					...
					if view_filter[filter_dict[fieldname]] == old:
						...
						update = True
		if update:
			dataent.db.sql("update __UserSettings set data=%s where doctype=%s and user=%s",
				(json.dumps(data), user_setting.doctype, user_setting.user))

			# clear that user settings from the redis cache
			dataent.cache().hset('_user_settings', '{0}::{1}'.format(user_setting.doctype,
