...
def set_default(doc, key):
	...
	if not doc.is_default:
		dataent.db.set(doc, "is_default", 1)

	dataent.db.sql("""update `tab%s` set `is_default`=0
...
def set_field_property(filters, key, value):
	...
	docs = [dataent.get_doc('DocType', d.parent) for d in \
		dataent.get_all("DocField", fields=['parent'], filters=filters)]
...
class InvalidIncludePath(dataent.ValidationError): pass
	...
	for i in range(5):
		if "{% include" in content:
			...
			for path in paths:
				...
				with io.open(dataent.get_app_path(app, app_path), 'r', encoding = 'utf-8') as f:
					include = f.read()
...
def get_fetch_values(doctype, fieldname, value):
	...
	for df in meta.get_fields_to_fetch(fieldname):
		...
		out[df.fieldname] = dataent.db.get_value(link_df.options, value, source_fieldname)
