...
import dataent, json, copy, re
import dataent.defaults
import dataent.share
import dataent.permissions
from dataent.utils import flt, cint, getdate, get_datetime, get_time, make_filter_tuple, get_filter, add_to_date
from dataent import _
from dataent.model import optional_fields
from dataent.client import check_parent_permission
from dataent.model.utils.user_settings import get_user_settings, update_user_settings
...
class DatabaseQuery(object):
	...
	def execute(self, query=None, fields=None, filters=None, or_filters=None,
		...
		if not ignore_permissions and not dataent.has_permission(self.doctype, "read", user=user):
			dataent.flags.error_message = _('Insufficient Permission for {0}').format(dataent.bold(self.doctype))
	...
	def build_and_run(self):
		...
		return dataent.db.sql(query, as_dict=not self.as_list, debug=self.debug, update=self.update)
	...
	def sanitize_fields(self):
		...
		def _raise_exception():
			dataent.throw(_('Use of sub-query or function is restricted'), dataent.DataError)
	...
	def append_table(self, table_name):
		...
		if (not self.flags.ignore_permissions) and (not dataent.has_permission(doctype)):
			dataent.flags.error_message = _('Insufficient Permission for {0}').format(dataent.bold(doctype))
	...
	def set_optional_columns(self):
		...
		columns = dataent.db.get_table_columns(self.doctype)
	...
	def prepare_filter_condition(self, f):
		...
		if f.operator.lower() in ('ancestors of', 'descendants of', 'not ancestors of', 'not descendants of'):
			...
			if dataent.get_meta(f.doctype).get_field(f.fieldname) is not None :
				ref_doctype = dataent.get_meta(f.doctype).get_field(f.fieldname).options

			result=[]
			lft, rgt = dataent.db.get_value(ref_doctype, f.value, ["lft", "rgt"])

			# Get descendants elements of a DocType with a tree structure
			if f.operator.lower() in ('descendants of', 'not descendants of') :
				result = dataent.db.sql_list("""select name from `tab{0}`
			...
			fallback = "''"
			value = (dataent.db.escape((v or '').strip(), percent=False) for v in result)
			...
			values = f.value or ''
			if isinstance(values, dataent.string_types):
				values = values.split(",")

			fallback = "''"
			value = (dataent.db.escape((v or '').strip(), percent=False) for v in values)
			value = '("{0}")'.format('", "'.join(value))
		else:
			df = dataent.get_meta(f.doctype).get("fields", {"fieldname": f.fieldname})
			...
			if isinstance(value, string_types) and not f.operator.lower() == 'between':
				value = '"{0}"'.format(dataent.db.escape(value, percent=False))
	...
	def build_match_conditions(self, as_condition=True):
		...
		if not self.user:
			self.user = dataent.session.user

		if not self.tables: self.extract_tables()

		meta = dataent.get_meta(self.doctype)
		role_permissions = dataent.permissions.get_role_permissions(meta, user=self.user)
		self.shared = dataent.share.get_shared(self.doctype, self.user)
		...
		if (not meta.istable and
			...
			only_if_shared = True
			if not self.shared:
				dataent.throw(_("No permission to read {0}").format(self.doctype), dataent.PermissionError)
			else:
			...
			if role_permissions.get("if_owner", {}).get("read"): #if has if_owner permission skip user perm check
				self.match_conditions.append("`tab{0}`.owner = '{1}'".format(self.doctype,
					dataent.db.escape(self.user, percent=False)))
			elif role_permissions.get("read"): # add user permission only if role has read perm
				# get user permissions
				user_permissions = dataent.permissions.get_user_permissions(self.user)
	...
	def get_share_condition(self):
		return """`tab{0}`.name in ({1})""".format(self.doctype, ", ".join(["'%s'"] * len(self.shared))) % \
			tuple([dataent.db.escape(s, percent=False) for s in self.shared])
	...
	def add_user_permissions(self, user_permissions):
		...
		for df in doctype_link_fields:
			...
			if user_permission_values:
				docs = []
				if dataent.get_system_settings("apply_strict_user_permissions"):
					condition = ""
				else:
				...
				if docs:
					condition += "`tab{doctype}`.`{fieldname}` in ({values})".format(
						...
						values=", ".join(
							[('"' + dataent.db.escape(doc, percent=False) + '"') for doc in docs])
						)
	...
	def get_permission_query_conditions(self):
		condition_methods = dataent.get_hooks("permission_query_conditions", {}).get(self.doctype, [])
		if condition_methods:
			conditions = []
			for method in condition_methods:
				c = dataent.call(dataent.get_attr(method), self.user)
				if c:
					conditions.append(c)
	...
	def run_custom_query(self, query):
		...
		return dataent.db.sql(query, as_dict = (not self.as_list))
	...
	def validate_order_by_and_group_by(self, parameters):
		...
		if 'select' in _lower and ' from ' in _lower:
			dataent.throw(_('Cannot use sub-query in order by'))
		...
		for field in parameters.split(","):
			if "." in field and field.strip().startswith("`tab"):
				...
				if tbl not in self.tables:
					if tbl.startswith('`'):
						tbl = tbl[4:-1]
					dataent.throw(_("Please select atleast 1 column from {0} to sort/group").format(tbl))
...
def get_list(doctype, *args, **kwargs):
	...
	if dataent.is_table(doctype):
		...
		if not kwargs.get('parent'):
			dataent.flags.error_message = _('Parent is required to get child table data')
			raise dataent.PermissionError(doctype)
...
def has_any_user_permission_for_doctype(doctype, user, applicable_for):
	user_permissions = dataent.permissions.get_user_permissions(user=user)
