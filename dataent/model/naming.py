...
from dataent.utils import now_datetime, cint, cstr
import re
...
def set_new_name(doc):
	...
	autoname = dataent.get_meta(doc.doctype).autoname or ""

	if autoname.lower() != "prompt" and not dataent.flags.in_import:
		doc.name = None
	...
	if autoname.startswith('field:') and not doc.name:
		...
		dataent.throw(_("{0} is required").format(doc.meta.get_label(fieldname)))
	...
	doc.name = validate_name(
		doc.doctype,
		doc.name,
		dataent.get_meta(doc.doctype).get_field("name_case")
	)
...
def set_name_by_naming_series(doc):
	...
	if not doc.naming_series:
		dataent.throw(dataent._("Naming Series mandatory"))
...
def make_autoname(key='', doctype='', doc=''):
   ...
   *Example:*
	...
	"""
	if key == "hash":
		return dataent.generate_hash(doctype, 10)

	if "#" not in key:
		key = key + ".#####"
	elif "." not in key:
		dataent.throw(_("Invalid naming series (. missing)") + (_(" for {0}").format(doctype) if doctype else ""))
...
def getseries(key, digits, doctype=''):
	# series created ?
	current = dataent.db.sql("select `current` from `tabSeries` where name=%s for update", (key,))
	if current and current[0][0] is not None:
		current = current[0][0]
		# yes, update it
		dataent.db.sql("update tabSeries set current = current+1 where name=%s", (key,))
		current = cint(current) + 1
	else:
		# no, create it
		dataent.db.sql("insert into tabSeries (name, current) values (%s, 1)", (key,))
		current = 1
...
def revert_series_if_last(key, name):
	...
	current = dataent.db.sql("select `current` from `tabSeries` where name=%s for update", (prefix,))

	if current and current[0][0]==count:
		dataent.db.sql("update tabSeries set current=current-1 where name=%s", prefix)


def get_default_naming_series(doctype):
	"""get default value for `naming_series` property"""
	naming_series = dataent.get_meta(doctype).get_field("naming_series").options or ""
	if naming_series:
...
def validate_name(doctype, name, case=None, merge=False):
	...
	if name.startswith('New '+doctype):
		dataent.throw(_('There were some errors setting the name, please contact the administrator'), dataent.NameError)
	if case == 'Title Case':
		name = name.title()
	if case == 'UPPER CASE':
		name = name.upper()
	...
	if not dataent.get_meta(doctype).get("issingle") and (doctype == name) and (name != "DocType"):
		dataent.throw(_("Name of {0} cannot be {1}").format(doctype, name), dataent.NameError)

	special_characters = "<>"
	if re.findall("[{0}]+".format(special_characters), name):
		message = ", ".join("'{0}'".format(c) for c in special_characters)
		dataent.throw(_("Name cannot contain special characters like {0}").format(message), dataent.NameError)
...
def append_number_if_name_exists(doctype, value, fieldname='name', separator='-', filters=None):
	...
	exists = dataent.db.exists(doctype, filters)

	regex = '^{value}{separator}\d+$'.format(value=re.escape(value), separator=separator)

	if exists:
		last = dataent.db.sql("""select {fieldname} from `tab{doctype}`
...
def _set_amended_name(doc):
	...
	if dataent.db.get_value(doc.doctype, doc.amended_from, "amended_from"):
...
def _prompt_autoname(autoname, doc):
	...
	if not doc.name:
		dataent.throw(_("Name not set via prompt"))
