...
def get_dynamic_link_map(for_delete=False):
	...
	'''
	if getattr(dataent.local, 'dynamic_link_map', None)==None or dataent.flags.in_test:
		# Build from scratch
		dynamic_link_map = {}
		for df in get_dynamic_links():
			...
			if meta.issingle:
				...
				links = dataent.db.sql_list("""select distinct {options} from `tab{parent}`""".format(**df))
				for doctype in links:
					dynamic_link_map.setdefault(doctype, []).append(df)

		dataent.local.dynamic_link_map = dynamic_link_map
	return dataent.local.dynamic_link_map
...
def get_dynamic_links():
	...
	for query in dynamic_link_queries:
		df += dataent.db.sql(query, as_dict=True)
	return df
