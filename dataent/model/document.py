...
import dataent
import time
from dataent import _, msgprint
from dataent.utils import flt, cstr, now, get_datetime_str, file_lock
from dataent.utils.background_jobs import enqueue
from dataent.model.base_document import BaseDocument, get_controller
from dataent.model.naming import set_new_name
from six import iteritems, string_types
from werkzeug.exceptions import NotFound, Forbidden
import hashlib, json
from dataent.model import optional_fields
from dataent.model.workflow import validate_workflow
from dataent.utils.file_manager import save_url
from dataent.utils.global_search import update_global_search
from dataent.integrations.doctype.webhook import run_webhooks

# once_only validation
# methods

def get_doc(*args, **kwargs):
	"""returns a dataent.model.Document object.
...
class Document(BaseDocument):
		...
		if args and args[0] and isinstance(args[0], string_types):
			...
			if len(args)==1:
				...
				if isinstance(args[1], dict):
					# filter
					self.name = dataent.db.get_value(args[0], args[1], "name")
					if self.name is None:
						dataent.throw(_("{0} {1} not found").format(_(args[0]), args[1]),
							dataent.DoesNotExistError)
				else:
					self.name = args[1]
	...
	def load_from_db(self):
		...
		if not getattr(self, "_metaclass", False) and self.meta.issingle:
			single_doc = dataent.db.get_singles_dict(self.doctype)
			if not single_doc:
				single_doc = dataent.new_doc(self.doctype).as_dict()
			...
			d = dataent.db.get_value(self.doctype, self.name, "*", as_dict=1)
			if not d:
				dataent.throw(_("{0} {1} not found").format(_(self.doctype), self.name), dataent.DoesNotExistError)

			super(Document, self).__init__(d)

		if self.name=="DocType" and self.doctype=="DocType":
			from dataent.model.meta import doctype_table_fields
			table_fields = doctype_table_fields
		else:
			table_fields = self.meta.get_table_fields()

		for df in table_fields:
			children = dataent.db.get_values(df.options,
	...
	def get_latest(self):
		if not getattr(self, "latest", None):
			self.latest = dataent.get_doc(self.doctype, self.name)
		return self.latest

	def check_permission(self, permtype='read', permlevel=None):
		"""Raise `dataent.PermissionError` if not permitted"""
		if not self.has_permission(permtype):
			self.raise_no_permission_to(permlevel or permtype)

	def has_permission(self, permtype="read", verbose=False):
		"""Call `dataent.has_permission` if `self.flags.ignore_permissions`
		is not set.

		:param permtype: one of `read`, `write`, `submit`, `cancel`, `delete`"""
		if self.flags.ignore_permissions:
			return True
		return dataent.has_permission(self.doctype, permtype, self, verbose=verbose)

	def raise_no_permission_to(self, perm_type):
		"""Raise `dataent.PermissionError`."""
		dataent.flags.error_message = _('Insufficient Permission for {0}').format(self.doctype)
		raise dataent.PermissionError
	...
	def update_child_table(self, fieldname, df=None):
		...
		if rows:
			...
			deleted_rows = dataent.db.sql("""select name from `tab{0}` where parent=%s
				and parenttype=%s and parentfield=%s
				and name not in ({1})""".format(df.options, ','.join(['%s'] * len(rows))),
					[self.name, self.doctype, fieldname] + rows)
			if len(deleted_rows) > 0:
				# delete rows that do not match the ones in the document
				dataent.db.sql("""delete from `tab{0}` where name in ({1})""".format(df.options,
					','.join(['%s'] * len(deleted_rows))), tuple(row[0] for row in deleted_rows))

		else:
			# no rows found, delete all rows
			dataent.db.sql("""delete from `tab{0}` where parent=%s
	...
	def get_doc_before_save(self):
		if not getattr(self, '_doc_before_save', None):
			try:
				self._doc_before_save = dataent.get_doc(self.doctype, self.name)
			except dataent.DoesNotExistError:
				self._doc_before_save = None
				dataent.clear_last_message()
		return self._doc_before_save

	def set_new_name(self, force=False):
		"""Calls `dataent.naming.se_new_name` for parent and child docs."""
	...
	def update_single(self, d):
		...
		dataent.db.sql("""delete from tabSingles where doctype=%s""", self.doctype)
		for field, value in iteritems(d):
			if field != "doctype":
				dataent.db.sql("""insert into tabSingles(doctype, field, value)
					values (%s, %s, %s)""", (self.doctype, field, value))

		if self.doctype in dataent.db.value_cache:
			del dataent.db.value_cache[self.doctype]
	...
	def validate_set_only_once(self):
		...
		if set_only_once_fields and self._doc_before_save:
			...
			for field in set_only_once_fields:
				...
				if fail:
					dataent.throw(_("Value cannot be changed for {0}").format(self.meta.get_label(field.fieldname)),
						dataent.CannotChangeConstantError)
	...
	def apply_fieldlevel_read_permissions(self):
		...
		for table_field in self.meta.get_table_fields():
			for df in dataent.get_meta(table_field.options).fields or []:
				if df.permlevel and not df.permlevel in has_access_to:
	...
	def validate_higher_perm_levels(self):
		...
		if self.flags.ignore_permissions or dataent.flags.in_install:
			return
		...
		for df in self.meta.get_table_fields():
			high_permlevel_fields = dataent.get_meta(df.options).meta.get_high_permlevel_fields()
			if high_permlevel_fields:
				for d in self.get(df.fieldname):
	...
	def get_permissions(self):
		if self.meta.istable:
			...
			permissions = dataent.get_meta(self.parenttype).permissions
		else:
	...
	def check_if_latest(self):
		...
		if not self.get('__islocal'):
			...
			if conflict:
				dataent.msgprint(_("Error: Document has been modified after you have opened it") \
				+ (" (%s, %s). " % (modified, self.modified)) \
				+ _("Please refresh to get the latest document."),
					raise_exception=dataent.TimestampMismatchError)
		else:
	...
	def check_docstatus_transition(self, docstatus):
		...
		if docstatus==0:
			if self.docstatus==0:
				...
				raise dataent.DocstatusTransitionError(_("Cannot change docstatus from 0 to 2"))
			...
			if self.docstatus==1:
				...
				raise dataent.DocstatusTransitionError(_("Cannot change docstatus from 1 to 0"))
	...
	def _validate_mandatory(self):
		...
		if dataent.flags.print_messages:
			print(self.as_json().encode("utf-8"))

		raise dataent.MandatoryError('[{doctype}, {name}]: {fields}'.format(
	...
	def _validate_links(self):
		...
		if invalid_links:
			...
			dataent.throw(_("Could not find {0}").format(msg),
				dataent.LinkValidationError)

		if cancelled_links:
			msg = ", ".join((each[2] for each in cancelled_links))
			dataent.throw(_("Cannot link cancelled document: {0}").format(msg),
				dataent.CancelledLinkError)
	...
	def run_notifications(self, method):
		...
		from dataent.email.doctype.notification.notification import evaluate_alert

		if self.flags.notifications == None:
			alerts = dataent.cache().hget('notifications', self.doctype)
			if alerts==None:
				alerts = dataent.get_all('Notification', fields=['name', 'event', 'method'],
					filters={'enabled': 1, 'document_type': self.doctype})
				dataent.cache().hset('notifications', self.doctype, alerts)
	...
	def delete(self):
		"""Delete document."""
		dataent.delete_doc(self.doctype, self.name, flags=self.flags)
	...
	def run_post_save_methods(self):
		...
		if (self.doctype, self.name) in dataent.flags.currently_saving:
			dataent.flags.currently_saving.remove((self.doctype, self.name))

		self.latest = None

	def clear_cache(self):
		dataent.clear_document_cache(self.doctype, self.name)

	def reset_seen(self):
		'''Clear _seen property and set current user as seen'''
		if getattr(self.meta, 'track_seen', False):
			self._seen = json.dumps([dataent.session.user])

	def notify_update(self):
		"""Publish realtime that the current document is modified"""
		dataent.publish_realtime("doc_update", {"modified": self.modified, "doctype": self.doctype, "name": self.name},
		...
		if not self.meta.get("read_only") and not self.meta.get("issingle") and \
			...
			dataent.publish_realtime("list_update", data, after_commit=True)
	...
	def db_set(self, fieldname, value=None, update_modified=True, notify=False, commit=False):
		...
		if update_modified and (self.doctype, self.name) not in dataent.flags.currently_saving:
		...
		dataent.db.set_value(self.doctype, self.name, fieldname, value,
	...
	def db_get(self, fieldname):
		...
		return dataent.db.get_value(self.doctype, self.name, fieldname)

	def check_no_back_links_exist(self):
		"""Check if document links to any active document before Cancel."""
		from dataent.model.delete_doc import check_if_doc_is_linked, check_if_doc_is_dynamically_linked
	...
	def save_version(self):
		'''Save version info'''
		version = dataent.new_doc('Version')
	...
	def validate_value(self, fieldname, condition, val2, doc=None, raise_exception=None):
		...
		if not dataent.compare(val1, condition, val2):
	...
	def validate_table_has_rows(self, parentfield, raise_exception=None):
		...
		if not (isinstance(self.get(parentfield), list) and len(self.get(parentfield)) > 0):
			...
			dataent.throw(_("Table {0} cannot be empty").format(label), raise_exception or dataent.EmptyTableError)
	...
	def add_comment(self, comment_type, text=None, comment_by=None, link_doctype=None, link_name=None):
		...
		if comment_type=='Comment':
			...
			out = dataent.get_doc(dict(
				doctype='Version',
				ref_doctype= self.doctype,
				docname= self.name,
				data = dataent.as_json(dict(comment_type=comment_type, comment=text))
			))
			if comment_by:
	...
	def add_seen(self, user=None):
		...
		if self.meta.track_seen:
			...
			_seen = dataent.parse_json(_seen)

			if user not in _seen:
				_seen.append(user)
				dataent.db.set_value(self.doctype, self.name, '_seen', json.dumps(_seen), update_modified=False)
	...
	def add_viewed(self, user=None):
		...
		if hasattr(self.meta, 'track_views') and self.meta.track_views:
			dataent.get_doc({
				"doctype": "View Log",
				"viewed_by": dataent.session.user,
				"reference_doctype": self.doctype,
				"reference_name": self.name,
			}).insert(ignore_permissions=True)
			dataent.local.flags.commit = True
	...
	def set_onload(self, key, value):
		if not self.get("__onload"):
			self.set("__onload", dataent._dict())
		self.get("__onload")[key] = value

	def get_onload(self, key=None):
		if not key:
			return self.get("__onload", dataent._dict())

		return self.get('__onload')[key]

	def update_timeline_doc(self):
		if dataent.flags.in_install or not self.meta.get("timeline_field"):
			return
	...
	def queue_action(self, action, **kwargs):
		...
		if file_lock.lock_exists(self.get_signature()):
			dataent.throw(_('This document is currently queued for execution. Please try again'),
				title=_('Document Queued'))

		self.lock()
		enqueue('dataent.model.document.execute_action', doctype=self.doctype, name=self.name,
			...
			if timeout:
				for i in range(timeout):
					...
					if not file_lock.lock_exists(signature):
						...
						break
			if lock_exists:
				raise dataent.DocumentLockedError
...
def execute_action(doctype, name, action, **kwargs):
	...
	try:
		...
		if dataent.local.message_log:
			msg = json.loads(dataent.local.message_log[-1]).get('message')
		else:
			msg = '<pre><code>' + dataent.get_traceback() + '</pre></code>'
