...
import dataent
from dataent.utils import nowdate, nowtime, now_datetime
...
from dataent.permissions import get_allowed_docs_for_doctype

def get_new_doc(doctype, parent_doc = None, parentfield = None, as_dict=False):
	if doctype not in dataent.local.new_doc_templates:
		# cache a copy of new doc as it is called
		# frequently for inserts
		dataent.local.new_doc_templates[doctype] = make_new_doc(doctype)

	doc = copy.deepcopy(dataent.local.new_doc_templates[doctype])
...
def get_user_default_value(df, defaults, doctype_user_permissions, allowed_records):
	...
	if df.fieldtype == "Link" and df.options != "User":
		...
		if (dataent.get_meta(df.options).document_type=="Setup"
...
def set_dynamic_default_values(doc, parent_doc, parentfield):
	...
	for df in dataent.get_meta(doc["doctype"]).get("fields"):
		if df.get("default"):
...
def get_default_based_on_another_field(df, user_permissions, parent_doc):
	...
	reference_name = parent_doc.get(ref_fieldname) if parent_doc else dataent.db.get_default(ref_fieldname)
	default_value = dataent.db.get_value(ref_doctype, reference_name, df.fieldname)
