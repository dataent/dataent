...
from dataent.utils import cstr, cint, flt

# imports - third-party imports
import pymysql
from pymysql.constants import ER

class InvalidColumnName(dataent.ValidationError): pass
...
def updatedb(dt, meta=None):
	...
	Syncs a `DocType` to the table
	   ...
	   * updates indices
	"""
	res = dataent.db.sql("select issingle from tabDocType where name=%s", (dt,))
	if not res:
...
class DbTable:
	def __init__(self, doctype, prefix = 'tab', meta = None):
		...
		self.meta = meta
		if not self.meta:
			self.meta = dataent.get_meta(self.doctype)
	...
	def validate(self):
		...
		columns = [dataent._dict({"fieldname": f, "fieldtype": "Data"}) for f in standard_varchar_columns]
		columns += self.columns.values()

		for col in columns:
			if len(col.fieldname) >= 64:
				dataent.throw(_("Fieldname is limited to 64 characters ({0})")
					.format(dataent.bold(col.fieldname)))
			...
			if col.fieldtype in type_map and type_map[col.fieldtype][0]=="varchar":
				...
				if not (1 <= new_length <= 1000):
					dataent.throw(_("Length of {0} should be between 1 and 1000").format(col.fieldname))
	...
	def is_new(self):
		return self.name not in DbManager(dataent.db).get_tables_list(dataent.db.cur_db_name)
	...
	def get_columns_from_docfields(self):
		...
		"""
		fl = dataent.db.sql("SELECT * FROM tabDocField WHERE parent = %s", self.doctype, as_dict = 1)
		lengths = {}
		precisions = {}
		uniques = {}
		...
		if not dataent.flags.in_install_db and (dataent.flags.in_install != "dataent" or dataent.flags.ignore_in_install):
			...
			for ps in dataent.get_all("Property Setter", fields=["field_name", "property", "value"],
				filters={
	...
	def get_columns_from_db(self):
		self.show_columns = dataent.db.sql("desc `%s`" % self.name)
	...
	def get_foreign_keys(self):
		fk_list = []
		txt = dataent.db.sql("show create table `%s`" % self.name)[0][1]
	...
	def drop_foreign_keys(self):
		...
		for col in self.drop_foreign_key:
			dataent.db.sql("set foreign_key_checks=0")
			dataent.db.sql("alter table `%s` drop foreign key `%s`" % (self.name, fk_dict[col.fieldname]))
			dataent.db.sql("set foreign_key_checks=1")
	...
	def alter(self):
		...
		for col in self.add_index:
			...
			if not dataent.db.sql("show index from `%s` where key_name = %s" %
		...
		for col in self.drop_index:
			if col.fieldname != 'name': # primary key
				# if index key exists
				if dataent.db.sql("""show index from `{0}`
					where key_name=%s
		...
		if query:
			try:
				dataent.db.sql("alter table `{}` {}".format(self.name, ", ".join(query)))
				...
				if e.args[0]==1060:
					...
					dataent.throw(_("{0} field cannot be set as unique in {1}, as there are non-unique existing values".format(fieldname, self.name)))
				else:
					raise e
...
class DbManager:
	...
	#TODO:
		...
		0a. Merge restore database and extract_sql(from dataent_server_tools).
	...
	def restore_database(self,target,source,user,password):
		from dataent.utils import make_esc
		esc = make_esc('$ ')
...
def validate_column_name(n):
	...
	if special_characters:
		...
		dataent.throw(_("Fieldname {0} cannot have special characters like {1}").format(dataent.bold(cstr(n)), special_characters), InvalidColumnName)
	return n
...
def validate_column_length(fieldname):
	...
	if len(fieldname) > 64:
		dataent.throw(_("Fieldname is limited to 64 characters ({0})").format(fieldname))

def remove_all_foreign_keys():
	dataent.db.sql("set foreign_key_checks = 0")
	dataent.db.commit()
	for t in dataent.db.sql("select name from tabDocType where issingle=0"):
		dbtab = DbTable(t[0])
		try:
		...
		for f in fklist:
			dataent.db.sql("alter table `tab%s` drop foreign key `%s`" % (t[0], f[1]))
...
def add_column(doctype, column_name, fieldtype, precision=None):
	if column_name in dataent.db.get_table_columns(doctype):
		# already exists
		return

	dataent.db.commit()
	dataent.db.sql("alter table `tab%s` add column %s %s" % (doctype,
