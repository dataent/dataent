...
from dataent.utils import cstr, cint
from dataent.model import default_fields, no_value_fields, optional_fields
from dataent.model.document import Document
from dataent.model.base_document import BaseDocument
from dataent.model.db_schema import type_map
...
def get_meta(doctype, cached=True):
	if cached:
		if not dataent.local.meta_cache.get(doctype):
			meta = dataent.cache().hget("meta", doctype)
			if meta:
				meta = Meta(meta)
			else:
				meta = Meta(doctype)
				dataent.cache().hset('meta', doctype, meta.as_dict())
			dataent.local.meta_cache[doctype] = meta

		return dataent.local.meta_cache[doctype]
	else:
...
def get_table_columns(doctype):
	return dataent.cache().hget("table_columns", doctype,
		lambda: dataent.db.get_table_columns(doctype))

def load_doctype_from_file(doctype):
	fname = dataent.scrub(doctype)
	with open(dataent.get_app_path("dataent", "core", "doctype", fname, fname + ".json"), "r") as f:
...
class Meta(Document):
	...
	def as_dict(self, no_nulls = False):
		def serialize(doc):
			...
			for key in doc.__dict__:
				...
				if (isinstance(value, (dataent.text_type, int, float, datetime, list, tuple))
	...
	def get_global_search_fields(self):
		...
		if getattr(self, 'show_name_in_global_search', None):
			fields.append(dataent._dict(fieldtype='Data', fieldname='name', label='Name'))
	...
	def add_custom_fields(self):
		try:
			self.extend("fields", dataent.db.sql("""SELECT * FROM `tabCustom Field`
	...
	def apply_property_setters(self):
		property_setters = dataent.db.sql("""select * from `tabProperty Setter` where
			doc_type=%s""", (self.name,), as_dict=1)

		if not property_setters: return

		integer_docfield_properties = [d.fieldname for d in dataent.get_meta('DocField').fields
	...
	def set_custom_permissions(self):
		...
		if dataent.flags.in_patch or dataent.flags.in_install:
			return

		if not self.istable and self.name not in ('DocType', 'DocField', 'DocPerm',
			'Custom DocPerm'):
			custom_perms = dataent.get_all('Custom DocPerm', fields='*',
	...
	def get_dashboard_data(self):
		...
		try:
			...
			if hasattr(module, 'get_data'):
				data = dataent._dict(module.get_data())
		except ImportError:
			pass
...
doctype_table_fields = [
	dataent._dict({"fieldname": "fields", "options": "DocField"}),
	dataent._dict({"fieldname": "permissions", "options": "DocPerm"})
]
...
def is_single(doctype):
	try:
		return dataent.db.get_value("DocType", doctype, "issingle")
	except IndexError:
		raise Exception('Cannot determine whether %s is single' % doctype)

def get_parent_dt(dt):
	parent_dt = dataent.db.sql("""select parent from tabDocField
		where fieldtype="Table" and options=%s and (parent not like "old_parent:%%")
		limit 1""", dt)
	return parent_dt and parent_dt[0][0] or ''

def set_fieldname(field_id, fieldname):
	dataent.db.set_value('DocField', field_id, 'fieldname', fieldname)
...
def get_field_currency(df, doc=None):
	...
	if not getattr(dataent.local, "field_currency", None):
		dataent.local.field_currency = dataent._dict()

	if not (dataent.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname) or
		(doc.parent and dataent.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname))):
		...
		if ":" in cstr(df.get("options")):
			...
			if len(split_opts)==3:
				currency = dataent.get_cached_value(split_opts[0], doc.get(split_opts[1]), split_opts[2])
		else:
			...
			if doc.parent:
				if currency:
					...
					currency = dataent.db.get_value(doc.parenttype, doc.parent, df.get("options"))

		if currency:
			dataent.local.field_currency.setdefault((doc.doctype, ref_docname), dataent._dict())\
				.setdefault(df.fieldname, currency)

	return dataent.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname) or \
		(doc.parent and dataent.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname))

def get_field_precision(df, doc=None, currency=None):
	"""get precision based on DocField options and fieldvalue in doc"""
	from dataent.utils import get_number_format_info

	if cint(df.precision):
		precision = cint(df.precision)

	elif df.fieldtype == "Currency":
		precision = cint(dataent.db.get_default("currency_precision"))
		if not precision:
			number_format = dataent.db.get_default("number_format") or "#,###.##"
			decimal_str, comma_str, precision = get_number_format_info(number_format)
	else:
		precision = cint(dataent.db.get_default("float_precision")) or 3
...
def trim_tables(doctype=None):
	...
	for doctype in dataent.db.get_all("DocType", filters=filters):
		doctype = doctype.name
		columns = dataent.db.get_table_columns(doctype)
		fields = dataent.get_meta(doctype).get_fieldnames_with_value()
