...
from dataent.model.naming import validate_name
from dataent.model.dynamic_links import get_dynamic_link_map
from dataent.utils.password import rename_password
from dataent.model.utils.user_settings import sync_user_settings, update_user_settings_data
...
def update_user_settings(old, new, link_fields):
	...
	user_settings_details = dataent.db.sql('''select user, doctype, data from `__UserSettings` where
...
def update_attachments(doctype, old, new):
	try:
		if old != "File Data" and doctype != "DocType":
			dataent.db.sql("""update `tabFile` set attached_to_name=%s
...
def rename_versions(doctype, old, new):
	dataent.db.sql("""update tabVersion set docname=%s where ref_doctype=%s and docname=%s""",
		(new, doctype, old))

def rename_parent_and_child(doctype, old, new, meta):
	# rename the doc
	dataent.db.sql("update `tab%s` set name=%s where name=%s" % (dataent.db.escape(doctype), '%s', '%s'),
		(new, old))
...
def update_autoname_field(doctype, new, meta):
	...
	if meta.get('autoname'):
		...
		if field and field[0] == "field":
			dataent.db.sql("update `tab%s` set %s=%s where name=%s" % (dataent.db.escape(doctype), field[1], '%s', '%s'),
				(new, new))

def validate_rename(doctype, new, meta, merge, force, ignore_permissions):
	# using for update so that it gets locked and someone else cannot edit it while this rename is going on!
	exists = dataent.db.sql("select name from `tab{doctype}` where name=%s for update".format(doctype=dataent.db.escape(doctype)), new)
	exists = exists[0][0] if exists else None

	if merge and not exists:
		dataent.msgprint(_("{0} {1} does not exist, select a new target to merge").format(doctype, new), raise_exception=1)
	...
	if (not merge) and exists:
		dataent.msgprint(_("Another {0} with name {1} exists, select another name").format(doctype, new), raise_exception=1)

	if not (ignore_permissions or dataent.permissions.has_permission(doctype, "write", raise_exception=False)):
		dataent.msgprint(_("You need write permission to rename"), raise_exception=1)

	if not (force or ignore_permissions) and not meta.allow_rename:
		dataent.msgprint(_("{0} not allowed to be renamed").format(_(doctype)), raise_exception=1)
...
def update_child_docs(old, new, meta):
	...
	for df in meta.get_table_fields():
		dataent.db.sql("update `tab%s` set parent=%s where parent=%s" \
			% (dataent.db.escape(df.options), '%s', '%s'), (new, old))

def update_link_field_values(link_fields, old, new, doctype):
	for field in link_fields:
		if field['issingle']:
			try:
				single_doc = dataent.get_doc(field['parent'])
			...
			dataent.db.sql("""\
				...
				where `%s`=%s""" \
				% (dataent.db.escape(parent), dataent.db.escape(field['fieldname']), '%s',
					dataent.db.escape(field['fieldname']), '%s'),
				(new, old))
...
def get_link_fields(doctype):
	...
	if not dataent.flags.link_fields:
		dataent.flags.link_fields = {}

	if not doctype in dataent.flags.link_fields:
		link_fields = dataent.db.sql("""\
		...
		custom_link_fields = dataent.db.sql("""\
		...
		property_setter_link_fields = dataent.db.sql("""\
		...
		dataent.flags.link_fields[doctype] = link_fields

	return dataent.flags.link_fields[doctype]

def update_options_for_fieldtype(fieldtype, old, new):
	if dataent.conf.developer_mode:
		for name in dataent.db.sql_list("""select parent from
			tabDocField where options=%s""", old):
			doctype = dataent.get_doc("DocType", name)
			save = False
		...
		dataent.db.sql("""update `tabDocField` set options=%s
			where fieldtype=%s and options=%s""", (new, fieldtype, old))

	dataent.db.sql("""update `tabCustom Field` set options=%s
		where fieldtype=%s and options=%s""", (new, fieldtype, old))

	dataent.db.sql("""update `tabProperty Setter` set value=%s
...
def get_select_fields(old, new):
	...
	select_fields = dataent.db.sql("""\
		...
		where
			...
			df.options like "%%%%%s%%%%" """ \
		% ('%s', dataent.db.escape(old)), (new,), as_dict=1)

	# get link fields from tabCustom Field
	custom_select_fields = dataent.db.sql("""\
		select dt as parent, fieldname,
			(select issingle from tabDocType dt
			where dt.name = df.dt) as issingle
		from `tabCustom Field` df
	...
	# remove fields whose options have been changed using property setter
	property_setter_select_fields = dataent.db.sql("""\
		select ps.doc_type as parent, ps.field_name as fieldname,
			(select issingle from tabDocType dt
			where dt.name = ps.doc_type) as issingle
		from `tabProperty Setter` ps
...
def update_select_field_values(old, new):
	dataent.db.sql("""\
		...
		where
			...
			(options like "%%%%\\n%s%%%%" or options like "%%%%%s\\n%%%%")""" % \
		('%s', '%s', '%s', dataent.db.escape(old), dataent.db.escape(old)), (old, new, new))
...
def update_parenttype_values(old, new):
	...
	property_setter_child_doctypes = dataent.db.sql("""\
...
def bulk_rename(doctype, rows=None, via_console = False):
	...
	if not rows:
		dataent.throw(_("Please select a valid csv file with data"))

	if not via_console:
		max_rows = 500
		if len(rows) > max_rows:
			dataent.throw(_("Maximum {0} rows allowed").format(max_rows))
...
def get_fetch_fields(doctype, linked_to, ignore_doctypes=None):
	...
	for d in product_list:
		linked_doctype_info = dataent._dict()
