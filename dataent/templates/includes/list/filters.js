function setup_list_filters() {
	...
	dataent.call('dataent.website.doctype.web_form.web_form.get_form_data', {
	...
	.then((r) => {
		...
		web_form_filters = web_form.web_form_fields.filter(df => {
			...
			const value = dataent.utils.get_query_params()[df.fieldname];
			const f = dataent.ui.form.make_control({
				df: {
					...
					is_filter: true,
					change: (e) => {
						const query_params = Object.assign(dataent.utils.get_query_params(), {
							[df.fieldname]: f.get_value()
						})
						const query_string = dataent.utils.make_query_string(query_params);
...
dataent.ready(setup_list_filters);
