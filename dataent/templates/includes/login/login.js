...
login.bind_events = function() {
	...
	$(".form-login").on("submit", function(event) {
		...
		var args = {};
		args.cmd = "login";
		args.usr = dataent.utils.xss_sanitise(($("#login_email").val() || "").trim());
		args.pwd = $("#login_password").val();
		args.device = "desktop";
		if(!args.usr || !args.pwd) {
			dataent.msgprint("{{ _("Both login and password required") }}");
			return false;
		}
		login.call(args);
		return false;
	...
	$(".form-signup").on("submit", function(event) {
		event.preventDefault();
		var args = {};
		args.cmd = "dataent.core.doctype.user.user.sign_up";
		args.email = ($("#signup_email").val() || "").trim();
		args.redirect_to = dataent.utils.get_url_arg("redirect-to") || '';
	...
	$(".form-forgot").on("submit", function(event) {
		event.preventDefault();
		var args = {};
		args.cmd = "dataent.core.doctype.user.user.reset_password";
	...
	var login_handlers = {
		200: function(data) {
			if(data.message == 'Logged In'){
				...
				window.location.href = dataent.utils.get_url_arg("redirect-to") || data.home_page;
			} else if(data.message=="No App") {
				login.set_indicator("{{ _("Success") }}", 'green');
				if(localStorage) {
					var last_visited =
						localStorage.getItem("last_visited")
						|| dataent.utils.get_url_arg("redirect-to");
				...
				if(cint(data.message[0])==0) {
					...
					login.set_indicator("{{ _('Success') }}", 'green');
					dataent.msgprint(data.message[1])
				}
...
var verify_token =  function(event) {
	$(".form-verify").on("submit", function(eventx) {
		...
		args.tmp_id = dataent.get_cookie('tmp_id');
		if(!args.otp) {
			dataent.msgprint('{{ _("Login token required") }}');
			return false;
		}
		login.call(args);
		return false;
