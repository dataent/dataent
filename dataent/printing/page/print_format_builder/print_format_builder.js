dataent.pages['print-format-builder'].on_page_load = function(wrapper) {
	dataent.print_format_builder = new dataent.PrintFormatBuilder(wrapper);
	dataent.breadcrumbs.add("Setup", "Print Format");
}

dataent.pages['print-format-builder'].on_page_show = function(wrapper) {
	var route = dataent.get_route();
	if(route.length>1) {
		dataent.model.with_doc('Print Format', route[1], function() {
			dataent.print_format_builder.print_format = dataent.get_doc('Print Format', route[1]);
			dataent.print_format_builder.refresh();
		});
	} else if(dataent.route_options) {
		if(dataent.route_options.make_new) {
			let { doctype, name, based_on } = dataent.route_options;
			dataent.route_options = null;
			dataent.print_format_builder.setup_new_print_format(doctype, name, based_on);
		} else {
			dataent.print_format_builder.print_format = dataent.route_options.doc;
			dataent.route_options = null;
			dataent.print_format_builder.refresh();
		}
	}
...
dataent.PrintFormatBuilder = Class.extend({
	...
	make: function() {
		this.page = dataent.ui.make_app_page({
			parent: this.parent,
	...
	show_start: function() {
		this.page.main.html(dataent.render_template("print_format_builder_start", {}));
	...
	start_edit_print_format: function() {
		...
		var me = this;
		this.print_format_input = dataent.ui.form.make_control({
		...
		this.page.main.find(".btn-edit-print-format").on("click", function() {
			...
			if(!name) return;
			dataent.model.with_doc("Print Format", name, function(doc) {
				dataent.set_route('print-format-builder', name);
			});
		});
	},
	start_new_print_format: function() {
		var me = this;
		this.doctype_input = dataent.ui.form.make_control({
		...
		this.name_input = dataent.ui.form.make_control({
		...
		this.page.main.find(".btn-new-print-format").on("click", function() {
			...
			if(!(doctype && name)) {
				dataent.msgprint(__("Both DocType and Name required"));
				return;
			}
	...
	setup_new_print_format: function(doctype, name, based_on) {
		dataent.call({
			method: 'dataent.printing.page.print_format_builder.print_format_builder.create_custom_format',
			args: {
				doctype: doctype,
				name: name,
				based_on: based_on
	...
	setup_print_format: function() {
		var me = this;
		dataent.model.with_doctype(this.print_format.doc_type, function(doctype) {
			me.meta = dataent.get_meta(me.print_format.doc_type);
			me.setup_sidebar();
			me.render_layout();
			...
			me.page.add_inner_button(__("Edit Properties"), function() {
				dataent.set_route("Form", "Print Format", me.print_format.name);
			});
		});
	},
	setup_sidebar: function() {
		...
		$(dataent.render_template("print_format_builder_sidebar",
			{fields: fields}))
	...
	render_layout: function() {
		...
		this.prepare_data();
		$(dataent.render_template("print_format_builder_layout", {
		...
		$.each(this.data, function(i, f) {
			...
			if(!f.name && f.fieldname) {
				...
				if(f.fieldname==="_custom_html") {
					...
					f = $.extend(dataent.meta.get_docfield(me.print_format.doc_type,
						f.fieldname) || {}, f);
				}
			}
	...
	init_visible_columns: function(f) {
		f.visible_columns = []
		$.each(dataent.get_meta(f.options).fields, function(i, _f) {
			...
			onAdd: function(evt) {
				...
				if(!$item.hasClass("print-format-builder-field")) {
					...
					if(fieldname==="_custom_html") {
						...
						var field = dataent.meta.get_docfield(me.print_format.doc_type,
							fieldname);
					}

					var html = dataent.render_template("print_format_builder_field",
						{field: field, me:me});
	...
	setup_add_section: function() {
		...
		this.page.main.find(".print-format-builder-add-section").on("click", function() {
			...
			var $section = $(dataent.render_template("print_format_builder_section",
	...
	setup_column_selector: function() {
		...
		this.page.main.on("click", ".select-columns", function() {
			...
			var doc_fields = dataent.get_meta(doctype).fields;
			...
			// render checkboxes
			$(dataent.render_template("print_format_builder_column_selector", {
				fields: fields,
	...
	save_print_format: function() {
		...
		dataent.call({
			...
			callback: function(r) {
				...
				dataent.show_alert({message: __("Saved"), indicator: 'green'});
			}
		});
	}
});
