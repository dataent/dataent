...
dataent.ui.form.on("Print Format", "onload", function(frm) {
	frm.add_fetch("doc_type", "module", "module");
});

dataent.ui.form.on("Print Format", {
	refresh: function(frm) {
		frm.set_intro("");
		frm.toggle_enable(["html", "doc_type", "module"], false);
		if (dataent.session.user==="Administrator" || frm.doc.standard==="No") {
			frm.toggle_enable(["html", "doc_type", "module"], true);
			frm.enable_save();
		}

		if(frm.doc.standard==="Yes" && dataent.session.user !== "Administrator") {
		...
		if(!frm.is_new()) {
			if(!frm.doc.custom_format) {
				frm.add_custom_button(__("Edit Format"), function() {
					if(!frm.doc.doc_type) {
						dataent.msgprint(__("Please select DocType first"));
						return;
					}
					dataent.set_route("print-format-builder", frm.doc.name);
				});
			}
			frm.add_custom_button(__("Make Default"), function() {
				dataent.call({
					method: "dataent.printing.doctype.print_format.print_format.make_default",
					args: {
						name: frm.doc.name
					}
				})
