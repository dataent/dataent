...
import dataent, json
from dataent.model.utils.user_settings import update_user_settings, sync_user_settings

def execute():
	""" Update list_view's order by property from __UserSettings """

	users = dataent.db.sql("select distinct(user) from `__UserSettings`", as_dict=True)

	for user in users:
		# get user_settings for each user
		settings = dataent.db.sql("select * from `__UserSettings` \
			where user='{0}'".format(dataent.db.escape(user.user)), as_dict=True)
