...
def execute():
	custom_doctypes = dataent.get_all('DocType', filters={
		'custom': 1
	})

	for doctype in custom_doctypes:
		property_setters = dataent.get_all('Property Setter', filters={
			'doc_type': doctype.name,
			'doctype_or_field': 'DocField'
		}, fields=['name', 'property', 'value', 'property_type', 'field_name'])

		custom_fields = dataent.get_all('Custom Field',
		...
		for prop in property_setters:
			...
			dataent.db.sql('DELETE FROM `tabProperty Setter` WHERE `name`=%s', prop.name)

		meta = dataent.get_meta(doctype.name)
		...
		for cf in custom_fields:
			...
			if field:
				...
				df = dataent.new_doc('DocField', meta, 'fields')
				df.update(cf)
				meta.fields.append(df)
			dataent.db.sql('DELETE FROM `tabCustom Field` WHERE name=%s', cf.name)
