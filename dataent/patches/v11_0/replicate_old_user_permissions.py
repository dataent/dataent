...
from dataent.permissions import get_valid_perms

def execute():
	dataent.reload_doctype("User Permission")
	user_permissions = dataent.get_all('User Permission', fields=['allow', 'name', 'user'])
	...
	for key, doctype_to_skip in doctype_to_skip_map.items():
		...
		if not dataent.db.has_column("User Permission", "applicable_for") \
			and dataent.db.has_column("User Permission", "skip_for_doctype"):
...
def get_linked_doctypes(doctype):
	from dataent.permissions import get_linked_doctypes
	linked_doctypes = get_linked_doctypes(doctype)
	child_doctypes = [d.options for d in dataent.get_meta(doctype).get_table_fields()]
