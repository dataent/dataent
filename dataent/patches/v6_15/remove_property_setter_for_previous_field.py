...
def execute():
	...
	dataent.db.sql("""delete from `tabProperty Setter` where property='previous_field'""")

	all_custom_fields = dataent._dict()
	for d in dataent.db.sql("""select name, dt, fieldname, insert_after from `tabCustom Field`
		where insert_after is not null and insert_after != ''""", as_dict=1):
			all_custom_fields.setdefault(d.dt, dataent._dict()).setdefault(d.fieldname, d.insert_after)
...
def get_sorted_fields(doctype, custom_fields):
	...
	fields_dict = dataent.get_meta(doctype).get("fields")

	standard_fields_count = dataent.db.sql("""select count(name) from `tabDocField`
