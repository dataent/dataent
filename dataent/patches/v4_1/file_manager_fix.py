...
import dataent
import os
from dataent.utils.file_manager import get_content_hash, get_file, get_file_name
from dataent.utils import get_files_path, get_site_path
...
def get_replaced_files():
	ret = []
	new_files = dict(dataent.db.sql("select name, file_name from `tabFile` where file_name not like 'files/%'"))
	old_files = dict(dataent.db.sql("select name, file_name from `tabFile` where ifnull(content_hash, '')=''"))
...
def rename_replacing_files():
	...
	for file_name, file_datas in replaced_files:
		...
		content_hash = dataent.db.get_value('File', file_datas[0], 'content_hash')
		if not content_hash:
			continue
