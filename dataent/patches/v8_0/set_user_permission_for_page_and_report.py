...
def execute():
	if not dataent.db.exists('DocType', 'Custom Role'):
		dataent.reload_doc("core", 'doctype', "custom_role")
...
def update_ref_doctype_in_custom_role():
	dataent.reload_doc("core", 'doctype', "custom_role")
	dataent.db.sql("""update `tabCustom Role` 
				set 
...
def make_custom_roles_for_page_and_report():
	for doctype in ['Page', 'Report']:
		for data in get_data(doctype):
			doc = dataent.get_doc(doctype, data.name)
...
def get_data(doctype):
	...
	return dataent.get_all(doctype, fields = fields)

def get_roles(doctype, data, doc):
	roles = []
	if doctype == 'Page':
		for d in doc.roles:
			if dataent.db.exists('Role', d.role):
				roles.append({'role': d.role})
	else:
		out = dataent.get_all('Custom DocPerm', fields='distinct role', filters=dict(parent = data.ref_doctype))
		for d in out:
			roles.append({'role': d.role})
	return roles
...
def make_custom_roles(doctype, name, roles):
	...
	if roles:
		custom_permission = dataent.get_doc({
			'doctype': 'Custom Role',
			field : name,
			'roles' : roles
		}).insert()
