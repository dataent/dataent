...
def execute():
	...
	doctypes_with_text_fields = dataent.get_all("DocField", fields=["parent", "fieldname"],
	...
	for opts in doctypes_with_text_fields:
		...
		try:
			result = dataent.get_all(opts.parent, fields=["name", opts.fieldname])
		...
		for data in result:
			...
			if html != old_value:
				...
				dataent.db.set_value(opts.parent, data.name, opts.fieldname, html, update_modified=False)
