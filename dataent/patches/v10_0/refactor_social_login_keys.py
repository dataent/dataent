...
def execute():
	...
	dataent.reload_doc("integrations", "doctype", "social_login_key", force=True)

	if not dataent.db.exists('DocType', 'Social Login Keys'):
		return

	social_login_keys = dataent.get_doc("Social Login Keys", "Social Login Keys")
	if social_login_keys.get("facebook_client_id") or social_login_keys.get("facebook_client_secret"):
		facebook_login_key = dataent.new_doc("Social Login Key")
	...
	if social_login_keys.get("dataent_server_url"):
		dataent_login_key = dataent.new_doc("Social Login Key")
		dataent_login_key.get_social_login_provider("Dataent", initialize=True)
		dataent_login_key.social_login_provider = "Dataent"
		dataent_login_key.base_url = social_login_keys.get("dataent_server_url")
		dataent_login_key.client_id = social_login_keys.get("dataent_client_id")
		dataent_login_key.client_secret = social_login_keys.get("dataent_client_secret")
		if not (dataent_login_key.client_secret and dataent_login_key.client_id and dataent_login_key.base_url):
			dataent_login_key.enable_social_login = 0
		dataent_login_key.save()

	if social_login_keys.get("github_client_id") or social_login_keys.get("github_client_secret"):
		github_login_key = dataent.new_doc("Social Login Key")
	...
	if social_login_keys.get("google_client_id") or social_login_keys.get("google_client_secret"):
		google_login_key = dataent.new_doc("Social Login Key")
	...
	dataent.delete_doc("DocType", "Social Login Keys")

def run_patch():
	dataent.reload_doc("core", "doctype", "user", force=True)
	dataent.reload_doc("core", "doctype", "user_social_login", force=True)

	users = dataent.get_all("User", fields=["*"], filters={"name":("not in", ["Administrator", "Guest"])})

	for user in users:
		idx = 0
		if user.dataent_userid:
			insert_user_social_login(user.name, user.modified_by, 'dataent', idx, userid=user.dataent_userid)
			idx += 1
...
def insert_user_social_login(user, modified_by, provider, idx, userid=None, username=None):
	...
	creation_time = dataent.utils.get_datetime_str(dataent.utils.get_datetime())
	values = [
		dataent.generate_hash(length=10),
		creation_time,
		creation_time,
		user,
		modified_by,
	...
	""".format(
		...
		values= "'" + "', '".join([dataent.db.escape(d) for d in values]) + "'"
	)
