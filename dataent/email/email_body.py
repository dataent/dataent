...
from dataent.email.smtp import get_outgoing_email_account
from dataent.utils import (get_url, scrub_urls, strip, expand_relative_urls, cint,
...
def get_email_html(template, args, subject, header=None):
	...
	email = dataent.utils.jinja.get_email_from_template(template, args)
...
def get_footer(email_account, footer=None):
	...
	company_address = dataent.db.get_default("email_footer_address")

	if company_address:
		args.update({'company_address': company_address})

	if not cint(dataent.db.get_default("disable_standard_email_footer")):
		args.update({'default_mail_footer': dataent.get_hooks('default_mail_footer')})

	footer += dataent.utils.jinja.get_email_from_template('email_footer', args)[0]
...
def get_filecontent_from_path(path):
	...
	if path.startswith('assets/'):
		...
		# public file
		full_path = dataent.get_site_path('public', path)
	elif path.startswith('private/files/'):
		# private file
		full_path = dataent.get_site_path(path)
	else:
		full_path = path
...
def get_header(header=None):
	...
	from dataent.utils.jinja import get_email_from_template
	...
	if not title:
		title = dataent.get_hooks('app_title')[-1]
