...
from dataent.utils import cint, parse_addr
...
def send(email, append_to=None, retry=1):
	...
	def _send(retry):
		try:
			...
			dataent.throw(_("Invalid login or password"), title='Email Failed')
			raise
		except smtplib.SMTPRecipientsRefused:
			dataent.msgprint(_("Invalid recipient address"), title='Email Failed')
			raise
...
def get_outgoing_email_account(raise_exception_not_set=True, append_to=None, sender=None):
	...
	if not dataent.local.outgoing_email_account.get(append_to) \
		...
		if not email_account and raise_exception_not_set and cint(dataent.db.get_single_value('System Settings', 'setup_complete')):
			dataent.throw(_("Please setup default Email Account from Setup > Email > Email Account"),
		...
		dataent.local.outgoing_email_account[append_to or sender_email_id or "default"] = email_account
...
def get_default_outgoing_email_account(raise_exception_not_set=True):
	...
	if not email_account and dataent.conf.get("mail_server"):
		...
		email_account.update({
			"smtp_server": dataent.conf.get("mail_server"),
			"smtp_port": dataent.conf.get("mail_port"),

			# legacy: use_ssl was used in site_config instead of use_tls, but meant the same thing
			"use_tls": cint(dataent.conf.get("use_tls") or 0) or cint(dataent.conf.get("use_ssl") or 0),
			"login_id": dataent.conf.get("mail_login"),
			"email_id": dataent.conf.get("auto_email_id") or dataent.conf.get("mail_login") or 'notifications@example.com',
			"password": dataent.conf.get("mail_password"),
			"always_use_account_email_id_as_sender": dataent.conf.get("always_use_account_email_id_as_sender", 0),
			"always_use_account_name_as_sender_name": dataent.conf.get("always_use_account_name_as_sender_name", 0)
		})
...
def _get_email_account(filters):
	name = dataent.db.get_value("Email Account", filters)
	return dataent.get_doc("Email Account", name) if name else None
...
class SMTPServer:
		...
		if self.email_account:
			...
			if not self.email_account.no_smtp_authentication:
				if self.email_account.ascii_encode_password:
					self.password = dataent.safe_encode(self.email_account.password, 'ascii')
				else:
	...
	def sess(self):
		...
		if not getattr(self, 'server'):
			...
			dataent.msgprint(err_msg)
			raise dataent.OutgoingEmailError(err_msg)
		...
		try:
			...
			if self.login and self.password:
				...
				if ret[0]!=235:
					dataent.msgprint(ret[1])
					raise dataent.OutgoingEmailError(ret[1])

			return self._sess

		except _socket.error as e:
			# Invalid mail server -- due to refusing connection
			dataent.msgprint(_('Invalid Outgoing Mail Server or Port'))
			traceback = sys.exc_info()[2]
			raise_(dataent.ValidationError, e, traceback)

		except smtplib.SMTPAuthenticationError as e:
			dataent.msgprint(_("Invalid login or password"))
			traceback = sys.exc_info()[2]
			raise_(dataent.ValidationError, e, traceback)

		except smtplib.SMTPException:
			dataent.msgprint(_('Unable to send emails at this time'))
			raise
