...
import dataent
from dataent import _, safe_decode, safe_encode
from dataent.utils import (extract_email_id, convert_utc_to_user_timezone, now,
	cint, cstr, strip, markdown, parse_addr)
from dataent.utils.scheduler import log
from dataent.utils.file_manager import get_random_filename, save_file, MaxFileSizeReachedError

class EmailSizeExceededError(dataent.ValidationError): pass
class EmailTimeoutError(dataent.ValidationError): pass
class TotalSizeExceededError(dataent.ValidationError): pass
class LoginLimitExceeded(dataent.ValidationError): pass
...
class EmailServer:
	...
	def setup(self, args=None):
		# overrride
		self.settings = args or dataent._dict()
	...
	def connect_imap(self):
		...
		try:
			if cint(self.settings.use_ssl):
				self.imap = Timed_IMAP4_SSL(self.settings.host, timeout=dataent.conf.get("pop_timeout"))
			else:
				self.imap = Timed_IMAP4(self.settings.host, timeout=dataent.conf.get("pop_timeout"))
			...
			dataent.msgprint(_('Invalid Mail Server. Please rectify and try again.'))
			raise

		except Exception as e:
			dataent.msgprint(_('Cannot connect: {0}').format(str(e)))
			raise
	...
	def connect_pop(self):
		...
		try:
			if cint(self.settings.use_ssl):
				self.pop = Timed_POP3_SSL(self.settings.host, timeout=dataent.conf.get("pop_timeout"))
			else:
				self.pop = Timed_POP3(self.settings.host, timeout=dataent.conf.get("pop_timeout"))
			...
			if self.is_temporary_system_problem(e):
				...
				dataent.msgprint(_('Invalid User Name or Support Password. Please rectify and try again.'))
				raise
	...
	def get_messages(self):
		...
		try:
			...
			# size limits
			self.total_size = 0
			self.max_email_size = cint(dataent.local.conf.get("max_email_size"))
	...
	def check_imap_uidvalidity(self):
		...
		dataent.db.set_value("Email Account", self.settings.email_account, "uidnext", uidnext)
...
class Email:
	...
	def set_from(self):
		...
		if _reply_to and not dataent.db.get_value('Email Account', {"email_id":_reply_to}, 'email_id'):
	...
	def decode_email(self, email):
		if not email: return
		decoded = ""
		for part, encoding in decode_header(dataent.as_unicode(email).replace("\""," ").replace("\'"," ")):
			if encoding:
