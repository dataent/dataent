...
from dataent import msgprint, throw, _, safe_decode
from dataent.email.smtp import SMTPServer, get_outgoing_email_account
from dataent.email.email_body import get_email, get_formatted_html, add_attachment
from dataent.utils.verified_command import get_signed_params, verify_request
from html2text import html2text
from dataent.utils import get_url, nowdate, encode, now_datetime, add_days, split_emails, cstr, cint
from dataent.utils.file_manager import get_file
from rq.timeouts import JobTimeoutException
from dataent.utils.scheduler import log
from six import text_type, string_types

class EmailLimitCrossedError(dataent.ValidationError): pass
...
def get_email_queue(recipients, sender, subject, **kwargs):
	...
	except dataent.InvalidEmailAddressError:
		# bad Email Address - don't add to queue
		dataent.log_error('Invalid Email ID Sender: {0}, Recipients: {1}'.format(mail.sender,
...
def check_email_limit(recipients):
	...
	if (smtp_server.email_account
		...
		monthly_email_limit = dataent.conf.get('limits', {}).get('emails')
		daily_email_limit = cint(dataent.conf.get('limits', {}).get('daily_emails'))
...
def get_emails_sent_this_month():
	return dataent.db.sql("""select count(name) from `tabEmail Queue` where
...
def clear_outbox():
	...
	email_queues = dataent.db.sql_list("""select name from `tabEmail Queue`
		where priority=0 and datediff(now(), modified) > 31""")

	if email_queues:
		dataent.db.sql("""delete from `tabEmail Queue` where name in (%s)"""
			% ','.join(['%s']*len(email_queues)), tuple(email_queues))

		dataent.db.sql("""delete from `tabEmail Queue Recipient` where parent in (%s)"""
			% ','.join(['%s']*len(email_queues)), tuple(email_queues))
