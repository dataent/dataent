...
def create_email_flag_queue(names, action):
	...
	def mark_as_seen_unseen(name, action):
		...
		if action == "Read":
			...
			_seen = [user for user in _seen if dataent.session.user != user]
	...
	for name in json.loads(names or []):
		uid, seen_status, email_account = dataent.db.get_value("Communication", name,
		...
		if (action =='Read' and seen_status == 0) or (action =='Unread' and seen_status == 1):
			...
			if create_new:
				...
				dataent.db.set_value("Communication", name, "seen", seen,
...
def mark_as_trash(communication):
	...
	dataent.db.set_value("Communication", communication, "email_status", "Trash")

@dataent.whitelist()
def mark_as_spam(communication, sender):
	""" set email status to spam """
	email_rule = dataent.db.get_value("Email Rule", { "email_id": sender })
	if not email_rule:
		dataent.get_doc({
			"doctype": "Email Rule",
			"email_id": sender,
	...
	dataent.db.set_value("Communication", communication, "email_status", "Spam")
