...
test_records = dataent.get_test_records('Email Account')

from dataent.core.doctype.communication.email import make
from dataent.desk.form.load import get_attachments
from dataent.utils.file_manager import delete_file_from_filesystem
from dataent.email.doctype.email_account.email_account import notify_unreplied
from datetime import datetime, timedelta

class TestEmailAccount(unittest.TestCase):
	def setUp(self):
		dataent.flags.mute_emails = False
		dataent.flags.sent_mail = None
	...
	def test_incoming(self):
		dataent.db.sql("delete from tabCommunication where sender='test_sender@example.com'")
		...
		comm = dataent.get_doc("Communication", {"sender": "test_sender@example.com"})
		self.assertTrue("test_receiver@example.com" in comm.recipients)

		# check if todo is created
		self.assertTrue(dataent.db.get_value(comm.reference_doctype, comm.reference_name, "name"))
	...
	def test_unread_notification(self):
		...
		dataent.db.sql("delete from `tabEmail Queue`")
		notify_unreplied()
		self.assertTrue(dataent.db.get_value("Email Queue", {"reference_doctype": comm.reference_doctype,
			"reference_name": comm.reference_name, "status":"Not Sent"}))

	def test_incoming_with_attach(self):
		dataent.db.sql("delete from tabCommunication where sender='test_sender@example.com'")
		existing_file = dataent.get_doc({'doctype': 'File', 'file_name': 'epaas-conf-14.png'})
		...
		self.assertTrue("epaas-conf-14.png" in [f.file_name for f in attachments])
	...
	def test_outgoing(self):
		...
		mail = email.message_from_string(dataent.get_last_doc("Email Queue").message)
		self.assertTrue("test-mail-000" in mail.get("Subject"))

	def test_sendmail(self):
		dataent.sendmail(sender="test_sender@example.com", recipients="test_recipient@example.com",
			content="test mail 001", subject="test-mail-001", delayed=False)

		sent_mail = email.message_from_string(dataent.safe_decode(dataent.flags.sent_mail))
	...
	def test_print_format(self):
		...
		sent_mail = email.message_from_string(dataent.get_last_doc("Email Queue").message)
	...
	def test_threading(self):
		...
		sent_name = make(subject = "Test", content="test content",
			recipients="test_receiver@example.com", sender="test@example.com",doctype="ToDo",name=dataent.get_last_doc("ToDo").name,
		...
		sent = dataent.get_doc("Communication", sent_name)
	...
	def test_threading_by_subject(self):
		...
		comm_list = dataent.get_all("Communication", filters={"sender":"test_sender@example.com"},
	...
	def test_threading_by_message_id(self):
		...
		event = dataent.get_doc(dict(doctype='Event', subject='test-message')).insert()

		# send a mail against this
		dataent.sendmail(recipients='test@example.com', subject='test message for threading',
			message='testing', reference_doctype=event.doctype, reference_name=event.name)

		last_mail = dataent.get_doc('Email Queue', dict(reference_name=event.name))
