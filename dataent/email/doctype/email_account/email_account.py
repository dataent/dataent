...
from dataent.utils import validate_email_add, cint, get_datetime, DATE_FORMAT, strip, comma_or, sanitize_html
from dataent.utils.user import is_system_user
from dataent.utils.jinja import render_template
from dataent.email.smtp import SMTPServer
from dataent.email.receive import EmailServer, Email
...
from dataent.core.doctype.communication.email import set_incoming_outgoing_accounts
...
class EmailAccount(Document):
	...
	def on_update(self):
		...
		from dataent.core.doctype.user.user import setup_user_email_inbox
	...
	def there_must_be_only_one_default(self):
		...
		for field in ("default_incoming", "default_outgoing"):
			...
			for email_account in dataent.get_all("Email Account", filters={ field: 1 }):
	...
	def get_domain(self, email_id):
		...
		try:
			...
			return dataent.db.get_value("Email Domain", domain[1], fields, as_dict=True)
		except Exception:
			pass
	...
	def check_smtp(self):
		...
		if self.enable_outgoing:
			if not self.smtp_server:
				dataent.throw(_("{0} is required").format("SMTP Server"))
	...
	def get_incoming_server(self, in_receive=False, email_sync_rule="UNSEEN"):
		...
		if not args.get("host"):
			dataent.throw(_("{0} is required").format("Email Server"))

		email_server = EmailServer(dataent._dict(args))
		try:
			...
			if in_receive:
				...
				description = dataent.message_log.pop() if dataent.message_log else "Socket Error"
				if test_internet():
					self.db_set("no_failed", self.no_failed + 1)
					if self.no_failed > 2:
						self.handle_incoming_connect_error(description=description)
				else:
					dataent.cache().set_value("workers:no-internet", True)
				return None
	...
	def set_failed_attempts_count(self, value):
		dataent.cache().set('{0}:email-account-failed-attempts'.format(self.name), value)

	def get_failed_attempts_count(self):
		return cint(dataent.cache().get('{0}:email-account-failed-attempts'.format(self.name)))
	...
	def set_sender_field_and_subject_field(self):
		...
		meta_module = dataent.get_meta_module(self.append_to)
		meta = dataent.get_meta(self.append_to)
	...
	def find_parent_from_in_reply_to(self, communication, email):
		...
		if in_reply_to and "@{0}".format(dataent.local.site) in in_reply_to:
			# reply to a communication sent from the system
			email_queue = dataent.db.get_value('Email Queue', dict(message_id=in_reply_to), ['communication','reference_doctype', 'reference_name'])
			if email_queue:
			...
			if dataent.db.exists(parent_doctype, parent_name):
				parent = dataent._dict(doctype=parent_doctype, name=parent_name)
				...
				if parent_doctype=='Communication':
					...
					if parent.reference_name:
						...
						parent = dataent.get_doc(parent.reference_doctype,
	...
	def on_trash(self):
		...
		from dataent.core.doctype.user.user import remove_user_email_inbox

		dataent.db.sql("update `tabCommunication` set email_account='' where email_account=%s", self.name)
		remove_user_email_inbox(email_account=self.name)

	def after_rename(self, old, new, merge=False):
		dataent.db.set_value("Email Account", new, "email_account_name", new)
	...
	def mark_emails_as_read_unread(self):
		...
		flags = dataent.db.sql("""select name, communication, uid, action from
			`tabEmail Flag Queue` where is_completed=0 and email_account='{email_account}'
			""".format(email_account=dataent.db.escape(self.name)), as_dict=True)
		...
		if flags and uid_list:
			...
			dataent.db.sql(""" update `tabEmail Flag Queue` set is_completed=1
	...
	def set_communication_seen_status(self, docnames, seen=0):
		...
		dataent.db.sql(""" update `tabCommunication` set seen={seen}
			where name in ({docnames})""".format(docnames=docnames, seen=seen))

@dataent.whitelist()
def get_append_to(doctype=None, txt=None, searchfield=None, start=None, page_len=None, filters=None):
	if not txt: txt = ""
	return [[d] for d in dataent.get_hooks("email_append_to") if txt in d]
...
def pull_from_email_account(email_account):
	...
	email_account = dataent.get_doc("Email Account", email_account)
...
def get_max_email_uid(email_account):
	...
	result = dataent.db.get_all("Communication", filters={
		"communication_medium": "Email",
		"sent_or_received": "Received",
