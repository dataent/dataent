...
from dataent.utils import (format_time, get_link_to_form, get_url_to_report,
...
max_reports_per_user = dataent.local.conf.max_reports_per_user or 3
...
class AutoEmailReport(Document):
	...
	def validate_report_count(self):
		...
		count = dataent.db.sql('select count(*) from `tabAuto Email Report` where user=%s and enabled=1', self.user)[0][0]
		if count > max_reports_per_user + (-1 if self.flags.in_insert else 0):
			dataent.throw(_('Only {0} emailed reports are allowed per user').format(max_reports_per_user))

	def validate_report_format(self):
		""" check if user has select correct report format """
		valid_report_formats = ["HTML", "XLSX", "CSV"]
		if self.format not in valid_report_formats:
			dataent.throw(_("%s is not a valid report format. Report format should \
				one of the following %s"%(dataent.bold(self.format), dataent.bold(", ".join(valid_report_formats)))))
	...
	def get_report_content(self):
		...
		# add serial numbers
		columns.insert(0, dataent._dict(fieldname='idx', label='', width='30px'))
	...
	def get_html_table(self, columns=None, data=None):
		...
		report_doctype = dataent.db.get_value('Report', self.report, 'ref_doctype')

		return dataent.render_template('dataent/templates/emails/auto_email_report.html', {
			'title': self.name,
	...
	def get_spreadsheet_data(columns, data):
		...
		for row in data:
			...
			for df in columns:
				...
				new_row.append(dataent.format(row[df.fieldname], df, row))
	...
	def send(self):
		if self.filter_meta and not self.filters:
			dataent.throw(_("Please set filters value in Report Filter table."))
...
def download(name):
	...
	auto_email_report = dataent.get_doc('Auto Email Report', name)
	auto_email_report.check_permission()
	data = auto_email_report.get_report_content()

	if not data:
		dataent.msgprint(_('No Data'))
		return

	dataent.local.response.filecontent = data
	dataent.local.response.type = "download"
	dataent.local.response.filename = auto_email_report.get_file_name()
...
def send_monthly():
	...
	for report in dataent.get_all('Auto Email Report', {'enabled': 1, 'frequency': 'Monthly'}):
		dataent.get_doc('Auto Email Report', report.name).send()
