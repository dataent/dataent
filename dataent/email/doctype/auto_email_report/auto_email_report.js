...
dataent.ui.form.on('Auto Email Report', {
	refresh: function(frm) {
		...
		if(!frm.is_new()) {
			frm.add_custom_button(__('Download'), function() {
				var w = window.open(
					dataent.urllib.get_full_url(
						"/api/method/dataent.email.doctype.auto_email_report.auto_email_report.download?"
			...
			frm.add_custom_button(__('Send Now'), function() {
				dataent.call({
					method: 'dataent.email.doctype.auto_email_report.auto_email_report.send_now',
					args: {name: frm.doc.name},
					callback: function() {
						dataent.msgprint(__('Scheduled to send'));
					}
				});
			});
		} else {
			if(!frm.doc.user) {
				frm.set_value('user', dataent.session.user);
			}
			if(!frm.doc.email_to) {
				frm.set_value('email_to', dataent.session.user);
			}
		}
	},
	report: function(frm) {
	...
	show_filters: function(frm) {
		...
		if(frm.doc.report_type === 'Custom Report' || (frm.doc.report_type !== 'Report Builder'
			&& dataent.query_reports[frm.doc.report]
			&& dataent.query_reports[frm.doc.report].filters)) {
			...
			if (frm.doc.report_type === 'Custom Report'
				&& dataent.query_reports[frm.doc.reference_report]
				&& dataent.query_reports[frm.doc.reference_report].filters) {
				report_filters = dataent.query_reports[frm.doc.reference_report].filters;
			} else {
				report_filters = dataent.query_reports[frm.doc.report].filters;
			}
			...
			report_filters.forEach(function(f) {
				$('<tr><td>' + f.label + '</td><td>'+ dataent.format(filters[f.fieldname], f) +'</td></tr>')
