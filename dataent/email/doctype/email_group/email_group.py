...
class EmailGroup(Document):
	def onload(self):
			singles = [d.name for d in dataent.db.get_all("DocType", "name", {"issingle": 1})]
			self.get("__onload").import_types = [{"value": d.parent, "label": "{0} ({1})".format(d.parent, d.label)} \
				for d in dataent.db.get_all("DocField", ("parent", "label"), {"options": "Email"}) 
	...
	def import_from(self, doctype):
		...
		for user in dataent.db.get_all(doctype, [email_field, unsubscribed_field or "name"]):
			try:
		...
		dataent.msgprint(_("{0} subscribers added").format(added))
	...
	def get_total_subscribers(self):
		return dataent.db.sql("""select count(*) from `tabEmail Group Member`
			where email_group=%s""", self.name)[0][0]

	def on_trash(self):
		for d in dataent.get_all("Email Group Member", "name", {"email_group": self.name}):
			dataent.delete_doc("Email Group Member", d.name)

@dataent.whitelist()
def import_from(name, doctype):
	nlist = dataent.get_doc("Email Group", name)
...
def add_subscribers(name, email_list):
	...
	for email in email_list:
		...
		if parsed_email:
			if not dataent.db.get_value("Email Group Member",
				{"email_group": name, "email": parsed_email}):
				dataent.get_doc({
					"doctype": "Email Group Member",
					"email_group": name,
					"email": parsed_email
				}).insert(ignore_permissions = dataent.flags.ignore_permissions)

				count += 1
			else:
				pass
		else:
			dataent.msgprint(_("{0} is not a valid Email Address").format(email))

	dataent.msgprint(_("{0} subscribers added").format(count))

	return dataent.get_doc("Email Group", name).update_total_subscribers()
...
def restrict_email_group(doc, method):
	...
	email_group = dataent.get_doc("Email Group", doc.email_group)
	if email_group.get_total_subscribers() >= email_group_limit:
		dataent.throw(_("Please Upgrade to add more than {0} subscribers").format(email_group_limit))
