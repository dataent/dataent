...
dataent.ui.form.on("Email Group", "refresh", function(frm) {
	if(!frm.is_new()) {
		frm.add_custom_button(__("View Subscribers"), function() {
			dataent.route_options = {"email_group": frm.doc.name};
			dataent.set_route("List", "Email Group Member");
		}, __("View"));

		frm.add_custom_button(__("Import Subscribers"), function() {
			dataent.prompt({fieldtype:"Select", options: frm.doc.__onload.import_types,
				label:__("Import Email From"), fieldname:"doctype", reqd:1},
				function(data) {
					dataent.call({
						method: "dataent.email.doctype.email_group.email_group.import_from",
						args: {
							"name": frm.doc.name,
		...
		frm.add_custom_button(__("Add Subscribers"), function() {
			dataent.prompt({fieldtype:"Text",
				label:__("Email Addresses"), fieldname:"email_list", reqd:1},
				function(data) {
					dataent.call({
						method: "dataent.email.doctype.email_group.email_group.add_subscribers",
						args: {
							"name": frm.doc.name,
