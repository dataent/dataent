...
def sendmail_to_system_managers(subject, content):
	dataent.sendmail(recipients=get_system_managers(), subject=subject, content=content)
...
def get_system_managers():
	return dataent.db.sql_list("""select parent FROM `tabHas Role`
...
def get_communication_doctype(doctype, txt, searchfield, start, page_len, filters):
	user_perms = dataent.utils.user.UserPermissions(dataent.session.user)
	...
	if len(txt)<2:
		...
		com_doctypes = [d[0] for d in dataent.db.get_values("DocType", {"issingle": 0, "istable": 0, "hide_toolbar": 0})]
...
def get_cached_contacts(txt):
	contacts = dataent.cache().hget("contacts", dataent.session.user) or []
...
def update_contact_cache(contacts):
	cached_contacts = dataent.cache().hget("contacts", dataent.session.user) or []

	uncached_contacts = [d for d in contacts if d not in cached_contacts]
	cached_contacts.extend(uncached_contacts)

	dataent.cache().hset("contacts", dataent.session.user, cached_contacts)
