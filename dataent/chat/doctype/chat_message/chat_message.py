...
from   dataent import _, _dict
import dataent

# imports - dataent module imports
from dataent.chat 	  import authenticate
from dataent.chat.util import (
	get_if_empty,
	check_url,
	dictify,
	get_emojis,
...
def get_message_mentions(content):
	...
	for token in tokens:
		if token.startswith('@'):
			what = token[1:]
			if dataent.db.exists('User', what):
				mentions.append(what)
		else:
			if dataent.db.exists('User', token):
...
def get_new_chat_message_doc(user, room, content, type = "Content", link = True):
	...
	room = dataent.get_doc('Chat Room', room)

	meta = get_message_meta(content)
	mess = dataent.new_doc('Chat Message')
	mess.room 	   = room.name
...
def send(user, room, content, type = "Content"):
	...
	dataent.publish_realtime('dataent.chat.message:create', mess, room = room,
		after_commit = True)
...
def seen(message, user = None):
	...
	if has_message:
		mess = dataent.get_doc('Chat Message', message)
		mess.add_seen(user)

		room = mess.room
		resp = dict(message = message, data = dict(seen = json.loads(mess._seen)))

		dataent.publish_realtime('dataent.chat.message:update', resp, room = room, after_commit = True)

def history(room, fields = None, limit = 10, start = None, end = None):
	room = dataent.get_doc('Chat Room', room)
	mess = dataent.get_all('Chat Message',
		filters  = [
	...
	dataent.enqueue('dataent.chat.doctype.chat_message.chat_message.mark_messages_as_seen',
		message_names=[m.name for m in mess], user=dataent.session.user)
...
def mark_messages_as_seen(message_names, user):
	...
	for name in message_names:
		seen = dataent.db.get_value('Chat Message', name, '_seen') or '[]'
		seen = json.loads(seen)
		seen.append(user)
		seen = json.dumps(seen)
		dataent.db.set_value('Chat Message', name, '_seen', seen, update_modified=False)
