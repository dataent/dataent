...
from   dataent.chat.doctype.chat_room       import chat_room
		...
		if not self.is_new():
			...
			if diff:
				...
				if 'status' in fields:
					...
					for room in rooms:
						dataent.publish_realtime('dataent.chat.profile:update', update, room = room.name, after_commit = True)

				if 'enable_chat' in fields:
					update = dict(user = user, data = dict(enable_chat = bool(self.enable_chat)))
					dataent.publish_realtime('dataent.chat.profile:update', update, user = user, after_commit = True)
...
def get(user, fields = None):
	duser   = dataent.get_doc('User', user)

	if dataent.db.exists('Chat Profile', user):
