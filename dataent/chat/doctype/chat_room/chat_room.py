...
from dataent.core.doctype.version.version import get_diff
...
def is_direct(owner, other, bidirectional = False):
	def get_room(owner, other):
		room = dataent.get_all('Chat Room', filters = [
...
class ChatRoom(Document):
	def validate(self):
		...
		if self.type == "Direct":
			if len(self.users) != 1:
				dataent.throw(_('{0} room must have atmost one user.').format(self.type))

			other = squashify(self.users)

			if self.is_new():
				if is_direct(self.owner, other.user, bidirectional = True):
					dataent.throw(_('Direct room with {0} already exists.').format(other.user))

		if self.type == "Group" and not self.room_name:
			dataent.throw(_('Group name cannot be empty.'))
	...
	def on_update(self):
		if not self.is_new():
			...
			if diff:
				...
				dataent.publish_realtime('dataent.chat.room:update', update, room = self.name, after_commit = True)
...
def get(user, rooms = None, fields = None, filters = None):
	...
	# - Achilles Rasquinha achilles@epaas.xyz
	authenticate(user)
...
def create(kind, owner, users = None, name = None):
	...
	for u in users:
		dataent.publish_realtime('dataent.chat.room:create', room, user = u, after_commit = True)
