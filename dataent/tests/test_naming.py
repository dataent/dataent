...
from dataent.model.naming import getseries
from dataent.model.naming import append_number_if_name_exists, revert_series_if_last

class TestNaming(unittest.TestCase):
	def tearDown(self):
		# Reset ToDo autoname to hash
		todo_doctype = dataent.get_doc('DocType', 'ToDo')
	...
	def test_format_autoname(self):
		...
		todo_doctype = dataent.get_doc('DocType', doctype)
		todo_doctype.autoname = 'format:TODO-{MM}-{status}-{##}'
		todo_doctype.save()

		description = 'Format'

		todo = dataent.new_doc(doctype)
	...
	def test_revert_series(self):
		...
		dataent.db.sql("""INSERT INTO `tabSeries` (name, current) values (%s, 1)""", (series,))
		revert_series_if_last(key, name)
		count = dataent.db.sql("""SELECT current from `tabSeries` where name = %s""", series, as_dict=True)[0]

		self.assertEqual(count.get('current'), 0)
		dataent.db.sql("""delete from `tabSeries` where name = %s""", series)

		series = 'TEST-{}-'.format(year)
		key = 'TEST-.YYYY.-.#####'
		name = 'TEST-{}-00002'.format(year)
		dataent.db.sql("""INSERT INTO `tabSeries` (name, current) values (%s, 2)""", (series,))
		revert_series_if_last(key, name)
		count = dataent.db.sql("""SELECT current from `tabSeries` where name = %s""", series, as_dict=True)[0]
		...
		key = 'TEST-'
		name = 'TEST-00003'
		dataent.db.sql("""INSERT INTO `tabSeries` (name, current) values (%s, 3)""", (series,))
		revert_series_if_last(key, name)
		count = dataent.db.sql("""SELECT current from `tabSeries` where name = %s""", series, as_dict=True)[0]
