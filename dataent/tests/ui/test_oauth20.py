...
class TestOAuth20(unittest.TestCase):
	...
	def test_login_using_authorization_code(self):
		...
		# Revoke Token
		revoke_token_response = requests.post(dataent.get_site_config().host_name + "/api/method/dataent.integrations.oauth2.revoke_token",
	...
	def test_login_using_implicit_token(self):
		...
		oauth_client = dataent.get_doc("OAuth Client", self.client_id)
...
def check_valid_openid_response(access_token=None):
	...
	openid_response = requests.get(dataent.get_site_config().host_name +
		"/api/method/dataent.integrations.oauth2.openid_profile", headers=headers)
