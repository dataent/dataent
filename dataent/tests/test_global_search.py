...
from dataent.utils import global_search
...
class TestGlobalSearch(unittest.TestCase):
	...
	def tearDown(self):
		dataent.db.sql('delete from `tabProperty Setter` where doc_type="Event"')
	...
	def insert_test_events(self):
		...
		for text in phrases:
			dataent.get_doc(dict(
				...
				starts_on=dataent.utils.now_datetime())).insert()
	...
	def test_update_doc(self):
		...
		event = dataent.get_doc('Event', dataent.get_all('Event')[0].name)
	...
	def test_delete_doc(self):
		...
		event_name = dataent.get_all('Event')[0].name
		event = dataent.get_doc('Event', event_name)
		test_subject = event.subject
		results = global_search.search(test_subject)
		self.assertEqual(len(results), 1)

		dataent.delete_doc('Event', event_name)
		global_search.sync_global_search()
