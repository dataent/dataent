...
import dataent
import unittest
from dataent.utils.password import update_password, check_password, passlibctx

class TestPassword(unittest.TestCase):
	def setUp(self):
		dataent.delete_doc('Email Account', 'Test Email Account Password')
		dataent.delete_doc('Email Account', 'Test Email Account Password-new')
	...
	def test_encrypted_password(self):
		...
		auth_password = dataent.db.sql('''select `password` from `__Auth`
	...
	def make_email_account(self, name='Test Email Account Password'):
		if not dataent.db.exists('Email Account', name):
			...
			return dataent.get_doc('Email Account', name)
	...
	def test_hashed_password(self, user='test@example.com'):
		...
		self.assertRaises(dataent.AuthenticationError, check_password, user, new_password)
	...
	def test_password_on_rename_user(self):
		...
		dataent.rename_doc(doc.doctype, old_name, new_name)

		new_doc = dataent.get_doc(doc.doctype, new_name)
		self.assertEqual(new_doc.get_password(), password)
		self.assertTrue(not dataent.db.sql('''select `password` from `__Auth`
			where doctype=%s and name=%s and fieldname="password"''', (doc.doctype, doc.name)))

		dataent.rename_doc(doc.doctype, new_name, old_name)
		self.assertTrue(dataent.db.sql('''select `password` from `__Auth`
			where doctype=%s and name=%s and fieldname="password"''', (doc.doctype, doc.name)))
