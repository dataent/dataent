...
from dataent.core.page.permission_manager.permission_manager import get_roles_and_doctypes
...
class TestDomainification(unittest.TestCase):
	...
	def tearDown(self):
		dataent.db.sql("delete from tabRole where name='_Test Role'")
		dataent.db.sql("delete from `tabHas Role` where role='_Test Role'")
		dataent.db.sql("delete from tabDomain where name in ('_Test Domain 1', '_Test Domain 2')")
		dataent.delete_doc('DocType', 'Test Domainification')
		self.remove_from_active_domains(remove_all=True)
	...
	def test_module_def_for_domainification(self):
		...
		test_module_def = dataent.get_doc("Module Def", "Contacts")
