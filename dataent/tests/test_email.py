...
import unittest, dataent, re, email
...
class TestEmail(unittest.TestCase):
	...
	def test_email_queue(self, send_after=None):
		dataent.sendmail(recipients = ['test@example.com', 'test1@example.com'],
			sender="admin@example.com",
			reference_doctype='User', reference_name='Administrator',
			subject='Testing Queue', message='This mail is queued!',
			unsubscribe_message="Unsubscribe", send_after=send_after)

		email_queue = dataent.db.sql("""select name,message from `tabEmail Queue` where status='Not Sent'""", as_dict=1)
		self.assertEqual(len(email_queue), 1)
		queue_recipients = [r.recipient for r in dataent.db.sql("""SELECT recipient FROM `tabEmail Queue Recipient`
	...
	def test_send_after(self):
		...
		email_queue = dataent.db.sql("""select name from `tabEmail Queue` where status='Sent'""", as_dict=1)
	...
	def test_flush(self):
		...
		self.assertTrue('test1@example.com' in queue_recipients)
		self.assertEqual(len(queue_recipients), 2)
		self.assertTrue('Unsubscribe' in dataent.safe_decode(dataent.flags.sent_mail))

	def test_cc_header(self):
		#test if sending with cc's makes it into header
		dataent.sendmail(recipients=['test@example.com'],
		...
		email_queue = dataent.db.sql("""select name from `tabEmail Queue` where status='Not Sent'""", as_dict=1)
		self.assertEqual(len(email_queue), 1)
		queue_recipients = [r.recipient for r in dataent.db.sql("""select recipient from `tabEmail Queue Recipient`
			where status='Not Sent'""", as_dict=1)]
		self.assertTrue('test@example.com' in queue_recipients)
	...
	def test_cc_footer(self):
		...
		self.assertTrue('This email was sent to test@example.com and copied to test1@example.com' in dataent.safe_decode(dataent.flags.sent_mail))
	...
	def test_expose(self):
		...
		email_obj = email.message_from_string(dataent.safe_decode(dataent.flags.sent_mail))
		for part in email_obj.walk():
			content = part.get_payload(decode=True)

			if content:
				dataent.local.flags.signed_query_string = re.search('(?<=/api/method/dataent.email.queue.unsubscribe\?).*(?=\n)', content.decode()).group(0)
				self.assertTrue(verify_request())
				break

	def test_expired(self):
		self.test_email_queue()
		dataent.db.sql("update `tabEmail Queue` set modified=DATE_SUB(curdate(), interval 8 day)")
		from dataent.email.queue import clear_outbox
		clear_outbox()
		email_queue = dataent.db.sql("""select name from `tabEmail Queue` where status='Expired'""", as_dict=1)
		self.assertEqual(len(email_queue), 1)
		queue_recipients = [r.recipient for r in dataent.db.sql("""select recipient from `tabEmail Queue Recipient`
	...
	def test_unsubscribe(self):
		...
		self.assertTrue(dataent.db.get_value("Email Unsubscribe",
			{"reference_doctype": "User", "reference_name": "Administrator", "email": "test@example.com"}))

		before = dataent.db.sql("""select count(name) from `tabEmail Queue` where status='Not Sent'""")[0][0]
		...
		email_queue = dataent.db.sql("""select name from `tabEmail Queue` where status='Not Sent'""",
			as_dict=1)
	...
	def test_email_queue_limit(self):
		from dataent.email.queue import send, EmailLimitCrossedError
	...
	def test_image_parsing(self):
		import re
		email_account = dataent.get_doc('Email Account', '_Test Email Account 1')

		dataent.db.sql('''delete from `tabCommunication` where sender = 'sukh@yyy.com' ''')

		with open(dataent.get_app_path('dataent', 'tests', 'data', 'email_with_image.txt'), 'r') as raw:
