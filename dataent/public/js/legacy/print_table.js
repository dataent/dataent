dataent.printTable = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
		if(!this.columns)
	...
	get_columns: function() {
		var perms = dataent.perm.get_perm(this.doctype);
		return ['Sr'].concat($.map(dataent.meta.docfield_list[this.tabletype], function(df) {
			return (cint(df.print_hide) || !(perms[df.permlevel] &&
				perms[df.permlevel].read)) ? null : df.fieldname;
		}));
	},
	get_data: function() {
		var children = dataent.get_doc(this.doctype, this.docname)[this.fieldname] || [];
	...
	add_table: function(data) {
		...
		$.each(me.columns, function(ci, fieldname) {
			var df = dataent.meta.docfield_map[me.tabletype][fieldname];
			if(me.head_labels) {
		...
		$.each(data, function(ri, row) {
			...
			if(allow) {
				...
				$.each(me.columns, function(ci, fieldname) {
					...
					var df = dataent.meta.docfield_map[me.tabletype][fieldname];
					value = dataent.format(value, df, {for_print:true});
