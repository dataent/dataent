...
window.set_field_tip = function(n,txt) {
	var df = dataent.meta.get_docfield(cur_frm.doctype, n, cur_frm.docname);
	if(df)df.description = txt;

	if(cur_frm && cur_frm.fields_dict) {
		if(cur_frm.fields_dict[n])
			cur_frm.fields_dict[n].comment_area.innerHTML = dataent.utils.replace_newlines(txt);
		else
...
window.refresh_field = function(n, docname, table_field) {
	...
	if (n && typeof n==='string' && table_field){
		var grid = cur_frm.fields_dict[table_field].grid,
			field = dataent.utils.filter_dict(grid.docfields, {fieldname: n});
		if (field && field.length){
			field = field[0];
			var meta = dataent.meta.get_docfield(field.parent, field.fieldname, docname);
			$.extend(field, meta);
			if (docname){
...
_f.Frm.prototype.set_currency_labels = function(fields_list, currency, parentfield) {
	...
	$.each(fields_list, function(i, fname) {
		var docfield = dataent.meta.docfield_map[doctype][fname];
		if(docfield) {
	...
	$.each(grid_field_label_map, function(fname, label) {
		fname = fname.split("-");
		var df = dataent.meta.get_docfield(fname[0], fname[1], me.doc.name);
...
_f.Frm.prototype.field_map = function(fnames, fn) {
	...
	for (var i=0, l=fnames.length; i<l; i++) {
		...
		var field = dataent.meta.get_docfield(cur_frm.doctype, fieldname, this.docname);
		if(field) {
			fn(field);
...
_f.Frm.prototype.get_docfield = function(fieldname1, fieldname2) {
	if(fieldname2) {
		...
		return dataent.meta.get_docfield(doctype, fieldname2, this.docname);
	} else {
		// for parent
		return dataent.meta.get_docfield(this.doctype, fieldname1, this.docname);
	}
};
...
_f.Frm.prototype.set_df_property = function(fieldname, property, value, docname, table_field) {
	...
	if (!docname && !table_field){
		...
		var grid = this.fields_dict[table_field].grid,
			fname = dataent.utils.filter_dict(grid.docfields, {'fieldname': fieldname});
		if (fname && fname.length)
			df = dataent.meta.get_docfield(fname[0].parent, fieldname, docname);
	}
...
_f.Frm.prototype.get_files = function() {
	return this.attachments
		? dataent.utils.sort(this.attachments.get_attachments(), "file_name", "string")
		: [] ;
};
...
_f.Frm.prototype.clear_table = function(fieldname) {
	dataent.model.clear_table(this.doc, fieldname);
};

_f.Frm.prototype.add_child = function(fieldname, values) {
	var doc = dataent.model.add_child(this.doc, dataent.meta.get_docfield(this.doctype, fieldname).options, fieldname);
	if(values) {
...
_f.Frm.prototype.set_value = function(field, value, if_missing) {
	...
	var _set = function(f, v) {
		...
		if(fieldobj) {
			if(!if_missing || !dataent.model.has_value(me.doctype, me.doc.name, f)) {
				if(fieldobj.df.fieldtype==="Table" && $.isArray(v)) {

					dataent.model.clear_table(me.doc, fieldobj.df.fieldname);

					for (var i=0, j=v.length; i < j; i++) {
						var d = v[i];
						var child = dataent.model.add_child(me.doc, fieldobj.df.options,
					...
					return dataent.model.set_value(me.doctype, me.doc.name, f, v);
				}
			}
		} else {
			dataent.msgprint(__("Field {0} not found.",[f]));
			throw "frm.set_value";
		}
	};
...
_f.Frm.prototype.call = function(opts, args, callback) {
	...
	if(!opts.doc) {
		if(opts.method.indexOf(".")===-1)
			opts.method = dataent.model.get_server_module_name(me.doctype) + "." + opts.method;
		...
		opts.callback = function(r) {
			if($.isPlainObject(r.message)) {
				if(opts.child) {
					...
					var std_field_list = ["doctype"].concat(dataent.model.std_fields_list);
...
_f.Frm.prototype.set_read_only = function() {
	var perm = [];
	var docperms = dataent.perm.get_perm(this.doc.doctype);
...
_f.Frm.prototype.get_formatted = function(fieldname) {
	return dataent.format(this.doc[fieldname],
		dataent.meta.get_docfield(this.doctype, fieldname, this.docname),
		{no_icon:true}, this.doc);
};

_f.Frm.prototype.open_grid_row = function() {
	return dataent.ui.form.get_open_grid_form();
};
...
_f.Frm.prototype.get_selected = function() {
	...
	dataent.meta.get_table_fields(this.doctype).forEach(function(df) {
...
_f.Frm.prototype.set_indicator_formatter = function(fieldname, get_color, get_text) {
	...
	var doctype;
	if(dataent.meta.docfield_map[this.doctype][fieldname]) {
		doctype = this.doctype;
	} else {
		dataent.meta.get_table_fields(this.doctype).every(function(df) {
			if(dataent.meta.docfield_map[df.options][fieldname]) {
				doctype = df.options;
				return false;
			} else {
				return true;
	...
	dataent.meta.docfield_map[doctype][fieldname].formatter =
		function(value, df, options, doc) {
			if(value) {
				var label;
				if(get_text) {
					label = get_text(doc);
				} else if(dataent.form.link_formatters[df.options]) {
					label = dataent.form.link_formatters[df.options](value, doc);
				} else {
					label = value;
				}
...
_f.Frm.prototype.can_create = function(doctype) {
	...
	if(!dataent.model.can_create(doctype)) {
		return false;
	}
...
_f.Frm.prototype.make_new = function(doctype) {
	...
	if(this.make_methods && this.make_methods[doctype]) {
		...
		dataent.model.with_doctype(doctype, function() {
			var new_doc = dataent.model.get_new_doc(doctype);

			// set link fields (if found)
			dataent.get_meta(doctype).fields.forEach(function(df) {
				if(df.fieldtype==='Link' && df.options===me.doctype) {
					new_doc[df.fieldname] = me.doc.name;
				}
			});

			dataent.ui.form.make_quick_entry(doctype, null, null, new_doc);
			// dataent.set_route('Form', doctype, new_doc.name);
		});
	}
};
