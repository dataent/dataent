...
dataent.urllib = {
	...
	get_base_url: function() {
		// var url= (dataent.base_url || window.location.href).split('#')[0].split('?')[0].split('desk')[0];
		var url = (dataent.base_url || window.location.origin)
	...
	get_full_url: function(url) {
		...
		return url.substr(0,1)==="/" ?
			(dataent.urllib.get_base_url() + url) :
			(dataent.urllib.get_base_url() + "/" + url);
	}
}

window.get_url_arg = dataent.urllib.get_arg;
window.get_url_dict = dataent.urllib.get_dict;
