...
dataent.utils.full_name = function(fn, ln) {
	return fn + (ln ? ' ' : '') + (ln ? ln : '')
}
...
function set_value_in(ele, v, ftype, fopt, doc) {
	$(ele).html(dataent.format(v, {fieldtype:ftype, options:fopt}, null, doc));
	return;
}
...
function validate_email(txt) {
	return dataent.utils.validate_type(txt, "email");
}
function validate_spl_chars(txt) {
	return dataent.utils.validate_type(txt, "alphanum")
}
function cstr(s) {
	if(s==null)return '';
	return s+'';
