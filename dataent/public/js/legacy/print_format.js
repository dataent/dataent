...
$.extend(_p, {
	...
	*/
	build: function(fmtname, onload, no_letterhead, only_body, no_heading) {
		...
		if(!cur_frm) {
			dataent.msgprint(__("No document selected"));
			return;
		}

		if(!dataent.model.can_print(cur_frm.doctype, cur_frm)) {
		...
		if(args.fmtname == 'Standard') {
			...
			if(!print_format_doc) {
				dataent.msgprint(__("Unknown Print Format: {0}", [args.fmtname]));
				return;
			}
			args.onload(_p.render({
...
<html>\
	...
	get_letter_head: function() {
		...
		if(cur_frm.doc.letter_head) {
			lh = cstr(dataent.boot.letter_heads[cur_frm.doc.letter_head].header);
		} else if (dataent.boot.sysdefaults.default_letter_head_content) {
			lh = dataent.boot.sysdefaults.default_letter_head_content;
		}
		return lh;
	},
	...
	print_std: function(no_letterhead, no_heading) {
		...
		var data = dataent.get_children("DocType", doctype, "fields");
			...
			build_data: function(data, doctype, docname) {
				...
				for(var i = 0; i < data.length; i++) {
					var fieldname = data[i].fieldname ? data[i].fieldname :
						data[i].label;
					var field = fieldname ?
						dataent.meta.get_docfield(doctype, fieldname, docname) : data[i];
					if(!field.print_hide) {
	...
	print_std_add_field: function(dt, dn, f, layout) {
		...
		if(f.fieldtype!='Button') {
			if(val || in_list(['Float', 'Int', 'Currency'], f.fieldtype)) {
				...
				row.cells[1].innerHTML = dataent.format(val, f, {for_print: true});
