...
dataent.ui.form.Controller = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
	}
});
...
_f.Frm = function(doctype, parent, in_form) {
	...
	this.sections = [];
	this.grids = [];
	this.cscript = new dataent.ui.form.Controller({frm:this});
	this.events = {};
	this.pformat = {};
	this.fetch_dict = {};
	this.parent = parent;
...
_f.Frm.prototype.check_doctype_conflict = function(docname) {
	if(this.doctype=='DocType' && docname=='DocType') {
		dataent.msgprint(__('Allowing DocType, DocType. Be careful!'));
	} else if(this.doctype=='DocType') {
		if (dataent.views.formview[docname] || dataent.pages['List/'+docname]) {
			window.location.reload();
			//	dataent.msgprint(__("Cannot open {0} when its instance is open", ['DocType']))
			// throw 'doctype open conflict'
		}
	} else {
		if (dataent.views.formview.DocType && dataent.views.formview.DocType.frm.opendocs[this.doctype]) {
			window.location.reload();
			//	dataent.msgprint(__("Cannot open instance when its {0} is open", ['DocType']))
			// throw 'doctype open conflict'
		}
	}
};
...
_f.Frm.prototype.setup = function() {
	this.fields = [];
	this.fields_dict = {};
	this.state_fieldname = dataent.workflow.get_state_fieldname(this.doctype);
	...
	this.toolbar = new dataent.ui.form.Toolbar({
		frm: this,
		page: this.page
	});
	...
	this.script_manager = new dataent.ui.form.ScriptManager({
		frm: this
	});
	...
	if(!this.meta.hide_toolbar) {
		this.footer = new dataent.ui.form.Footer({
			frm: this,
...
_f.Frm.prototype.setup_drag_drop = function() {
	...
	this.$wrapper.on('dragenter dragover', false)
		.on('drop', function(e) {
			...
			if(me.doc.__islocal) {
				dataent.msgprint(__("Please save before attaching."));
				throw "attach error";
			}

			if(me.attachments.max_reached()) {
				dataent.msgprint(__("Maximum Attachment Limit for this record reached."));
				throw "attach error";
			}
...
_f.Frm.prototype.setup_print_layout = function() {
	var me = this;
	this.print_preview = new dataent.ui.form.PrintPreview({
		frm: this
	});
...
_f.Frm.prototype.print_doc = function() {
	...
	if(!dataent.model.can_print(this.doc.doctype, this)) {
		dataent.msgprint(__("You are not allowed to print this document"));
		return;
	}
...
_f.Frm.prototype.watch_model_updates = function() {
	...
	// on main doc
	dataent.model.on(me.doctype, "*", function(fieldname, value, doc) {
		// set input
	...
	// on table fields
	var table_fields = dataent.get_children("DocType", me.doctype, "fields", {fieldtype:"Table"});

	// using $.each to preserve df via closure
	$.each(table_fields, function(i, df) {
		dataent.model.on(df.options, "*", function(fieldname, value, doc) {
...
_f.Frm.prototype.setup_std_layout = function() {
	...
	// layout
	this.layout = new dataent.ui.form.Layout({
		parent: this.body,
	...
	this.document_flow = new dataent.ui.form.DocumentFlow({
		frm: this
	});

	this.dashboard = new dataent.ui.form.Dashboard({
		frm: this,
	});

	// state
	this.states = new dataent.ui.form.States({
		frm: this
	});
};
...
_f.Frm.prototype.email_doc = function(message) {
	new dataent.views.CommunicationComposer({
		doc: this.doc,
		frm: this,
...
_f.Frm.prototype.rename_doc = function() {
	dataent.model.rename_doc(this.doctype, this.docname);
};
...
_f.Frm.prototype.rename_notify = function(dt, old, name) {
	...
	// cleanup
	if(this && this.opendocs[old] && dataent.meta.docfield_copy[dt]) {
		// delete docfield copy
		dataent.meta.docfield_copy[dt][name] = dataent.meta.docfield_copy[dt][old];
		delete dataent.meta.docfield_copy[dt][old];
	}
	...
	dataent.re_route[window.location.hash] = '#Form/' + encodeURIComponent(this.doctype) + '/' + encodeURIComponent(name);
	dataent.set_route('Form', this.doctype, name);
};

// SETUP

_f.Frm.prototype.setup_meta = function() {
	this.meta = dataent.get_doc('DocType', this.doctype);
	this.perm = dataent.perm.get_perm(this.doctype); // for create
	if(this.meta.istable) {
...
_f.Frm.prototype.refresh_header = function(is_a_different_doc) {
	...
	// main title
	if(!this.meta.in_dialog || this.in_form) {
		dataent.utils.set_title(this.meta.issingle ? this.doctype : this.docname);
	}
	...
	if(this.meta.is_submittable
		...
		&& !this.is_dirty()
		&& !this.is_new()
		&& !dataent.model.has_workflow(this.doctype) // show only if no workflow
...
_f.Frm.prototype.check_doc_perm = function() {
	...
	this.perm = dataent.perm.get_perm(dt, this.doc);
...
_f.Frm.prototype.refresh = function(docname) {
	...
	if(this.docname) { // document to show
		...
		// set the doc
		this.doc = dataent.get_doc(this.doctype, this.docname);

		// check permissions
		if(!this.check_doc_perm()) {
			dataent.show_not_permitted(__(this.doctype) + " " + __(this.docname));
			return;
		}

		// read only (workflow)
		this.read_only = dataent.workflow.is_read_only(this.doctype, this.docname);
...
_f.Frm.prototype.render_form = function(is_a_different_doc) {
	if(!this.meta.istable) {
		...
		this.sidebar = new dataent.ui.form.Sidebar({
			frm: this,
			page: this.page
		});
		this.sidebar.make();
...
_f.Frm.prototype.setnewdoc = function() {
	...
	this.script_manager.trigger("before_load", this.doctype, this.docname)
		.then(() => {
			...
			dataent.after_ajax(function() {
				me.trigger_link_fields();
			});

			dataent.breadcrumbs.add(me.meta.module, me.doctype);
		});
...
_f.Frm.prototype.copy_doc = function(onload, from_amend) {
	...
	var newdoc = dataent.model.copy_doc(this.doc, from_amend);
	...
	dataent.set_route('Form', newdoc.doctype, newdoc.name);
};
...
_f.Frm.prototype.reload_doc = function() {
	...
	if(!me.doc.__islocal) {
		dataent.model.remove_from_locals(me.doctype, me.docname);
		dataent.model.with_doc(me.doctype, me.docname, function() {
			me.refresh();
		});
	}
};
...
dataent.validated = 0;
// Proxy for dataent.validated
Object.defineProperty(window, 'validated', {
	get: function() {
		console.warn('Please use `dataent.validated` instead of `validated`. It will be deprecated soon.');
		return dataent.validated;
	},
	set: function(value) {
		console.warn('Please use `dataent.validated` instead of `validated`. It will be deprecated soon.');
		dataent.validated = value;
		return dataent.validated;
	}
});
...
_f.Frm.prototype._save = function(save_action, callback, btn, on_error, resolve, reject) {
	...
	var after_save = function(r) {
		if (!r.exc) {
			if (["Save", "Update", "Amend"].indexOf(save_action) !== -1) {
				dataent.utils.play_sound("click");
			}
	...
	if(save_action != "Update") {
		// validate
		dataent.validated = true;
		dataent.run_serially([
			() => this.script_manager.trigger("validate"),
			() => this.script_manager.trigger("before_save"),
			() => {
				if(!dataent.validated) {
					fail();
					return;
				}

				dataent.ui.form.save(me, save_action, after_save, btn);
			}
		]).catch(fail);
	} else {
		dataent.ui.form.save(me, save_action, after_save, btn);
...
_f.Frm.prototype.savesubmit = function(btn, callback, on_error) {
	...
	return new Promise(resolve => {
		...
		dataent.confirm(__("Permanently Submit {0}?", [this.docname]), function() {
			dataent.validated = true;
			me.script_manager.trigger("before_submit").then(function() {
				...
				me.save('Submit', function(r) {
					if(r.exc) {
						...
						dataent.utils.play_sound("submit");
						callback && callback();
...
_f.Frm.prototype.savecancel = function(btn, callback, on_error) {
	...
	dataent.confirm(__("Permanently Cancel {0}?", [this.docname]), function() {
		dataent.validated = true;
		me.script_manager.trigger("before_cancel").then(function() {
			...
			var after_cancel = function(r) {
				if(r.exc) {
					...
					dataent.utils.play_sound("cancel");
					me.refresh();
					callback && callback();
					me.script_manager.trigger("after_cancel");
				}
			};
			dataent.ui.form.save(me, "cancel", after_cancel, btn);
		});
	}, () => handle_fail());
};
...
_f.Frm.prototype.savetrash = function() {
	...
	dataent.model.delete_doc(this.doctype, this.docname, function() {
		window.history.back();
	});
};
...
_f.Frm.prototype.amend_doc = function() {
	...
	var fn = function(newdoc) {
		...
		if(me.fields_dict && me.fields_dict['amendment_date'])
			newdoc.amendment_date = dataent.datetime.obj_to_str(new Date());
	};
	this.copy_doc(fn, 1);
	dataent.utils.play_sound("click");
};
...
_f.Frm.prototype.get_docinfo = function() {
	return dataent.model.docinfo[this.doctype][this.docname];
};
...
_f.Frm.prototype.reload_docinfo = function(callback) {
	var me = this;
	dataent.call({
		method: "dataent.desk.form.load.get_docinfo",
		args: {
			doctype: me.doctype,
			name: me.doc.name
		},
...
_f.Frm.prototype.set_footnote = function(txt) {
	this.footnote_area = dataent.utils.set_footnote(this.footnote_area, this.body, txt);
};
...
_f.Frm.prototype.validate_form_action = function(action, resolve) {
	...
	var perms = dataent.perm.get_perm(this.doc.doctype)[0];
	...
	if ((dataent.workflow.is_read_only(this.doctype, this.docname) && (perms["write"] ||
		perms["create"] || perms["submit"] || perms["cancel"])) || !dataent.workflow.is_read_only(this.doctype, this.docname)) {
	...
	if (!this.perm[0][perm_to_check] && !allowed_for_workflow) {
		...
		dataent.throw (__("No permission to '{0}' {1}", [__(action), __(this.doc.doctype)]));
	}
};

_f.Frm.prototype.has_perm = function(ptype) {
	return dataent.perm.has_perm(this.doctype, 0, ptype, this.doc);
};

_f.Frm.prototype.scroll_to_element = function() {
	if (dataent.route_options && dataent.route_options.scroll_to) {
		var scroll_to = dataent.route_options.scroll_to;
		delete dataent.route_options.scroll_to;
