...
dataent.upload = {
	make: function(opts) {
		...
		var $upload = $(dataent.render_template("upload", {opts:opts})).appendTo(opts.parent);
		...
		new dataent.ui.DropZone($dropzone, {
			drop: function (files) {
				$dropzone.hide();
		...
		$file_input.on("change", function() {
			if (this.files.length > 0 || opts.files) {
				...
				var file_pills = file_array.map(
					file => dataent.upload.make_file_row(file, opts)
				);
				$uploaded_files_wrapper.append(file_pills);
			} else {
				dataent.upload.show_empty_state($upload);
			}
		});
		...
		opts.btn.click(function() {
			...
			if(opts.args.gs_template) {
				dataent.integration_service.gsuite.create_gsuite_file(opts.args,opts);
			} else if(file_url) {
				opts.args.file_url = file_url;
				dataent.upload.upload_file(null, opts.args, opts);
			} else {
				var files = $upload.data('attached_files');
				dataent.upload.upload_multiple_files(files, opts.args, opts);
			}
		});
	},
	make_file_row: function(file, { show_private } = {}) {
		const safe_file_name = dataent.utils.xss_sanitise(file.name);
		var template = `
	...
	upload_multiple_files: function(files /*FileData array*/, args, opts) {
		var i = -1;
		dataent.upload.total_files = files ? files.length : 0;
		...
		function upload_next() {
			if(files) {
				...
				if(!opts.progress) {
					dataent.show_progress(__('Uploading'), i, files.length);
				}
			}
			dataent.upload.upload_file(file, args, opts);
		}
	...
	upload_file: function(fileobj, args, opts) {
		if(!fileobj && !args.file_url) {
			if(opts.on_no_attach) {
				...
				dataent.msgprint(__("Please attach a file or set a URL"));
			}
			return;
		}

		if(fileobj) {
			dataent.upload.read_file(fileobj, args, opts);
		} else {
			// with file_url
			dataent.upload._upload_file(fileobj, args, opts);
		}
	},

	_upload_file: function(fileobj, args, opts, dataurl) {
		if (args.file_size) {
			dataent.upload.validate_max_file_size(args.file_size);
		}
		if(opts.on_attach) {
			...
			if (opts.confirm_is_private) {
				dataent.prompt({
					...
					dataent.upload.upload_to_server(fileobj, args, opts);
	...
	read_file: function(fileobj, args, opts) {
		...
		if(opts.options && opts.options.toLowerCase()=="image") {
			if(!dataent.utils.is_image_file(args.filename)) {
				dataent.msgprint(__("Only image extensions (.gif, .jpg, .jpeg, .tiff, .png, .svg) allowed"));
				return;
			}
		}

		let start_complete = dataent.cur_progress ? dataent.cur_progress.percent : 0;
		...
		var upload_with_filedata = function() {
			...
			freader.onload = function() {
				...
				dataent.upload._upload_file(fileobj, args, opts, dataurl);
			};
		...
		if (!dataent.socketio || opts.no_socketio ||
				dataent.flags.no_socketio || file_not_big_enough) {
			upload_with_filedata();
			return;
		} else {
			args.file_size = fileobj.size;
			dataent.call({
				method: 'dataent.utils.file_manager.validate_filename',
		...
		var upload_through_socketio = function() {
			dataent.socketio.uploader.start({
				file: fileobj,
				...
				on_progress: (percent_complete) => {
					let increment = (flt(percent_complete) / dataent.upload.total_files);
					dataent.show_progress(__('Uploading'),
	...
	validate_max_file_size: function(file_size) {
		var max_file_size = dataent.boot.max_file_size || 5242880;

		if (file_size > max_file_size) {
			// validate max file size
			dataent.throw(__("File size exceeded the maximum allowed size of {0} MB", [max_file_size / 1048576]));
		}
	},
			...
			primary_action: function(){
				...
				opts.loopcallback = function (){
					if (i < j) {
						...
						dataent.upload.upload_file(fileobjs[i], args, opts);
						i++;
					}
				};
