...
$.extend(dataent.perm, {
	...
	has_perm: (doctype, permlevel, ptype, doc) => {
		...
		if (!dataent.perm.doctype_perm[doctype]) {
			dataent.perm.doctype_perm[doctype] = dataent.perm.get_perm(doctype);
		}

		let perms = dataent.perm.doctype_perm[doctype];
		...
		if (permlevel === 0 && perm && doc) {
			let docinfo = dataent.model.get_docinfo(doctype, doc.name);
	...
	get_perm: (doctype, doc) => {
		...
		let meta = dataent.get_doc("DocType", doctype);
		const user  = dataent.session.user;

		if (user === "Administrator" || dataent.user_roles.includes("Administrator")) {
			perm[0].read = 1;
		}

		if (!meta) return perm;

		dataent.perm.build_role_permissions(perm, meta);
				...
				for (let i = 0; i < docinfo.shared.length; i++) {
					...
					if (s.user === user) {
						...
						if (s.read) {
							...
							perm[0].email = dataent.boot.user.can_email.indexOf(doctype) !== -1 ? 1 : 0;
							perm[0].print = dataent.boot.user.can_print.indexOf(doctype) !== -1 ? 1 : 0;
						}
					}
				}
			}
		...
		if (dataent.model.can_read(doctype) && !perm[0].read) {
			// read via sharing
			perm[0].read = 1;
		}
	...
	build_role_permissions: (perm, meta) => {
		...
		$.each(meta.permissions || [], (i, p) => {
			...
			if (dataent.user_roles.includes(p.role)) {
				...
				$.each(dataent.perm.rights, (i, key) => {
	...
	get_match_rules: (doctype, ptype) => {
		...
		let perm = dataent.perm.get_perm(doctype);

		let user_permissions = dataent.defaults.get_user_permissions();

		if (user_permissions && !$.isEmptyObject(user_permissions)) {
			let rules = {};
			let fields_to_check = dataent.meta.get_fields_to_check_permissions(doctype);
			$.each(fields_to_check, (i, df) => {
				const user_permissions_for_doctype = user_permissions[df.options] || [];
				const allowed_records = dataent.perm.get_allowed_docs_for_doctype(user_permissions_for_doctype, doctype);
		...
		if (perm[0].if_owner && perm[0].read) {
			match_rules.push({ "Owner": dataent.session.user });
		}
		return match_rules;
	},
	...
	get_field_display_status: (df, doc, perm, explain) => {
		...
		if (!perm && doc) {
			perm = dataent.perm.get_perm(doc.doctype, doc);
		}
	...
	is_visible: (df, doc, perm) => {
		if (typeof df === 'string') {
			// df is fieldname
			df = dataent.meta.get_docfield(doc.doctype, df, doc.parent || doc.name);
		}

		let status = dataent.perm.get_field_display_status(df, doc, perm);
