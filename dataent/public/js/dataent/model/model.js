...
$.extend(dataent.model, {
	...
	init: function() {
		...
		dataent.realtime.on("doc_update", function(data) {
			// set list dirty
			dataent.views.ListView.trigger_list_update(data);
			var doc = locals[data.doctype] && locals[data.doctype][data.name];

			if(doc) {
				// current document is dirty, show message if its not me
				if(dataent.get_route()[0]==="Form" && cur_frm.doc.doctype===doc.doctype && cur_frm.doc.name===doc.name) {
					if(!dataent.ui.form.is_saving && data.modified!=cur_frm.doc.modified) {
					...
					if(!doc.__unsaved) {
						...
						dataent.model.remove_from_locals(doc.doctype, doc.name);
					} else {
		...
		dataent.realtime.on("list_update", function(data) {
			dataent.views.ListView.trigger_list_update(data);
		});
	...
	is_value_type: function(fieldtype) {
		...
		return dataent.model.no_value_type.indexOf(fieldtype)===-1;
	},

	is_non_std_field: function(fieldname) {
		return !dataent.model.std_fields_list.includes(fieldname);
	},

	get_std_field: function(fieldname) {
		var docfield = $.map([].concat(dataent.model.std_fields).concat(dataent.model.std_fields_table),
			function(d) {
				if(d.fieldname==fieldname) return d;
			});
		if(!docfield.length) {
			dataent.msgprint(__("Unknown Column: {0}", [fieldname]));
		}
		return docfield[0];
	},
	...
	with_doctype: function(doctype, callback, async) {
		if(locals.DocType[doctype]) {
			...
			return dataent.call({
				method:'dataent.desk.form.load.getdoctype',
				type: "GET",
				args: {
					doctype: doctype,
					with_parent: 1,
				...
				callback: function(r) {
					if(r.exc) {
						dataent.msgprint(__("Unable to load: {0}", [__(doctype)]));
						throw "No doctype";
					}
					if(r.message=="use_cache") {
						dataent.model.sync(cached_doc);
					} else {
						localStorage["_doctype:" + doctype] = JSON.stringify(r.docs);
					}
					dataent.model.init_doctype(doctype);

					if(r.user_settings) {
						// remember filters and other settings from last view
						dataent.model.user_settings[doctype] = JSON.parse(r.user_settings);
						dataent.model.user_settings[doctype].updated_on = moment().toString();
					}
					callback && callback(r);
				}
			});
	...
	init_doctype: function(doctype) {
		...
		if(meta.__templates) {
			$.extend(dataent.templates, meta.__templates);
		}
	},
	...
	with_doc: function(doctype, name, callback) {
		return new Promise(resolve => {
			...
			if(locals[doctype] && locals[doctype][name] && dataent.model.get_docinfo(doctype, name)) {
				callback && callback(name);
				resolve(dataent.get_doc(doctype, name));
			} else {
				return dataent.call({
					method: 'dataent.desk.form.load.getdoc',
					type: "GET",
					args: {
						doctype: doctype,
						name: name
	...
	get_docinfo: function(doctype, name) {
		return dataent.model.docinfo[doctype] && dataent.model.docinfo[doctype][name] || null;
	},

	set_docinfo: function(doctype, name, key, value) {
		if (dataent.model.docinfo[doctype] && dataent.model.docinfo[doctype][name]) {
			dataent.model.docinfo[doctype][name][key] = value;
		}
	},

	get_shared: function(doctype, name) {
		return dataent.model.get_docinfo(doctype, name).shared;
	},

	get_server_module_name: function(doctype) {
		var dt = dataent.model.scrub(doctype);
		var module = dataent.model.scrub(locals.DocType[doctype].module);
		var app = dataent.boot.module_app[module];
	...
	can_create: function(doctype) {
		return dataent.boot.user.can_create.indexOf(doctype)!==-1;
	},

	can_read: function(doctype) {
		return dataent.boot.user.can_read.indexOf(doctype)!==-1;
	},

	can_write: function(doctype) {
		return dataent.boot.user.can_write.indexOf(doctype)!==-1;
	},

	can_get_report: function(doctype) {
		return dataent.boot.user.can_get_report.indexOf(doctype)!==-1;
	},

	can_delete: function(doctype) {
		if(!doctype) return false;
		return dataent.boot.user.can_delete.indexOf(doctype)!==-1;
	},

	can_cancel: function(doctype) {
		if(!doctype) return false;
		return dataent.boot.user.can_cancel.indexOf(doctype)!==-1;
	},

	has_workflow: function(doctype) {
		return dataent.get_list('Workflow', {'document_type': doctype,
			'is_active': 1}).length;
	},
	...
	is_single: function(doctype) {
		if(!doctype) return false;
		return dataent.boot.single_types.indexOf(doctype) != -1;
	},

	can_import: function(doctype, frm) {
		// system manager can always import
		if(dataent.user_roles.includes("System Manager")) return true;

		if(frm) return frm.perm[0].import===1;
		return dataent.boot.user.can_import.indexOf(doctype)!==-1;
	},
	...
	can_export: function(doctype, frm) {
		...
		return dataent.boot.user.can_export.indexOf(doctype)!==-1;
	},

	can_print: function(doctype, frm) {
		if(frm) return frm.perm[0].print===1;
		return dataent.boot.user.can_print.indexOf(doctype)!==-1;
	},

	can_email: function(doctype, frm) {
		if(frm) return frm.perm[0].email===1;
		return dataent.boot.user.can_email.indexOf(doctype)!==-1;
	},
	...
	can_share: function(doctype, frm) {
		...
		return dataent.boot.user.can_share.indexOf(doctype)!==-1;
	},
	...
	can_set_user_permissions: function(doctype, frm) {
		...
		return dataent.boot.user.can_set_user_permissions.indexOf(doctype)!==-1;
	},

	has_value: function(dt, dn, fn) {
		// return true if property has value
		var val = locals[dt] && locals[dt][dn] && locals[dt][dn][fn];
		var df = dataent.meta.get_docfield(dt, fn, dn);
	...
	get_list: function(doctype, filters) {
		...
		if($.isEmptyObject(docsdict))
			return [];
		return dataent.utils.filter_dict(docsdict, filters);
	},
	...
	get_value: function(doctype, filters, fieldname, callback) {
		if(callback) {
			...
			if(typeof filters==="string" && locals[doctype] && locals[doctype][filters]) {
				...
				var l = dataent.get_list(doctype, filters);
	...
	set_value: function(doctype, docname, fieldname, value, fieldtype) {
		...
		$.each(to_update, (key, value) => {
			if (doc && doc[key] !== value) {
				...
				doc[key] = value;
				tasks.push(() => dataent.model.trigger(key, value, doc));
			} else {
	...
	on: function(doctype, fieldname, fn) {
		...
		*/
		/* example: dataent.model.on("Customer", "age", function(fieldname, value, doc) {
		  if(doc.age < 16) {
		   	dataent.msgprint("Warning, Customer must atleast be 16 years old.");
		    raise "CustomerAgeError";
		  }
		}) */
		dataent.provide("dataent.model.events." + doctype);
		if(!dataent.model.events[doctype][fieldname]) {
			dataent.model.events[doctype][fieldname] = [];
		}
		dataent.model.events[doctype][fieldname].push(fn);
	},
		...
		var runner = function(events, event_doc) {
			$.each(events || [], function(i, fn) {
				if(fn) {
					...
					// or use the default promise dataent.after_ajax
					if (_promise && _promise.then) {
						return _promise;
					} else {
						return dataent.after_server_call();
					}
				}
			});
		};

		if(dataent.model.events[doc.doctype]) {
			tasks.push(() => {
				return runner(dataent.model.events[doc.doctype][fieldname]);
			});

			tasks.push(() => {
				return runner(dataent.model.events[doc.doctype]['*']);
			});
		}
	...
	get_doc: function(doctype, name) {
		...
		if($.isPlainObject(name)) {
			var doc = dataent.get_list(doctype, name);
	...
	get_children: function(doctype, parent, parentfield, filters) {
		if($.isPlainObject(doctype)) {
			...
			var doc = dataent.get_doc(doctype, parent);
		}

		var children = doc[parentfield] || [];
		if(filters) {
			return dataent.utils.filter_dict(children, filters);
		} else {
			return children;
		}
	},
	...
	remove_from_locals: function(doctype, name) {
		...
		if(dataent.views.formview[doctype]) {
			delete dataent.views.formview[doctype].frm.opendocs[name];
		}
	},
	...
	get_no_copy_list: function(doctype) {
		...
		var docfields = dataent.get_doc("DocType", doctype).fields || [];
	...
	delete_doc: function(doctype, docname, callback) {
		dataent.confirm(__("Permanently delete {0}?", [docname]), function() {
			return dataent.call({
				method: 'dataent.client.delete',
				args: {
					doctype: doctype,
					name: docname
				},
				callback: function(r, rt) {
					if(!r.exc) {
						...
						dataent.model.clear_doc(doctype, docname);
	...
	round_floats_in: function(doc, fieldnames) {
		if(!fieldnames) {
			fieldnames = dataent.meta.get_fieldnames(doc.doctype, doc.parent,
	...
	validate_missing: function(doc, fieldname) {
		if(!doc[fieldname]) {
			dataent.throw(__("Please specify") + ": " +
				__(dataent.meta.get_label(doc.doctype, fieldname, doc.parent || doc.name)));
		}
	},
	...
	is_numeric_field: function(fieldtype) {
		...
		return dataent.model.numeric_fieldtypes.includes(fieldtype);
	}
});

// legacy
dataent.get_doc = dataent.model.get_doc;
dataent.get_children = dataent.model.get_children;
dataent.get_list = dataent.model.get_list;
