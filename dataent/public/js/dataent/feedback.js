...
dataent.utils.Feedback = Class.extend({
	...
	manual_feedback_request: function(doc) {
		...
		if(dataent.boot.feedback_triggers[doc.doctype]) {
			var feedback_trigger = dataent.boot.feedback_triggers[doc.doctype]
	...
	get_feedback_request_details: function(args, is_resend) {
		...
		return dataent.call({
			method: "dataent.core.doctype.feedback_trigger.feedback_trigger.get_feedback_request_details",
			'args': args,
			callback: function(r) {
				if(r.message) {
	...
	send_feedback_request: function(args) {
		...
		return dataent.call({
			method: "dataent.core.doctype.feedback_trigger.feedback_trigger.send_feedback_request",
			'args': args,
			freeze: true,
			callback: function(r) {
				if(r.message) {
					dataent.msgprint(__("Feedback Request for {0} is sent to {1}",
