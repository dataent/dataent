...
dataent.ui.form.MultiSelectDialog = Class.extend({
	init: function(opts) {
	...
	make: function() {
		...
		fields = fields.concat([
			...
			{ fieldtype: "Button", fieldname: "more_btn", label: __("More"),
				click: function(){
					me.start += 20;
					dataent.flags.auto_scroll = true;
					me.get_results();
				}
			}
		]);
				...
				if (e) {
					...
					if($.isArray(me.setters)) {
						for (let df of me.setters) {
							dataent.route_options[df.fieldname] = me.dialog.fields_dict[df.fieldname].get_value() || undefined;
						}
					} else {
						Object.keys(me.setters).forEach(function(setter) {
							dataent.route_options[setter] = me.dialog.fields_dict[setter].get_value() || undefined;
						});
					}

					dataent.new_doc(me.doctype, true);
				}
			}
		});
	...
	bind_events: function() {
		...
		this.$parent.find('.input-with-feedback').on('change', (e) => {
			dataent.flags.auto_scroll = false;
			this.get_results();
		});
	...
	make_list_row: function(result={}) {
		...
		columns.forEach(function(column) {
			contents += `<div class="list-item__content ellipsis">
				${
					head ? `<span class="ellipsis">${__(dataent.model.unscrub(column))}</span>`
		...
		dataent.call({
			...
			callback: function(r) {
				...
				if (r.values.length) {
					...
					results.map( (result) => {
						result["Date"] = dataent.format(result["Date"], {"fieldtype":"Date"});
					})
