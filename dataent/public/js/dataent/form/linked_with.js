...
dataent.ui.form.LinkedWith = class LinkedWith {
	...
	load_doctypes() {
		...
		const promises = doctypes_to_load.map(dt => {
			return dataent.model.with_doctype(dt, () => {
				if(dataent.listview_settings[dt]) {
					// add additional fields to __linked_doctypes
					this.frm.__linked_doctypes[dt].add_fields =
						dataent.listview_settings[dt].add_fields;
				}
			});
		});
	...
	links_not_permitted_or_missing() {
		...
		if (this.frm.__linked_doctypes) {
			links =
				Object.keys(this.frm.__linked_doctypes)
					.filter(dataent.model.can_get_report);
		}
	...
	get_linked_docs() {
		return dataent.call({
			method: "dataent.desk.form.linked_with.get_linked_docs",
			args: {
				doctype: this.frm.doctype,
