...
dataent.ui.form.States = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
		this.state_fieldname = dataent.workflow.get_state_fieldname(this.frm.doctype);

		// no workflow?
		if(!this.state_fieldname)
			return;

		this.update_fields = dataent.workflow.get_update_fields(this.frm.doctype);
	...
	setup_help: function() {
		...
		this.frm.page.add_action_item(__("Help"), function() {
			dataent.workflow.setup(me.frm.doctype);
			var state = me.get_state();
			var d = new dataent.ui.Dialog({
				title: "Workflow: "
					+ dataent.workflow.workflows[me.frm.doctype].name
			});

			dataent.workflow.get_transitions(me.frm.doc).then((transitions) => {
				...
				$(d.body).html("<p>"+__("Current status")+": " + state.bold() + "</p>"
					+ "<p>"+__("Document is only editable by users of role")+": "
						+ dataent.workflow.get_document_state(me.frm.doctype,
	...
	show_actions: function() {
		...
		dataent.workflow.get_transitions(this.frm.doc).then(transitions => {
			$.each(transitions, function(i, d) {
				if(dataent.user_roles.includes(d.allowed) && has_approval_access(d)) {
					added = true;
					me.frm.page.add_action_item(__(d.action), function() {
						dataent.xcall('dataent.model.workflow.apply_workflow',
							{doc: me.frm.doc, action: d.action})
							.then((doc) => {
								dataent.model.sync(doc);
								me.frm.refresh();
							});
					});
				}
	...
	set_default_state: function() {
		var default_state = dataent.workflow.get_default_state(this.frm.doctype, this.frm.doc.docstatus);
		if(default_state) {
