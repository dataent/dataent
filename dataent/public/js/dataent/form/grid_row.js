...
export default class GridRow {
		...
		if(this.grid.is_editable()) {
			if(this.frm) {
				...
				dataent.run_serially([
					...
					() => {
						dataent.model.clear_doc(this.doc.doctype, this.doc.name);
	...
	render_template() {
		...
		this.row_display = $('<div class="row-data sortable-handle template-row">'+
			...
			.html(dataent.render(this.grid.template, {
				doc: this.doc ? dataent.get_format_helper(this.doc) : null,
				frm: this.frm,
				row: this
			}));
	}
	...
	setup_columns() {
		...
		for(var ci in this.grid.visible_columns) {
			var df = this.grid.visible_columns[ci][0],
				...
				txt = this.doc ?
					dataent.format(this.doc[df.fieldname], df, null, this.doc) :
					__(df.label);
	...
	make_column(df, colsize, txt, ci) {
		...
		var $col = $('<div class="col grid-static-col col-xs-'+colsize+' '+add_class+'"></div>')
			...
			.appendTo(this.row)
			.on('click', function() {
				if(dataent.ui.form.editable_row===me) {
					return;
				}
	...
	toggle_editable_row(show) {
		...
		if(this.grid.allow_on_grid_editing() && this.grid.is_editable() && this.doc && show !== false) {
			...
			if(dataent.ui.form.editable_row
				&& dataent.ui.form.editable_row !== this) {
			...
			dataent.ui.form.editable_row = null;
		}
	}
	...
	make_control(column) {
		...
		var field = dataent.ui.form.make_control({
			df: df,
			parent: parent,
			only_input: true,
			with_link_btn: true,
	...
	set_arrow_keys(field) {
		...
		if(field.$input) {
			field.$input.on('keydown', function(e) {
				var { TAB, UP_ARROW, DOWN_ARROW } = dataent.ui.keyCode;
	...
	show_form() {
		...
		dataent.dom.freeze("", "dark");
		if(cur_frm) cur_frm.cur_grid = this;
		this.wrapper.addClass("grid-row-open");
		if(!dataent.dom.is_element_in_viewport(this.wrapper)) {
			dataent.utils.scroll_to(this.wrapper, true, 15);
		}
	...
	refresh_field(fieldname, txt) {
		...
		if(!df) {
			...
			if(df && this.doc) {
				var txt = dataent.format(this.doc[fieldname], df[0],
					null, this.doc);
			}
		}

		if(txt===undefined && this.frm) {
			var txt = dataent.format(this.doc[fieldname], df,
				null, this.frm.doc);
		}
	...
	get_visible_columns(blacklist) {
		...
		var visible_columns = $.map(this.docfields, function(df) {
			var visible = !df.hidden && df.in_list_view && me.grid.frm.get_perm(df.permlevel, "read")
				&& !in_list(dataent.model.layout_fields, df.fieldtype) && !in_list(blacklist, df.fieldname);
