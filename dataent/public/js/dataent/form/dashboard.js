...
dataent.ui.form.Dashboard = Class.extend({
	...
	render_links: function() {
		...
		$(dataent.render_template('form_links', this.data))
	...
	open_document_list: function($link, show_open) {
		...
		if(this.data.internal_links[doctype]) {
			if(names.length) {
				dataent.route_options = {'name': ['in', names]};
			} else {
				return false;
			}
		} else if(this.data.fieldname) {
			dataent.route_options = this.get_document_filter(doctype);
			if(show_open) {
	...
	set_open_count: function() {
		...
		var method = this.data.method || 'dataent.desk.notifications.get_open_count';
		dataent.call({
			type: "GET",
			method: method,
			args: {
	...
	render_heatmap: function() {
		if(!this.heatmap) {
			this.heatmap = new Chart("#heatmap-" + dataent.model.scrub(this.frm.doctype), {
				type: 'heatmap',
				height: 120,
