...
dataent.ui.form.Layout = Class.extend({
	init: function(opts) {
		this.views = {};
		this.pages = [];
		this.sections = [];
	...
	show_empty_form_message: function() {
		if(!(this.wrapper.find(".dataent-control:visible").length || this.wrapper.find(".section-head.collapsed").length)) {
	...
	get_doctype_fields: function() {
		...
		fields = fields.concat(dataent.meta.sort_docfields(dataent.meta.docfield_map[this.doctype]));
		return fields;
	},
	...
	init_field: function(df, render = false) {
		const fieldobj = dataent.ui.form.make_control({
			df: df,
	...
	make_page: function(df) {
		...
		this.fold_btn = head.find(".btn-fold").on("click", function() {
			...
			if(page.hasClass("hide")) {
				...
				dataent.utils.scroll_to($(this), true, 30);
				me.folded = false;
			} else {
	...
	make_section: function(df) {
		this.section = new dataent.ui.form.Section(this, df);
	...
	make_column: function(df) {
		this.column = new dataent.ui.form.Column(this.section, df);
		if(df && df.fieldname) {
	...
	refresh_sections: function() {
		...
		this.wrapper.find(".form-section:not(.hide-control)").each(function() {
			...
			if(!$this.find(".dataent-control:not(.hide-control)").length
	...
	attach_doc_and_docfields: function(refresh) {
		...
		for(var i=0, l=this.fields_list.length; i<l; i++) {
			...
			if(me.doc) {
				...
				fieldobj.df = dataent.meta.get_docfield(me.doc.doctype,
	...
	focus_on_next_field: function(start_idx, fields) {
		...
		for(var i= start_idx + 1, len = fields.length; i < len; i++) {
			...
			if(this.is_visible(field)) {
				...
				} else if(!in_list(dataent.model.no_value_type, field.df.fieldtype)) {
	...
	evaluate_depends_on_value: function(expression) {
		...
		if(typeof(expression) === 'boolean') {
			...
			try {
				...
				dataent.throw(__('Invalid "depends_on" expression'));
			}
...
dataent.ui.form.Section = Class.extend({
...
dataent.ui.form.Column = Class.extend({
