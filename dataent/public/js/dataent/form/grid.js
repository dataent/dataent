...
dataent.ui.form.get_open_grid_form = function() {
	return $(".grid-row-open").data("grid_row");
}

dataent.ui.form.close_grid_form = function() {
	var open_form = dataent.ui.form.get_open_grid_form();
	open_form && open_form.hide_form();

	// hide editable row too
	if(dataent.ui.form.editable_row) {
		dataent.ui.form.editable_row.toggle_editable_row(false);
	}
}
...
export default class Grid {
	...
	setup_check() {
		...
		this.remove_rows_button.on('click', function() {
			...
			me.get_selected().forEach((docname) => {
				...
				tasks.push(() => dataent.timeout(0.1));
			});
	...
	refresh(force) {
		...
		if(this.frm) {
			this.display_status = dataent.perm.get_field_display_status(this.df, this.frm.doc,
				this.perm);
		} else {
			// not in form
		...
		// dataent.utils.scroll_to(_scroll_y);
	...
	setup_fields() {
		...
		if (this.frm && this.frm.docname) {
			...
			this.df = dataent.meta.get_docfield(this.frm.doctype, this.df.fieldname,
				this.frm.docname);
		} else {
			// use non-doc specific docfield
			if(this.df.options) {
				this.df = dataent.meta.get_docfield(this.df.options, this.df.fieldname);
			}
		}

		if(this.doctype && this.frm) {
			this.docfields = dataent.meta.get_docfields(this.doctype, this.frm.docname);
		} else {
				...
				row.columns_list.forEach(function(column) {
					...
					if (column.df.fieldname == fieldname) {
						if (show) {
							...
							if  (row != dataent.ui.form.editable_row) {
	...
	get_docfield(fieldname) {
		return dataent.meta.get_docfield(this.doctype, fieldname, this.frm ? this.frm.docname : null);
	}
	get_row(key) {
	...
	add_new_row(idx, callback, show) {
		if(this.is_editable()) {
			if(this.frm) {
				var d = dataent.model.add_child(this.frm.doc, this.df.options, this.df.fieldname, idx);
				d.__unedited = true;
	...
	setup_visible_columns() {
		...
		for(var ci in fields) {
			...
			if(!df.hidden
				...
				&& !in_list(dataent.model.layout_fields, df.fieldtype)) {
				...
				if (df.fieldtype == 'Link' && !df.formatter && df.parent && dataent.meta.docfield_map[df.parent]) {
					const docfield = dataent.meta.docfield_map[df.parent][df.fieldname];
		...
		while(total_colsize < 11 && passes < 12) {
			for(var i in this.visible_columns) {
				...
				if(colsize > 1 && colsize < 11
					&& !in_list(dataent.model.std_fields_list, df.fieldname)) {
	...
	set_multiple_add(link, qty) {
		...
		var me = this;
		var link_field = dataent.meta.get_docfield(this.df.options, link);
	...
	setup_allow_bulk_edit() {
		...
		if(this.frm && this.frm.get_docfield(this.df.fieldname).allow_bulk_edit) {
			...
			// upload
			dataent.flags.no_socketio = true;
			$(this.wrapper).find(".grid-upload").removeClass("hide").on("click", function() {
				dataent.prompt({fieldtype:"Attach", label:"Upload File", fieldname: "upload_file"},
					function(data) {
						var data = dataent.utils.csv_to_array(dataent.upload.get_string(data.upload_file));
							...
							if(i > 6) {
								...
								if(!blank_row) {
									...
									$.each(row, function(ci, value) {
										...
										var df = dataent.meta.get_docfield(me.df.options, fieldname);

										// convert date formatting
										if(df.fieldtype==="Date" && value) {
											value = dataent.datetime.user_to_str(value);
										}
						...
						dataent.msgprint({message:__('Table updated'), title:__('Success'), indicator:'green'})
	...
	setup_download() {
		var me = this;
		let title = me.df.label || dataent.model.unscrub(me.df.fieldname);
		$(this.wrapper).find(".grid-download").removeClass("hide").on("click", function() {
			...
			data.push(["------"]);
			$.each(dataent.get_meta(me.df.options).fields, function(i, df) {
				// don't include the read-only field in the template
				if(dataent.model.is_value_type(df.fieldtype)) {
					data[1].push(df.label);
					data[2].push(df.fieldname);
					let description = (df.description || "") + ' ';
					if (df.fieldtype === "Date") {
						description += dataent.boot.sysdefaults.date_format;
					}
			...
			$.each(me.frm.doc[me.df.fieldname] || [], function(i, d) {
				...
				$.each(data[2], function(i, fieldname) {
					...
					if(docfields[i].fieldtype==="Date" && value) {
						value = dataent.datetime.str_to_user(value);
					}
			...
			dataent.tools.downloadify(data, null, title);
			return false;
		});
	}
