...
dataent.provide('dataent.document_flow');

dataent.ui.form.DocumentFlow = Class.extend({
	init: function(opts) {
		$.extend(this, opts);

		this.module = dataent.get_meta(this.frm.doctype).module
		if (!dataent.document_flow || !dataent.document_flow[this.module]) {
			return;
		}
		this.doctypes = dataent.document_flow[this.module][this.frm.doctype];
		if (!this.doctypes) {
			return;
		}
	...
	render: function() {
		...
		$(dataent.render_template('form_document_flow', {
			frm: this.frm,
	...
	get_linked_docs: function(for_doctype) {
		if(!this.linked_with[for_doctype]) {
			this.linked_with[for_doctype] = new dataent.ui.form.LinkedWith({
				frm: this.frm,
	...
	mark_completed_flow: function() {
		var me = this;
		dataent.call({
			method: "dataent.desk.form.document_flow.get_document_completion_status",
			args: {
