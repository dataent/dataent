...
dataent.ui.form.Share = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
	},
	refresh: function() {
	...
	render_sidebar: function() {
		...
		for (var i=0, l=shared.length; i < l; i++) {
			...
			if (s.everyone) {
				...
				var user_info = dataent.user_info(s.user);
				users.push({
	...
	show: function() {
		...
		dataent.call({
			method: "dataent.share.get_users",
			args: {
	...
	render_shared: function(shared) {
		...
		$(dataent.render_template("set_sharing", {frm: this.frm, shared: this.shared, everyone: everyone}))
			.appendTo(d.body);

		if(dataent.model.can_share(null, this.frm)) {
	...
	make_user_input: function() {
		// make add-user input
		this.dialog.share_with = dataent.ui.form.make_control({
	...
	set_edit_share_events: function() {
		...
		$(d.body).find(".edit-share").on("click", function() {
			...
			dataent.call({
				method: "dataent.share.set_permission",
				args: {
					doctype: me.frm.doctype,
					name: me.frm.doc.name,
					user: user,
