...
dataent.provide("dataent.ui.form.handlers");

dataent.ui.form.get_event_handler_list = function(doctype, fieldname) {
	if(!dataent.ui.form.handlers[doctype]) {
		dataent.ui.form.handlers[doctype] = {};
	}
	if(!dataent.ui.form.handlers[doctype][fieldname]) {
		dataent.ui.form.handlers[doctype][fieldname] = [];
	}
	return dataent.ui.form.handlers[doctype][fieldname];
}

dataent.ui.form.on = dataent.ui.form.on_change = function(doctype, fieldname, handler) {
	var add_handler = function(fieldname, handler) {
		var handler_list = dataent.ui.form.get_event_handler_list(doctype, fieldname);
...
dataent.ui.form.off = function(doctype, fieldname, handler) {
	var handler_list = dataent.ui.form.get_event_handler_list(doctype, fieldname);
...
dataent.ui.form.trigger = function(doctype, fieldname) {
	cur_frm.script_manager.trigger(fieldname, doctype);
}

dataent.ui.form.ScriptManager = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
	},
	...
	trigger: function(event_name, doctype, name) {
		...
		this.frm.selected_doc = dataent.get_doc(doctype, name);
	...
	get_handlers: function(event_name, doctype) {
		...
		if(dataent.ui.form.handlers[doctype] && dataent.ui.form.handlers[doctype][event_name]) {
			$.each(dataent.ui.form.handlers[doctype][event_name], function(i, fn) {
	...
	setup: function() {
		...
		$.each(this.frm.fields, function(i, field) {
			...
			if(field.df.fieldtype==="Table") {
				$.each(dataent.meta.get_docfields(field.df.options, me.frm.docname), function(i, df) {
					setup_add_fetch(df);
				});
			}
		});
		...
		// css
		doctype.__css && dataent.dom.set_style(doctype.__css);

		this.trigger('setup');
	},
	log_error: function(caller, e) {
		dataent.show_alert("Error in Client Script.");
	...
	copy_from_first_row: function(parentfield, current_row, fieldnames) {
		...
		$.each(fieldnames, function(i, fieldname) {
			dataent.model.set_value(current_row.doctype, current_row.name, fieldname,
				data[0][fieldname]);
		});
	}
});
