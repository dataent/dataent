...
dataent.ui.form.Toolbar = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
		this.refresh();
	...
	set_title: function() {
		...
		if(this.frm.meta.title_field) {
			dataent.utils.set_title(title + " - " + this.frm.docname);
		}
	...
	setup_editable_title: function() {
		...
		this.page.$title_area.find(".title-text").on("click", function() {
			if(me.is_title_editable()) {
				dataent.prompt({fieldname: "title", fieldtype:"Data",
	...
	set_indicator: function() {
		...
		var indicator = dataent.get_indicator(this.frm.doc);
		if(indicator) {
	...
	make_menu: function() {
		...
		var is_submittable = dataent.model.is_submittable(this.frm.doc.doctype)
		var issingle = this.frm.meta.issingle;
		var print_settings = dataent.model.get_doc(":Print Settings", "Print Settings")
		...
		if(!is_submittable || docstatus == 1  ||
			...
			if(dataent.model.can_print(null, me.frm) && !issingle) {
		...
		// email
		if(dataent.model.can_email(null, me.frm) && me.frm.doc.docstatus < 2) {
		...
		// copy
		if(in_list(dataent.boot.user.can_create, me.frm.doctype) && !me.frm.meta.allow_copy) {
		...
		if(me.frm.meta.issingle) {
			this.page.add_menu_item(__('Add to Desktop'), function () {
				dataent.add_to_desktop(me.frm.doctype, me.frm.doctype);
			}, true);
		}

		// delete
		if((cint(me.frm.doc.docstatus) != 1) && !me.frm.doc.__islocal
			&& dataent.model.can_delete(me.frm.doctype)) {
			this.page.add_menu_item(__("Delete"), function() {
				me.frm.savetrash();}, true);
		}

		if(dataent.user_roles.includes("System Manager") && me.frm.meta.issingle === 0) {
			...
			if (dataent.boot.developer_mode===1 && me.frm.meta.issingle) {
				// edit doctype
				this.page.add_menu_item(__("Edit DocType"), function() {
					dataent.set_route('Form', 'DocType', me.frm.doctype);
				}, true);
			}
		}
		...
		if(!this.frm.doc.__unsaved) {
			if(is_submittable && docstatus == 1) {
				this.page.add_menu_item(__("Request Feedback"), function() {
					var feedback = new dataent.utils.Feedback();
		...
		if(p[CREATE] && !this.frm.meta.issingle) {
			this.page.add_menu_item(__("New {0} (Ctrl+B)", [__(me.frm.doctype)]), function() {
				dataent.new_doc(me.frm.doctype, true);}, true);
		}
	},
	can_save: function() {
	...
	has_workflow: function() {
		if(this._has_workflow === undefined)
			this._has_workflow = dataent.get_list("Workflow", {document_type: this.frm.doctype}).length;
	...
	show_linked_with: function() {
		if(!this.frm.linked_with) {
			this.frm.linked_with = new dataent.ui.form.LinkedWith({
				frm: this.frm
			});
		}
