...
dataent.ui.form.PrintPreview = Class.extend({
	init: function (opts) {
		$.extend(this, opts);
		this.make();
		this.bind_events();
	...
	make: function () {
		this.wrapper = this.frm.page.add_view("print", dataent.render_template("print_layout", {}));

		// only system manager can edit
		this.wrapper.find(".btn-print-edit").toggle(dataent.user.has_role("System Manager"));
	},
	bind_events: function () {
		...
		this.print_formats = dataent.meta.get_print_formats(this.frm.meta.name);
		this.print_letterhead = this.wrapper
			.find(".print-letterhead")
			.on("change", function () { me.print_sel.trigger("change"); })
			.prop("checked", cint(
				(dataent.model.get_doc(":Print Settings", "Print Settings")
		...
		this.wrapper.find(".btn-download-pdf").click(function () {
			if (!me.is_old_style()) {
				var w = window.open(
					dataent.urllib.get_full_url("/api/method/dataent.utils.print_format.download_pdf?"
		...
		this.wrapper.find(".btn-print-edit").on("click", function () {
			...
			if (is_standard_but_editable) {
				dataent.set_route("Form", "Print Format", print_format.name);
				return;
			}
			if (is_custom_format) {
				dataent.set_route("print-format-builder", print_format.name);
				return;
			}
			...
			dataent.prompt([
				...
				dataent.set_route("print-format-builder");
	...
	set_user_lang: function () {
		...
		this.language_sel.empty()
			.add_options([{value:'', label:__("Select Language...")}]
				.concat(dataent.get_languages()))
			.val(this.lang_code);
		this.preview();
	},
	set_default_print_language: function () {
		var print_format = this.get_print_format();
		this.lang_code = print_format.default_print_language || this.frm.doc.language || dataent.boot.lang;
	...
	preview: function () {
		...
		this.get_print_html(function (out) {
			...
			const print_height_inches = dataent.dom.pixel_to_inches(print_height);
	...
	printit: function () {
		...
		var me = this;
		dataent.call({
			method: "dataent.printing.doctype.print_settings.print_settings.is_print_server_enabled",
			callback: function (data) {
				if (data.message) {
					dataent.call({
						"method": "dataent.utils.print_format.print_by_server",
						args: {
	...
	new_page_preview: function (printit) {
		var me = this;
		var w = window.open(dataent.urllib.get_full_url("/printview?"
			+ "doctype=" + encodeURIComponent(me.frm.doc.doctype)
			+ "&name=" + encodeURIComponent(me.frm.doc.name)
	...
	get_print_html: function (callback) {
		...
		this._req = dataent.call({
			method: "dataent.www.printview.get_html_and_style",
			args: {
				doc: this.frm.doc,
	...
	refresh_print_options: function () {
		this.print_formats = dataent.meta.get_print_formats(this.frm.doctype);
		return this.print_sel
			.empty().add_options(this.print_formats);

	},
	with_old_style: function (opts) {
		dataent.require("/assets/js/print_format_v3.min.js", function () {
	...
	set_style: function (style) {
		dataent.dom.set_style(style || dataent.boot.print_css, "print-style");
	}
});

dataent.ui.get_print_settings = function (pdf, callback, letter_head) {
	var print_settings = locals[":Print Settings"]["Print Settings"];

	var default_letter_head = locals[":Company"] && dataent.defaults.get_default('company')
		? locals[":Company"][dataent.defaults.get_default('company')]["default_letter_head"]
		: '';
	...
	var columns = [{
		...
		options: $.map(dataent.boot.letter_heads, function (i, d) { return d }),
	...
	dataent.prompt(columns, function (data) {
		var data = $.extend(print_settings, data);
		if (!data.with_letter_head) {
			data.letter_head = null;
		}
		if (data.letter_head) {
			data.letter_head = dataent.boot.letter_heads[print_settings.letter_head];
		}
		callback(data);
	}, __("Print Settings"));
}
