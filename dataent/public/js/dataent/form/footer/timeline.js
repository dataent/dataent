...
dataent.provide('dataent.timeline');
dataent.separator_element = '<div>---</div>';

dataent.ui.form.Timeline = class Timeline {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	make() {
		...
		this.list.on("click", ".btn-more", function() {
			...
			dataent.call({
				btn: this,
				method: "dataent.desk.form.load.get_communications",
				args: {
				...
				callback: function(r) {
					if (!r.exc) {
						if (r.message) {
							...
							dataent.model.set_docinfo(me.frm.doc.doctype, me.frm.doc.name, "communications", communications);
		...
		this.email_button = this.wrapper.find(selector)
			.on("click", function() {
				...
				if(me.frm.doctype === "Communication") {
					...
					const comment_value = dataent.markdown(me.comment_area.get_value());
					$.extend(args, {
						txt: strip_html(comment_value) ? comment_value : ''
					});
				}
				new dataent.views.CommunicationComposer(args)
			});
	}
	...
	setup_interaction_button() {
		...
		this.activity_button = this.wrapper.find(selector)
			.on("click", function() {
				...
				$.extend(args, {
					txt: dataent.markdown(me.comment_area.get_value())
				});
				new dataent.views.InteractionComposer(args);
			});
	}
	...
	setup_editing_area() {
		...
		this.editing_area = new dataent.ui.CommentArea({
	...
	make_editing_area(container) {
		return dataent.ui.form.make_control({
			parent: container,
			df: {
				fieldtype: 'Comment',
				fieldname: 'comment',
	...
	render_timeline_item(c) {
		...
		var $timeline_item = $(dataent.render_template("timeline_item", {data:c, frm:this.frm}))
			.appendTo(me.list)
			...
			.on('click', '.edit-comment', function(e) {
				...
				if($timeline_item.hasClass('is-editing')) {
					...
					me.current_editing_area.on_submit = (value) => {
						...
						c.content = value;
						dataent.timeline.update_communication(c);
	...
	prepare_timeline_item(c) {
		...
		c.futur_date = communication_date > dataent.datetime.now_datetime() ? true : false;
		if(!c.fullname) {
			c.fullname = c.sender_full_name || dataent.user.full_name(c.sender);
		}
		...
		if(c.comment_type==="Workflow" || c.comment_type==="Label") {
			c.comment_html = repl('<span class="label label-%(style)s">%(text)s</span>', {
				style: dataent.utils.guess_style(c.content),
				text: __(c.content)
			});
		} else {
			if(c.communication_type=="Communication" && c.communication_medium=="Email") {
				c.content = c.content.split(dataent.separator_element)[0];
				c.content = dataent.utils.strip_original_content(c.content);

				c.original_content = c.content;
				c.content = dataent.utils.toggle_blockquote(c.content);
			...
			if(!dataent.utils.is_html(c.content)) {
				c.content_html = dataent.markdown(__(c.content));
			} else {
				c.content_html = c.content;
				c.content_html = dataent.utils.strip_whitespace(c.content_html);
			}
			...
			if (this.is_communication_or_comment(c)) {
				...
				c.liked_by_user = c._liked_by.indexOf(dataent.session.user)!==-1;
			}
		}

		// basic level of XSS protection
		c.content_html = dataent.dom.remove_script_and_style(c.content_html);
	...
	set_icon_and_color(c) {
		if(c.communication_type == "Feedback"){
			...
			c.rating_icons = dataent.render_template("rating_icons", {rating: c.rating, show_label: true})
			c.color = "#f39c12"
		} else {
			c.icon = {
			...
			if(data.changed && data.changed.length) {
				...
				data.changed.every(function(p) {
					if(p[0]==='docstatus') {
						...
						var df = dataent.meta.get_docfield(me.frm.doctype, p[0], me.frm.docname);

						if(df && !df.hidden) {
							var field_display_status = dataent.perm.get_field_display_status(df, null,
								me.frm.perm);
							if(field_display_status === 'Read' || field_display_status === 'Write') {
								parts.push(__('{0} from {1} to {2}', [
									__(df.label),
									(dataent.ellipsis(p[1], 40) || '""').bold(),
									(dataent.ellipsis(p[2], 40) || '""').bold()
								]));
							}
						}
					}
				...
				data.row_changed.every(function(row) {
					row[3].every(function(p) {
						var df = me.frm.fields_dict[row[0]] &&
							dataent.meta.get_docfield(me.frm.fields_dict[row[0]].grid.doctype,
								p[0], me.frm.docname);

						if(df && !df.hidden) {
							var field_display_status = dataent.perm.get_field_display_status(df,
								null, me.frm.perm);

							if(field_display_status === 'Read' || field_display_status === 'Write') {
								parts.push(__('{0} from {1} to {2} in row #{3}', [
									dataent.meta.get_label(me.frm.fields_dict[row[0]].grid.doctype,
										p[0]),
									(dataent.ellipsis(p[1], 40) || '""').bold(),
									(dataent.ellipsis(p[2], 40) || '""').bold(),
									row[1]
				...
				if(data[key] && data[key].length) {
					parts = (data[key] || []).map(function(p) {
						...
						if(df && !df.hidden) {
							...
							if(field_display_status === 'Read' || field_display_status === 'Write') {
								return dataent.meta.get_label(me.frm.doctype, p[0])
							}
						}
					});
	...
	insert_comment(comment_type, comment, btn) {
		var me = this;
		return dataent.call({
			method: "dataent.desk.form.utils.add_comment",
			args: {
				doc:{
	...
	delete_comment(name) {
		...
		dataent.confirm(__('Delete comment?'), function() {
			return dataent.call({
	...
	update_comment(name, content){
		return dataent.call({
			method: 'dataent.desk.form.utils.update_comment',
			args: { name, content },
			callback: function(r) {
				if(!r.exc) {
					dataent.utils.play_sound('click');
	...
	get_names_for_mentions() {
		var valid_users = Object.keys(dataent.boot.user_info)
			.filter(user => !["Administrator", "Guest"].includes(user));

		return valid_users.map(user => dataent.boot.user_info[user].name);
	}

	setup_comment_like() {
		this.wrapper.on("click", ".comment-likes .octicon-heart", dataent.ui.click_toggle_like);

		dataent.ui.setup_like_popover(this.wrapper, ".comment-likes");
	}
};

$.extend(dataent.timeline, {
	new_communication: function(communication) {
		var docinfo = dataent.model.get_docinfo(communication.reference_doctype, communication.reference_name);
	...
	delete_communication: function(communication) {
		var docinfo = dataent.model.get_docinfo(communication.reference_doctype, communication.reference_name);
		var index = dataent.timeline.index_of_communication(communication, docinfo);
		if (index !== -1) {
