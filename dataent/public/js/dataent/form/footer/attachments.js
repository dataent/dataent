...
dataent.ui.form.Attachments = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
		this.make();
	},
	...
	add_attachment: function(attachment) {
		...
		var $attach = $(dataent.render_template("attachment", { 
			"file_path": "/desk#Form/File/" + fileid,
			"icon": attachment.is_private ? "fa fa-lock" : "fa fa-unlock-alt",
			"file_name": file_name,
			"file_url": dataent.urllib.get_full_url(file_url)
		...
		var $close =
			...
			.click(function() {
				var remove_btn = this;
				dataent.confirm(__("Are you sure you want to delete the attachment?"),
					function() {
		...
		if(!dataent.model.can_write(this.frm.doctype, this.frm.name)) {
			$close.remove();
		}
	},
	...
	remove_attachment: function(fileid, callback) {
		...
		var me = this;
		return dataent.call({
			method: 'dataent.desk.form.utils.remove_attach',
			args: {
				fid: fileid,
				dt: me.frm.doctype,
				dn: me.frm.docname
			...
			callback: function(r,rt) {
				if(r.exc) {
					if(!r._server_messages)
						dataent.msgprint(__("There were errors"));
					return;
				}
	...
	new_attachment: function(fieldname) {
		...
		// make upload dialog
		this.dialog = dataent.ui.get_upload_dialog({
			"args": me.get_args(),
...
dataent.ui.get_upload_dialog = function(opts){
	var dialog = new dataent.ui.Dialog({
		...
		fields: [
			...
			{
				"hidden": !opts.args.doctype || !dataent.boot.gsuite_enabled,
				"fieldtype": "Section Break",
