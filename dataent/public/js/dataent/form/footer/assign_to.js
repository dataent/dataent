...
dataent.ui.form.AssignTo = Class.extend({
	init: function(opts) {
		var me = this;
	...
	render: function(d) {
		...
		if(d && d.length) {
			for(var i=0; i<d.length; i++) {
				var info = dataent.user_info(d[i].owner);
				info.assign_to_name = d[i].name
				info.owner = d[i].owner;
				info.avatar = dataent.avatar(d[i].owner);
				...
				if(d[i].owner===dataent.session.user) {
					me.primary_action = this.frm.page.add_menu_item(__("Assignment Complete"), function() {
						me.remove(dataent.session.user);
					}, "fa fa-check", "btn-success")
				}

				if(!(d[i].owner === dataent.session.user || me.frm.perm[0].write)) {
	...
	add: function() {
		...
		if(this.frm.is_new()) {
			dataent.throw(__("Please save the document before assignment"));
			return;
		}

		if(!me.assign_to) {
			me.assign_to = new dataent.ui.form.AssignToDialog({
				obj: me,
	...
	remove: function(owner) {
		...
		if(this.frm.is_new()) {
			dataent.throw(__("Please save the document before removing assignment"));
			return;
		}

		dataent.call({
			method:'dataent.desk.form.assign_to.remove',
			args: {
				doctype: me.frm.doctype,
				name: me.frm.docname,
				assign_to: owner
...
dataent.ui.form.AssignToDialog = Class.extend({
	init: function(opts){
		var me = this
		var dialog = new dataent.ui.Dialog({
			...
			primary_action: function() { dataent.ui.add_assignment(opts, this) },
		...
		me.dialog.fields_dict.assign_to.get_query = "dataent.core.doctype.user.user.user_query";
	...
	toggle_myself: function(myself) {
		...
		if($(myself).prop("checked")) {
			me.dialog.set_value("assign_to", dataent.session.user);
...
dataent.ui.add_assignment = function(opts, dialog) {
