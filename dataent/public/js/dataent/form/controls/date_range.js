dataent.ui.form.ControlDateRange = dataent.ui.form.ControlData.extend({
	make_input: function() {
		this._super();
	...
	set_date_options: function() {
		...
		this.datepicker_options.dateFormat =
			(dataent.boot.sysdefaults.date_format || 'yyyy-mm-dd');
	...
	parse: function(value) {
		...
		if(value && value.includes(',')) {
			...
			var from_date = moment(dataent.datetime.user_to_obj(vals[0])).format('YYYY-MM-DD');
			var to_date = moment(dataent.datetime.user_to_obj(vals[vals.length-1])).format('YYYY-MM-DD');
			return [from_date, to_date];
		}
	},
	format_for_input: function(value1, value2) {
		if(value1 && value2) {
			value1 = dataent.datetime.str_to_user(value1);
			value2 = dataent.datetime.str_to_user(value2);
