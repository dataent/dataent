...
dataent.ui.form.ControlDate = dataent.ui.form.ControlData.extend({
	make_input: function() {
		this._super();
	...
	set_formatted_input: function(value) {
		...
		if(should_refresh) {
			this.datepicker.selectDate(dataent.datetime.str_to_obj(value));
		}
	},
	set_date_options: function() {
		// webformTODO:
		let sysdefaults = dataent.boot.sysdefaults;

		let lang = dataent.boot.user.language || 'en';
		...
		this.date_format = dataent.defaultDateFormat;
	...
	get_now_date: function() {
		return dataent.datetime.now_date(true);
	},
		...
		this.$input.on("keydown", function(e) {
			if(e.which===84) { // 84 === t
				if(me.df.fieldtype=='Date') {
					me.set_value(dataent.datetime.nowdate());
				} if(me.df.fieldtype=='Datetime') {
					me.set_value(dataent.datetime.now_datetime());
				} if(me.df.fieldtype=='Time') {
					me.set_value(dataent.datetime.now_time());
				}
				return false;
			}
		});
	...
	parse: function(value) {
		if(value) {
			return dataent.datetime.user_to_str(value);
		}
	},
	...
	validate: function(value) {
		if(value && !dataent.datetime.validate(value)) {
			let sysdefaults = dataent.sys_defaults;
			let date_format = sysdefaults && sysdefaults.date_format
				? sysdefaults.date_format : 'yyyy-mm-dd';
			dataent.msgprint(__("Date {0} must be in format: {1}", [value, date_format]));
			return '';
		}
		return value;
	}
