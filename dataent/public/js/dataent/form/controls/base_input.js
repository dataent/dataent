dataent.ui.form.ControlInput = dataent.ui.form.Control.extend({
	horizontal: true,
	make: function() {
		// parent element
		this._super();
	...
	make_wrapper: function() {
		if(this.only_input) {
			this.$wrapper = $('<div class="form-group dataent-control">').appendTo(this.parent);
		} else {
			this.$wrapper = $('<div class="dataent-control">\
	...
	refresh_input: function() {
		...
		if (me.disp_status != "None") {
			// refresh value
			if(me.doctype && me.docname) {
				me.value = dataent.model.get_value(me.doctype, me.docname, me.df.fieldname);
			}
	...
	set_disp_area: function(value) {
		...
		this.disp_area && $(this.disp_area)
			.html(dataent.format(value, this.df, {no_icon:true, inline:true},
	...
	bind_focusout: function() {
		...
		if (dataent.dom.is_touchscreen()) {
			...
			this.$input && this.$input.on("focusout", function() {
				if (dataent.dom.is_touchscreen()) {
					dataent.utils.scroll_to(me.$wrapper);
				}
			});
		}
	},
