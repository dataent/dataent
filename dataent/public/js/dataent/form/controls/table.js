...
dataent.ui.form.ControlTable = dataent.ui.form.Control.extend({
	make: function() {
		this._super();
		...
		this.$wrapper.on('paste',':text', function(e) {
			...
			var data = dataent.utils.csv_to_array(pastedData,'\t');
			if (data.length === 1 & data[0].length === 1) return;
			if (data.length > 100){
				data = data.slice(0, 100);
				dataent.msgprint(__('For performance, only the first 100 rows were processed.'));
			}
			var fieldnames = [];
			...
			$.each(data, function(i, row) {
				...
				if(!blank_row) {
					...
					$.each(row, function(ci, value) {
						if (fieldnames[ci]) dataent.model.set_value(cur_doctype, row_name, fieldnames[ci], value);
					});
					dataent.show_progress(__('Processing'), i, data.length);
				}
			});
