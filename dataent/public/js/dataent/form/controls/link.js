...
dataent.ui.form.ControlLink = dataent.ui.form.ControlData.extend({
	make_input: function() {
		var me = this;
		...
		this.$input.on("focus", function() {
			setTimeout(function() {
				if(me.$input.val() && me.get_options()) {
					...
					me.$link.toggle(true);
					me.$link_open.attr('href', dataent.utils.get_form_link(doctype, name));
				}
	...
	get_reference_doctype() {
		...
		return this.doctype || dataent.get_route()[0] === 'List' ? dataent.get_route()[1] : null;
	},
	...
	new_doc: function() {
		...
		if(this.df.get_route_options_for_new_doc) {
			dataent.route_options = this.df.get_route_options_for_new_doc(this);
		} else {
			dataent.route_options = {};
		}

		// partially entered name field
		dataent.route_options.name_field = this.get_value();

		// reference to calling link
		dataent._from_link = this;
		dataent._from_link_scrollY = $(document).scrollTop();

		dataent.ui.form.make_quick_entry(doctype, (doc) => {
	...
	setup_awesomeplete: function() {
		...
		this.$input.on("input", dataent.utils.debounce(function(e) {
		...
		this.$input.on("awesomplete-select", function(e) {
			...
			if(me.df.remember_last_selected_value) {
				dataent.boot.user.last_selected_values[me.df.options] = item.value;
			}
	...
	validate_link_and_fetch: function(df, doctype, docname, value) {
		...
		if(value) {
			return new Promise((resolve) => {
				...
				return dataent.call({
					method:'dataent.desk.form.utils.validate_link',
					type: "GET",
					args: {
						'value': value,
						'options': doctype,
	...
	set_fetch_values: function(df, docname, fetch_values) {
		...
		for(var i=0; i < fl.length; i++) {
			dataent.model.set_value(df.parent, docname, fl[i], fetch_values[i], df.fieldtype);
		}
	}
});
