dataent.ui.form.ControlGeolocation = dataent.ui.form.ControlCode.extend({
	make_wrapper() {
		// Create the elements for map area
		this._super();

		let $input_wrapper = this.$wrapper.find('.control-input-wrapper');
		this.map_id = dataent.dom.get_unique_id();
		this.map_area = $(
		...
		if ($input_wrapper.is(':visible')) {
			...
			$(document).on('dataent.ui.Dialog:shown', () => {
				this.make_map();
			});
		}
	},
	...
	bind_leaflet_map() {
		...
		L.Icon.Default.imagePath = '/assets/dataent/images/leaflet/';
	...
	bind_leaflet_draw_control() {
		...
		var options = {
			...
			draw: {
				polyline: {
					shapeOptions: {
						color: dataent.ui.color.get('blue'),
						weight: 10
					}
				},
				polygon: {
					...
					drawError: {
						color: dataent.ui.color.get('orange'), // Color the shape will turn when intersects
