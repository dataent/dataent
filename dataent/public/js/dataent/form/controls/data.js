dataent.ui.form.ControlData = dataent.ui.form.ControlInput.extend({
	html_element: "input",
	input_type: "text",
	make_input: function() {
	...
	setup_autoname_check: function() {
		...
		this.meta = dataent.get_meta(this.df.parent);
			...
			this.$input.on('keyup', () => {
				...
				if (this.doc && this.doc.__islocal) {
					...
					let timeout = setTimeout(() => {
						...
						dataent.db.get_value(this.doctype, this.$input.val(),
							'name', (val) => {
								if (val) {
	...
	validate: function(v) {
		...
		if(this.df.options == 'Phone') {
			...
			var email_list = dataent.utils.split_emails(v);
			if (!email_list) {
				// invalid email
				return '';
			} else {
				...
				email_list.forEach(function(email) {
					if (!validate_email(email)) {
						dataent.msgprint(__("Invalid Email: {0}", [email]));
						invalid_email = true;
					}
				});
