...
dataent.ui.form.ControlComment = dataent.ui.form.ControlTextEditor.extend({
	make_wrapper() {
		...
		// wrapper should point to dataent-control
		this.$wrapper = !this.no_wrapper
			? this.comment_wrapper.find('.dataent-control')
	...
	bind_events() {
		...
		this.$wrapper.on('keydown', e => {
			const key = dataent.ui.keys.get_key(e);
			if (key === 'ctrl+enter') {
				e.preventDefault();
				this.submit();
			}
		...
		this.quill.on('text-change', dataent.utils.debounce(() => {
			this.update_state();
		}, 300));
	},
