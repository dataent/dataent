dataent.ui.form.make_control = function (opts) {
	var control_class_name = "Control" + opts.df.fieldtype.replace(/ /g, "");
	if(dataent.ui.form[control_class_name]) {
		return new dataent.ui.form[control_class_name](opts);
	} else {
...
dataent.ui.form.Control = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
		this.make();

		// if developer_mode=1, show fieldname as tooltip
		if(dataent.boot.user && dataent.boot.developer_mode===1 && this.$wrapper) {
	...
	make_wrapper: function() {
		this.$wrapper = $("<div class='dataent-control'></div>").appendTo(this.parent);
	...
	get_status: function(explain) {
		...
		var status = dataent.perm.get_field_display_status(this.df,
			dataent.model.get_doc(this.doctype, this.docname), this.perm || (this.frm && this.frm.perm), explain);

		// hide if no value
		if (this.doctype && status==="Read" && !this.only_input
			&& is_null(dataent.model.get_value(this.doctype, this.docname, this.df.fieldname))
	...
	show_translatable_button(value) {
		...
		if (!dataent.model
			...
			|| !dataent.model.can_write('Translation')
			|| !value) return;

		// Disable translation in website
		if (!dataent.views || !dataent.views.TranslationManager) return;
		...
		$(translation_btn)
			...
			.on('click', () => {
				if (!this.doc.__islocal) {
					new dataent.views.TranslationManager({
						'df': this.df,
						'source_name': value,
	...
	set_model_value: function(value) {
		if(this.doctype && this.docname) {
			...
			return dataent.model.set_value(this.doctype, this.docname, this.df.fieldname,
