...
dataent.provide('dataent.success_action');

dataent.ui.form.SuccessAction = class SuccessAction {
	constructor(form) {
		this.form = form;
		this.load_setting();
	}
	...
	load_setting() {
		this.setting = dataent.boot.success_action
	...
	show_alert() {
		dataent.db.count(this.form.doctype)
			.then(count => {
	...
	get default_actions() {
		return {
			'new': {
				label: __('New'),
				action: (frm) => dataent.new_doc(frm.doctype)
			},
			'print': {
				label: __('Print'),
			...
			'list': {
				label: __('View All'),
				action: (frm) => {
					dataent.set_route('List', frm.doctype);
				}
			}
		};
	}
