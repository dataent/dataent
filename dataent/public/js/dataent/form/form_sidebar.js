...
dataent.ui.form.Sidebar = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
	},

	make: function() {
		var sidebar_content = dataent.render_template("form_sidebar", {doctype: this.frm.doctype, frm:this.frm});
		...
		this.bind_events();
		dataent.ui.form.setup_user_image_event(this.frm);
	...
	bind_events: function() {
		...
		this.comments.on("click", function() {
			dataent.utils.scroll_to(me.frm.footer.wrapper.find(".form-comments"), true);
		});

		this.like_icon.on("click", function() {
			dataent.ui.toggle_like(me.like_icon, me.frm.doctype, me.frm.doc.name, function() {
				me.refresh_like();
			});
		})
	},
	...
	refresh: function() {
		if(this.frm.doc.__islocal) {
			...
			this.sidebar.find(".modified-by").html(__("{0} edited this {1}",
				["<strong>" + dataent.user.full_name(this.frm.doc.modified_by) + "</strong>",
					"<br>" + comment_when(this.frm.doc.modified)]));
			this.sidebar.find(".created-by").html(__("{0} created this {1}",
				["<strong>" + dataent.user.full_name(this.frm.doc.owner) + "</strong>",
					"<br>" + comment_when(this.frm.doc.creation)]));

			this.refresh_like();
			this.setup_ratings();
			dataent.ui.form.set_user_image(this.frm);
		}
	},
	...
	make_tags: function() {
		...
		this.frm.tags = new dataent.ui.TagEditor({
	...
	make_attachments: function() {
		var me = this;
		this.frm.attachments = new dataent.ui.form.Attachments({
			parent: me.sidebar.find(".form-attachments"),
			frm: me.frm
		});
	},
	make_assignments: function() {
		this.frm.assign_to = new dataent.ui.form.AssignTo({
			parent: this.sidebar.find(".form-assignments"),
			frm: this.frm
		});
	},
	make_shared: function() {
		this.frm.shared = new dataent.ui.form.Share({
			frm: this.frm,
			parent: this.sidebar.find(".form-shared")
		});
	},
	make_viewers: function() {
		this.frm.viewers = new dataent.ui.form.Viewers({
			frm: this.frm,
	...
	make_like: function() {
		...
		dataent.ui.setup_like_popover(this.sidebar.find(".liked-by-parent"), ".liked-by");
	},
	...
	refresh_like: function() {
		...
		this.like_icon.toggleClass("text-extra-muted not-liked",
			!dataent.ui.is_liked(this.frm.doc))
	...
	setup_ratings: function() {
		...
		if(_ratings) {
			...
			var rating_icons = dataent.render_template("rating_icons", {rating: _ratings, show_label: false});
