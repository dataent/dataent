...
dataent.ui.form.LinkSelector = Class.extend({
	init: function (opts) {
		...
		if (this.doctype != "[Select]") {
			dataent.model.with_doctype(this.doctype, function (r) {
				me.make();
			});
		} else {
			this.make();
	...
	search: function () {
		...
		dataent.link_search(this.doctype, args, function (r) {
			...
			dataent.prompt({
				...
				$.each(me.target.frm.doc[me.target.df.fieldname] || [], function (i, d) {
					if (d[me.fieldname] === value) {
						dataent.model.set_value(d.doctype, d.name, me.qty_fieldname, data.qty);
						dataent.show_alert(__("Added {0} ({1})", [value, d[me.qty_fieldname]]));
						updated = true;
						return false;
					}
				});
				if (!updated) {
					dataent.run_serially([
						...
						() => dataent.timeout(0.1),
						() => dataent.model.set_value(d.doctype, d.name, me.fieldname, value),
						() => dataent.timeout(0.5),
						() => dataent.model.set_value(d.doctype, d.name, me.qty_fieldname, data.qty),
						() => dataent.show_alert(__("Added {0} ({1})", [value, data.qty]))
					]);
				}
			}, __("Set Quantity"), __("Set"));
		} else if (me.dynamic_link_field) {
			var d = me.target.add_new_row();
			dataent.model.set_value(d.doctype, d.name, me.dynamic_link_field, me.dynamic_link_reference);
			dataent.model.set_value(d.doctype, d.name, me.fieldname, value);
			dataent.show_alert(__("{0} {1} added", [me.dynamic_link_reference, value]));
		} else {
			var d = me.target.add_new_row();
			dataent.model.set_value(d.doctype, d.name, me.fieldname, value);
			dataent.show_alert(__("{0} added", [value]));
		}
	}
});

dataent.link_search = function (doctype, args, callback, btn) {
	if (!args) {
		args = {
			txt: ''
		}
	...
	dataent.call({
		method: "dataent.desk.search.search_widget",
		type: "GET",
		args: args,
		callback: function (r) {
			callback && callback(r);
