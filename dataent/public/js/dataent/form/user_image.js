dataent.ui.form.set_user_image = function(frm) {
	...
	if (image) {
		...
		image_section
			...
			.removeClass('hide')
			.find('.standard-image')
			.css({'background-color': dataent.get_palette(title)})
			.html(dataent.get_abbr(title));
...
dataent.ui.form.setup_user_image_event = function(frm) {
	// re-draw image on change of user image
	if(frm.meta.image_field) {
		dataent.ui.form.on(frm.doctype, frm.meta.image_field, function(frm) {
			dataent.ui.form.set_user_image(frm);
		});
	}
