/**
 * dataent.views.ImageView
 */
dataent.provide("dataent.views");

dataent.views.ImageView = class ImageView extends dataent.views.ListView {
	get view_name() {
		return 'Image';
	}
	...
	get_image_url(data) {
		...
		if (window.cordova && !dataent.utils.is_url(url)) {
			url = dataent.base_url + url;
		}
		if (url) {
			return url;
		}
	...
	get_attached_images() {
		return dataent.call({
			method: 'dataent.core.doctype.file.file.get_attached_images',
			args: {
	...
	setup_gallery() {
		var me = this;
		this.gallery = new dataent.views.GalleryView({
			doctype: this.doctype,
			items: this.items,
			wrapper: this.$result,
...
dataent.views.GalleryView = Class.extend({
	init: function (opts) {
		$.extend(this, opts);
		var me = this;
	...
	prepare: function () {
		...
		if (this.pswp_root.length === 0) {
			var pswp = dataent.render_template('photoswipe_dom');
	...
	load_lib: function () {
		return new Promise(resolve => {
			var asset_dir = 'assets/dataent/js/lib/photoswipe/';
			dataent.require([
