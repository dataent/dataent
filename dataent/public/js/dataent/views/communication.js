...
dataent.last_edited_communication = {};
dataent.standard_replies = {};

dataent.views.CommunicationComposer = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
		this.make();
	},
	make: function() {
		...
		['recipients', 'cc', 'bcc'].forEach(field => {
			this.dialog.fields_dict[field].get_data = function() {
				...
				dataent.call({
					method: "dataent.email.get_contact_list",
					args: {
						txt: txt,
					},
					callback: (r) => {
	...
	get_fields: function() {
		...
		var fields= [
			...
			{fieldtype: "Section Break"},
			{
				...
				fieldname:"content",
				onchange: dataent.utils.debounce(this.save_as_draft.bind(this), 300)
			},

			{fieldtype: "Section Break"},
			{fieldtype: "Column Break"},
			{label:__("Send me a copy"), fieldtype:"Check",
				fieldname:"send_me_a_copy", 'default': dataent.boot.user.send_me_a_copy},
		...
		var email_accounts = dataent.boot.email_accounts.filter(function(account, idx){
			return !in_list(["All Accounts", "Sent", "Spam", "Trash"], account.email_account) &&
				account.enable_outgoing
		})
		if(dataent.boot.email_accounts && email_accounts.length > 1) {
			fields = [
	...
	setup_email_template: function() {
		...
		this.dialog.fields_dict["email_template"].df.onchange = () => {
			...
			dataent.call({
				method: 'dataent.email.doctype.email_template.email_template.get_email_template',
				args: {
	...
	get_last_edited_communication: function() {
		if (!dataent.last_edited_communication[this.doc]) {
			dataent.last_edited_communication[this.doc] = {};
		}

		if(!dataent.last_edited_communication[this.doc][this.key]) {
			dataent.last_edited_communication[this.doc][this.key] = {};
		}

		return dataent.last_edited_communication[this.doc][this.key];
	},
	...
	setup_print_language: function() {
		...
		$(fields.language_sel.input)
			.empty()
			.add_options(dataent.get_languages());
	...
	setup_attach: function() {
		...
		attach.find(".add-more-attachments a").on('click',this,function() {
			me.upload = dataent.ui.get_upload_dialog(args);
		})
		me.render_attach()
	...
	render_attach:function(){
		...
		if(files.length) {
			$.each(files, function(i, f) {
				...
				f.file_url = dataent.urllib.get_full_url(f.file_url);
	...
	setup_email: function() {
		...
		$(fields.send_me_a_copy.input).on('click', () => {
			...
			dataent.db.set_value('User', dataent.session.user, 'send_me_a_copy', val);
			dataent.boot.user.send_me_a_copy = val;
		});
	...
	save_as_draft: function() {
		if (this.dialog) {
			try {
				...
				message = message.split(dataent.separator_element)[0];
	...
	send_email: function(btn, form_values, selected_attachments, print_html, print_format) {
		...
		if(!form_values.recipients) {
			dataent.msgprint(__("Enter Email Recipient(s)"));
			return;
		}
		...
		if(cur_frm && !dataent.model.can_email(me.doc.doctype, cur_frm)) {
			dataent.msgprint(__("You are not allowed to send emails related to this document"));
			return;
		}


		return dataent.call({
			method:"dataent.core.doctype.communication.email.make",
			args: {
				...
				sender_full_name: form_values.sender?dataent.user.full_name():undefined,
			...
			callback: function(r) {
				if(!r.exc) {
					dataent.utils.play_sound("email");

					if(r.message["emails_not_sent_to"]) {
						dataent.msgprint(__("Email not sent to {0} (unsubscribed / disabled)",
							[ dataent.utils.escape_html(r.message["emails_not_sent_to"]) ]) );
					}

					if ((dataent.last_edited_communication[me.doc] || {})[me.key]) {
						delete dataent.last_edited_communication[me.doc][me.key];
					}
					if (cur_frm) {
						// clear input
					...
					dataent.msgprint(__("There were errors while sending email. Please try again."));
	...
	is_print_letterhead_checked: function() {
		if (this.frm && $(this.frm.wrapper).find('.form-print-wrapper').is(':visible')){
			...
			return (dataent.model.get_doc(":Print Settings", "Print Settings") ||
	...
	setup_earlier_reply: function() {
		...
		let signature = dataent.boot.user.email_signature || "";

		if(!dataent.utils.is_html(signature)) {
		...
		if (this.is_a_reply) {
			...
			content = `
				<div><br></div>
				${reply}
				${dataent.separator_element}
				<p>${__("On {0}, {1} wrote:", [dataent.datetime.global_date_format(communication_date) , last_email.sender])}</p>
				<blockquote>
				${last_email_content}
				</blockquote>
			`;
