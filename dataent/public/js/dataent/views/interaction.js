...
dataent.provide("dataent.interaction_settings");

dataent.views.InteractionComposer = class InteractionComposer {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	make() {
		let me = this;
		me.dialog = new dataent.ui.Dialog({
	...
	get_event_categories() {
		let me = this;
		dataent.model.with_doctype('Event', () => {
			let categories = dataent.meta.get_docfield("Event", "event_category").options.split("\n");
	...
	create_interaction(btn, form_values, selected_attachments) {
		...
		if (!("owner" in interaction_values)){
			interaction_values["owner"] = dataent.session.user;
		}
		return dataent.call({
			...
			callback: function(r) {
				if(!r.exc) {
					...
					dataent.msgprint(__("There were errors while creating the document. Please try again."));
				}
			}
		});
	...
	add_attachments(doc, attachments) {
		dataent.call({
			method:'dataent.utils.file_manager.add_attachments',
			args: {
				doctype: doc.doctype,
				name: doc.name,
