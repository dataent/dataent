...
dataent.provide("dataent.views.calendar");
dataent.provide("dataent.views.calendars");

dataent.views.CalendarView = class CalendarView extends dataent.views.ListView {
	static load_last_view() {
		const route = dataent.get_route();
		if (route.length === 3) {
			const doctype = route[1];
			const user_settings = dataent.get_user_settings(doctype)['Calendar'] || {};
	...
	setup_defaults() {
		...
		this.calendar_settings = dataent.views.calendar[this.doctype] || {};
		this.calendar_name = dataent.get_route()[3];
	}
	...
	render() {
		...
		this.load_lib
			...
			.then(options => {
				this.calendar = new dataent.views.Calendar(options);
			});
	}
	...
	get_calendar_preferences() {
		...
		return new Promise(resolve => {
			if (calendar_name === 'Default') {
				Object.assign(options, dataent.views.calendar[this.doctype]);
				resolve(options);
			} else {
				dataent.model.with_doc('Calendar View', calendar_name, () => {
					const doc = dataent.get_doc('Calendar View', calendar_name);
	...
	get required_libs() {
		return [
			'assets/dataent/js/lib/fullcalendar/fullcalendar.min.css',
			'assets/dataent/js/lib/fullcalendar/fullcalendar.min.js',
			'assets/dataent/js/lib/fullcalendar/locale-all.js'
		];
	}
};

dataent.views.Calendar = Class.extend({
	...
	make_page: function() {
		...
		$.each(dataent.boot.calendars, function(i, doctype) {
			if(dataent.model.can_read(doctype)) {
				me.page.add_menu_item(__(doctype), function() {
					dataent.set_route("List", doctype, "Calendar");
				});
			}
		});
	...
	make: function() {
		...
		this.footnote_area = dataent.utils.set_footnote(this.footnote_area, this.$wrapper,
	...
	get_system_datetime: function(date) {
		date._offset = (moment(date).tz(dataent.sys_defaults.time_zone)._offset);
		return dataent.datetime.convert_to_system_tz(date);
	},
	setup_options: function(defaults) {
		var me = this;
		this.cal_options = {
			locale: dataent.boot.user.language || "en",
			header: {
				left: 'title',
				center: '',
			...
			events: function(start, end, timezone, callback) {
				return dataent.call({
					method: me.get_events_method || "dataent.desk.calendar.get_events",
					type: "GET",
			...
			eventClick: function(event) {
				...
				if(dataent.model.can_read(doctype)) {
					dataent.set_route("Form", doctype, event.name);
				}
			},
			...
			select: function(startDate, endDate, jsEvent, view) {
				...
				var event = dataent.model.get_new_doc(me.doctype);
				...
				dataent.set_route("Form", me.doctype, event.name);
			},
	...
	prepare_events: function(events) {
		...
		return (events || []).map(d => {
			d.id = d.name;
			d.editable = dataent.model.can_write(d.doctype || me.doctype);
			...
			// convert to user tz
			d.start = dataent.datetime.convert_to_user_tz(d.start);
			d.end = dataent.datetime.convert_to_user_tz(d.end);

			// show event on single day if start or end date is invalid
			if (!dataent.datetime.validate(d.start) && d.end) {
				d.start = dataent.datetime.add_days(d.end, -1);
			}

			if (d.start && !dataent.datetime.validate(d.end)) {
				d.end = dataent.datetime.add_days(d.start, 1);
			}
	...
	prepare_colors: function(d) {
		...
		if(this.get_css_class) {
			...
			color_name = dataent.ui.color.validate_hex(color_name) ?
				color_name : 'blue';
			d.backgroundColor = dataent.ui.color.get(color_name, 'extra-light');
			d.textColor = dataent.ui.color.get(color_name, 'dark');
		} else {
			color = d.color;
			if (!dataent.ui.color.validate_hex(color) || !color) {
				color = dataent.ui.color.get('blue', 'extra-light');
			}
			d.backgroundColor = color;
			d.textColor = dataent.ui.color.get_contrast_color(color);
		}
		return d;
	},
	update_event: function(event, revertFunc) {
		var me = this;
		dataent.model.remove_from_locals(me.doctype, event.name);
		return dataent.call({
			method: me.update_event_method || "dataent.desk.calendar.update_event",
			args: me.get_update_args(event),
			callback: function(r) {
				if(r.exc) {
					dataent.show_alert(__("Unable to update event"));
					revertFunc();
				}
			},
			error: function() {
