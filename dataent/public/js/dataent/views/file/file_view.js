...
dataent.views.FileView = class FileView extends dataent.views.ListView {
	static load_last_view() {
		const route = dataent.get_route();
		if (route.length === 2) {
			const view_user_settings = dataent.get_user_settings('File', 'File');
			dataent.set_route('List', 'File', view_user_settings.last_folder || dataent.boot.home_folder);
			return true;
		}
		...
		const items = [
			{
				label: __('Home'),
				action: () => {
					dataent.set_route('List', 'File', 'Home');
				},
			},
			{
				label: __('Cut'),
				action: () => {
					dataent.file_manager.cut(this.get_checked_items(), this.current_folder);
				},
			...
			},
			{
				label: __('Paste'),
				action: () => {
					dataent.file_manager.paste(this.current_folder);
				},
			...
			},
			{
				label: __('New Folder'),
				action: () => {
					dataent.prompt(__('Name'), (values) => {
						if((values.value.indexOf("/") > -1)) {
							dataent.throw(__("Folder name should not include '/' (slash)"));
						}
						const data =  {
						...
						dataent.call({
							method: "dataent.core.doctype.file.file.create_new_folder",
							args: data
						});
			...
			},
			{
				...
				action: () => {
					dataent.views.FileView.grid_view = !dataent.views.FileView.grid_view;
					this.refresh();
				}
			},
			{
				...
				action: () => {
					...
					dataent.ui.get_upload_dialog({
						...
						callback: (attachment, r) => {
							dataent.call({
								method: 'dataent.core.doctype.file.file.unzip_file',
								args: {
									name: r.message.name,
								},
								callback: function (r) {
									if(r.exc) {
										dataent.msgprint(__('Error in uploading files' + r.exc));
									}
								}
							});
						},
	...
	set_fields() {
		this.fields = this.meta.fields
			.filter(df => dataent.model.is_value_type(df.fieldtype) && !df.hidden)
			.map(df => df.fieldname)
	...
	prepare_datum(d) {
		...
		} else if (dataent.utils.is_image_file(d.file_name)) {
	...
	before_render() {
		super.before_render();
		dataent.model.user_settings.save('File', 'grid_view', dataent.views.FileView.grid_view);
	...
	render() {
		...
		if (dataent.views.FileView.grid_view) {
	...
	get_left_html(file) {
		...
		const file_size = dataent.form.formatters.FileSize(file.file_size);
		const route_url = this.get_route_url(file);

		let created_on;
		const [date] = file.creation.split(' ');
		if (date === dataent.datetime.now_date()) {
			created_on = comment_when(file.creation);
		} else {
			created_on = dataent.datetime.str_to_user(date);
		}
	...
	toggle_cut_paste_buttons() {
		...
		const hide = !dataent.file_manager.can_paste ||
			dataent.file_manager.old_folder === this.current_folder;
...
dataent.views.FileView.grid_view = dataent.get_user_settings('File').grid_view || false;
