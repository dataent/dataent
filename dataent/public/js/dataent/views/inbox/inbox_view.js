/**
 * dataent.views.InboxView
 */

dataent.provide("dataent.views");

dataent.views.InboxView = class InboxView extends dataent.views.ListView {
	static load_last_view() {
		const route = dataent.get_route();
		if (!route[3] && dataent.boot.email_accounts.length) {
			let email_account;
			if (dataent.boot.email_accounts[0].email_id == "All Accounts") {
				email_account = "All Accounts";
			} else {
				email_account = dataent.boot.email_accounts[0].email_account;
			}
			dataent.set_route("List", "Communication", "Inbox", email_account);
			return true;
		} else if (!route[3] || (route[3] !== "All Accounts" && !is_valid(route[3]))) {
			dataent.msgprint(__('Invalid Email Account'));
			window.history.back();
			return true;
		}
		return false;
		...
		function is_valid(email_account) {
			return dataent.boot.email_accounts.find(d => d.email_account === email_account);
		}
	}
	...
	setup_defaults() {
		...
		this.email_account = dataent.get_route()[3];
	...
	get_meta_html(email) {
		...
		const form_link = dataent.utils.get_form_link(email.reference_doctype, email.reference_name);
	...
	get_inbox_filters() {
		...
		if (email_account === "Sent") {
			...
			filters = default_filters.concat([
				...
				["Communication", "email_account", "in", dataent.boot.all_accounts, true]
			]);
		} else {
			var op = "=";
			if (email_account == "All Accounts") {
				op = "in";
				email_account = dataent.boot.all_accounts;
			}
	...
	get_no_result_message() {
		...
		} else if (!email_account && !dataent.boot.email_accounts.length) {
		...
		const html = dataent.model.can_create(args.doctype) ?
			`<p>${args.msg}</p>
			<p>
	...
	make_new_doc() {
		if (!this.email_account && !dataent.boot.email_accounts.length) {
			dataent.route_options = {
				'email_id': dataent.session.user_email
			};
