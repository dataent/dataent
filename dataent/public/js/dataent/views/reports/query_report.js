...
dataent.provide('dataent.query_reports');

dataent.standard_pages['query-report'] = function() {
	var wrapper = dataent.container.add_page('query-report');
	...
	dataent.query_report = new dataent.views.QueryReport({
		parent: wrapper,
	});

	$(wrapper).bind('show', function() {
		dataent.query_report.show();
	});
};

dataent.views.QueryReport = class QueryReport extends dataent.views.BaseList {
	show() {
	...
	init() {
		...
		this.init_promise = dataent.run_serially(tasks);
		return this.init_promise;
	}

	setup_defaults() {
		this.route = dataent.get_route();
		...
		this.refresh = dataent.utils.throttle(this.refresh, 300);

		this.menu_items = [];
	}

	setup_events() {
		dataent.realtime.on("report_generated", (data) => {
			if(data.report_name) {
				...
				if(data.name == this.prepared_report_doc_name) {
					...
					dataent.show_alert({message: alert_message, indicator: 'orange'});
				}
			}
		});
	...
	load() {
		if (dataent.get_route().length < 2) {
			this.toggle_nothing_to_show(true);
			return;
		}

		if (this.report_name !== dataent.get_route()[1]) {
			// this.toggle_loading(true);
			// different report
			this.load_report();
		}
		else if (dataent.has_route_options()) {
	...
	get_report_doc() {
		return dataent.model.with_doc('Report', this.report_name)
			.then(doc => {
				this.report_doc = doc;
			})
			.then(() => dataent.model.with_doctype(this.report_doc.ref_doctype));
	}

	get_report_settings() {
		if (dataent.query_reports[this.report_name]) {
			this.report_settings = this.get_local_report_settings();
			return this._load_script;
		}

		this._load_script = (new Promise(resolve => dataent.call({
			method: 'dataent.desk.query_report.get_script',
			args: { report_name: this.report_name },
			callback: resolve
		}))).then(r => {
			dataent.dom.eval(r.message.script || '');
			return r;
		}).then(r => {
			return dataent.after_ajax(() => {
				this.report_settings = this.get_local_report_settings();
	...
	get_local_report_settings() {
		let report_script_name = this.report_doc.report_type === 'Custom Report'
			...
			: this.report_name;
		return dataent.query_reports[report_script_name];
	}
	...
	setup_progress_bar() {
		...
		this.interval = setInterval(function()  {
			seconds_elapsed += 1;
			dataent.show_progress(__('Preparing Report'), seconds_elapsed, execution_time);
		}, 1000);
	}
	...
	set_route_filters() {
		if(dataent.route_options) {
			const fields = Object.keys(dataent.route_options);

			const filters_to_set = this.filters.filter(f => fields.includes(f.df.fieldname));

			const promises = filters_to_set.map(f => {
				return () => {
					const value = dataent.route_options[f.df.fieldname];
			...
			return dataent.run_serially(promises);
		}
	}
	...
	refresh() {
		...
		let query = dataent.utils.get_query_string(dataent.get_route_str());

		if(query) {
			let obj = dataent.utils.get_query_params(query);
		...
		return new Promise(resolve => {
			this.last_ajax = dataent.call({
				method: 'dataent.desk.query_report.run',
				type: 'GET',
				args: {
			...
			if (data.prepared_report) {
				...
				const query_string = dataent.utils.get_query_string(dataent.get_route_str());
				const query_params = dataent.utils.get_query_params(query_string);
	...
	add_prepared_report_buttons(doc) {
			...
			this.page.add_inner_button(__("Download Report"), function (){
				window.open(
					dataent.urllib.get_full_url(
						"/api/method/dataent.core.doctype.prepared_report.prepared_report.download_attachment?"
						+"dn="+encodeURIComponent(doc.name)));
			});

			const part1 = __('This report was generated {0}.', [dataent.datetime.comment_when(doc.report_end_time)]);
		...
		if(this.prepared_report_action == "New") {
			...
			this.page.set_primary_action(
				__("Edit"),
				() => {
					dataent.set_route(dataent.get_route());
				}
			);
	...
	generate_background_report() {
		...
		if (!missing_mandatory.length){
			let filters = this.get_filter_values(true);
			return new Promise(resolve => dataent.call({
				method: 'dataent.desk.query_report.background_enqueue_run',
				type: 'GET',
				args: {
					report_name: this.report_name,
					filters: filters
	...
	render_datatable() {
		...
		if (this.datatable) {
			...
			let datatable_options = {
				...
				direction: dataent.utils.is_rtl() ? 'rtl' : 'ltr',
				hooks: {
					columnTotal: dataent.utils.report_column_total
				}
			};
	...
	get_user_settings() {
		return dataent.model.user_settings.get(this.report_name)
			.then(user_settings => {
	...
	prepare_columns(columns) {
		return columns.map(column => {
			...
			const format_cell = (value, row, column, data) => {
				...
				return dataent.format(value, column,
			...
			if (column.fieldtype === 'Date') {
				compareFn = (cell, keyword) => {
					...
					const keywordValue = dataent.datetime.user_to_obj(keyword);
					const cellValue = dataent.datetime.str_to_obj(cell.content);
	...
	set_breadcrumbs() {
		...
		const ref_doctype = dataent.get_meta(this.report_doc.ref_doctype);
		dataent.breadcrumbs.add(ref_doctype.module);
	}
	...
	pdf_report(print_settings) {
		const base_url = dataent.urllib.get_base_url();
		const print_css = dataent.boot.print_css;
		const landscape = print_settings.orientation == 'Landscape';
		...
		const filters_html = this.get_filters_html_for_print();
		const content = dataent.render_template(custom_format || 'print_grid', {
			title: __(this.report_name),
			subtitle: filters_html,
		...
		const html = dataent.render_template('print_template', {
			title: __(this.report_name),
			content: content,
			base_url: base_url,
			print_css: print_css,
		...
		dataent.render_pdf(html, print_settings);
	}
	...
	get_filters_html_for_print() {
		...
		return Object.keys(applied_filters)
			.map(fieldname => {
				const label = dataent.query_report.get_filter(fieldname).df.label;
	...
	export_report() {
		...
		this.export_dialog = dataent.prompt([
			{
			...
			if (file_format === 'CSV') {
				...
				dataent.tools.downloadify(out, null, this.report_name);
			} else {
				let filters = this.get_filter_values(true);
				if (dataent.urllib.get_dict("prepared_report_name")) {
					filters = Object.assign(dataent.urllib.get_dict("prepared_report_name"), filters);
				}
				...
				const args = {
					cmd: 'dataent.desk.query_report.export_query',
					report_name: this.report_name,
	...
	get_menu_items() {
		return [
			...
			{
				label: __('Edit'),
				action: () => dataent.set_route('Form', 'Report', this.report_name),
				condition: () => dataent.user.is_report_manager(),
				standard: true
			},
			{
				label: __('Print'),
				action: () => {
					dataent.ui.get_print_settings(
						false,
						print_settings => this.print_report(print_settings),
						this.report_doc.letter_head
					);
				},
				condition: () => dataent.model.can_print(this.report_doc.ref_doctype),
				standard: true
			},
			{
				label: __('PDF'),
			...
			},
			{
				...
				condition: () => dataent.model.can_export(this.report_doc.ref_doctype),
				standard: true
			},
			{
				label: __('Setup Auto Email'),
				action: () => dataent.set_route('List', 'Auto Email Report', {'report' : this.report_name}),
				standard: true
			},
			{
				label: __('Add Column'),
				action: () => {
					let d = new dataent.ui.Dialog({
						title: __('Add Column'),
						fields: [
							{
								fieldtype: 'Select',
								...
								change: () => {
									...
									dataent.model.with_doctype(doctype, () => {
										let options = dataent.meta.get_docfields(doctype)
											.filter(dataent.model.is_value_type)
						...
						primary_action: (values) => {
							...
							let df = dataent.meta.get_docfield(values.doctype, values.field);
							custom_columns.push({
								...
								options: df.fieldtype === "Link" ? dataent.model.unscrub(df.fieldname) : undefined,
								width: 100
							});

							dataent.call({
								method: 'dataent.desk.query_report.get_data_for_custom_field',
								args: {
									field: values.field,
									doctype: values.doctype
								},
			...
			},
			{
				...
				action: () => {
					let d = new dataent.ui.Dialog({
						...
						primary_action: (values) => {
							dataent.call({
								method: "dataent.desk.query_report.save_report",
								args: {
								...
								callback: function(r) {
									this.show_save = false;
									d.hide();
									dataent.set_route('query-report', r.message);
								}
							});
						}
					});
			...
			},
			{
				...
				action: () => dataent.set_route('List', 'User Permission', {
					doctype: 'Report',
					name: this.report_name
				}),
				condition: () => dataent.model.can_set_user_permissions('Report'),
				standard: true
			},
			{
				label: __('Add to Desktop'),
				action: () => dataent.add_to_desktop(this.report_name, null, this.report_name),
				standard: true
			},
		];
	}
...
Object.defineProperty(dataent, 'query_report_filters_by_name', {
	get() {
		console.warn('[Query Report] dataent.query_report_filters_by_name is deprecated. Please use the new api: dataent.query_report.get_filter_value(fieldname) and dataent.query_report.set_filter_value(fieldname, value)');
		return null;
	}
});
