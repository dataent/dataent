...
dataent.provide("dataent.report_dump");

$.extend(dataent.report_dump, {
	data: {},
	last_modified: {},
	with_data: function(doctypes, callback) {
		var pre_loaded = Object.keys(dataent.report_dump.last_modified);
		return dataent.call({
			method: "dataent.desk.report_dump.get_data",
			type: "GET",
			args: {
				doctypes: doctypes,
				last_modified: dataent.report_dump.last_modified
			},
			freeze: true,
			callback: function(r) {
				// creating map of data from a list
				$.each(r.message, function(doctype, doctype_data) {
					dataent.report_dump.set_data(doctype, doctype_data);
				});
				...
				$.each(r.message, function(doctype, doctype_data) {
					...
					if(!in_list(pre_loaded, doctype)) {
						if(doctype_data.links) {
							$.each(dataent.report_dump.data[doctype], function(row_idx, row) {
								$.each(doctype_data.links, function(link_key, link) {
									if(dataent.report_dump.data[link[0]][row[link_key]]) {
										row[link_key] = dataent.report_dump.data[link[0]][row[link_key]][link[1]];
									} else {
										row[link_key] = null;
									}
								});
	...
	set_data: function(doctype, doctype_data) {
		...
		if(dataent.report_dump.last_modified[doctype]) {
			...
			// replace old data
			$.each(dataent.report_dump.data[doctype], function(i, d) {
		...
		dataent.report_dump.last_modified[doctype] = doctype_data.last_modified;
		dataent.report_dump.data[doctype] = data;
	}
});

dataent.provide("dataent.views");
dataent.views.GridReport = Class.extend({
	init: function(opts) {
		this.filter_inputs = {};
		this.preset_checks = [];
	...
	get_data: function(callback) {
		...
		dataent.report_dump.with_data(this.doctypes, function() {
	...
	setup_filters: function() {
		...
		$.each(me.filter_inputs, function(i, v) {
			...
			if(opts.fieldtype == "Select" && in_list(me.doctypes, opts.link)) {
				$(v).add_options(dataent.report_dump.data[opts.link].map(d => d.name));
			} else if(opts.fieldtype == "Link" && in_list(me.doctypes, opts.link)) {
				opts.list = dataent.report_dump.data[opts.link].map(d => d.name);
		...
		this.page.add_menu_item(__("Print"), function() {
			dataent.ui.get_print_settings(false, function(print_settings) {
				dataent.render_grid({grid: me.grid, title: me.page.title, print_settings: print_settings, report: me});
			});
	...
	set_filter: function(key, value) {
		...
		if(filters) {
			...
			if(opts.fieldtype === "Check") {
				...
				filters.val(dataent.datetime.str_to_user(value));
			} else {
				filters.val(value);
			}
		} else {
			dataent.msgprint(__("Invalid Filter: {0}", [key]));
		}
	},
	...
	init_filter_values: function() {
		$.each(this.filter_inputs, function(key, filter) {
			...
			if(dataent.sys_defaults[key]) {
				filter.val(dataent.sys_defaults[key]);
	...
	set_default_values: function() {
		var values = {
			from_date: dataent.datetime.str_to_user(dataent.sys_defaults.year_start_date),
			to_date: dataent.datetime.str_to_user(dataent.sys_defaults.year_end_date)
		};
	...
	make_waiting: function() {
		this.waiting = dataent.messages.waiting(this.wrapper, __("Loading Report")+"...");
	},
	load_filter_values: function() {
		...
		$.each(this.filter_inputs, function(i, f) {
			...
			if(opts.fieldtype=='Check') {
				...
				if(opts.fieldtype=="Date") {
					me[opts.fieldname] = dataent.datetime.user_to_str(me[opts.fieldname]);
				} else if (opts.fieldtype == "Select") {
		...
		if(this.filter_inputs.from_date && this.filter_inputs.to_date && (this.to_date < this.from_date)) {
			dataent.msgprint(__("From Date must be before To Date"));
			return;
		}
	...
	round_item_values: function(item) {
		...
		$.each(this.columns, function(i, col) {
			if (col.formatter==me.currency_formatter) {
				item[col.id] = flt(item[col.id], dataent.defaults.get_default("float_precision") || 3);
			}
		});
	},
	...
	make: function() {
		...
		this.id = dataent.dom.set_unique_id(this.grid_wrapper.get(0));

		this.bind_show();

		dataent.cur_grid_report = this;
	...
	apply_filters_from_route: function() {
		...
		if(dataent.route_options) {
			$.each(dataent.route_options, function(key, value) {
	...
	export: function() {
		dataent.tools.downloadify(dataent.slickgrid_tools.get_view_data(this.columns, this.dataView),
	...
	date_formatter: function(row, cell, value) {
		return dataent.datetime.str_to_user(value);
	},
		...
		$.each(this.columns, function(i, col) {
			if(col.link_formatter) {
				col.formatter = function(row, cell, value, columnDef, dataContext, for_print) {
					...
					var me = dataent.cur_grid_report;
					...
					if (link_formatter.filter_input) {
						html = repl('<a href="#" \
							onclick="dataent.cur_grid_report.set_filter(\'%(col_name)s\', \'%(value)s\'); \
								dataent.cur_grid_report.refresh(); return false;">\
							%(value)s</a>', {
								value: value,
								col_name: link_formatter.filter_input,
								page_name: dataent.container.page.page_name
							});
					} else {
						html = value;
					}
	...
	make_date_range_columns: function() {
		...
		this.from_date = dataent.datetime.user_to_str(this.filter_inputs.from_date.val());
		this.to_date = dataent.datetime.user_to_str(this.filter_inputs.to_date.val());
		var date_diff = dataent.datetime.get_diff(this.to_date, this.from_date);
		...
		var add_column = function(date) {
			me.columns.push({
				id: date,
				name: dataent.datetime.str_to_user(date),
				field: date,
		...
		var build_columns = function(condition) {
			...
			for(var i=0; i <= date_diff; i++) {
				var date = dataent.datetime.add_days(me.from_date, i);
		...
		if(range=='Daily') {
			...
			build_columns(function(date) {
				...
				return !(dataent.datetime.get_diff(date, me.from_date) % 7);
			});
		} else if(range=='Monthly') {
			build_columns(function(date) {
				if(!me.last_date) return true;
				return dataent.datetime.str_to_obj(me.last_date).getMonth() != dataent.datetime.str_to_obj(date).getMonth();
			});
		} else if(range=='Quarterly') {
			build_columns(function(date) {
				if(!me.last_date) return true;
				return dataent.datetime.str_to_obj(date).getDate()==1 && in_list([0,3,6,9], dataent.datetime.str_to_obj(date).getMonth());
			});
		} else if(range=='Yearly') {
			build_columns(function(date) {
				if(!me.last_date) return true;
				return $.map(dataent.report_dump.data['Fiscal Year'], function(v) {
		...
		$.each(this.columns, function(i, col) {
			col.name = me.columns[i+1]
				? dataent.datetime.str_to_user(dataent.datetime.add_days(me.columns[i+1].id, -1))
				: dataent.datetime.str_to_user(me.to_date);
		});
	},
...
dataent.views.GridReportWithPlot = dataent.views.GridReport.extend({
...
dataent.views.TreeGridReport = dataent.views.GridReportWithPlot.extend({
	make_transaction_list: function(parent_doctype, doctype) {
		var me = this;
		var tmap = {};
		$.each(dataent.report_dump.data[doctype], function(i, v) {
		...
		$.each(dataent.report_dump.data[parent_doctype], function(i, parent) {
			if(tmap[parent.name]) {
	...
	prepare_tree: function(item_dt, group_dt) {
		var group_data = dataent.report_dump.data[group_dt];
		var item_data = dataent.report_dump.data[item_dt];
		...
		$.each(item_data, function(i, item) {
			...
			if(group_ids.indexOf(item.name)==-1) {
				...
				dataent.msgprint(__("Ignoring Item {0}, because a group exists with the same name!", [item.name.bold()]));
			}
		});
	...
	export: function() {
		var msgbox =	dataent.msgprint($.format('<p>{0}</p>\
		...
		$(msgbox.body).find("button").click(function() {
			...
			var data = dataent.slickgrid_tools.get_view_data(me.columns, me.data,
				function(row, item) {
					if(with_groups) {
						// add row
			...
			dataent.tools.downloadify(data, ["Report Manager", "System Manager"], me.title);
			return false;
		});
