...
dataent.views.ReportFactory = dataent.views.Factory.extend({
	make: function(route) {
		new dataent.views.ReportViewPage(route[1], route[2]);
	}
});

dataent.views.ReportViewPage = Class.extend({
	init: function(doctype, docname) {
		if(!dataent.model.can_get_report(doctype)) {
			dataent.show_not_permitted(dataent.get_route_str());
			return;
		}
		...
		var me = this;
		dataent.model.with_doctype(this.doctype, function() {
			me.make_report_view();
			if(me.docname) {
				dataent.model.with_doc('Report', me.docname, function(r) {
					me.parent.reportview.set_columns_and_filters(
						JSON.parse(dataent.get_doc("Report", me.docname).json || '{}'));
	...
	make_report_view: function() {
		...
		dataent.breadcrumbs.add(module, this.doctype);

		this.parent.reportview = new dataent.views.ReportView({
...
dataent.views.ReportView = dataent.ui.BaseList.extend({
	init: function(opts) {
		var me = this;
		$.extend(this, opts);
		this.can_delete = dataent.model.can_delete(me.doctype);
	...
	setup: function() {
		...
		this.page = this.parent.page;
		this.meta = dataent.get_meta(this.doctype);
		...
		this.make({
			...
			method: 'dataent.desk.reportview.get',
		...
		this.page.add_menu_item(__("Add to Desktop"), function() {
			dataent.add_to_desktop(me.docname || __('{0} Report', [me.doctype]), me.doctype, me.docname);
		}, true);
	...
	setup_auto_email: function() {
		...
		this.page.add_menu_item(__("Setup Auto Email"), function() {
			if(me.docname) {
				dataent.set_route('List', 'Auto Email Report', {'report' : me.docname});
			} else {
				dataent.msgprint({message:__('Please save the report first'), indicator: 'red'});
			}
		}, true);
	},
	...
	set_init_columns: function() {
		...
		if(!columns.length) {
			...
			$.each(dataent.meta.docfield_list[this.doctype], function(i, df) {
				if((df.in_standard_filter || df.in_list_view) && df.fieldname!='naming_series'
					&& !in_list(dataent.model.no_value_type, df.fieldtype)
					&& !df.report_hide) {
	...
	set_columns_and_filters: function(opts) {
		...
		if(opts.filters) {
			$.each(opts.filters, function(i, f) {
				...
				var df = dataent.meta.get_docfield(f[0], f[1]);
	...
	setup_print: function() {
		...
		this.page.add_menu_item(__("Print"), function() {
			dataent.ui.get_print_settings(false, function(print_settings) {
				...
				dataent.render_grid({grid:me.grid, title:title, print_settings:print_settings});
			})
	...
	build_columns: function() {
		...
		return $.map(this.columns, function(c) {
			var docfield = dataent.meta.docfield_map[c[1] || me.doctype][c[0]];
			if(!docfield) {
				var docfield = dataent.model.get_std_field(c[0]);
				if(docfield) {
				...
				formatter: function(row, cell, value, columnDef, dataContext, for_print) {
					...
					if(docfield.fieldtype==="Link" && docfield.fieldname!=="name") {
						...
						docfield.link_onclick =
							repl('dataent.container.page.reportview.filter_or_open("%(parent)s", "%(fieldname)s", "%(value)s")',
								{parent: docfield.parent, fieldname:docfield.fieldname, value:value});
					}
					return dataent.format(value, docfield, {for_print: for_print, always_show_decimals: true}, dataContext);
				}
			}
			return coldef;
		});
	...
	filter_or_open: function(parent, fieldname, value) {
		...
		if(!filter_set) {
			...
			var df = dataent.meta.get_docfield(parent, fieldname);
			if(df.fieldtype==='Link') {
				dataent.set_route('Form', df.options, value);
			}
		}
	},
	...
	render_view: function() {
		...
		dataent.slickgrid_tools.add_property_setter_on_resize(this.grid);
	...
	edit_cell: function(row, docfield) {
		if(!docfield || docfield.fieldname !== "idx"
			&& dataent.model.std_fields_list.indexOf(docfield.fieldname)!==-1) {
			return;
		} else if(dataent.boot.user.can_write.indexOf(this.doctype)===-1) {
			dataent.throw({message:__("No permission to edit"), title:__('Not Permitted')});
		}
		var me = this;
	...
	update_value: function(docfield, dialog, row) {
		...
		if (!args.name) {
			dataent.throw(__("ID field is required to edit values using Report. Please select the ID field using the Column Picker"));
		}
				...
				if(!r.exc) {
					...
					$.each(me.dataView.getItems(), function(i, item) {
						if (item.name === doc.name) {
							...
							$.each(dataent.model.get_all_docs(doc), function(i, d) {
								...
								if(name===d.name) {
									for(var k in d) {
										var v = d[k];
										if(dataent.model.std_fields_list.indexOf(k)===-1
											&& item[k]!==undefined) {
											new_item[k] = v;
										}
									}
	...
	make_column_picker: function() {
		var me = this;
		this.column_picker = new dataent.ui.ColumnPicker(this);
	...
	make_sorter: function() {
		var me = this;
		this.sort_dialog = new dataent.ui.Dialog({title:__('Sorting Preferences')});
		...
		// first
		this.sort_by_select = new dataent.ui.FieldSelect({
		...
		// second
		this.sort_by_next_select = new dataent.ui.FieldSelect({
	...
	make_export: function() {
		...
		var export_btn = this.page.add_menu_item(__('Export'), function() {
			...
			dataent.prompt({fieldtype:"Select", label: __("Select File Type"), fieldname:"file_format_type",
	...
	save_report: function(save_type) {
			...
			return dataent.call({
				...
				callback: function(r) {
					...
					if(r.message != me.docname)
						dataent.set_route('Report', me.doctype, r.message);
				}
			});
		...
		if(me.docname && save_type == "save") {
			...
			dataent.prompt({fieldname: 'name', label: __('New Report name'), reqd: 1, fieldtype: 'Data'}, function(data) {
		...
		if(this.can_delete) {
			...
			this.page.add_menu_item(__("Delete"), function() {
				...
				if(!delete_list.length)
					return;
				if(dataent.confirm(__("This is PERMANENT action and you cannot undo. Continue?"),
					function() {
						return dataent.call({
	...
	make_user_permissions: function() {
		var me = this;
		if(this.docname && dataent.model.can_set_user_permissions("Report")) {
	...
	setup_listview_settings: function() {
		if(dataent.listview_settings[this.doctype] && dataent.listview_settings[this.doctype].onload) {
			dataent.listview_settings[this.doctype].onload(this);
		}
	},
...
dataent.ui.ColumnPicker = Class.extend({
	init: function(list) {
		this.list = list;
	...
	add_column: function(c) {
		...
		var fieldselect = new dataent.ui.FieldSelect({parent:w.find('.col-xs-10'), doctype:this.doctype});
