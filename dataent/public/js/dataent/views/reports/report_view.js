/**
 * dataent.views.ReportView
 */
import DataTable from 'dataent-datatable';

dataent.provide('dataent.views');

dataent.views.ReportView = class ReportView extends dataent.views.ListView {
	get view_name() {
		return 'Report';
	}
	...
	setup_sort_selector() {
		this.sort_selector = new dataent.ui.SortSelector({
	...
	before_refresh() {
		if (this.report_doc) {
			// don't parse dataent.route_options if this is a Custom Report
	...
	set_dirty_state_for_custom_report() {
		...
		if (!dataent.utils.deep_equal(current_settings, report_settings)) {
	...
	prepare_data(r) {
		let data = r.message || {};
		data = dataent.utils.dict(data.keys, data.values);
	...
	on_update(data) {
		if (this.doctype === data.doctype && data.name) {
			...
			const flash_row = data.user !== dataent.session.user;
			if (this.data.find(d => d.name === data.name)) {
				// update existing
				dataent.db.get_doc(data.doctype, data.name)
					...
					const d = new dataent.ui.Dialog({
						...
						primary_action: ({ column, insert_before }) => {
							if (!columns_in_picker.map(col => col.value).includes(column)) {
								dataent.show_alert(__('Invalid column'));
								d.hide();
								return;
							}
	...
	setup_charts() {
		this.get_chart_data()
			.then(args => {
				...
				const get_df = (field) => dataent.meta.get_docfield(this.doctype, field);
				...
				this.chart = new Chart(this.$charts_wrapper[0], {
					...
					format_tooltip_y:
						value => dataent.format(value, get_df(value.field), { always_show_decimals: true, inline: true }, get_doc(value.doc))
				});
	...
	get_chart_data() {
		return new Promise(resolve => {
			...
			const y_fields = this.meta.fields.filter(df =>
				!df.hidden && dataent.model.is_numeric_field(df)
	...
	get_chart_args(x_axis, y_axes, chart_type) {
		...
		return {
			...
			datasets: y_axes.map(y_axis => ({
				name: dataent.meta.get_docfield(this.doctype, y_axis).label,
	...
	set_control_value(doctype, docname, fieldname, value) {
		...
		return new Promise((resolve, reject) => {
			dataent.db.set_value(doctype, docname, {[fieldname]: value})
				.then(r => {
					if (r.message) {
						resolve(r.message);
					} else {
	...
	render_editing_input(colIndex, value, parent) {
		...
		if (col.docfield.fieldtype === 'Text Editor') {
			...
			// make control
			control = dataent.ui.form.make_control({
				df: col.docfield,
				parent: parent,
				render_input: true
			});
	...
	is_editable(df, data) {
		...
		const is_standard_field = dataent.model.std_fields_list.includes(df.fieldname);
		const can_edit = !(
			is_standard_field
			|| df.read_only
			|| df.hidden
			|| !dataent.model.can_write(this.doctype)
		);
		return can_edit;
	}
	...
	set_fields() {
		...
		const fields = this.meta.fields.filter(df => {
			return (df.in_list_view || df.in_standard_filter)
				&& dataent.perm.has_perm(this.doctype, df.permlevel, 'read')
				&& dataent.model.is_value_type(df.fieldtype)
				&& !df.report_hide;
		});
	...
	get_fields() {
		let fields = this.fields.map(f => {
			let column_name = dataent.model.get_full_column_name(f[0], f[1]);
			if (f[1] !== this.doctype) {
				// child table field
		...
		const cdt_name_fields =
			this.get_unique_cdt_in_view()
				.map(cdt => dataent.model.get_full_column_name('name', cdt) + ' as ' + `'${cdt}:name'`);
	...
	add_currency_column(fieldname, doctype, col_index) {
		...
		const df = dataent.meta.get_docfield(doctype, fieldname);
		if (df && df.fieldtype === 'Currency' && df.options &&
			!df.options.includes(':') && dataent.meta.has_field(doctype, df.options)
		) {
			...
			dataent.show_alert(__('Also adding the dependent currency field {0}', [field[0].bold()]));
		}
	}
	...
	remove_column_from_datatable(column) {
		...
		if (field[0] === 'name') {
			this.refresh();
			dataent.throw(__('Cannot remove ID field'));
		}
	...
	get_columns_for_picker() {
		...
		const standard_fields_filter = df =>
			!in_list(dataent.model.no_value_type, df.fieldtype) && !df.report_hide;

		let doctype_fields = dataent.meta.get_docfields(this.doctype).filter(standard_fields_filter);

		doctype_fields = [{
			label: __('ID'),
			fieldname: 'name',
			fieldtype: 'Data'
		}].concat(doctype_fields, dataent.model.std_fields);

		out[this.doctype] = doctype_fields;

		const table_fields = dataent.meta.get_table_fields(this.doctype)
			.filter(df => !df.hidden);

		table_fields.forEach(df => {
			const cdt = df.options;
			const child_table_fields = dataent.meta.get_docfields(cdt).filter(standard_fields_filter);
	...
	build_column(c) {
		...
		let docfield = dataent.meta.docfield_map[doctype || this.doctype][fieldname];

		if (!docfield) {
			docfield = dataent.model.get_std_field(fieldname);
		...
		const editable = dataent.model.is_non_std_field(fieldname) && !docfield.read_only;

		const align = (() => {
			const is_numeric = dataent.model.is_numeric_field(docfield);
			if (is_numeric) {
				return 'right';
			}
		...
		return {
			...
			format: (value, row, column, data) => {
				...
				return dataent.format(value, column.docfield, { always_show_decimals: true }, doc);
			}
		};
	}
		...
		if (this.add_totals_row) {
			...
			const totals_row = this.columns.map((col, i) => {
				return {
					...
					format: value => {
						return dataent.format(value, col.docfield, { always_show_decimals: true });
					}
				}
			})
	...
	save_report(save_type) {
		const _save_report = (name) => {
			...
			return dataent.call({
				method: 'dataent.desk.reportview.save_report',
				args: {
					name: name,
					doctype: this.doctype,
				...
				callback:(r) => {
					if(r.exc) {
						dataent.msgprint(__("Report was not saved (there were errors)"));
						return;
					}
					if(r.message != this.report_name) {
						// Rerender the reports dropdown,
						// so that this report is included in the dropdown as well.
						dataent.boot.user.all_reports[r.message] = {
							ref_doctype: "Item",
							report_type: "Report Builder",
							title: r.message,
						};
						this.list_sidebar.setup_reports();
						dataent.set_route('List', this.doctype, 'Report', r.message);
						return;
					}
		...
		if(this.report_name && save_type == "save") {
			...
			dataent.prompt({fieldname: 'name', label: __('New Report name'), reqd: 1, fieldtype: 'Data'}, (data) => {
	...
	get_report_doc() {
		return new Promise(resolve => {
			dataent.model.with_doc('Report', this.report_name, () => {
				resolve(dataent.get_doc('Report', this.report_name));
			});
		});
	}
	...
	get_filters_html_for_print() {
		...
		return filters.map(f => {
			...
			const label = dataent.meta.get_label(doctype, fieldname);
	...
	get_columns_totals(data) {
		...
		this.columns.forEach((col, i) => {
			const totals = data.reduce((totals, d) => {
				if (col.id in d && dataent.model.is_numeric_field(col.docfield)) {
					totals += flt(d[col.id]);
					return totals;
				}
			}, 0);
	...
	report_menu_items() {
		let items = [
			...
			},
			{
				...
				action: () => {
					...
					dataent.ui.get_print_settings(false, (print_settings) => {
		...
		if (dataent.model.can_export(this.doctype)) {
			items.push({
				label: __('Export'),
				action: () => {
					...
					const d = new dataent.ui.Dialog({
						...
						primary_action: (data) => {
							args.cmd = 'dataent.desk.reportview.export_query';
		...
		items.push({
			...
			action: () => {
				if(this.report_name) {
					dataent.set_route('List', 'Auto Email Report', {'report' : this.report_name});
				} else {
					dataent.msgprint(__('Please save the report first'));
				}
			}
		});

		// save buttons
		if(dataent.user.is_report_manager()) {
			items = items.concat([
		...
		// user permissions
		if(this.report_name && dataent.model.can_set_user_permissions("Report")) {
			items.push({
				...
				action: () => {
					...
					dataent.set_route('List', 'User Permission', args);
				}
			});
		}
