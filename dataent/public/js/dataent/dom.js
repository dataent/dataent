...
dataent.dom = {
	...
	get_unique_id: function() {
		const id = 'unique-' + dataent.dom.id_count;
		dataent.dom.id_count++;
		return id;
	},
	set_unique_id: function(ele) {
		...
		var id = 'unique-' + dataent.dom.id_count;
		$ele.attr('id', id);
		dataent.dom.id_count++;
		return id;
	},
	...
	add: function(parent, newtag, className, cs, innerHTML, onclick) {
		if(parent && parent.substr)parent = dataent.dom.by_id(parent);
	...
	freeze: function(msg, css_class) {
		...
		dataent.dom.freeze_count++;
	},
	unfreeze: function() {
		if(!dataent.dom.freeze_count) return; // anything open?
		dataent.dom.freeze_count--;
		if(!dataent.dom.freeze_count) {
...
dataent.ellipsis = function(text, max) {
	if(!max) max = 20;
	text = cstr(text);
	if(text.length > max) {
...
dataent.run_serially = function(tasks) {
...
dataent.load_image = (src, onload, onerror, preprocess = () => {}) => {
...
dataent.timeout = seconds => {
	return new Promise((resolve) => {
		setTimeout(() => resolve(), seconds * 1000);
	});
};

dataent.scrub = function(text) {
	return text.replace(/ /g, "_").toLowerCase();
};

dataent.get_modal = function(title, content) {
...
dataent.is_online = function() {
	if (dataent.boot.developer_mode == 1) {
