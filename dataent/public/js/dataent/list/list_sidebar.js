...
dataent.views.ListSidebar = class ListSidebar {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
		this.get_stats();
	...
	make() {
		var sidebar_content = dataent.render_template("list_sidebar", { doctype: this.doctype });
		...
		let limits = dataent.boot.limits;

		if (limits.upgrade_url && limits.expiry && !dataent.flags.upgrade_dismissed) {
	...
	setup_views() {
		...
		if (dataent.views.calendar[this.doctype]) {
		...
		if (this.doctype === "Communication" && dataent.boot.email_accounts.length) {
			this.sidebar.find('.list-link[data-view="Inbox"]').removeClass('hide');
			show_list_link = true;
		}

		if (dataent.treeview_settings[this.doctype]) {
			this.sidebar.find(".tree-link").removeClass("hide");
		}

		this.current_view = 'List';
		var route = dataent.get_route();
		if (route.length > 2 && dataent.views.view_modes.includes(route[2])) {
	...
	setup_reports() {
		...
		var reports = Object.values(dataent.boot.user.all_reports).sort((a,b) => a.title.localeCompare(b.title)) || [];
	...
	setup_kanban_boards() {
		...
		dataent.views.KanbanView.setup_dropdown_in_sidebar(this.doctype, $dropdown);
	}

	setup_calendar_view() {
		const doctype = this.doctype;

		dataent.db.get_list('Calendar View', {
			filters: {
	...
	setup_email_inbox() {
		...
		if (has_common(dataent.user_roles, ["System Manager", "Administrator"])) {
			$(`<li class="new-email-account"><a>${__("New Email Account")}</a></li>`)
				.appendTo($dropdown);
		}

		let accounts = dataent.boot.email_accounts;
		...
		$dropdown.find('.new-email-account').click(function() {
			dataent.new_doc("Email Account");
		});
	}

	setup_assigned_to_me() {
		this.page.sidebar.find(".assigned-to-me a").on("click", () => {
			this.list_view.filter_area.add(this.list_view.doctype, "_assign", "like", `%${dataent.session.user}%`);
		});
	}
	...
	setup_upgrade_box() {
		...
		let upgrade_date = dataent.datetime.add_days(dataent.datetime.get_today(), 30);
		if (dataent.boot.limits.users === 1 || upgrade_date >= dataent.boot.limits.expiry) {
			...
			upgrade_box.find('.btn-upgrade').on('click', () => {
				dataent.set_route('usage-info');
			});

			upgrade_box.find('.close').on('click', () => {
				upgrade_list.remove();
				dataent.flags.upgrade_dismissed = 1;
			});
		}
	}
	...
	get_stats() {
		var me = this;
		dataent.call({
			method: 'dataent.desk.reportview.get_sidebar_stats',
			args: {
				stats: me.stats,
				doctype: me.doctype,
	...
	render_stat(field, stat, tags) {
		...
		var sum = 0;
		var stats = [];
		var label = dataent.meta.docfield_map[this.doctype][field] ?
			dataent.meta.docfield_map[this.doctype][field].label : field;
		...
		$(dataent.render_template("list_sidebar_stat", context))
