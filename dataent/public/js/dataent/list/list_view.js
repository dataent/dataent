...
dataent.views.ListView = class ListView extends dataent.views.BaseList {
	static load_last_view() {
		...
		if (route.length === 2) {
			...
			const user_settings = dataent.get_user_settings(doctype);
			const last_view = user_settings.last_view;
			dataent.set_route('List', doctype, dataent.views.is_valid(last_view) ? last_view : 'List');
			return true;
		}
		return false;
	}
	...
	has_permissions() {
		const can_read = dataent.perm.has_perm(this.doctype, 0, 'read');
		return can_read;
	}
	...
	show() {
		if (!this.has_permissions()) {
			dataent.set_route('');
			dataent.msgprint(__(`Not permitted to view ${this.doctype}`));
			return;
		}
	...
	show_restricted_list_indicator_if_applicable() {
		const match_rules_list = dataent.perm.get_match_rules(this.doctype);
	...
	show_restrictions(match_rules_list=[]) {
		dataent.msgprint(dataent.render_template('list_view_permission_restrictions', {
	...
	set_fields() {
		...
		this.fields.forEach(f => {
			const df = dataent.meta.get_docfield(f[1], f[0]);
	...
	patch_refresh_and_load_lib() {
		...
		this.refresh = dataent.utils.throttle(this.refresh, 1000);
		this.load_lib = new Promise(resolve => {
			if (this.required_libs) {
				dataent.require(this.required_libs, resolve);
			} else {
				resolve();
			}
		});
		...
		setInterval(() => {
			...
			if (dataent.get_route_str() === this.page_name) {
				this.refresh();
			}
		}, interval);
	}
	...
	make_new_doc() {
		...
		this.filter_area.get().forEach(f => {
			if (f[2] === "=" && dataent.model.is_non_std_field(f[1])) {
				options[f[1]] = f[3];
			}
		});
		dataent.new_doc(doctype, options);
	}
	...
	setup_columns() {
		...
		const get_df = dataent.meta.get_docfield.bind(null, this.doctype);
		...
		if (dataent.has_indicator(this.doctype)) {
			// indicator
			this.columns.push({
				type: 'Status'
			});
		...
		this.columns = this.columns.concat(
			fields_in_list_view
				.filter(df => {
					if (dataent.has_indicator(this.doctype) && df.fieldname === 'status') {
						return false;
					}
					if (!df.in_list_view) {
						return false;
	...
	before_render() {
		...
		dataent.model.user_settings.save(this.doctype, 'last_view', this.view_name);
	...
	render_tags() {
		...
		this.data.forEach((d, i) => {
			...
			// add tags
			let tag_editor = new dataent.ui.TagEditor({
	...
	get_header_html() {
		...
		const $columns = this.columns.map(col => {
			let classes = [
				...
				dataent.model.is_numeric_field(col.df) ? 'text-right' : ''
			].join(' ');
	...
	get_column_html(col, doc) {
		...
		const format = () => {
			if (formatters && formatters[fieldname]) {
				...
				return dataent.format(value, df, null, doc);
			}
		};

		const field_html = () => {
			let html;
			const _value = typeof value === 'string' ? dataent.utils.escape_html(value) : value;
			...
			if (df.fieldtype === 'Image') {
				...
				html = `<span class="filterable indicator ${dataent.utils.guess_colour(_value)} ellipsis"
		...
		const css_class = [
			'list-row-col ellipsis',
			class_map[col.type],
			dataent.model.is_numeric_field(df) ? 'text-right' : ''
		].join(' ');
	...
	get_count_str() {
		...
		return dataent.call({
			...
			args: {
				...
				fields: [`count(${dataent.model.get_full_column_name('name', this.doctype)}) as total_count`],
			}
		}).then(r => {
	...
	get_subject_html(doc) {
		let user = dataent.session.user;
		let subject_field = this.columns[0].df;
		let value = doc[subject_field.fieldname] || doc.name;
		let subject = strip_html(value);
		let escaped_subject = dataent.utils.escape_html(subject);
	...
	get_indicator_html(doc) {
		const indicator = dataent.get_indicator(doc, this.doctype);
		if (indicator) {
	...
	setup_filterable() {
		...
		this.$result.on('click', '.filterable', e => {
			...
			const filters_to_apply = filters.map(f => {
				f = f.split(',');
				if (f[2] === 'Today') {
					f[2] = dataent.datetime.get_today();
				} else if (f[2] == 'User') {
					f[2] = dataent.session.user;
				}
	...
	setup_like() {
		this.$result.on('click', '.like-action', dataent.ui.click_toggle_like);
		this.$result.on('click', '.list-liked-by-me', e => {
			const $this = $(e.currentTarget);
			$this.toggleClass('active');

			if ($this.hasClass('active')) {
				this.filter_area.add(this.doctype, '_liked_by', 'like', '%' + dataent.session.user + '%');
			} else {
				this.filter_area.remove('_liked_by');
			}
		});

		dataent.ui.setup_like_popover(this.$result, '.liked-by');
	}
	...
	setup_realtime_updates() {
		dataent.realtime.on('list_update', data => {
			...
			dataent.call(call_args)
				.then(({ message }) => {
					if (!message) return;
					const data = dataent.utils.dict(message.keys, message.values);
					if (!(data && data.length)) {
	...
	save_view_user_settings(obj) {
		return dataent.model.user_settings.save(this.doctype, this.view_name, obj);
	}
	...
	get_menu_items() {
		...
		if (dataent.model.can_import(doctype)) {
			items.push({
				label: __('Import'),
				action: () => dataent.set_route('List', 'Data Import', {
		...
		if (dataent.model.can_set_user_permissions(doctype)) {
			items.push({
		...
		if (dataent.user_roles.includes('System Manager')) {
			items.push({
				...
				action: () => dataent.set_route('permission-manager', {
					doctype
				}),
				standard: true
			});
		...
		items.push({
			...
			action: () => dataent.add_to_desktop(doctype, doctype),
			standard: true
		});

		if (dataent.user.has_role('System Manager') && dataent.boot.developer_mode === 1) {
			// edit doctype
			items.push({
				label: __('Edit DocType'),
				action: () => dataent.set_route('Form', 'DocType', doctype),
				standard: true
			});
		}
	...
	get_actions_menu_items() {
		...
		const is_field_editable = (field_doc) => {
			return field_doc.fieldname && dataent.model.is_value_type(field_doc)
				&& field_doc.fieldtype !== 'Read Only' && !field_doc.hidden && !field_doc.read_only;
		};

		const has_editable_fields = (doctype) => {
			return dataent.meta.get_docfields(doctype).some(field_doc => is_field_editable(field_doc));
		};

		const has_submit_permission = (doctype) => {
			return dataent.perm.has_perm(doctype, 0, 'submit');
		};
		...
		const bulk_delete = () => {
			return {
				...
				action: () => {
					...
					dataent.confirm(__('Delete {0} items permanently?', [docnames.length]),
			...
			return {
				...
				action: () => {
					...
					if (docnames.length > 0) {
						dataent.confirm(__('Cancel {0} documents?', [docnames.length]),
		...
		const bulk_submit = () => {
			return {
				...
				action: () => {
					const docnames = this.get_checked_items(true);
					if (docnames.length > 0) {
						dataent.confirm(__('Submit {0} documents?', [docnames.length]),
		...
		const bulk_edit = () => {
			return {
				...
				action: () => {
					...
					dataent.meta.get_docfields(doctype).forEach(field_doc => {
		...
		// bulk printing
		if (dataent.model.can_print(doctype)) {
			actions_menu_items.push(bulk_printing());
		}

		// Bulk submit
		if (dataent.model.is_submittable(doctype) && has_submit_permission(doctype)) {
			actions_menu_items.push(bulk_submit());
		}

		// Bulk cancel
		if (dataent.model.can_cancel(doctype)) {
			actions_menu_items.push(bulk_cancel());
		}

		// bulk delete
		if (dataent.model.can_delete(doctype)) {
	...
	parse_filters_from_route_options() {
		...
		for (let field in dataent.route_options) {

			let doctype = null;
			let value = dataent.route_options[field];
			...
			if (!doctype) {
				doctype = dataent.meta.get_doctype_for_field(this.doctype, field);
			}
	...
	static trigger_list_update(data) {
		...
		// refresh tree view
		if (dataent.views.trees[doctype]) {
			dataent.views.trees[doctype].tree.refresh();
			return;
		}

		// refresh list view
		const page_name = dataent.get_route_str();
		const list_view = dataent.views.list_view[page_name];
		list_view && list_view.on_update(data);
	}
};

$(document).on('save', (event, doc) => {
	dataent.views.ListView.trigger_list_update(doc);
});
