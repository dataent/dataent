...
dataent.views.BaseList = class BaseList {
	constructor(opts) {
	...
	setup_defaults() {
		...
		this.user_settings = dataent.get_user_settings(this.doctype);

		this.start = 0;
		this.page_length = 20;
		this.data = [];
		this.method = 'dataent.desk.reportview.get';

		this.can_create = dataent.model.can_create(this.doctype);
	...
	get_fields_in_list_view() {
		return this.meta.fields.filter(df => {
			return dataent.model.is_value_type(df.fieldtype) && (
				df.in_list_view
	...
	_add_field(fieldname) {
		...
		const is_valid_field = dataent.model.std_fields_list.includes(fieldname)
			|| dataent.meta.has_field(doctype, fieldname)
			|| fieldname === '_seen';
	...
	set_breadcrumbs() {
		dataent.breadcrumbs.add(this.meta.module, this.doctype);
	}

	setup_side_bar() {
		this.list_sidebar = new dataent.views.ListSidebar({
	...
	setup_list_wrapper() {
		this.$dataent_list = $('<div class="dataent-list">').appendTo(this.page.main);
	}
	...
	setup_result_area() {
		...
		this.$dataent_list.append(this.$result);
	}
	...
	setup_no_result_area() {
		...
		`).hide();
		this.$dataent_list.append(this.$no_result);
	}

	setup_freeze_area() {
		this.$freeze = $('<div class="freeze"></div>').hide();
		this.$dataent_list.append(this.$freeze);
	}
	...
	setup_paging_area() {
		...
		this.$dataent_list.append(this.$paging_area);
	...
	get_fields() {
		...
		return this.fields.map(f => dataent.model.get_full_column_name(f[0], f[1]));
	}
	...
	refresh() {
		...
		return dataent.call(this.get_call_args()).then(r => {
			// render
			this.prepare_data(r);
	...
	prepare_data(r) {
		let data = r.message || {};
		data = !Array.isArray(data) ? dataent.utils.dict(data.keys, data.values) : data;
...
class FilterArea {
	constructor(list_view) {
		...
		this.$filter_list_wrapper = $('<div class="filter-list">').appendTo(this.list_view.$dataent_list);
	...
	make_filter_list() {
		this.filter_list = new dataent.ui.FilterGroup({
...
dataent.views.view_modes = ['List', 'Gantt', 'Kanban', 'Calendar', 'Image', 'Inbox', 'Report'];
dataent.views.is_valid = view_mode => dataent.views.view_modes.includes(view_mode);
