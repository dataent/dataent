...
export default class ListFilter {
	constructor({ wrapper, doctype }) {
		...
		this.can_add_global = dataent.user.has_role(['System Manager', 'Administrator']);
		this.filters = [];
		this.make();
		this.bind();
		this.refresh();
	...
	make() {
		...
		this.filter_input = dataent.ui.form.make_control({
			df: {
				fieldtype: 'Data',
		...
		this.is_global_input = dataent.ui.form.make_control({
			df: {
				fieldtype: 'Check',
				label: __('Is Global')
			},
	...
	bind_save_filter() {
		this.filter_input.$input.keydown(dataent.utils.debounce(e => {
	...
	save_filter(filter_name) {
		return dataent.db.insert({
			doctype: 'List Filter',
			reference_doctype: this.list_view.doctype,
			filter_name,
			for_user: this.is_global_input.get_value() ? '' : dataent.session.user,
	...
	remove_filter(name) {
		if (!name) return;
		return dataent.db.delete_doc('List Filter', name);
	}
