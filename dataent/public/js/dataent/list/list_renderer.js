...
dataent.views.ListRenderer = Class.extend({
	name: 'List',
	init: function (opts) {
		$.extend(this, opts);
	...
	setup_cache: function () {
		dataent.provide('dataent.views.list_renderers.' + this.doctype);
		dataent.views.list_renderers[this.doctype][this.list_view.current_view] = this;
	},
	init_settings: function () {
		this.settings = dataent.listview_settings[this.doctype] || {};
		...
		if(dataent.model.is_submittable(this.doctype) && (!this.filters || !this.filters.length)) {
			this.filters = [[this.doctype, "docstatus", "!=", 2]];
		}
	},
	init_user_settings: function () {
		dataent.provide('dataent.model.user_settings.' + this.doctype + '.' + this.name);
		this.user_settings = dataent.get_user_settings(this.doctype)[this.name];
	},
	...
	load_last_view: function() {
		...
		// load it using dataent.set_route and return true
		// else return false
		return false;
	},
	...
	set_fields: function () {
		...
		// enabled / disabled
		if (dataent.meta.has_field(this.doctype, 'enabled')) { add_field('enabled'); }
		if (dataent.meta.has_field(this.doctype, 'disabled')) { add_field('disabled'); }

		// add workflow field (as priority)
		this.workflow_state_fieldname = dataent.workflow.get_state_fieldname(this.doctype);
		if (this.workflow_state_fieldname) {
			if (!dataent.workflow.workflows[this.doctype]['override_status']) {
		...
		this.meta.fields.forEach(function (df, i) {
			if (df.in_list_view && dataent.perm.has_perm(me.doctype, df.permlevel, 'read')) {
	...
	set_columns: function () {
		...
		if (this.meta.title_field) {
			name_column.title = dataent.meta.get_docfield(this.doctype, this.meta.title_field).label;
		}
		...
		var docfields_in_list_view =
			dataent.get_children('DocType', this.doctype, 'fields', { 'in_list_view': 1 })
				.sort(function (a, b) {
					return a.idx - b.idx
				});
		...
		if (this.settings.add_columns) {
			this.settings.add_columns.forEach(function (d) {
				if (me.total_colspans < 12) {
					if (typeof d === 'string') {
						me.add_column(dataent.meta.get_docfield(me.doctype, d));
					} else {
						me.columns.push(d);
	...
	get_item_html: function (data) {
		var main = this.columns.map(column =>
			dataent.render_template('list_item_main', {
				data: data,
				col: column,
		...
		return dataent.render_template('list_item_row', {
			data: data,
			main: main,
	...
	get_header_html: function () {
		var main = this.columns.map(column =>
			dataent.render_template('list_item_main_head', {
				col: column,
				_checkbox: ((dataent.model.can_delete(this.doctype) || this.settings.selectable)
					&& !this.no_delete)
			})
		).join("");

		return dataent.render_template('list_item_row_head', { main: main, list: this });
	},
	...
	render_tags: function (element, data) {
		...
		// add tags
		var tag_editor = new dataent.ui.TagEditor({
	...
	get_subject_html: function (data, without_workflow) {
		...
		return dataent.render_template('list_item_subject', data);
	},

	get_indicator_html: function (doc) {
		var indicator = dataent.get_indicator(doc, this.doctype);
		if (indicator) {
	...
	prepare_data: function (data) {
		...
		data._checkbox = (dataent.model.can_delete(this.doctype) || this.settings.selectable) && !this.no_delete
		...
		data._submittable = dataent.model.is_submittable(this.doctype);
		...
		if (this.workflow_state_fieldname) {
			data._workflow = {
				...
				style: dataent.utils.guess_style(data[this.workflow_state_fieldname])
			}
		}

		data._user = dataent.session.user;
	...
	prepare_when: function (data, date_str) {
		...
		// when
		data.when = (dataent.datetime.str_to_user(date_str)).split(' ')[0];
		var diff = dataent.datetime.get_diff(dataent.datetime.get_today(), date_str.split(' ')[0]);
		if (diff === 0) {
	...
	load_lib: function (callback) {
		dataent.require(this.required_libs, callback);
	},
	...
	make_no_result: function () {
		var new_button = dataent.boot.user.can_create.includes(this.doctype)
