dataent.RoleEditor = Class.extend({
	init: function(wrapper, frm, disable) {
		...
		dataent.call({
			method: 'dataent.core.doctype.user.user.get_all_roles',
			callback: function(r) {
				me.roles = r.message;
				me.show_roles();
	...
	set_roles_in_table: function() {
		...
		$.each(opts.unchecked_roles, function(i, role) {
			if(existing_roles_list.indexOf(role)!=-1) {
				dataent.model.clear_doc("Has Role", existing_roles_map[role]);
			}
		});
		...
		$.each(opts.checked_roles, function(i, role) {
			if(existing_roles_list.indexOf(role)==-1) {
				var user_role = dataent.model.add_child(me.frm.doc, "Has Role", "roles");
				user_role.role = role;
			}
		});
	...
	show_permissions: function(role) {
		...
		return dataent.call({
			method: 'dataent.core.doctype.user.user.get_perm_info',
			args: {role: role},
			callback: function(r) {
	...
	make_perm_dialog: function() {
		this.perm_dialog = new dataent.ui.Dialog({
