...
if (!window.dataent) window.dataent = {};
...
window.format_number = function (v, format, decimals) {
	if (!format) {
		...
		if (decimals == null) decimals = cint(dataent.defaults.get_default("float_precision")) || 3;
	}
...
function format_currency(v, currency, decimals) {
	...
	if(decimals === undefined) {
		decimals = dataent.boot.sysdefaults.currency_precision || null;
	}
...
function get_currency_symbol(currency) {
	if (dataent.boot) {
		if (dataent.boot.sysdefaults.hide_currency_symbol == "Yes")
			return null;

		if (!currency)
			currency = dataent.boot.sysdefaults.currency;

		return dataent.model.get_value(":Currency", currency, "symbol") || currency;
	} else {
		// load in template
		return dataent.currency_symbols[currency];
	}
}

function get_number_format(currency) {
	return (dataent.boot && dataent.boot.sysdefaults && dataent.boot.sysdefaults.number_format) || "#,###.##";
}

function get_number_format_info(format) {
	var info = dataent.number_format_info[format];
...
function precision(fieldname, doc) {
	if (cur_frm) {
		...
		var df = dataent.meta.get_docfield(doc.doctype, fieldname, doc.parent || doc.name);
		if (!df) console.log(fieldname + ": could not find docfield in method precision()");
		return dataent.meta.get_field_precision(df, doc);
	} else {
		return dataent.boot.sysdefaults.float_precision
	}
}
...
function round_based_on_smallest_currency_fraction(value, currency, precision) {
	var smallest_currency_fraction_value = flt(dataent.model.get_value(":Currency",
