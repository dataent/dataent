...
Object.assign(dataent.utils, {
	...
	get_file_link: function(filename) {
		...
		if(dataent.utils.is_url(filename)) {
			return filename;
	...
	comma_or: function(list) {
		return dataent.utils.comma_sep(list, " " + __("or") + " ");
	},
	comma_and: function(list) {
		return dataent.utils.comma_sep(list, " " + __("and") + " ");
	},
	...
	guess_colour: function(text) {
		return dataent.utils.guess_style(text, null, true);
	},
	...
	arrays_equal: function(arr1, arr2) {
		...
		for (var i = 0; i < arr1.length; i++) {
			if ($.isArray(arr1[i])) {
				if (!dataent.utils.arrays_equal(arr1[i], arr2[i])) {
					return false;
				}
			}
	...
	warn_page_name_change: function(frm) {
		dataent.msgprint(__("Note: Changing the Page Name will break previous URL to this page."));
	},
	...
	set_title: function(title) {
		dataent._original_title = title;
		if(dataent._title_prefix) {
			title = dataent._title_prefix + " " + title.replace(/<[^>]*>/g, "");
		}
		document.title = title;
	},

	set_title_prefix: function(prefix) {
		dataent._title_prefix = prefix;

		// reset the original title
		dataent.utils.set_title(dataent._original_title);
	},
	...
	play_sound: function(name) {
		try {
			if (dataent.boot.user.mute_sounds) {
				return;
			}
	...
	report_column_total: function(values, column, type) {
		if (values.length > 0) {
			...
			} else if (dataent.model.is_numeric_field(column.column.fieldtype)) {
	...
	is_rtl() {
		return ["ar", "he", "fa"].includes(dataent.boot.lang);
	}
});
