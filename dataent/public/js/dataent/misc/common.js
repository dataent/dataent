...
dataent.avatar = function(user, css_class, title) {
	if(user) {
		// desk
		var user_info = dataent.user_info(user);
	} else {
		// website
		user_info = {
			image: dataent.get_cookie("user_image"),
			fullname: dataent.get_cookie("full_name"),
			abbr: dataent.get_abbr(dataent.get_cookie("full_name")),
			color: dataent.get_palette(dataent.get_cookie("full_name"))
		}
	}
	...
	if(user_info.image) {
		...
		var image = (window.cordova && user_info.image.indexOf('http')===-1) ?
			dataent.base_url + user_info.image : user_info.image;
...
dataent.ui.scroll = function(element, animate, additional_offset) {
...
dataent.get_palette = function(txt) {
	return '#fafbfc';
	// //return '#8D99A6';
	...
	// if(idx > 47) idx = 47;
	// return dataent.palette[idx][0]
}

dataent.get_abbr = function(txt, max_length) {
	if (!txt) return "";
	var abbr = "";
...
dataent.gravatars = {};
dataent.get_gravatar = function(email_id, size = 0) {
	var param = size ? ('s=' + size) : 'd=retro';
	if(!dataent.gravatars[email_id]) {
		// TODO: check if gravatar exists
		dataent.gravatars[email_id] = "https://secure.gravatar.com/avatar/" + md5(email_id) + "?" + param;
	}
	return dataent.gravatars[email_id];
}
...
dataent.get_cookie = function getCookie(name) {
	return dataent.get_cookies()[name];
}

dataent.get_cookies = function getCookies() {
...
dataent.is_mobile = function() {
	return $(document).width() < 768;
}

dataent.utils.xss_sanitise = function (string, options) {
