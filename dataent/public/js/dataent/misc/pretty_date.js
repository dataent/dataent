...
function prettyDate(time, mini) {
	...
	if ('moment' in window) { // use dataent.ImportError ;)
		let ret;
		if (dataent.sys_defaults && dataent.sys_defaults.time_zone) {
			ret = moment.tz(time, dataent.sys_defaults.time_zone).locale(dataent.boot.lang).fromNow(mini);
		} else {
			ret = moment(time).locale(dataent.boot.lang).fromNow(mini);
		}
		if (mini) {
			if (ret === moment().locale(dataent.boot.lang).fromNow(mini)) {
				ret = __("now");
			} else {
				var parts = ret.split(" ");
				if (parts.length > 1) {
...
window.comment_when = function(datetime, mini) {
	var timestamp = dataent.datetime.str_to_user ?
		dataent.datetime.str_to_user(datetime) : datetime;
	return '<span class="dataent-timestamp '
		+ (mini ? " mini" : "") + '" data-timestamp="' + datetime
		+ '" title="' + timestamp + '">'
		+ prettyDate(datetime, mini) + '</span>';
};
dataent.datetime.comment_when = comment_when;

dataent.datetime.refresh_when = function() {
	if (jQuery) {
		$(".dataent-timestamp").each(function() {
