dataent.ui.FilterGroup = class {
	...
	validate_args(doctype, fieldname) {
		if(doctype && fieldname
			...
			&& !dataent.model.std_fields_list.includes(fieldname)) {

			dataent.throw(__(`Invalid filter: "${[fieldname.bold()]}"`));
			return false;
		}
		return true;
	}
	...
	_push_new_filter(doctype, fieldname, condition, value, hidden = false) {
		...
		let filter = new dataent.ui.Filter(args);
	...
	filter_exists(filter_value) {
		...
		this.filters.filter(f => f.field).map(f => {
			...
			let equal = dataent.utils.arrays_equal;
