...
dataent.ui.FilterList = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
		this.filters = [];
	...
	add_filter: function(doctype, fieldname, condition, value, hidden) {
		...
		if(doctype && fieldname
			...
			&& !in_list(dataent.model.std_fields_list, fieldname)) {
			dataent.msgprint({
	...
	push_new_filter: function(doctype, fieldname, condition, value) {
		...
		var filter = new dataent.ui.Filter({
			flist: this,
			_doctype: doctype,
			fieldname: fieldname,
			condition: condition,
		...
		for(var i in this.filters) {
			if(this.filters[i].field) {
				...
				if(f[0]==doctype && f[1]==fieldname && f[2]==condition && f[3]==value) {
					flag = true;
				} else if($.isArray(value) && dataent.utils.arrays_equal(value, f[3])) {
					flag = true;
				}
			}
		}
	...
	get_formatted_value: function(field, val){
		...
		value = dataent.format(value, field.df, {only_value: 1});
		return value;
	}
});

dataent.ui.Filter = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
	...
	make: function() {
		this.wrapper = $(dataent.render_template("edit_filter", {}))
			.appendTo(this.flist.wrapper.find('.filter_area'));
	},
	make_select: function() {
		var me = this;
		this.fieldselect = new dataent.ui.FieldSelect({
	...
	set_field: function(doctype, fieldname, fieldtype, condition) {
		...
		if(!original_docfield) {
			dataent.msgprint(__("Field {0} is not selectable.", [fieldname]));
			return;
		}
		...
		var f = dataent.ui.form.make_control({
			df: df,
			parent: field_area,
			only_input: true,
		})
...
dataent.ui.FieldSelect = Class.extend({
	...
	build_options: function() {
		var me = this;
		me.table_fields = [];
		var std_filters = $.map(dataent.model.std_fields, function(d) {
		...
		// main table
		var main_table_fields = std_filters.concat(dataent.meta.docfield_list[me.doctype]);
		$.each(dataent.utils.sort(main_table_fields, "label", "string"), function(i, df) {
			// show fields where user has read access and if report hide flag is not set
			if(dataent.perm.has_perm(me.doctype, df.permlevel, "read") && !df.report_hide)
		...
		$.each(me.table_fields, function(i, table_df) {
			if(table_df.options) {
				var child_table_fields = [].concat(dataent.meta.docfield_list[table_df.options]);
				$.each(dataent.utils.sort(child_table_fields, "label", "string"), function(i, df) {
					// show fields where user has read access and if report hide flag is not set
					if(dataent.perm.has_perm(me.doctype, df.permlevel, "read") && !df.report_hide)
						me.add_field_option(df);
				});
	...
	add_field_option: function(df) {
		...
		if(dataent.model.no_value_type.indexOf(df.fieldtype) == -1 &&
