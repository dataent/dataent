dataent.ui.Filter = class {
	constructor(opts) {
		...
		this.utils = dataent.ui.filter_utils;
		this.conditions = [
			["=", __("Equals")],
			["!=", __("Not Equals")],
			["like", __("Like")],
	...
	make() {
		this.filter_edit_area = $(dataent.render_template("edit_filter", {
	...
	make_field(df, old_fieldtype) {
		...
		let f = dataent.ui.form.make_control({
			df: df,
			parent: field_area,
			only_input: true,
		});
	...
	toggle_nested_set_conditions(df) {
		let show_condition = df.fieldtype === "Link" && dataent.boot.nested_set_doctypes.includes(df.options);
...
dataent.ui.filter_utils = {
	get_formatted_value(field, value) {
		...
		return dataent.format(value, field.df, {only_value: 1});
	},
