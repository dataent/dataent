...
dataent.ui.color = {
	get: function(color_name, shade) {
		...
		return dataent.ui.color_map;
	},
	get_color: function(color_name) {
		const color_names = Object.keys(dataent.ui.color_map);
		if(color_names.includes(color_name)) {
			return dataent.ui.color_map[color_name];
		} else {
	...
	get_color_shade: function(color_name, shade) {
		...
		if(Object.keys(shades).includes(shade)) {
			return dataent.ui.color_map[color_name][shades[shade]];
		} else {
			// eslint-disable-next-line
	...
	all: function() {
		return Object.values(dataent.ui.color_map)
			.reduce((acc, curr) => acc.concat(curr) , []);
	},
	names: function() {
		return Object.keys(dataent.ui.color_map);
	},
	...
	get_color_name: function(hex) {
		for (const key in dataent.ui.color_map) {
			const colors = dataent.ui.color_map[key];
