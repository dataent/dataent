...
dataent.ui.Tags = class {
	...
	bind() {
		this.$input.keypress((e) => {
			if(e.which == 13 || e.keyCode == 13) {
				const tagValue = dataent.utils.xss_sanitise(this.$input.val());
	...
	removeTag(label) {
		label = dataent.utils.xss_sanitise(label);
	...
	clearTags() {
		this.$ul.find('.dataent-tag').remove();
		this.tagsList = [];
	}
	...
	getTag(label) {
		let $tag = $(`<div class="dataent-tag btn-group" data-tag-label="${label}">
