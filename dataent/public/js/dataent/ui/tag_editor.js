dataent.ui.TagEditor = Class.extend({
	init: function(opts) {
		/* docs:
		Arguments
	...
	setup_tags: function() {
		...
		this.tags = new dataent.ui.Tags({
			parent: this.wrapper,
			placeholder: __("Add a tag ..."),
			onTagAdd: (tag) => {
				if(me.initialized && !me.refreshing) {
					return dataent.call({
						method: 'dataent.desk.tags.add_tag',
			...
			onTagRemove: (tag) => {
				if(!me.refreshing) {
					return dataent.call({
						method: 'dataent.desk.tags.remove_tag',
						args: me.get_args(tag),
						callback: function(r) {
	...
	setup_awesomplete: function() {
		...
		$input.on("input", function(e) {
			...
			dataent.call({
				method:"dataent.desk.tags.get_tags",
				args:{
