...
dataent.ui.BaseList = Class.extend({
	init: function (opts) {
		this.opts = opts || {};
		this.set_defaults();
		if (opts) {
	...
	make: function (opts) {
		...
		// make dom
		this.wrapper = $(dataent.render_template('listing', this.opts));
	...
	prepare_opts: function () {
		if (this.opts.new_doctype) {
			if (!dataent.boot.user.can_create.includes(this.opts.new_doctype)) {
			...
			if (me.custom_new_doc) {
				...
				if (me.filter_list) {
					...
					me.filter_list.get_filters().forEach(function (f, i) {
						if (f[2] === "=" && !dataent.model.std_fields_list.includes(f[1])) {
							dataent.route_options[f[1]] = f[3];
						}
					});
				}
				dataent.new_doc(doctype, true);
			}
		});
	},
	...
	make_filters: function () {
		...
		this.filter_list = new dataent.ui.FilterList({
			base_list: this,
		...
		if (dataent.model.is_submittable(this.doctype)) {
	...
	make_standard_filters: function() {
		...
		if (this.meta) {
			...
			this.meta.fields.forEach(function(df, i) {
				if(df.in_standard_filter && !dataent.model.no_value_type.includes(df.fieldtype)) {
	...
	set_filters_from_route_options: function ({clear_filters=true} = {}) {
		...
		for(var field in dataent.route_options) {
			var value = dataent.route_options[field];
			var doctype = null;
			...
			if (!doctype) {
				doctype = dataent.meta.get_doctype_for_field(me.doctype, field);
			}
	...
	_run: function (more) {
		...
		$.extend(args, {
			user_settings: dataent.model.user_settings[this.doctype]
		});

		return dataent.call({
			method: this.opts.method || 'dataent.desk.query_builder.runquery',
	...
	save_user_settings_locally: function (args) {
		if (this.opts.save_user_settings && this.doctype && !this.docname) {
			...
			var user_settings = dataent.model.user_settings[this.doctype];
			var different = false;
			...
			if (!dataent.utils.arrays_equal(args.filters, user_settings.filters)) {
	...
	get_values_from_response: function (data) {
		...
		if (data.keys && $.isArray(data.keys)) {
			return dataent.utils.dict(data.keys, data.values);
		} else {
			return data;
		}
	},
	...
	init_user_settings: function () {
		this.user_settings = dataent.model.user_settings[this.doctype] || {};
	},
