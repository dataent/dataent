...
dataent.ui.open_dialogs = [];

dataent.ui.Dialog = class Dialog extends dataent.ui.FieldGroup {
	constructor(opts) {
		super();
		this.display = false;
		this.is_dialog = true;
	...
	make() {
		this.$wrapper = dataent.get_modal("", "");
		...
		this.$wrapper
			.on("hide.bs.modal", function() {
				...
				if(dataent.ui.open_dialogs[dataent.ui.open_dialogs.length-1]===me) {
					dataent.ui.open_dialogs.pop();
					if(dataent.ui.open_dialogs.length) {
						window.cur_dialog = dataent.ui.open_dialogs[dataent.ui.open_dialogs.length-1];
					} else {
						window.cur_dialog = null;
					}
				}
			...
			.on("shown.bs.modal", function() {
				...
				dataent.ui.open_dialogs.push(me);
				me.focus_on_first_input();
				me.on_page_show && me.on_page_show();
				$(document).trigger('dataent.ui.Dialog:shown');
			})
			.on('scroll', function() {
