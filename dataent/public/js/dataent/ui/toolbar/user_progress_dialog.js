...
dataent.setup.UserProgressSlide = class UserProgressSlide extends dataent.ui.Slide {
	...
	make_done_state() {
		...
		if(this.done_state_title) {
			...
			this.$done_state_title.on('click', () => {
				dataent.set_route(this.done_state_title_route);
			});
		}
		...
		if(this.help_links) {
			this.help_links.map(link => {
				...
				if(link.url) {
					...
					$link.on('click', () => {
						dataent.help.show_video(link.video_id, link.label);
					})
				}
...
dataent.setup.UserProgressDialog  = class UserProgressDialog {
	constructor({
		slides = []
	}) {
		this.slides = slides;
	...
	setup() {
		this.dialog = new dataent.ui.Dialog({title: __("Complete Setup")});
		this.$wrapper = $(this.dialog.$wrapper).addClass('user-progress-dialog');
		this.slide_container = new dataent.ui.Slides({
			parent: this.dialog.body,
			slides: this.slides,
			slide_class: dataent.setup.UserProgressSlide,
			done_state: 1,
			before_load: ($footer) => {
	...
	get_and_update_progress_state() {
		var me = this;
		dataent.call({
			method: "dataent.desk.user_progress.update_and_get_user_progress",
			callback: function(r) {
