...
dataent.search.SearchDialog = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
		this.make();
	},
	...
	make: function() {
		...
		$(d.header).html($(dataent.render_template("search_header")));
	...
	bind_events: function() {
		...
		this.$modal_body.on('click', '.list-more', function() {
			...
			if(fetch_type === "Global") {
				dataent.search.utils.get_global_results(me.current_keyword,
		...
		// Help results
		// this.$modal_body.on('click', 'a[data-path]', dataent.help.show_results);
		...
		this.$search_modal.on('keydown', function(e) {
			...
			if(me.$modal_body.find('.list-link').length > 1) {
				if(me.modal_state === 0) {
					...
					var { UP_ARROW, DOWN_ARROW, TAB } = dataent.ui.keyCode;
	...
	render_data: function(result_sets) {
		var me = this;
		var $search_results = $(dataent.render_template("search")).addClass('hide');
	...
	render_result: function(type, result) {
		...
		if(result.image) {
			...
			$result.append('<a '+ get_link(result) +
				'><div class="result-image"><div class="flex-text"><span>'
					+ dataent.get_abbr(result.label) +'</span></div></div></a>');
		}
		...
		if(result.description) {
			...
			if(result.route_options) {
				dataent.route_options = result.route_options;
			}
			$result.on('click', (e) => {
				...
				if(result.onclick) {
					...
					dataent.set_route(result.route);
	...
	searches: {
		global_search: {
			...
			get_results: function(keywords, callback) {
				...
				var results = dataent.search.utils.get_nav_results(keywords);
				dataent.search.utils.get_global_results(keywords, start, limit)
