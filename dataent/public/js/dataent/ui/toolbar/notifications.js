dataent.provide("dataent.ui.notifications");

dataent.ui.notifications = {
	config: {
		"ToDo": { label: __("To Do") },
		...
		"Likes": { label: __("Likes"),
			click: function() {
				dataent.route_options = { show_likes: true };
				if (dataent.get_route()[0]=="activity") {
					dataent.pages['activity'].page.list.refresh();
				} else {
					dataent.set_route("activity");
				}
			}
		},
	},
	...
	update_notifications: function() {
		...
		this.boot_info = dataent.boot.notification_info;
	...
	bind_list: function() {
		...
		$("#dropdown-notification a").on("click", function() {
			...
			if(!doc) {
				...
				if (config.route) {
					dataent.set_route(config.route);
				...
				dataent.set_route("Form", doctype, doc);
			}
		});
	},
	...
	show_open_count_list: function(doctype) {
		...
		if(filters && $.isPlainObject(filters)) {
			...
			$.extend(dataent.route_options, filters);
		}
