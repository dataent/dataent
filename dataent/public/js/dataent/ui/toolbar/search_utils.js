...
dataent.search.utils = {
	setup_recent: function() {
		this.recent = JSON.parse(dataent.boot.user.recent || "[]") || [];
	},
	...
	get_recent_pages: function(keywords) {
		...
		dataent.route_history.forEach(function(route, i) {
			if(route[0]==='Form') {
				...
				values.push([dataent.route_titles[route[0]] || route[0], route]);
			}
		});
	...
	get_search_in_list: function(keywords) {
		...
		if(in_list(keywords.split(" "), "in") && (keywords.slice(-2) !== "in")) {
			...
			dataent.boot.user.can_read.forEach(function (item) {
				if(dataent.boot.user.can_search.includes(item)) {
	...
	get_creatables: function(keywords) {
		...
		if(firstKeyword.toLowerCase() === __("new")) {
			dataent.boot.user.can_create.forEach(function (item) {
				...
				if(level) {
					out.push({
						...
						index: 1 + level,
						match: item,
						onclick: function() { dataent.new_doc(item, true); }
					});
				}
			});
		}
	...
	get_doctypes: function(keywords) {
		...
		dataent.boot.user.can_read.forEach(function (item) {
			...
			if (level) {
				target = item;
				if (in_list(dataent.boot.single_types, item)) {
					out.push(option("", ["Form", item, item], 0.05));

				} else if (dataent.boot.user.can_search.includes(item)) {
					// include 'making new' option
					if (in_list(dataent.boot.user.can_create, item)) {
						var match = item;
						out.push({
							type: "New",
							...
							index: level + 0.015,
							match: item,
							onclick: function () { dataent.new_doc(match, true); }
						});
					}
					if (in_list(dataent.boot.treeviews, item)) {
						out.push(option("Tree", ["Tree", item], 0.05));

					} else {
						out.push(option("List", ["List", item], 0.05));
						if (dataent.model.can_get_report(item)) {
	...
	get_reports: function(keywords) {
		...
		var out = [];
		var route;
		Object.keys(dataent.boot.user.all_reports).forEach(function(item) {
			var level = me.fuzzy_search(keywords, item);
			if(level > 0) {
				var report = dataent.boot.user.all_reports[item];
	...
	get_modules: function(keywords) {
		var me = this;
		var out = [];
		Object.keys(dataent.modules).forEach(function(item) {
			var level = me.fuzzy_search(keywords, item);
			if(level > 0) {
				var module = dataent.modules[item];
				if (module._doctype) return;

				// disallow restricted modules
				if (dataent.boot.user.allow_modules &&
					!dataent.boot.user.allow_modules.includes(module.module_name)) {
					return;
				}
				var ret = {
					type: "Module",
	...
	get_global_results: function (keywords, start, limit, doctype = "") {
		...
		return new Promise(function(resolve, reject) {
			dataent.call({
				method: "dataent.utils.global_search.search",
				args: {
					text: keywords,
					start: start,
					limit: limit,
