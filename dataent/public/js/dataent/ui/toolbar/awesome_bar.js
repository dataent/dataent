...
dataent.search.AwesomeBar = Class.extend({
	setup: function(element) {
			...
			$this.data('timeout', setTimeout(function(){
				...
				if(txt && txt.length > 1) {
					...
					me.options = me.options.concat(
						me.deduplicate(dataent.search.utils.get_recent_pages(txt || "")));
				}
				me.add_help();
		...
		$input.on("awesomplete-select", function(e) {
			...
			if(item.route_options) {
				dataent.route_options = item.route_options;
			}
		...
		dataent.search.utils.setup_recent();
	},
	...
	add_help: function() {
		this.options.push({
			...
			onclick: function() {
				...
				dataent.msgprint(txt, __("Search Help"));
			}
		});
	},
	...
	build_options: function(txt) {
		var options = dataent.search.utils.get_creatables(txt).concat(
			dataent.search.utils.get_search_in_list(txt),
			dataent.search.utils.get_doctypes(txt),
			dataent.search.utils.get_reports(txt),
			dataent.search.utils.get_pages(txt),
			dataent.search.utils.get_modules(txt),
			dataent.search.utils.get_recent_pages(txt || "")
		);
	...
	make_global_search: function(txt) {
		...
		this.options.push({
			...
			default: "Search",
			onclick: function() {
				dataent.searchdialog.search.init_search(txt, "global_search");
			}
		});
	},
	...
	make_search_in_current: function(txt) {
		...
		if(route[0]==="List" && txt.indexOf(" in") === -1) {
			...
			var meta = dataent.get_meta(dataent.container.page.list_view.doctype);
		...
		if(first==parseInt(first) || first==="(" || first==="=") {
			...
			try {
				...
				this.options.push({
					...
					default: "Calculator",
					onclick: function() {
						dataent.msgprint(formatted_value, "Result");
					}
				});
			} catch(e) {
				// pass
