...
dataent.ui.toolbar.ModulesSelect = class {
	constructor() {
		this.user = dataent.boot.user.name;
		this.setup();
	}
	...
	setup() {
		this.dialog = new dataent.ui.Dialog({
			...
			fields: [
				...
				{ fieldtype: 'Column Break' },
				{
					...
					fieldtype: 'Link',
					options: 'User',
					default: dataent.boot.user.name,
					onchange: () => {
						this.user = this.get_value() || dataent.boot.user.name;
						this.$icons_list.refresh();
					}
				},
				{ fieldtype: 'Section Break' },
				{
					...
					get_data: () => {
						return new Promise((resolve) => {
							dataent.call({
								method: 'dataent.desk.doctype.desktop_icon.desktop_icon.get_module_icons',
								args: {user: this.user},
								freeze: true,
								callback: (r) => {
		...
		this.dialog.set_primary_action(__('Save'), () => {
			dataent.call({
				method: 'dataent.desk.doctype.desktop_icon.desktop_icon.update_icons',
				args: {
	...
	show(user) {
		if(user) {
			this.user = user || dataent.boot.user.name;
			this.$icons_list.refresh();
		}
		this.dialog.show();
	}
