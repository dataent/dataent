...
dataent.provide("dataent.messages");

import './dialog';

dataent.messages.waiting = function(parent, msg) {
	return $(dataent.messages.get_waiting_message(msg))
		.appendTo(parent);
};

dataent.messages.get_waiting_message = function(msg) {
	return repl('<div class="msg-box" style="width: 63%; margin: 30px auto;">\
		<p class="text-center">%(msg)s</p></div>', { msg: msg });
}

dataent.throw = function(msg) {
	if(typeof msg==='string') {
		msg = {message: msg, title: __('Error')};
	}
	if(!msg.indicator) msg.indicator = 'red';
	dataent.msgprint(msg);
	throw new Error(msg.message);
}

dataent.confirm = function(message, ifyes, ifno) {
	var d = new dataent.ui.Dialog({
		title: __("Confirm"),
		fields: [
			{fieldtype:"HTML", options:`<p class="dataent-confirm-message">${message}</p>`}
		],
...
dataent.prompt = function(fields, callback, title, primary_label) {
...
var msg_dialog=null;
dataent.msgprint = function(msg, title) {
	if(!msg) return;
	...
	if(data.alert) {
		dataent.show_alert(data);
		return;
	}

	if(!msg_dialog) {
		msg_dialog = new dataent.ui.Dialog({
			title: __("Message"),
			onhide: function() {
		...
		msg_dialog.loading_indicator = $('<div class="loading-indicator text-center" \
				style="margin: 15px;">\
				<img src="/assets/dataent/images/ui/ajax-loader.gif"></div>')
			.appendTo(msg_dialog.body);
	...
	if(data.message.search(/<br>|<p>|<li>/)==-1) {
		msg = dataent.utils.replace_newlines(data.message);
	}
...
window.msgprint = dataent.msgprint;

dataent.hide_msgprint = function(instant) {
	// clear msgprint
...
dataent.update_msgprint = function(html) {
	if(!msg_dialog || (msg_dialog && !msg_dialog.$wrapper.is(":visible"))) {
		dataent.msgprint(html);
	} else {
		msg_dialog.msg_area.html(html);
	}
}

dataent.verify_password = function(callback) {
	dataent.prompt({
		fieldname: "password",
		...
		dataent.call({
			method: "dataent.core.doctype.user.user.verify_password",
			args: {
...
dataent.show_progress = function(title, count, total=100, description) {
	if(dataent.cur_progress && dataent.cur_progress.title === title && dataent.cur_progress.is_visible) {
		var dialog = dataent.cur_progress;
	} else {
...
dataent.hide_progress = function() {
	if(dataent.cur_progress) {
		dataent.cur_progress.hide();
		dataent.cur_progress = null;
	}
}

// Floating Message
dataent.show_alert = function(message, seconds=7, actions={}) {
...
// Proxy for dataent.show_alert
Object.defineProperty(window, 'show_alert', {
	get: function() {
		console.warn('Please use `dataent.show_alert` instead of `show_alert`. It will be deprecated soon.');
		return dataent.show_alert;
	}
});
