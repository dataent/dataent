...
from dataent.modules import scrub, get_module_path, scrub_dt_dn
...
def export_to_files(record_list=None, record_module=None, verbose=0, create_init=None):
	...
	if record_list:
		for record in record_list:
			write_document_file(dataent.get_doc(record[0], record[1]), record_module, create_init=create_init)
...
def get_module_name(doc):
	if doc.doctype  == 'Module Def':
		...
		module = dataent.db.get_value("DocType", doc.document_type, "module")
	elif hasattr(doc, 'module'):
		module = doc.module
	else:
		module = dataent.db.get_value("DocType", doc.doctype, "module")
