...
from dataent.utils import get_datetime_str
	...
	if docs:
		...
		for doc in docs:
			if not force:
				...
				db_modified = dataent.db.get_value(doc['doctype'], doc['name'], 'modified')
			...
			if original_modified:
				...
				if doc["doctype"] == doc["name"] and doc["name"]!="DocType":
					dataent.db.sql("""update tabSingles set value=%s where field="modified" and doctype=%s""",
						(original_modified, doc["name"]))
				else:
					dataent.db.sql("update `tab%s` set modified=%s where name=%s" % \
						(doc['doctype'], '%s', '%s'),
						(original_modified, doc['name']))
...
def import_doc(docdict, force=False, data_import=False, pre_process=None,
	...
	if dataent.db.exists(doc.doctype, doc.name):
		old_doc = dataent.get_doc(doc.doctype, doc.name)
		...
		# delete old
		dataent.delete_doc(doc.doctype, doc.name, force=1, ignore_doctypes=ignore, for_reload=True)
