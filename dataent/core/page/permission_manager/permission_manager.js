dataent.pages['permission-manager'].on_page_load = (wrapper) => {
	...
	wrapper.permission_engine = new dataent.PermissionEngine(wrapper);

};

dataent.pages['permission-manager'].refresh = function(wrapper) {
	wrapper.permission_engine.set_from_route();
};

dataent.PermissionEngine = Class.extend({
	...
	setup_page: function() {
		...
		this.doctype_select
			= this.wrapper.page.add_select(__("Document Types"),
				...
				.change(function() {
					dataent.set_route("permission-manager", $(this).val());
				});
		this.role_select
		...
		this.page.add_inner_button(__('Set User Permissions'), () => {
			return dataent.set_route('List', 'User Permission');
		});
		this.set_from_route();
	},
	set_from_route: function() {
		...
		if(dataent.get_route()[1]) {
			this.doctype_select.val(dataent.get_route()[1]);
		} else if(dataent.route_options) {
			if(dataent.route_options.doctype) {
				this.doctype_select.val(dataent.route_options.doctype);
			}
			if(dataent.route_options.role) {
				this.role_select.val(dataent.route_options.role);
			}
	...
	reset_std_permissions: function(data) {
		var me = this;
		var d = dataent.confirm(__("Reset Permissions for {0}?", [me.get_doctype()]), function() {
			return dataent.call({
				module:"dataent.core",
				page:"permission_manager",
				method:"reset",
		...
		var $d = $(d.wrapper).find(".dataent-confirm-message").append("<hr><h4>Standard Permissions:</h4><br>");
			...
			.click(function() {
				...
				dataent.call({
					...
					callback: function(r) {
						...
						dataent.msgprint(__("Users with role {0}:", [__(role)])
		...
		$("<button class='btn btn-default btn-sm'><i class='fa fa-remove'></i></button>")
			...
			.click(function() {
				return dataent.call({
					...
					callback: function(r) {
						if(r.exc) {
							dataent.msgprint(__("Did not remove"));
						} else {
							me.refresh();
						}
					}
	...
	add_check_events: function() {
		...
		this.body.on("click", ".show-user-permissions", function() {
			dataent.route_options = { allow: me.get_doctype() || "" };
			...
			.click(function() {
				...
				d.set_primary_action(__('Add'), function() {
					...
					dataent.call({
						...
						callback: function(r) {
							if(r.exc) {
								dataent.msgprint(__("Did not add"));
							} else {
								me.refresh();
							}
						}
	...
	get_link_fields: function(doctype) {
		return dataent.get_children("DocType", doctype, "fields",
