...
dataent.ui.form.on('Data Import', {
	onload: function(frm) {
		...
		frm.set_query("reference_doctype", function() {
			return {
				"filters": {
					"issingle": 0,
					"istable": 0,
					"name": ['in', dataent.boot.user.can_import]
				}
			};
		});
		...
		dataent.realtime.on("data_import_progress", function(data) {
	...
	reference_doctype: function(frm){
		if (frm.doc.reference_doctype) {
			dataent.model.with_doctype(frm.doc.reference_doctype);
		}
	},
	...
	refresh: function(frm) {
		...
		if (frm.doc.reference_doctype && !frm.doc.import_file) {
			...
			if (frm.doc.import_status) {
				const listview_settings = dataent.listview_settings['Data Import'];
		...
		frm.add_custom_button(__("Help"), function() {
			dataent.help.show_video("6wiriRKPhmg");
		});

		if (frm.doc.reference_doctype && frm.doc.docstatus === 0) {
			frm.add_custom_button(__("Download template"), function() {
				dataent.data_import.download_dialog(frm).show();
			});
		}
		...
		if (frm.doc.reference_doctype && frm.doc.import_file && frm.doc.total_rows &&
			...
			frm.page.set_primary_action(__("Start Import"), function() {
				dataent.call({
					method: "dataent.core.doctype.data_import.data_import.import_data",
					args: {
	...
	action: function(frm) {
		...
		if(!frm.doc.reference_doctype) {
			dataent.msgprint(__("Please select document type first."));
			frm.set_value("action", "");
			return;
		}
	...
	create_log_table: function(frm) {
		...
		$(dataent.render_template("log_details", {
			data: msg.messages,
...
dataent.provide('dataent.data_import');
dataent.data_import.download_dialog = function(frm) {
	var dialog;
	const filter_fields = df => dataent.model.is_value_type(df) && !df.hidden;
	const get_fields = dt => dataent.meta.get_docfields(dt).filter(filter_fields);
	...
	const child_table_fields = dataent.meta.get_table_fields(frm.doc.reference_doctype)
		.map(df => {
			return {
				"label": df.options,
				"fieldname": df.fieldname + '_fields',
				"fieldtype": "MultiCheck",
				"options": dataent.meta.get_docfields(df.options)
	...
	dialog = new dataent.ui.Dialog({
		...
		primary_action: function(values) {
			...
			if (frm.doc.reference_doctype) {
				...
				let get_template_url = '/api/method/dataent.core.doctype.data_export.exporter.export_data';
				open_url_post(get_template_url, export_params());
			} else {
				dataent.msgprint(__("Please select the Document Type."));
			}
			dialog.hide();
		},
	...
	$(button_container).find('.dataent-control').map((index, button) => {
