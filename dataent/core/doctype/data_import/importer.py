...
from dataent.utils import cint, cstr, flt, getdate, get_datetime, get_url, get_absolute_url
...
def upload(rows = None, submit_after_import=None, ignore_encoding_errors=False, no_email=True, overwrite=None,
	...
	def bad_template():
		dataent.throw(_("Please do not change the rows above {0}").format(get_data_keys_definition().data_separator))

	def check_data_length():
		if not data:
			dataent.throw(_("No data found in the file. Please reattach the new file with data."))
	...
	def filter_empty_columns(columns):
		...
		if empty_cols:
			if columns[-1*len(empty_cols):] == empty_cols:
				...
				dataent.msgprint(_("Please make sure that there are no empty columns in the file."),
					raise_exception=1)
		...
		if autoname:
			...
			if (autoname not in doc) or (not doc[autoname]):
				...
				if not hasattr(get_controller(doctype), "autoname"):
					dataent.throw(_("{0} is a mandatory field".format(autoname)))
		return True
	...
	def attach_file_to_doc(doctype, docname, file_url):
		...
		files = dataent.db.sql("""Select name from `tabFile` where attached_to_doctype='{doctype}' and
	...
	if submit_after_import and not cint(dataent.db.get_value("DocType",
	...
	def publish_progress(achieved, reload=False):
		if data_import_doc:
			dataent.publish_realtime("data_import_progress", {"progress": str(int(100.0*achieved/total)),
				"data_import": data_import_doc.name, "reload": reload}, user=dataent.session.user)
		...
		for i, row in enumerate(batch):
			...
			try:
				...
				if parentfield:
					...
					if overwrite and doc.get("name") and dataent.db.exists(doctype, doc["name"]):
				...
				if dataent.local.message_log:
					err_msg = "\n".join(['<p class="border-bottom small">{}</p>'.format(json.loads(msg).get('message')) for msg in dataent.local.message_log])
				else:
				...
				log(**{
					"row": row_idx + 1,
					"title": 'Error for row %s' % (len(row)>1 and dataent.safe_decode(row[1]) or ""),
					"message": err_msg,
					"indicator": "red",
					"link":error_link
				})
...
def get_parent_field(doctype, parenttype):
	...
	# get parentfield
	if parenttype:
		for d in dataent.get_meta(parenttype).get_table_fields():
			if d.options==doctype:
				parentfield = d.fieldname
				break

		if not parentfield:
			dataent.msgprint(_("Did not find {0} for {0} ({1})").format("parentfield", parenttype, doctype))
			raise Exception
...
def delete_child_rows(rows, doctype):
	...
	for p in list(set([r[1] for r in rows])):
		if p:
			dataent.db.sql("""delete from `tab{0}` where parent=%s""".format(doctype), p)
