...
from dataent.utils import cstr, formatdate, format_datetime
...
def get_template(doctype=None, parent_doctype=None, all_doctypes="No", with_data="No", select_columns=None,
	...
	if all_doctypes:
		child_doctypes = []
		for df in dataent.get_meta(doctype).get_table_fields():
	...
	def build_field_columns(dt, parentfield=None):
		...
		if dt==doctype:
			...
			_column_start_end = dataent._dict(start=len(columns))

			append_field_column(dataent._dict({
				"fieldname": "name",
				"parent": dt,
				"label": "ID",
				"fieldtype": "Data",
	...
	def add_data():
		...
		if with_data=='Yes':
			dataent.permissions.can_export(parent_doctype, raise_exception=True)

			# sort nested set doctypes by `lft asc`
			order_by = None
			table_columns = dataent.db.get_table_columns(parent_doctype)
			if 'lft' in table_columns and 'rgt' in table_columns:
				order_by = '`tab{doctype}`.`lft` asc'.format(doctype=parent_doctype)

			# get permitted data only
			data = dataent.get_list(doctype, fields=["*"], limit_page_length=None, order_by=order_by)
	...
	if from_data_import == "Yes" and excel_format == "Yes":
		...
		dataent.response['result'] = cstr(w.getvalue())
