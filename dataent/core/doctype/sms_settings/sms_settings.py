...
from dataent import _, throw, msgprint
...
def get_contact_number(contact_name, ref_doctype, ref_name):
	...
	number = dataent.db.sql("""select mobile_no, phone from tabContact 
		where name=%s 
			and exists(
...
def send_via_gateway(arg):
	...
	if len(success_list) > 0:
		...
		if arg.get('success_msg'):
			dataent.msgprint(_("SMS sent to following numbers: {0}").format("\n" + "\n".join(success_list)))

def get_headers(sms_settings=None):
	if not sms_settings:
		sms_settings = dataent.get_doc('SMS Settings', 'SMS Settings')
