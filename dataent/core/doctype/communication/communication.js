dataent.ui.form.on("Communication", {
	onload: function(frm) {
		if(frm.doc.content) {
			frm.doc.content = dataent.dom.remove_script_and_style(frm.doc.content);
		}
	...
	show_relink_dialog: function(frm){
		...
		var d = new dataent.ui.Dialog ({
			...
			fields: [{
				...
				"get_query": function() {return {"query": "dataent.email.get_communication_doctype"}}
			},
			{
		...
		d.set_primary_action(__("Relink"), function () {
			...
			if (values) {
				dataent.confirm(
					...
					function () {
						dataent.show_alert('Document not Relinked')
					}
				);
			}
		});
	...
	mark_as_read_unread: function(frm) {
		...
		return dataent.call({
			method: "dataent.email.inbox.create_email_flag_queue",
			args: {
				'names': [frm.doc.name],
				'action': action,
				'flag': flag
	...
	get_mail_args: function(frm) {
		var sender_email_id = ""
		$.each(dataent.boot.email_accounts, function(idx, account) {
	...
	add_to_contact: function(frm) {
		...
		dataent.new_doc("Contact")
	},

	mark_as_spam: function(frm) {
		dataent.call({
			method: "dataent.email.inbox.mark_as_spam",
			args: {
			...
			freeze: true,
			callback: function(r) {
				dataent.msgprint(__("Email has been marked as spam"))
			}
		})
	},
	...
	move_to_trash: function(frm) {
		dataent.call({
			method: "dataent.email.inbox.mark_as_trash",
			args: {
				communication: frm.doc.name
			},
			freeze: true,
			callback: function(r) {
				dataent.msgprint(__("Email has been moved to trash"))
			}
		})
	}
});
