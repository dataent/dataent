...
from dataent.utils import get_fullname, get_link_to_form
from dataent.website.render import clear_cache
from dataent.model.db_schema import add_column
from dataent.exceptions import ImplicitCommitError
...
def notify_mentions(doc):
	...
	if doc.reference_doctype and doc.reference_name and doc.content and doc.comment_type=="Comment":
		...
		recipients = [dataent.db.get_value("User", {"enabled": 1, "name": name, "user_type": "System User"}, "email")
...
def get_comments_from_parent(doc):
	try:
		_comments = dataent.db.get_value(doc.reference_doctype, doc.reference_name, "_comments") or "[]"
...
def update_comments_in_parent(reference_doctype, reference_name, _comments):
	...
	if not reference_doctype or dataent.db.get_value("DocType", reference_doctype, "issingle"):
		return

	try:
		# use sql, so that we do not mess with the timestamp
		dataent.db.sql("""update `tab%s` set `_comments`=%s where name=%s""" % (reference_doctype,
			"%s", "%s"), (json.dumps(_comments), reference_name))

	except Exception as e:
		if e.args[0] == 1054 and getattr(dataent.local, 'request', None):
			# missing column and in request, add column and update after commit
			dataent.local._comments = (getattr(dataent.local, "_comments", [])
		...
		if not dataent.flags.in_patch:
			reference_doc = dataent.get_doc(reference_doctype, reference_name)
...
def add_info_comment(**kwargs):
	...
	return dataent.get_doc(kwargs).insert(ignore_permissions=True)

def update_comments_in_parent_after_request():
	"""update _comments in parent if _comments column is missing"""
	if hasattr(dataent.local, "_comments"):
		for (reference_doctype, reference_name, _comments) in dataent.local._comments:
