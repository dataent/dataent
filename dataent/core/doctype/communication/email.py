...
from dataent.core.utils import get_parent_doc
from dataent.utils import (get_url, get_formatted_email, cint,
  validate_email_add, split_emails, time_diff_in_seconds, parse_addr, get_datetime)
from dataent.utils.file_manager import get_file, add_attachments
from dataent.email.queue import check_email_limit
from dataent.utils.scheduler import log
from dataent.email.email_body import get_message_id
import dataent.email.smtp
import time
...
def make(doctype=None, name=None, content=None, subject=None, sent_or_received = "Sent",
	...
	is_error_report = (doctype=="User" and name==dataent.session.user and subject=="Error Report")
	send_me_a_copy = cint(send_me_a_copy)

	if doctype and name and not is_error_report and not dataent.has_permission(doctype, "email", name) and not (flags or {}).get('ignore_doctype_permissions'):
		raise dataent.PermissionError("You are not allowed to send emails related to: {doctype} {name}".format(
...
def notify(doc, print_html=None, print_format=None, attachments=None,
	...
	if dataent.flags.in_test:
		...
		enqueue(sendmail, queue="default", timeout=300, event="sendmail",
			...
			recipients=recipients, cc=cc, bcc=bcc, lang=dataent.local.lang,
			session=dataent.local.session, print_letterhead=dataent.flags.print_letterhead)
...
def prepare_to_notify(doc, print_html=None, print_format=None, attachments=None):
	...
	view_link = dataent.utils.cint(dataent.db.get_value("Print Settings", "Print Settings", "attach_view_link"))
		...
		for a in attachments:
			if isinstance(a, string_types):
				...
				try:
					...
					dataent.throw(_("Unable to find attachment {0}").format(a))
			else:
...
def set_incoming_outgoing_accounts(doc):
	...
	if not doc.incoming_email_account and doc.sender:
		doc.incoming_email_account = dataent.db.get_value("Email Account",
	...
	if not doc.incoming_email_account and doc.reference_doctype:
		...
		doc.outgoing_email_account = dataent.db.get_value("Email Account",
	...
	if not doc.outgoing_email_account:
		...
		doc.outgoing_email_account = dataent.db.get_value("Email Account",
			...
			["email_id", "always_use_account_email_id_as_sender", "name",
			"send_unsubscribe_message", "always_use_account_name_as_sender_name"], as_dict=True) or dataent._dict()
...
def get_cc(doc, recipients=None, fetched_from_email_account=False):
	...
	if cc:
		...
		exclude += [d[0] for d in dataent.db.get_all("User", ["email"], {"thread_notify": 0}, as_list=True)]
		...
		if doc.reference_doctype and doc.reference_name:
			exclude += [d[0] for d in dataent.db.get_all("Email Unsubscribe", ["email"],
...
def filter_email_list(doc, email_list, exclude, is_cc=False, is_bcc=False):
	...
	for email in list(set(email_list)):
		...
		if is_cc:
			is_user_enabled = dataent.db.get_value("User", email_address, "enabled")
...
def get_assignees(doc):
	return [( get_formatted_email(d.owner) or d.owner ) for d in
		dataent.db.get_all("ToDo", filters={
...
def sendmail(communication_name, print_html=None, print_format=None, attachments=None,
	...
	try:
		...
		traceback = log("dataent.core.doctype.communication.email.sendmail", dataent.as_json({
...
def update_mins_to_first_communication(parent, communication):
	if parent.meta.has_field('mins_to_first_response') and not parent.get('mins_to_first_response'):
		if dataent.db.get_all('User', filters={'email': communication.sender,
...
def mark_email_as_seen(name=None):
	try:
		if name and dataent.db.exists("Communication", name) and not dataent.db.get_value("Communication", name, "read_by_recipient"):
			dataent.db.set_value("Communication", name, "read_by_recipient", 1)
			dataent.db.set_value("Communication", name, "delivery_status", "Read")
			dataent.db.set_value("Communication", name, "read_by_recipient_on", get_datetime())
			dataent.db.commit()
	except Exception:
