...
from dataent.utils import validate_email_add, get_fullname, strip_html, cstr
from dataent.core.doctype.communication.comment import (notify_mentions,
	update_comment_in_doc, on_trash)
from dataent.core.doctype.communication.email import (validate_email,
	notify, _notify, update_parent_mins_to_first_response)
from dataent.core.utils import get_parent_doc, set_timeline_doc
from dataent.utils.bot import BotReply
...
class Communication(Document):
	...
	def onload(self):
		...
		if self.communication_type == "Communication" and self.communication_medium == "Email" \
			...
			email_flag_queue = dataent.db.get_value("Email Flag Queue", {
	...
	def validate_reference(self):
		if self.reference_doctype and self.reference_name:
			if not self.reference_owner:
				self.reference_owner = dataent.db.get_value(self.reference_doctype, self.reference_name, "owner")

			# prevent communication against a child table
			if dataent.get_meta(self.reference_doctype).istable:
				dataent.throw(_("Cannot create a {0} against a child document: {1}")
			...
			if self.reference_doctype == "Communication":
				...
				if circular_linking:
					dataent.throw(_("Please make sure the Reference Communication Docs are not circularly linked."), dataent.CircularLinkingError)
	...
	def after_insert(self):
		...
		if self.reference_doctype == "Communication" and self.sent_or_received == "Sent" and \
			...
			dataent.db.set_value("Communication", self.reference_name, "status", "Replied")

		if self.communication_type in ("Communication", "Comment"):
			# send new comment to listening clients
			dataent.publish_realtime('new_communication', self.as_dict(),
			...
			if self.reference_name == dataent.session.user:
				...
				dataent.publish_realtime('new_message', message, after_commit=True)
			else:
				# reference_name contains the user who is addressed in the messages' page comment
				dataent.publish_realtime('new_message', self.as_dict(),
	...
	def on_trash(self):
		if (not self.flags.ignore_permissions
			...
			dataent.throw(_("Sorry! You cannot delete auto-generated comments"))

		if self.communication_type in ("Communication", "Comment"):
			# send delete comment to listening clients
			dataent.publish_realtime('delete_communication', self.as_dict(),
				doctype= self.reference_doctype, docname = self.reference_name,
				after_commit=True)
	...
	def set_status(self):
		...
		email_rule = dataent.db.get_value("Email Rule", { "email_id": self.sender, "is_spam":1 })
		if self.communication_type == "Communication" and self.communication_medium == "Email" \
	...
	def set_sender_full_name(self):
		if not self.sender_full_name and self.sender:
			if self.sender == "Administrator":
				self.sender_full_name = dataent.db.get_value("User", "Administrator", "full_name")
				self.sender = dataent.db.get_value("User", "Administrator", "email")
				...
				self.sender_full_name = sender_name or dataent.db.exists("Contact", {"email_id": sender_email}) or sender_email
	...
	def set_delivery_status(self, commit=False):
		...
		status_counts = Counter(dataent.db.sql_list('''select status from `tabEmail Queue` where communication=%s''', self.name))
		...
		if delivery_status:
			...
			dataent.publish_realtime('update_communication', self.as_dict(),
...
def on_doctype_update():
	...
	dataent.db.add_index("Communication", ["reference_doctype", "reference_name"])
	dataent.db.add_index("Communication", ["timeline_doctype", "timeline_name"])
	dataent.db.add_index("Communication", ["link_doctype", "link_name"])
	dataent.db.add_index("Communication", ["status", "communication_type"])
...
def has_permission(doc, ptype, user):
	if ptype=="read":
		...
		if doc.reference_doctype and doc.reference_name:
			if dataent.has_permission(doc.reference_doctype, ptype="read", doc=doc.reference_name):
				return True
		if doc.timeline_doctype and doc.timeline_name:
			if dataent.has_permission(doc.timeline_doctype, ptype="read", doc=doc.timeline_name):
				return True
...
def get_permission_query_conditions_for_communication(user):
	...
	if "Super Email User" in roles or "System Manager" in roles:
		...
		accounts = dataent.get_all("User Email", filters={ "parent": user },
