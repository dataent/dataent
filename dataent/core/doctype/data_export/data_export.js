...
dataent.ui.form.on('Data Export', {
	refresh: frm => {
		frm.disable_save();
		...
		frm.set_query("reference_doctype", () => {
			return {
				"filters": {
					"issingle": 0,
					"istable": 0,
					"name": ['in', dataent.boot.user.can_export]
				}
			};
		});
	},
	reference_doctype: frm => {
		...
		if (doctype) {
			dataent.model.with_doctype(doctype, () => set_field_options(frm));
		} else {
...
const can_export = frm => {
	...
	if (!doctype) {
		...
		dataent.msgprint(__('Atleast one field of Parent Document Type is mandatory'));
	} else {
		is_valid_form = true;
	}
...
const set_field_options = (frm) => {
	...
	frm.filter_list = new dataent.ui.FilterGroup({
...
const make_multiselect_buttons = parent_wrapper => {
	...
	["Select All", "Unselect All"].map(d => {
		dataent.ui.form.make_control({
			parent: $(button_container),
			df: {
				label: __(d),
				fieldname: dataent.scrub(d),
				fieldtype: "Button",
				click: () => {
...
const get_doctypes = parentdt => {
	return [parentdt].concat(
		dataent.meta.get_table_fields(parentdt).map(df => df.options)
	);
};
...
const add_doctype_field_multicheck_control = (doctype, parent_wrapper) => {
	...
	const multicheck_control = dataent.ui.form.make_control({
