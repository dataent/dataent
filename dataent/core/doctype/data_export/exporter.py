...
import re, csv, os
from dataent.utils.csvutils import UnicodeWriter
from dataent.utils import cstr, formatdate, format_datetime, parse_json, cint
from dataent.core.doctype.data_import.importer import get_data_keys
...
class DataExporter:
	...
	def prepare_args(self):
		...
		if self.all_doctypes:
			self.child_doctypes = []
			for df in dataent.get_meta(self.doctype).get_table_fields():
	...
	def build_response(self):
		...
		if self.with_data and not self.data:
			dataent.respond_as_web_page(_('No Data'), _('There is no data to be exported'), indicator_color='orange')
	...
	def build_field_columns(self, dt, parentfield=None):
		...
		tablecolumns = []
		for f in dataent.db.sql('desc `tab%s`' % dt):
		...
		_column_start_end = dataent._dict(start=0)
		...
		if dt==self.doctype:
			...
			if meta.get('istable') and not self.with_data:
				self.append_field_column(dataent._dict({
					"fieldname": "parent",
					"parent": "",
					"label": "Parent",
					"fieldtype": "Data",
			...
			_column_start_end = dataent._dict(start=len(self.columns))
	...
	def add_data(self):
		...
		dataent.permissions.can_export(self.parent_doctype, raise_exception=True)

		# sort nested set doctypes by `lft asc`
		order_by = None
		table_columns = dataent.db.get_table_columns(self.parent_doctype)
		if 'lft' in table_columns and 'rgt' in table_columns:
			order_by = '`tab{doctype}`.`lft` asc'.format(doctype=self.parent_doctype)
		# get permitted data only
		self.data = dataent.get_list(self.doctype, fields=["*"], filters=self.filters, limit_page_length=None, order_by=order_by)
		...
		for doc in self.data:
			...
			if self.all_doctypes:
				...
				for c in self.child_doctypes:
					for ci, child in enumerate(dataent.db.sql("""select * from `tab{0}`
	...
	def build_response_as_excel(self):
		filename = dataent.generate_hash("", 10)
		...
		dataent.response['filename'] = self.doctype + '.xlsx'
