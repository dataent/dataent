...
class DomainSettings(Document):
	...
	def on_update(self):
		for i, d in enumerate(self.active_domains):
			...
			if i >= 1:
				dataent.flags.keep_desktop_icons = True
			domain = dataent.get_doc('Domain', d.domain)
			domain.setup_domain()
		...
		for domain in all_domains:
			...
			if 'modules' in data:
				for module in data.get('modules'):
					dataent.db.set_value('Module Def', module, 'restrict_to_domain', domain)

			if 'restricted_roles' in data:
				for role in data['restricted_roles']:
					if not dataent.db.get_value('Role', role):
						dataent.get_doc(dict(doctype='Role', role_name=role)).insert()
					dataent.db.set_value('Role', role, 'restrict_to_domain', domain)
...
def get_active_domains():
	...
	def _get_active_domains():
		domains = dataent.get_all("Has Domain", filters={ "parent": "Domain Settings" },
	...
	return dataent.cache().get_value("active_domains", _get_active_domains)
...
def get_active_modules():
	...
	def _get_active_modules():
		...
		for m in dataent.get_all("Module Def", fields=['name', 'restrict_to_domain']):
			if (not m.restrict_to_domain) or (m.restrict_to_domain in active_domains):
				active_modules.append(m.name)
		return active_modules

	return dataent.cache().get_value('active_modules', _get_active_modules)
