dataent.listview_settings['User Permission'] = {
		...
		list_view.page.add_inner_button( __("Add / Update"), function() {
			let dialog =new dataent.ui.Dialog({
				...
				primary_action: (data) => {
					...
					dataent.call({
						async: false,
						method: "dataent.core.doctype.user_permission.user_permission.add_user_permissions",
						args: {
							data : data
						},
						callback: function(r) {
							if(r.message === 1) {
								dataent.show_alert({message:__("User Permissions created sucessfully"), indicator:'blue'});
							} else {
								dataent.show_alert({message:__("Nothing to update"), indicator:'red'});
		...
		list_view.page.add_inner_button( __("Bulk Delete"), function() {
			const dialog = new dataent.ui.Dialog({
				...
				primary_action: (data) => {
					...
					dataent.confirm(__('Are you sure?'), () => {
						dataent
							.xcall('dataent.core.doctype.user_permission.user_permission.clear_user_permissions', data)
							.then(data => {
								dialog.hide();
								let message = '';
								if (data === 0) {
	...
	validate: function(dialog, data) {
		...
		if(data.apply_to_all_doctypes == 0 && !("applicable_doctypes" in data)) {
			dataent.throw(__("Please select applicable Doctypes"));
		}
		return data;
	},
	...
	get_applicable_doctype: function(dialog) {
		return new Promise(resolve => {
			dataent.call({
				method: 'dataent.core.doctype.user_permission.user_permission.check_applicable_doc_perm',
				async: false,
				args:{
	...
	get_multi_select_options: function(dialog, applicable){
		return new Promise(resolve => {
			dataent.call({
				method: 'dataent.desk.form.linked_with.get_linked_doctypes',
				async: false,
				args:{
					user: dialog.fields_dict.user.value,
					doctype: dialog.fields_dict.doctype.value,
