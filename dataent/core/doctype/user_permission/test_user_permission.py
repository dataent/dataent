...
from dataent.core.doctype.user_permission.user_permission import add_user_permissions
...
class TestUserPermission(unittest.TestCase):
	def setUp(self):
		dataent.db.sql("Delete from `tabUser Permission` where user='test_bulk_creation_update@example.com'")
	...
	def test_for_applicable_on_update_from_apply_to_all(self):
		...
		removed_apply_to_all = dataent.db.exists("User Permission", get_exists_param(user))
		is_created_applicable_first = dataent.db.exists("User Permission", get_exists_param(user, applicable = "Chat Room"))
		is_created_applicable_second = dataent.db.exists("User Permission", get_exists_param(user, applicable = "Chat Message"))
	...
	def test_for_apply_to_all_on_update_from_applicable(self):
		...
		is_created = add_user_permissions(param)
		is_created_apply_to_all = dataent.db.exists("User Permission", get_exists_param(user))
		removed_applicable_first = dataent.db.exists("User Permission", get_exists_param(user, applicable = "Chat Room"))
		removed_applicable_second = dataent.db.exists("User Permission", get_exists_param(user, applicable = "Chat Message"))
...
def get_user():
	if dataent.db.exists('User', 'test_bulk_creation_update@example.com'):
		return dataent.get_doc('User', 'test_bulk_creation_update@example.com')
	else:
