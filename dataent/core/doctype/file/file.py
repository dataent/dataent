...
from dataent.utils.file_manager import delete_file_data_content, get_content_hash, get_random_filename
from dataent import _
from dataent.utils.nestedset import NestedSet
from dataent.utils import strip, get_files_path
from PIL import Image, ImageOps
from six import StringIO, string_types
from six.moves.urllib.parse import unquote
import zipfile

class FolderNotEmpty(dataent.ValidationError): pass
...
class File(NestedSet):
	...
	def before_insert(self):
		dataent.local.rollback_observers.append(self)
		self.set_folder_name()

	def get_name_based_on_parent_folder(self):
		if self.folder:
			path = get_breadcrumbs(self.folder)
			folder_name = dataent.get_value("File", self.folder, "file_name")
	...
	def autoname(self):
		...
		if self.is_folder:
			...
			self.name = dataent.generate_hash("", 10)
	...
	def get_successor(self):
		return dataent.db.sql_list("select name from tabFile where folder='%s'"%self.name) or []
	...
	def validate(self):
		...
		if dataent.db.exists('File', {'name': self.name, 'is_folder': 0}):
			old_file_url = self.file_url
			if not self.is_folder and (self.is_private != self.db_get('is_private')):
				private_files = dataent.get_site_path('private', 'files')
				public_files = dataent.get_site_path('public', 'files')
			...
			if self.attached_to_doctype and self.attached_to_name:
				if not self.attached_to_field:
					field_name = None
					reference_dict = dataent.get_doc(self.attached_to_doctype, self.attached_to_name).as_dict()
				...
				if self.attached_to_field:
					dataent.db.set_value(self.attached_to_doctype, self.attached_to_name, self.attached_to_field, self.file_url)


	def set_folder_size(self):
		"""Set folder size if folder"""
		if self.is_folder and not self.is_new():
			self.file_size = dataent.utils.cint(self.get_folder_size())
			self.db_set('file_size', self.file_size)

			for folder in self.get_ancestors():
				dataent.db.set_value("File", folder, "file_size", self.get_folder_size(folder))
	...
	def get_folder_size(self, folder=None):
		...
		file_size =  dataent.db.sql("""select ifnull(sum(file_size), 0)
	...
	def validate_file(self):
		...
		if (self.file_url or "").startswith("/files/"):
			...
			if not os.path.exists(get_files_path(dataent.as_unicode(self.file_name.lstrip("/")))):
				dataent.throw(_("File {0} does not exist").format(self.file_url), IOError)
	...
	def validate_duplicate_entry(self):
		if not self.flags.ignore_duplicate_entry_error and not self.is_folder:
			...
			n_records = dataent.db.sql("""select name from `tabFile`
			...
			if len(n_records) > 0:
				...
				dataent.throw(dataent._("Same file has already been attached to the record"), dataent.DuplicateEntryError)
	...
	def generate_content_hash(self):
		...
		if self.file_url.startswith("/files/"):
			try:
				...
				dataent.msgprint(_("File {0} does not exist").format(self.file_url))
				raise

	def on_trash(self):
		if self.is_home_folder or self.is_attachments_folder:
			dataent.throw(_("Cannot delete Home and Attachments folders"))
	...
	def make_thumbnail(self, set_as_thumbnail=True, width=300, height=300, suffix="small", crop=False):
		if self.file_url:
			...
			path = os.path.abspath(dataent.get_site_path("public", thumbnail_url.lstrip("/")))
			...
			try:
				...
				dataent.msgprint(_("Unable to write file format for {0}").format(path))
				return
	...
	def check_folder_is_empty(self):
		...
		files = dataent.get_all("File", filters={"folder": self.name}, fields=("name", "file_name"))

		if self.is_folder and files:
			dataent.throw(_("Folder {0} is not empty").format(self.name), FolderNotEmpty)

	def check_reference_doc_permission(self):
		"""Check if permission exists for reference document"""
		if not dataent.db.exists(self.attached_to_doctype, self.attached_to_name):
		...
		if self.attached_to_name:
			...
			try:
				if not self.flags.ignore_permissions and \
					not dataent.has_permission(self.attached_to_doctype,
						"write", self.attached_to_name):
					dataent.throw(dataent._("Cannot delete file as it belongs to {0} {1} for which you do not have permissions").format(self.attached_to_doctype, self.attached_to_name),
	...
	def delete_file(self):
		...
		if self.file_name and self.content_hash and (not dataent.db.count("File",
	...
	def unzip(self):
		...
		if not ".zip" in self.file_name:
			dataent.msgprint(_("Not a zip file"))
			return

		zip_path = dataent.get_site_path(self.file_url.strip('/'))
		...
		with zipfile.ZipFile(zip_path) as zf:
			...
			for info in zf.infolist():
				if not info.filename.startswith('__MACOSX'):
					...
					file_name = dataent.db.get_value('File', dict(file_url=file_url))
					if file_name:
						file_doc = dataent.get_doc('File', file_name)
					else:
						file_doc = dataent.new_doc("File")
		...
		dataent.delete_doc('File', self.name)

def on_doctype_update():
	dataent.db.add_index("File", ["attached_to_doctype", "attached_to_name"])

def make_home_folder():
	home = dataent.get_doc({
		"doctype": "File",
		"is_folder": 1,
		"is_home_folder": 1,
		"file_name": _("Home")
...
def get_breadcrumbs(folder):
	...
	values = dataent.db.get_value("File", folder, ["lft", "rgt"], as_dict=True)
	if not values:
		dataent.throw(_("Folder {0} does not exist").format(folder))

	return dataent.db.sql("""select name, file_name from tabFile
		where lft < %s and rgt > %s order by lft asc""", (values.lft, values.rgt), as_dict=1)

@dataent.whitelist()
def create_new_folder(file_name, folder):
	""" create new folder under current parent folder """
	file = dataent.new_doc("File")
...
def move_file(file_list, new_parent, old_parent):
	...
	# recalculate sizes
	dataent.get_doc("File", old_parent).save()
	dataent.get_doc("File", new_parent).save()
...
def setup_folder_path(filename, new_parent):
	...
	if file.is_folder:
		dataent.rename_doc("File", file.name, file.get_name_based_on_parent_folder(), ignore_permissions=True)
...
def get_local_image(file_url):
	file_path = dataent.get_site_path("public", file_url.lstrip("/"))

	try:
		image = Image.open(file_path)
	except IOError:
		dataent.msgprint(_("Unable to read file format for {0}").format(file_url))
		raise
...
def check_file_permission(file_url):
	for file in dataent.get_all("File", filters={"file_url": file_url, "is_private": 1}, fields=["name", "attached_to_doctype", "attached_to_name"]):

		if (dataent.has_permission("File", ptype="read", doc=file.name)
			or dataent.has_permission(file.attached_to_doctype, ptype="read", doc=file.attached_to_name)):
			return True
...
def unzip_file(name):
	...
	file_obj = dataent.get_doc('File', name)
	file_obj.unzip()
...
def get_attached_images(doctype, names):
	...
	img_urls = dataent.db.get_list('File', filters={
...
def on_doctype_update():
	dataent.db.add_index("File", ["lft", "rgt"])