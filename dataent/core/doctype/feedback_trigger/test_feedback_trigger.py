...
# test_records = dataent.get_test_records('Feedback Trigger')
def get_feedback_request(todo, feedback_trigger):
	return dataent.db.get_value("Feedback Request", {
		"is_sent": 1,
...
class TestFeedbackTrigger(unittest.TestCase):
	def setUp(self):
		new_user = dataent.get_doc(dict(doctype='User', email='test-feedback@example.com',
			first_name='Tester')).insert(ignore_permissions=True)
		new_user.add_roles("System Manager")

	def tearDown(self):
		dataent.db.sql("delete from tabContact where email_id='test-feedback@example.com'")
		dataent.delete_doc("User", "test-feedback@example.com")
		dataent.delete_doc("Feedback Trigger", "ToDo")
		dataent.db.sql('delete from `tabEmail Queue`')
		dataent.db.sql('delete from `tabFeedback Request`')

	def test_feedback_trigger(self):
		""" Test feedback trigger """
		from dataent.www.feedback import accept

		dataent.delete_doc("Feedback Trigger", "ToDo")
		dataent.db.sql('delete from `tabEmail Queue`')
		dataent.db.sql('delete from `tabFeedback Request`')

		feedback_trigger = dataent.get_doc({
			"enabled": 1,
		...
		email_queue = dataent.db.sql("""select name from `tabEmail Queue` where
		...
		docname = dataent.db.get_value("Communication", {
			"reference_doctype": "ToDo",
			"reference_name": todo.name,
		...
		communication = dataent.get_doc("Communication", docname)
		...
		email_queue = dataent.db.sql("""select name from `tabEmail Queue` where
			reference_doctype='ToDo' and reference_name='{0}'""".format(todo.name))
		self.assertFalse(email_queue)
		dataent.delete_doc("ToDo", todo.name)
