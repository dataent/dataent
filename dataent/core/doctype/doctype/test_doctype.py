...
import dataent
import unittest
from dataent.core.doctype.doctype.doctype import UniqueFieldnameError, IllegalMandatoryError, DoctypeLinkError, WrongOptionsDoctypeLinkError,\
...
class TestDocType(unittest.TestCase):
	...
	def test_validate_name(self):
		self.assertRaises(dataent.NameError, self.new_doctype("_Some DocType").insert)
		self.assertRaises(dataent.NameError, self.new_doctype("8Some DocType").insert)
		self.assertRaises(dataent.NameError, self.new_doctype("Some (DocType)").insert)
		for name in ("Some DocType", "Some_DocType"):
			if dataent.db.exists("DocType", name):
				dataent.delete_doc("DocType", name)

			doc = self.new_doctype(name).insert()
			doc.delete()

	def test_doctype_unique_constraint_dropped(self):
		if dataent.db.exists("DocType", "With_Unique"):
			dataent.delete_doc("DocType", "With_Unique")

		dt = self.new_doctype("With_Unique", unique=1)
		dt.insert()

		doc1 = dataent.new_doc("With_Unique")
		doc2 = dataent.new_doc("With_Unique")
		...
		doc1.insert()
		self.assertRaises(dataent.UniqueValidationError, doc2.insert)
	...
	def test_all_depends_on_fields_conditions(self):
		...
		docfields = dataent.get_all("DocField", or_filters={
			"ifnull(depends_on, '')": ("!=", ''),
