...
from dataent.utils import now, cint
from dataent.model import no_value_fields, default_fields
from dataent.model.document import Document
from dataent.custom.doctype.property_setter.property_setter import make_property_setter
from dataent.desk.notifications import delete_notification_count_for
from dataent.modules import make_boilerplate, get_doc_path
from dataent.model.db_schema import validate_column_name, validate_column_length, type_map
...
class InvalidFieldNameError(dataent.ValidationError): pass
class UniqueFieldnameError(dataent.ValidationError): pass
class IllegalMandatoryError(dataent.ValidationError): pass
class DoctypeLinkError(dataent.ValidationError): pass
class WrongOptionsDoctypeLinkError(dataent.ValidationError): pass
class HiddenAndMandatoryWithoutDefaultError(dataent.ValidationError): pass
class NonUniqueError(dataent.ValidationError): pass
class CannotIndexedError(dataent.ValidationError): pass
class CannotCreateStandardDoctypeError(dataent.ValidationError): pass
...
class DocType(Document):
	...
	def validate(self):
		...
		if not self.is_new():
			self.before_update = dataent.get_doc('DocType', self.name)

		if not self.is_new():
			self.setup_fields_to_fetch()

		if self.default_print_format and not self.custom:
			dataent.throw(_('Standard DocType cannot have default print format, use Customize Form'))
	...
	def check_developer_mode(self):
		...
		if dataent.flags.in_patch or dataent.flags.in_test:
			return

		if not dataent.conf.get("developer_mode") and not self.custom:
			dataent.throw(_("Not in Developer Mode! Set in site_config.json or make 'Custom' DocType."), CannotCreateStandardDoctypeError)

	def setup_fields_to_fetch(self):
		'''Setup query to update values for newly set fetch values'''
		try:
			old_meta = dataent.get_meta(dataent.get_doc('DocType', self.name), cached=False)
			old_fields_to_fetch = [df.fieldname for df in old_meta.get_fields_to_fetch()]
		except dataent.DoesNotExistError:
			old_fields_to_fetch = []

		new_meta = dataent.get_meta(self, cached=False)
	...
	def validate_website(self):
		...
		if self.has_web_view:
			...
			if not 'route' in [d.fieldname for d in self.fields]:
				dataent.throw(_('Field "route" is mandatory for Web Views'), title='Missing Field')
	...
	def change_modified_of_parent(self):
		...
		if dataent.flags.in_import:
			return
		parent_list = dataent.db.sql("""SELECT parent
			from tabDocField where fieldtype="Table" and options=%s""", self.name)
		for p in parent_list:
			dataent.db.sql('UPDATE tabDocType SET modified=%s WHERE `name`=%s', (now(), p[0]))
	...
	def validate_series(self, autoname=None, name=None):
		...
		if autoname and autoname.startswith('field:'):
			...
			if not field or field not in [ df.fieldname for df in self.fields ]:
				dataent.throw(_("Invalid fieldname '{0}' in autoname".format(field)))
			else:
		...
		if autoname and (not autoname.startswith('field:')) \
			...
			used_in = dataent.db.sql('select name from tabDocType where substring_index(autoname, ".", 1) = %s and name!=%s', (prefix, name))
			if used_in:
				dataent.throw(_("Series {0} already used in {1}").format(prefix, used_in[0][0]))
	...
	def on_update(self):
		...
		if not self.custom and not (dataent.flags.in_import or dataent.flags.in_test) and conf.get('developer_mode'):
			self.export_doc()
		...
		dataent.clear_cache(doctype=self.name)

		if not dataent.flags.in_install and hasattr(self, 'before_update'):
			self.sync_global_search()

		# clear from local cache
		if self.name in dataent.local.meta_cache:
			del dataent.local.meta_cache[self.name]

		clear_linked_doctype_cache()

	def delete_duplicate_custom_fields(self):
		if not (dataent.db.table_exists(self.name) and dataent.db.table_exists("Custom Field")):
			return
		fields = [d.fieldname for d in self.fields if d.fieldtype in type_map]
		dataent.db.sql('''delete from
				`tabCustom Field`
			where
	...
	def sync_global_search(self):
		...
		if set(global_search_fields_before_update) != set(global_search_fields_after_update):
			now = (not dataent.request) or dataent.flags.in_test or dataent.flags.in_install
	...
	def set_base_class_for_controller(self):
		...
		controller_path = dataent.get_module_path(dataent.scrub(self.module),
			'doctype', dataent.scrub(self.name), dataent.scrub(self.name) + '.py')
		...
		if '\nfrom dataent.model.document import Document' in code and class_string in code:
			code = code.replace('from dataent.model.document import Document',
	...
	def before_rename(self, old, new, merge=False):
		...
		if not self.custom and dataent.session.user != "Administrator":
			dataent.throw(_("DocType can only be renamed by Administrator"))

		self.check_developer_mode()
		self.validate_name(new)

		if merge:
			dataent.throw(_("DocType can not be merged"))

		# Do not rename and move files and folders for custom doctype
		if not self.custom and not dataent.flags.in_test and not dataent.flags.in_patch:
	...
	def after_rename(self, old, new, merge=False):
		...
		if self.issingle:
			dataent.db.sql("""update tabSingles set doctype=%s where doctype=%s""", (new, old))
			dataent.db.sql("""update tabSingles set value=%s
				where doctype=%s and field='name' and value = %s""", (new, new, old))
		else:
			dataent.db.sql("rename table `tab%s` to `tab%s`" % (old, new))
	...
	def rename_files_and_folders(self, old, new):
		...
		for fname in os.listdir(new_path):
			if dataent.scrub(old) in fname:
				subprocess.check_output(['mv', os.path.join(new_path, fname),
					os.path.join(new_path, fname.replace(dataent.scrub(old), dataent.scrub(new)))])

		self.rename_inside_controller(new, old, new_path)
		dataent.msgprint(_('Renamed files and replaced code in controllers, please check!'))

	def rename_inside_controller(self, new, old, new_path):
		for fname in ('{}.js', '{}.py', '{}_list.js', '{}_calendar.js', 'test_{}.py', 'test_{}.js'):
			fname = os.path.join(new_path, fname.format(dataent.scrub(new)))
			if os.path.exists(fname):
				with open(fname, 'r') as f:
					code = f.read()
				with open(fname, 'w') as f:
					f.write(code.replace(dataent.scrub(old).replace(' ', ''), dataent.scrub(new).replace(' ', '')))
	...
	def preserve_naming_series_options_in_property_setter(self):
		...
		if not (dataent.db.table_exists(self.name) and dataent.db.sql("select name from `tab{}` limit 1".format(self.name))):
			return

		existing_property_setter = dataent.db.get_value("Property Setter", {"doc_type": self.name,
	...
	def import_doc(self):
		...
		from dataent.modules.import_module import import_from_files
	...
	def make_controller_template(self):
		...
		if not self.istable:
			...
			if not os.path.exists(dataent.get_module_path(dataent.scrub(self.module),
				'doctype', dataent.scrub(self.name), 'tests')):
				make_boilerplate("test_controller.js", self.as_dict())

		if self.has_web_view:
			templates_path = dataent.get_module_path(dataent.scrub(self.module), 'doctype', dataent.scrub(self.name), 'templates')
	...
	def make_amendable(self):
		...
		if self.is_submittable:
			if not dataent.db.sql("""select name from tabDocField
	...
	def get_max_idx(self):
		...
		max_idx = dataent.db.sql("""select max(idx) from `tabDocField` where parent = %s""",
			self.name)
	...
	def validate_name(self, name=None):
		...
		if not is_a_valid_name:
			dataent.throw(_("DocType's name should start with a letter and it can only consist of letters, numbers, spaces and underscores"), dataent.NameError)

def validate_fields_for_doctype(doctype):
	doc = dataent.get_doc("DocType", doctype)
	doc.delete_duplicate_custom_fields()
	validate_fields(dataent.get_meta(doctype, cached=False))
...
def validate_fields(meta):
	...
	:param meta: `dataent.model.meta.Meta` object to check."""
	...
	def check_unique_fieldname(docname, fieldname):
		...
		if len(duplicates) > 1:
			dataent.throw(_("{0}: Fieldname {1} appears multiple times in rows {2}").format(docname, fieldname, ", ".join(duplicates)), UniqueFieldnameError)
	...
	def check_illegal_mandatory(docname, d):
		if (d.fieldtype in no_value_fields) and d.fieldtype!="Table" and d.reqd:
			dataent.throw(_("{0}: Field {1} of type {2} cannot be mandatory").format(docname, d.label, d.fieldtype), IllegalMandatoryError)

	def check_link_table_options(docname, d):
		if d.fieldtype in ("Link", "Table"):
			if not d.options:
				dataent.throw(_("{0}: Options required for Link or Table type field {1} in row {2}").format(docname, d.label, d.idx), DoctypeLinkError)
			if d.options=="[Select]" or d.options==d.parent:
				return
			if d.options != d.parent:
				options = dataent.db.get_value("DocType", d.options, "name")
				if not options:
					dataent.throw(_("{0}: Options must be a valid DocType for field {1} in row {2}").format(docname, d.label, d.idx), WrongOptionsDoctypeLinkError)
				elif not (options == d.options):
					dataent.throw(_("{0}: Options {1} must be the same as doctype name {2} for the field {3}", DoctypeLinkError)
	...
	def check_hidden_and_mandatory(docname, d):
		if d.hidden and d.reqd and not d.default:
			dataent.throw(_("{0}: Field {1} in row {2} cannot be hidden and mandatory without default").format(docname, d.label, d.idx), HiddenAndMandatoryWithoutDefaultError)

	def check_width(d):
		if d.fieldtype == "Currency" and cint(d.width) < 100:
			dataent.throw(_("Max width for type Currency is 100px in row {0}").format(d.idx))

	def check_in_list_view(d):
		if d.in_list_view and (d.fieldtype in not_allowed_in_list_view):
			dataent.throw(_("'In List View' not allowed for type {0} in row {1}").format(d.fieldtype, d.idx))

	def check_in_global_search(d):
		if d.in_global_search and d.fieldtype in no_value_fields:
			dataent.throw(_("'In Global Search' not allowed for type {0} in row {1}")
	...
	def check_dynamic_link_options(d):
		if d.fieldtype=="Dynamic Link":
			...
			if not doctype_pointer or (doctype_pointer[0].fieldtype not in ("Link", "Select")) \
				...
				dataent.throw(_("Options 'Dynamic Link' type of field must point to another Link Field with options as 'DocType'"))

	def check_illegal_default(d):
		if d.fieldtype == "Check" and d.default and d.default not in ('0', '1'):
			dataent.throw(_("Default for 'Check' type of field must be either '0' or '1'"))
		if d.fieldtype == "Select" and d.default and (d.default not in d.options.split("\n")):
			dataent.throw(_("Default for {0} must be an option").format(d.fieldname))

	def check_precision(d):
		if d.fieldtype in ("Currency", "Float", "Percent") and d.precision is not None and not (1 <= cint(d.precision) <= 6):
			dataent.throw(_("Precision should be between 1 and 6"))
	...
	def check_unique_and_text(docname, d):
		...
		if getattr(d, "unique", False):
			if d.fieldtype not in ("Data", "Link", "Read Only"):
				dataent.throw(_("{0}: Fieldtype {1} for {2} cannot be unique").format(docname, d.fieldtype, d.label), NonUniqueError)

			if not d.get("__islocal"):
				try:
					has_non_unique_values = dataent.db.sql("""select `{fieldname}`, count(*)
					...
					if has_non_unique_values and has_non_unique_values[0][0]:
						dataent.throw(_("{0}: Field '{1}' cannot be set as Unique as it has non-unique values").format(docname, d.label), NonUniqueError)

		if d.search_index and d.fieldtype in ("Text", "Long Text", "Small Text", "Code", "Text Editor"):
			dataent.throw(_("{0}:Fieldtype {1} for {2} cannot be indexed").format(docname, d.fieldtype, d.label), CannotIndexedError)
	...
	def check_fold(fields):
		...
		for i, f in enumerate(fields):
			if f.fieldtype=="Fold":
				if fold_exists:
					dataent.throw(_("There can be only one Fold in a form"))
				fold_exists = True
				if i < len(fields)-1:
					nxt = fields[i+1]
					if nxt.fieldtype != "Section Break":
						dataent.throw(_("Fold must come before a Section Break"))
				else:
					dataent.throw(_("Fold can not be at the end of the form"))
	...
	def check_search_fields(meta, fields):
		...
		for fieldname in search_fields:
			...
			if (fieldtype_mapper.get(fieldname) in no_value_fields) or \
				...
				dataent.throw(_("Search field {0} is not valid").format(fieldname))
	...
	def check_title_field(meta):
		...
		if meta.title_field not in fieldname_list:
			dataent.throw(_("Title field must be a valid fieldname"), InvalidFieldNameError)
		...
		def _validate_title_field_pattern(pattern):
			...
			for fieldname in re.findall("{(.*?)}", pattern, re.UNICODE):
				...
				if fieldname not in fieldname_list:
					dataent.throw(_("{{{0}}} is not a valid fieldname pattern. It should be {{field_name}}.").format(fieldname),
	...
	def check_image_field(meta):
		...
		if not df:
			dataent.throw(_("Image field must be a valid fieldname"), InvalidFieldNameError)
		if df[0].fieldtype != 'Attach Image':
			dataent.throw(_("Image field must be of type Attach Image"), InvalidFieldNameError)
	...
	def check_is_published_field(meta):
		...
		if meta.is_published_field not in fieldname_list:
			dataent.throw(_("Is Published Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_timeline_field(meta):
		...
		if meta.timeline_field not in fieldname_list:
			dataent.throw(_("Timeline field must be a valid fieldname"), InvalidFieldNameError)

		df = meta.get("fields", {"fieldname": meta.timeline_field})[0]
		if df.fieldtype not in ("Link", "Dynamic Link"):
			dataent.throw(_("Timeline field must be a Link or Dynamic Link"), InvalidFieldNameError)
		...
		if meta.sort_field:
			...
			for fieldname in sort_fields:
				if not fieldname in fieldname_list + list(default_fields):
					dataent.throw(_("Sort field {0} must be a valid fieldname").format(fieldname),
						InvalidFieldNameError)
	...
	def check_illegal_depends_on_conditions(docfield):
		...
		for field in depends_on_fields:
			...
			if depends_on and ("=" in depends_on) and \
				...
				dataent.throw(_("Invalid {0} condition").format(dataent.unscrub(field)), dataent.ValidationError)
...
def validate_permissions_for_doctype(doctype, for_remove=False):
	...
	doctype = dataent.get_doc("DocType", doctype)
...
def clear_permissions_cache(doctype):
	...
	for user in dataent.db.sql_list("""select
		...
		dataent.clear_cache(user=user)

def validate_permissions(doctype, for_remove=False):
	permissions = doctype.get("permissions")
	if not permissions:
		dataent.msgprint(_('No Permissions Specified'), alert=True, indicator='orange')
	...
	def check_atleast_one_set(d):
		if not d.read and not d.write and not d.submit and not d.cancel and not d.create:
			dataent.throw(_("{0}: No basic permissions set").format(get_txt(d)))
	...
	def check_double(d):
		...
		if has_similar:
			dataent.throw(_("{0}: Only one rule allowed with the same Role, Level and {1}")\
	...
	def check_level_zero_is_set(d):
		if cint(d.permlevel) > 0 and d.role != 'All':
			...
			if not has_zero_perm:
				dataent.throw(_("{0}: Permission at level 0 must be set before higher levels are set").format(get_txt(d)))
	...
	def check_permission_dependency(d):
		if d.cancel and not d.submit:
			dataent.throw(_("{0}: Cannot set Cancel without Submit").format(get_txt(d)))

		if (d.submit or d.cancel or d.amend) and not d.write:
			dataent.throw(_("{0}: Cannot set Submit, Cancel, Amend without Write").format(get_txt(d)))
		if d.amend and not d.write:
			dataent.throw(_("{0}: Cannot set Amend without Cancel").format(get_txt(d)))
		if d.get("import") and not d.create:
			dataent.throw(_("{0}: Cannot set Import without Create").format(get_txt(d)))
	...
	def remove_rights_for_single(d):
		...
		if d.report:
			dataent.msgprint(_("Report cannot be set for Single types"))
			d.report = 0
			d.set("import", 0)
			d.set("export", 0)
		...
		for ptype, label in [["set_user_permissions", _("Set User Permissions")]]:
			if d.get(ptype):
				d.set(ptype, 0)
				dataent.msgprint(_("{0} cannot be set for Single types").format(label))

	def check_if_submittable(d):
		if d.submit and not issubmittable:
			dataent.throw(_("{0}: Cannot set Assign Submit if not Submittable").format(get_txt(d)))
		elif d.amend and not issubmittable:
			dataent.throw(_("{0}: Cannot set Assign Amend if not Submittable").format(get_txt(d)))

	def check_if_importable(d):
		if d.get("import") and not isimportable:
			dataent.throw(_("{0}: Cannot set import as {1} is not importable").format(get_txt(d), doctype))
...
def make_module_and_roles(doc, perm_fieldname="permissions"):
	...
	try:
		if hasattr(doc,'restrict_to_domain') and doc.restrict_to_domain and \
			not dataent.db.exists('Domain', doc.restrict_to_domain):
			dataent.get_doc(dict(doctype='Domain', domain=doc.restrict_to_domain)).insert()

		if not dataent.db.exists("Module Def", doc.module):
			m = dataent.get_doc({"doctype": "Module Def", "module_name": doc.module})
			m.app_name = dataent.local.module_app[dataent.scrub(doc.module)]
		...
		for role in list(set(roles)):
			if not dataent.db.exists("Role", role):
				r = dataent.get_doc(dict(doctype= "Role", role_name=role, desk_access=1))
				r.flags.ignore_mandatory = r.flags.ignore_permissions = True
				r.insert()
	except dataent.DoesNotExistError as e:
		pass
...
def check_if_fieldname_conflicts_with_methods(doctype, fieldname):
	...
	if fieldname in method_list:
		dataent.throw(_("Fieldname {0} conflicting with meta object").format(fieldname))

def clear_linked_doctype_cache():
	dataent.cache().delete_value('linked_doctypes_without_ignore_user_permissions_enabled')
