...
cur_frm.cscript.refresh = function(doc) {
	cur_frm.add_custom_button("Show Report", function() {
		switch(doc.report_type) {
			case "Report Builder":
				dataent.set_route('List', doc.ref_doctype, 'Report', doc.name);
				break;
			case "Query Report":
				dataent.set_route("query-report", doc.name);
				break;
			case "Script Report":
				dataent.set_route("query-report", doc.name);
				break;
...
dataent.ui.form.on('Report', {
	refresh: function(frm) {
		if(!dataent.boot.developer_mode && dataent.session.user != 'Administrator') {
