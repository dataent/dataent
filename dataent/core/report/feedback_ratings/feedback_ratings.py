...
def get_data(filters):
	...
	party_details = dataent.get_list("Communication", filters=filters, fields=fields,
...
def get_document_type(doctype, txt, searchfield, start, page_len, filters):
	...
	document_type = dataent.get_all("Feedback Trigger", filters={ "enabled": 1, "document_type": ("like", txt) },
		fields=["document_type"], as_list=True)

	document_type = map(list, document_type)
	to_ignore = [ doc[0] for doc in document_type ]

	documents = dataent.get_all("Feedback Request", filters={ "reference_doctype": ["not in", to_ignore] },
