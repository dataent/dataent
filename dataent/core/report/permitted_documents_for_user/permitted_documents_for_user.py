...
from dataent.permissions import check_admin_or_system_manager
...
def execute(filters=None):
	...
	if show_permissions:
		...
		for i,item in enumerate(data):
			temp = dataent.permissions.get_doc_permissions(dataent.get_doc(doctype, item[0]), False,user)
...
def get_columns_and_fields(doctype):
	...
	fields = ["`name`"]
	for df in dataent.get_meta(doctype).fields:
...
def query_doctypes(doctype, txt, searchfield, start, page_len, filters):
	...
	user_perms = dataent.utils.user.UserPermissions(user)
	user_perms.build_permissions()
	can_read = user_perms.can_read

	single_doctypes = [d[0] for d in dataent.db.get_values("DocType", {"issingle": 1})]
