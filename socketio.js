...
io.on('connection', function (socket) {
	...
	// dataent.chat
	socket.on("dataent.chat.room:subscribe", function (rooms) {
		if (!Array.isArray(rooms)) {
			rooms = [rooms];
		}

		for (var room of rooms) {
			console.log('dataent.chat: Subscribing ' + socket.user + ' to room ' + room);
			room = get_chat_room(socket, room);

			console.log('dataent.chat: Subscribing ' + socket.user + ' to event ' + room);
			socket.join(room);
		}
	});

	socket.on("dataent.chat.message:typing", function (data) {
		const user = data.user;
		const room = get_chat_room(socket, data.room);

		console.log('dataent.chat: Dispatching ' + user + ' typing to room ' + room);

		io.to(room).emit('dataent.chat.room:typing', {
			room: data.room,
			user: user
		});
	});
	...
	request.get(get_url(socket, '/api/method/dataent.realtime.get_user_info'))
		.type('form')
		.query({
			sid: sid
		})
...
function get_site_name(socket) {
	if (socket.request.headers['x-dataent-site-name']) {
		return get_hostname(socket.request.headers['x-dataent-site-name']);
...
function can_subscribe_doc(args) {
	...
	request.get(get_url(args.socket, '/api/method/dataent.realtime.can_subscribe_doc'))
		.type('form')
		.query({
			sid: args.sid,
